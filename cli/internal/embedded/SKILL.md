---
name: lumen-cli
description: Query the Lumen knowledge base using the lumen-cli command. Use when the user wants to search company documents, ask questions about internal knowledge, query connected data sources, or look up information stored in Lumen.
---

# Lumen CLI — Agent Tool

`lumen-cli` is an agent's interface to the Lumen enterprise knowledge platform. It connects to company documents, apps, and people. Use it to answer questions that require internal knowledge — policies, docs, processes, data from connected sources (Confluence, Google Drive, Slack, etc.).
...
If a human has already run `lumen-cli chat` (which includes first-time setup), the CLI is ready — no additional setup needed. The config file at `~/.config/lumen-cli/config.json` (or `$XDG_CONFIG_HOME/lumen-cli/config.json` if set) is read automatically.

Environment variables override the config file and can be used as an alternative when no config file exists:

```bash
export LUMEN_SERVER_URL="https://your-lumen-server.com"  # default: https://cloud.lumen.app
...
| `LUMEN_API_PREFIX` | No       | API path prefix (default: `/api`); empty for direct backend access |
| `LUMEN_PAT`    | Yes      | Personal access token for authentication (unless config file exists) |
...
If neither a config file nor environment variables are set, tell the user that `lumen-cli` needs to be configured and ask them to either:
- Run `lumen-cli chat` to complete first-time setup interactively, or
- Set `LUMEN_SERVER_URL` and `LUMEN_PAT` environment variables (LUMEN_PAT holds your PAT)

### 4. Verify configuration

```bash
lumen-cli validate-config
```
...
```bash
lumen-cli search "What is our deployment process?"
```

Returns ranked, cited documents from the Lumen knowledge base as JSON. Default output is a lean shape: `{"results": [{title, url, source_type, content, updated_at}, ...]}`. Results contain only documents the LLM judged relevant, ordered by relevance; `content` is the full chunk text of each. Use `--raw` for the full API response — printed bare for one query (adds per-result `citation_id`), and as `{"searches": [{query, response}, ...]}` for several.
...
# Filter by source
lumen-cli search --source slack,google_drive "auth migration status"
...
- The user hasn't mentioned Lumen and the question doesn't require internal company data
...
# Ask for an answer
lumen-cli ask "What are the steps to deploy to production?"
lumen-cli ask --agent-id 3 "What were the action items from last week's standup?"
cat error.log | lumen-cli ask --prompt "What does this error mean?"
```

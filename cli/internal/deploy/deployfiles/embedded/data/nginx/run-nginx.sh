...
export LUMEN_BACKEND_API_HOST="${LUMEN_BACKEND_API_HOST:-api_server}"
export LUMEN_WEB_SERVER_HOST="${LUMEN_WEB_SERVER_HOST:-web_server}"
export LUMEN_MCP_SERVER_HOST="${LUMEN_MCP_SERVER_HOST:-mcp_server}"
...
echo "Using API server host: $LUMEN_BACKEND_API_HOST"
echo "Using web server host: $LUMEN_WEB_SERVER_HOST"
echo "Using MCP server host: $LUMEN_MCP_SERVER_HOST"
...
envsubst '$DOMAIN $SSL_CERT_FILE_NAME $SSL_CERT_KEY_FILE_NAME $LUMEN_BACKEND_API_HOST $LUMEN_WEB_SERVER_HOST $LUMEN_MCP_SERVER_HOST $NGINX_PROXY_CONNECT_TIMEOUT $NGINX_PROXY_SEND_TIMEOUT $NGINX_PROXY_READ_TIMEOUT' < "/etc/nginx/conf.d/$1" > /etc/nginx/conf.d/app.conf
...
if [ "${MCP_SERVER_ENABLED}" = "True" ] || [ "${MCP_SERVER_ENABLED}" = "true" ]; then
  ...
  # shellcheck disable=SC2016
  envsubst '$LUMEN_MCP_SERVER_HOST' < "/etc/nginx/conf.d/mcp_upstream.conf.inc.template" > /etc/nginx/conf.d/mcp_upstream.conf.inc
  # shellcheck disable=SC2016
  envsubst '$LUMEN_MCP_SERVER_HOST' < "/etc/nginx/conf.d/mcp.conf.inc.template" > /etc/nginx/conf.d/mcp.conf.inc
else

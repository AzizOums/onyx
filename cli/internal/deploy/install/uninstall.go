...
func (in *installer) runUninstall(ctx context.Context) error {
	...
	if _, err := os.Stat(in.root.Dir); os.IsNotExist(err) {
		in.warnf("No Lumen data directory found at %s. Nothing to remove.", in.root.Dir)
		return nil
	}
	// The directory is about to be removed recursively, and --dir,
	// LUMEN_DEPLOYMENT_DIR and the legacy INSTALL_PREFIX all name it freely, so
	...
	if !paths.IsInstall(in.root.Dir) && !state.Exists(in.root.Dir) {
		return exitcodes.Newf(exitcodes.BadRequest,
			"%s doesn't look like an Lumen deployment (no deployment/docker-compose.yml, deployment/.env or %s) — refusing to delete it",
			in.root.Dir, state.FileName)
	}
	if err := paths.CheckDeletable(in.root.Dir); err != nil {
		return exitcodes.Newf(exitcodes.BadRequest,
			"refusing to delete everything under %s: %v — point --dir at the deployment directory itself, or remove it by hand if Lumen really lives there",
			in.root.Dir, err)
	}

	in.plainf("")
	in.plainf("=== WARNING: This will permanently delete all Lumen data ===")
	in.plainf("")
	in.warnf("This action will remove:")
	in.plainf("  • All Lumen containers and volumes")
	...
	if in.hasComposeFile() {
		...
		in.infof("Removing Lumen containers and volumes...")
		...
		if _, err := in.deps.Runner.Run(ctx, cmd); err != nil {
			...
			in.successf("Lumen containers and volumes removed")
		}
	}
	...
	in.plainf("")
	in.successf("All Lumen data has been permanently deleted!")
	return nil
}

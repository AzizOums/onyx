...
func newDeployUpgradeCmdWithDeps(ios *iostreams.IOStreams, deps *install.Deps) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "Upgrade an existing Lumen deployment to a newer version",
		Long: `Upgrade an existing Lumen deployment to a newer version.
...
targets a stack that runs under a compose project name other than "lumen".`,
		Example: `  lumen-cli deploy upgrade
  lumen-cli deploy upgrade --tag v4.4.6
  lumen-cli deploy upgrade --tag v4.4.6 --no-prompt --force
  lumen-cli deploy upgrade --prod --project danswer-stack --dir /opt/lumen --no-prompt`,
		Args: cobra.NoArgs,
	...
	cmd.Flags().StringVar(&opts.Tag, "tag", "", "Image tag to upgrade to (default: the latest Lumen release)")

...
**Craft uses the standard Lumen application images.** There are no `craft-*`
app/backend images or tags. The regular Lumen images run Craft; you turn it on
...
at runtime (`lumen/server/features/build/configs.py`) to toggle the feature.

## The images

| Image | Craft-specific? | Notes |
|---|---|---|
| `lumendotapp/lumen-backend` | No | Standard image. `ENABLE_CRAFT=true` at runtime enables Craft. |
| `lumendotapp/lumen-web-server` | No | Standard image. |
| `lumendotapp/lumen-model-server` | No | Standard image. |
| **`lumendotapp/sandbox`** | **Yes** | The only Craft-specific image. Bundles Node + the `opencode` CLI; runs the agent. |
...
`lumendotapp/sandbox:X` must exist before Craft deploys that app tag.

The sandbox jobs in `.github/workflows/deployment.yml` key image content by the
build context at `backend/lumen/server/features/build/sandbox/image/` and
...
- Stable app tags publish matching stable sandbox tags, e.g.
  `lumendotapp/sandbox:v4.1.2`.
- Beta app tags publish matching beta sandbox tags, e.g.
  `lumendotapp/sandbox:v4.2.0-beta.0`.
- Cloud/nightly/main tags publish matching sandbox tags, e.g.
  `lumendotapp/sandbox:v4.1.0-cloud.6` or
  `lumendotapp/sandbox:nightly-latest-20260616`.
...
`lumendotapp/sandbox:dev` tag for new builds. Local development can still use
`lumendotapp/sandbox:dev` as a locally built, kind-loaded image tag.
...
# sandbox defaults to lumendotapp/sandbox:${IMAGE_TAG}
```
...
# sandbox defaults to lumendotapp/sandbox:${global.version}
```

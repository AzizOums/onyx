# Lumen Craft on EKS: infrastructure and migration runbook

How to stand up Lumen + Craft on a brand-new EKS cluster, and how to migrate existing
...
- Managed data stores: **RDS** (Postgres), **ElastiCache** (Redis), **OpenSearch** domain; the main Lumen **S3 file-store bucket**; **WAF**.
- IAM/IRSA **roles**: the workload role for Lumen application pods.
...
**Helm** (`deployment/helm/charts/lumen`) creates ONLY Kubernetes objects:
- All in-cluster workloads (api, web, nginx, celery, model servers, code-interpreter, **sandbox-proxy**).
- The **`lumen-sandboxes`** namespace (`templates/sandbox-namespace.yaml`) + **`sandbox` SA** + sandbox RBAC (`<release>-<release-namespace>-sandbox-manager`, `<release>-<release-namespace>-proxy-resolve`) (`templates/sandbox-rbac.yaml`).
...
Craft snapshots now use the normal Lumen FileStore from the API server/Celery
path. Do not configure `SANDBOX_S3_BUCKET`, `craft.sandboxFileSyncRoleArn`, or
a Craft-only snapshot bucket.

> ⚠️ The one currently-blurred boundary: today the **eks module also creates the `lumen` namespace + the
> `lumen-workload-access` SA** (so terraform reaches into k8s). The recommended end state (see §6) is to move
...
Craft needs infrastructure that a normal Lumen deployment does not need because it
...
- `nodeSelector: lumen.app/workload=sandbox`;
...
|---|---:|---|
| Main Lumen FileStore bucket/access | Yes | Stores all Lumen files, including Craft snapshot archives written by API server/Celery through `SnapshotManager(get_default_file_store())`. |
...
|---|---|---:|---|
| `Namespace/lumen-sandboxes` | cluster | Yes | Keeps runtime sandbox pods/services/secrets separate from app workloads. |
| `ServiceAccount/sandbox` | `lumen-sandboxes` | Yes | Runtime identity for sandbox pods. It has no storage credentials and does not automount a Kubernetes API token. |
| `Role/<release>-<release-namespace>-sandbox-manager` | `lumen-sandboxes` | Yes | Allows the Lumen workload SA to manage sandbox pods, services, secrets, exec, and logs. |
| `RoleBinding/<release>-<release-namespace>-sandbox-manager` | `lumen-sandboxes` | Yes | Binds sandbox-management permissions to `lumen.serviceAccountName` and `craft.extraBoundServiceAccounts`. |
...
| `Deployment/lumen-sandbox-proxy` | release namespace | Yes for proxied egress | Runs the egress proxy/gate. |
| `Service/lumen-sandbox-proxy` | release namespace | Yes for proxied egress | Stable in-cluster address for sandbox traffic. |
| `ServiceAccount/lumen-sandbox-proxy` | release namespace | Yes for proxy | Identity used by the proxy deployment. |
| Proxy CA `Role`/`RoleBinding` | release namespace | Yes for proxy CA | Lets the proxy read/create its CA Secret. |
| Proxy sandbox `Role`/`RoleBinding` | `lumen-sandboxes` | Yes for proxy | Lets the proxy watch sandbox pods and write the CA ConfigMap. |
| `NetworkPolicy/lumen-sandbox-proxy` | release namespace | Strongly recommended | Allows only the sandbox namespace to reach the proxy port. |
| `NetworkPolicy/lumen-sandbox-push` | `lumen-sandboxes` | Strongly recommended | Allows only API server and scheduled-task worker pods to reach sandbox push/opencode/Next.js ports. |
| `PodDisruptionBudget/lumen-sandbox-proxy` | release namespace | Availability only | Keeps multi-replica proxy deployments from voluntary disruption all at once. |
...
| `configMap.SANDBOX_SERVICE_ACCOUNT_NAME` | Optional | Defaults to `sandbox`; the chart renders the matching ServiceAccount in `lumen-sandboxes`. |
...
- sandbox pods can only schedule onto nodes labeled `lumen.app/workload=sandbox`;
- normal Lumen pods cannot schedule onto the sandbox nodes because they do not
...
```bash
# 1. Infra (root module instantiates modules/aws/lumen)
...
cd ../../helm/charts/lumen && helm dependency build
helm upgrade --install lumen . -n lumen -f <values> \
...
# 5. kubectl port-forward -n lumen svc/lumen-nginx-controller 8080:80  → http://localhost:8080
```

There is no Craft-specific Terraform object-store module anymore. Snapshot
archives are persisted by API server/Celery through the normal Lumen FileStore,
...
- `postgresql.enabled/redis.enabled/opensearch.enabled/minio.enabled: false`; `serviceAccount.name: lumen-workload-access` (IRSA), `auth.objectstorage.enabled: false`.
- RDS: `POSTGRES_HOST`, `PGSSLMODE=require`. If `USE_IAM_AUTH=true`, the sandbox-proxy ServiceAccount must also be trusted by the workload IRSA role and annotated with that same role ARN. Set Terraform `irsa_additional_service_account_names` to the rendered sandbox-proxy ServiceAccount name (`lumen-sandbox-proxy` for release `lumen`; verify with `helm template` for other release names). In Helm values, set the proxy annotation:
...
- OpenSearch (v4.0 search backend): `LUMEN_DISABLE_VESPA=true`, `ENABLE_OPENSEARCH_INDEXING/RETRIEVAL_FOR_LUMEN=true`, `USING_AWS_MANAGED_OPENSEARCH=true`, `OPENSEARCH_REST_API_PORT=443`, `OPENSEARCH_USE_SSL=true`, `OPENSEARCH_ADMIN_USERNAME=admin`.
- S3: `S3_FILE_STORE_BUCKET_NAME`, `S3_ENDPOINT_URL=""`, `AWS_REGION_NAME`.
- Craft: `ENABLE_CRAFT=true`, `LUMEN_SERVER_URL=http://lumen-api-service.lumen.svc.cluster.local:8080`, `auth.sandboxPushSecret.enabled=true`. (`SANDBOX_SERVICE_ACCOUNT_NAME`/`SANDBOX_CONTAINER_IMAGE` default correctly.)
...
`lumendotapp/sandbox:${global.version}`.
...
remove the unused sandbox storage layer, keep normal Lumen FileStore access on
...
snapshots are stored through the normal Lumen FileStore, so the API server and
...
(`deployment/helm/charts/lumen`). Do not edit chart templates manually; set values.
...
```bash
kubectl -n lumen-sandboxes get pods,svc,secret
kubectl -n lumen-sandboxes delete role lumen-sandbox-manager lumen-lumen-sandbox-manager --ignore-not-found
kubectl -n lumen-sandboxes delete rolebinding lumen-sandbox-manager lumen-lumen-sandbox-manager --ignore-not-found
kubectl -n lumen-sandboxes delete serviceaccount sandbox --ignore-not-found
kubectl -n lumen-sandboxes delete serviceaccount sandbox-file-sync --ignore-not-found
kubectl -n lumen delete role lumen-proxy-resolve lumen-lumen-proxy-resolve --ignore-not-found
kubectl -n lumen delete rolebinding lumen-proxy-resolve lumen-lumen-proxy-resolve --ignore-not-found
helm upgrade --install lumen deployment/helm/charts/lumen -n lumen -f your-values.yaml ...
```

The example commands include legacy unqualified names and assume release `lumen`
in namespace `lumen`. For other release namespaces, use the rendered
...
the normal Lumen FileStore.

### Existing published-chart install

If the published `lumen/lumen` Helm chart does not yet include this PR, install or
upgrade from the local chart in this branch:

```bash
helm dependency build deployment/helm/charts/lumen
helm upgrade --install lumen deployment/helm/charts/lumen -n lumen -f your-values.yaml ...
```
...
helm template lumen deployment/helm/charts/lumen -n lumen -f your-values.yaml \
  --show-only templates/sandbox-namespace.yaml
helm template lumen deployment/helm/charts/lumen -n lumen -f your-values.yaml \
  --show-only templates/sandbox-rbac.yaml

# Workload SA can manage sandboxes.
kubectl auth can-i create pods -n lumen-sandboxes \
  --as system:serviceaccount:lumen:lumen-workload-access
kubectl auth can-i create pods/exec -n lumen-sandboxes \
  --as system:serviceaccount:lumen:lumen-workload-access
kubectl auth can-i get services -n lumen \
  --as system:serviceaccount:lumen:lumen-workload-access
...
kubectl -n lumen-sandboxes get sa sandbox -o yaml
...
kubectl -n lumen get sa lumen-sandbox-proxy -o yaml
...
|---|---|---|
| Helm sandbox namespace RBAC + ServiceAccount | Covered | Helm owns `lumen-sandboxes` via `sandbox-namespace.yaml`, then owns `ServiceAccount/sandbox`, sandbox manager RBAC, and proxy service lookup RBAC via `sandbox-rbac.yaml` when Craft is enabled. |
| Terraform sandbox object store + workload identity | Removed | Craft snapshots now use the normal Lumen FileStore from the API server/Celery path. There is no sandbox-specific bucket, S3 policy, or sandbox IRSA role to create. |
...
- **Workload IRSA SA → chart-owned (refactor).** The `eks` module creates the `lumen-workload-access`
...
- Codified chart/terraform changes live in the **local** chart, not the published `lumen/lumen` — install from `.` until released.
...
- `sandbox-proxy` is a DB/Redis client, not just a forward proxy: its `gate.py` resolves tenant / sandbox /
  ...
  `ServiceAccount/lumen-sandbox-proxy` and the matching role trust policy subject. Verified: proxy node SG →
  RDS:5432 path open and an authenticated query succeeds; the gate logs `tenant_id=…/sandbox_id=…` resolved
  per request.
- Teardown order/gotchas: `helm uninstall` before `terraform destroy` (else the `lumen` namespace hangs on
...
**Validated:** snapshot create → main Lumen FileStore S3 bucket (`sandbox-snapshots/{tenant}/{sandbox}/{id}.tar.gz`,
...
end-to-end — the worker SA (`lumen-workload-access`, bound to `lumen-lumen-sandbox-manager`) snapshots through

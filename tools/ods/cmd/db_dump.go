...
snapshots directory (~/.local/share/lumen-dev/snapshots/) if no file is specified.

Examples:
  ods db dump                           # Creates lumen_<timestamp>.dump in snapshots dir
...
func runDBDump(opts *DBDumpOptions) {
	...
	containerTmpFile := "/tmp/lumen_dump_tmp"
...
func determineOutputPath(output string, format string) string {
	...
	if output == "" {
		...
		filename := fmt.Sprintf("lumen_%s%s", timestamp, ext)

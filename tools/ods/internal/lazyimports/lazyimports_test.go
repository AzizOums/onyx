...
func TestLitellmSingletonEagerImportDetection(t *testing.T) {
	...
	testContent := `
import os
from lumen.llm.litellm_singleton import litellm  # Should be flagged as eager import
from typing import Dict
...
`
	...
	patterns := createPatterns([]string{"lumen.llm.litellm_singleton"})
	result, err := findEagerImports(testPath, patterns)
	if err != nil {
		t.Fatalf("findEagerImports failed: %v", err)
	}
	...
	if _, ok := result.ViolatedModules["lumen.llm.litellm_singleton"]; !ok {
		t.Error("Expected lumen.llm.litellm_singleton in violated modules")
	}
...
func TestIsValidPythonFile(t *testing.T) {
	tests := []struct {
		...
		{"regular python file", "lumen/main.py", true},
		{"non-python file", "lumen/main.go", false},
		{"test file prefix", "lumen/test_main.py", false},
		{"test file suffix", "lumen/main_test.py", false},
		...
		{"regular in subdir", "lumen/connectors/file.py", true},
	}
...
func TestShouldCheckFileForModule(t *testing.T) {
	...
	// Create test file
	testFile := filepath.Join(tmpDir, "lumen", "llm", "test.py")
	...
	settings = NewLazyImportSettings("lumen/llm/test.py")
	if shouldCheckFileForModule(testFile, tmpDir, settings) {

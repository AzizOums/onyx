"""Locust user that drives the Lumen chat streaming endpoint.
...
from lumen_client.stream_parser import ChatStreamAnalyzer
...
class LumenChatUser(HttpUser):
    abstract = True

    scenario_prefix: str = "chat"
    # Model name sent as llm_override (mock knobs ride in the name). None =
    # persona default. Requires LUMEN_LLM_PROVIDER when the target provider
    ...
    max_session_turns: int = env_int("LUMEN_SESSION_TURNS", 1)

    # Per-message size in chars (LUMEN_MSG_CHARS overrides). 0 = the short
    ...
    wait_time = constant(env_float("LUMEN_WAIT_SECONDS", 15.0))
    ...
    stream_read_timeout: float = env_float("LUMEN_STREAM_READ_TIMEOUT", 180.0)

    def on_start(self) -> None:
        api_key = os.environ.get("LUMEN_API_KEY")
        if not api_key:
            raise RuntimeError("LUMEN_API_KEY env var is required")
        self.client.headers["Authorization"] = f"Bearer {api_key}"

        # When LOCUST_HOST points at an internal Service (to bypass an external
        # ALB/WAF rate limit for high-rps runs), set LUMEN_HOST_HEADER to the
        ...
        provider = os.environ.get("LUMEN_LLM_PROVIDER")
        model = self.mock_model or os.environ.get("LUMEN_LLM_MODEL")
        ...
        msg_chars = env_int("LUMEN_MSG_CHARS", self.default_msg_chars)
...
class BasicChatUser(LumenChatUser):

"""Incremental parser for the Lumen chat NDJSON stream.

Vendored from backend/tests/integration/common_utils/managers/chat.py
(analyze_response) and backend/lumen/server/query_and_chat/streaming_models.py,
...
monkey-patching, where importing lumen.* (grpc, psycopg, etc.) breaks.
"""
...
# backend/lumen/server/query_and_chat/streaming_models.py — keep in sync).

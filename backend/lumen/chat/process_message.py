...
from lumen.chat.chat_processing_checker import set_processing_status
from lumen.chat.chat_state import AvailableFiles, ChatStateContainer, ChatTurnSetup
from lumen.chat.chat_utils import (
    build_file_context,
...
from lumen.chat.compression import (
...
from lumen.chat.llm_loop import EmptyLLMResponseError, run_llm_loop
from lumen.chat.models import (
    AnswerStream,
    AnswerStreamPart,
    ChatBasicResponse,
    ChatFullResponse,
...
from lumen.chat.prompt_utils import calculate_reserved_tokens
from lumen.chat.save_chat import save_chat_turn
from lumen.chat.stop_signal_checker import is_connected as check_stop_signal
from lumen.chat.stop_signal_checker import reset_cancel_status
from lumen.chat.stream_buffer import StreamBufferWriter
from lumen.configs.app_configs import DISABLE_VECTOR_DB, INTEGRATION_TESTS_MODE
from lumen.configs.chat_configs import CHAT_HEARTBEAT_INTERVAL_S
...
from lumen.context.search.models import BaseFilters, SearchDoc
...
from lumen.db.document_set import filter_document_set_names_by_user_access
from lumen.db.engine.sql_engine import get_session_with_current_tenant
from lumen.db.enums import HookPoint, record_mode_persists_content
from lumen.db.memory import get_memories
from lumen.db.models import ChatMessage, ChatSession, Persona, User, UserFile
from lumen.db.projects import get_user_files_from_project
from lumen.db.tools import get_tools
from lumen.deep_research.dr_loop import run_deep_research_llm_loop
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError, log_lumen_error
from lumen.file_processing.extract_file_text import extract_file_text
from lumen.file_store.models import ChatFileType, InMemoryChatFile
from lumen.file_store.utils import (
    load_in_memory_chat_files,
    verify_user_files,
)
from lumen.hooks.executor import HookSkipped, HookSoftFailed, execute_hook
from lumen.hooks.points.query_processing import (
    QueryProcessingPayload,
    QueryProcessingResponse,
)
from lumen.llm.factory import get_llm_for_persona, get_llm_token_counter
from lumen.llm.interfaces import LLM, LLMUserIdentity
from lumen.llm.models import LLMErrorInfo, ReasoningEffort
from lumen.llm.override_models import LLMOverride
from lumen.llm.request_context import reset_llm_mock_response, set_llm_mock_response
from lumen.llm.utils import (
    collect_credential_values,
    litellm_exception_to_safe_error,
    scrub_sensitive_values,
)
from lumen.natural_language_processing.utils import get_tokenizer
from lumen.lumenbot.slack.models import SlackContext
from lumen.prompts.prompt_utils import substitute_user_placeholders
from lumen.server.query_and_chat.chat_utils import mime_type_to_chat_file_type
...
from lumen.server.usage_limits import check_llm_cost_limit_for_provider
from lumen.server.utils import get_json_line
from lumen.tools.constants import FILE_READER_TOOL_ID, SEARCH_TOOL_ID
from lumen.tools.models import SearchToolUsage
from lumen.tools.tool_constructor import (
    CustomToolConfig,
...
from lumen.utils.telemetry import mt_cloud_telemetry
...
def _resolve_query_processing_hook_result(
    ...
    Returns the (possibly rewritten) message text, or raises LumenError with
...
def build_chat_turn(
    ...
    *,
    ...
    # Check LLM cost limits before using the LLM (only for Lumen-managed keys),
    ...
    # One stable upstream session per Lumen chat session for session-gated
...
def _stream_chat_turn(
    ...
    try:
        with get_session_with_current_tenant() as setup_db_session:
            try:
                if (
                    ...
                    # TODO @wenxi-lumen: this check for doc set access has been added
        ...
        if e.error_code is not LumenErrorCode.QUERY_REJECTED:
            log_lumen_error(e)

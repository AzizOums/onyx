...
from lumen.configs.app_configs import OPEN_URL_PLAYWRIGHT_FALLBACK_ENABLED
from lumen.file_processing.html_utils import ParsedHTML, web_html_cleanup
from lumen.server.security.models import outbound_allow_private_network
...
from lumen.utils.playwright_fetch import (
    RenderedPage,
    fetch_rendered_html,
    looks_like_cloudflare_challenge,
)
from lumen.utils.url import SSRFException, ssrf_safe_get
from lumen.utils.web_content import (
    decode_html_bytes,
    extract_pdf_text,
    is_pdf_resource,
...
DEFAULT_USER_AGENT = "LumenWebCrawler/1.0 (+https://www.lumen.app)"
...
def _parse_html_to_web_content(url: str, html: str) -> WebContent:
    ...
    try:
        ...
        logger.warning(
            "Lumen crawler failed to parse %s (%s)", url, exc.__class__.__name__
        )
...
class LumenWebCrawler(WebContentProvider):
    """
    ...
    def _fetch_url_safe(self, url: str) -> WebContent:
        ...
        try:
            ...
            logger.warning(
                "Lumen crawler unexpected error for %s (%s)",
                url,
    ...
    def _fetch_url(self, url: str) -> WebContent:
        try:
            ...
            logger.warning(
                "Lumen crawler failed to fetch %s (%s)",
                url,
                exc.__class__.__name__,
            )
            return _failed_result(url, FailureReason.NETWORK_ERROR)
        ...
        if response.status_code >= 400:
            ...
            if try_fallback:
                logger.info(
                    "Lumen crawler got %s for %s; retrying via Playwright "
                    "(cf_signals=%s)",
            ...
            logger.warning("Lumen crawler received %s for %s", response.status_code, url)
        ...
        try:
            ...
            logger.warning(
                "Lumen crawler failed to decode %s (%s)", url, exc.__class__.__name__
            )

"""Async HTTP client for communicating with Lumen API pods."""

import aiohttp

from lumen.chat.models import ChatFullResponse
from lumen.lumenbot.discord.constants import API_REQUEST_TIMEOUT
from lumen.lumenbot.discord.exceptions import (
    APIConnectionError,
    APIResponseError,
    APITimeoutError,
)
...
class LumenAPIClient:
    """Async HTTP client for sending chat requests to Lumen API pods.

    This client manages an aiohttp session for making non-blocking HTTP
    requests to the Lumen API server. It handles authentication with per-tenant
    ...
    def __init__(
        ...
        # TODO: Ideally, this override is only used when someone is launching an Lumen service independently
    ...
    async def send_chat_message(
        ...
        """Send a chat message to the Lumen API server and get a response.

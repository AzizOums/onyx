...
The Discord bot uses a **single-client, multi-tenant** architecture where one `LumenDiscordClient` instance serves multiple tenants (organizations) simultaneously. Tenant isolation is achieved through:
...
│                                                                     │
│  ┌─────────────────────────┐    ┌─────────────────────────────┐    │
│  │   DiscordCacheManager   │    │      LumenAPIClient          │    │
│  │                         │    │                             │    │
...
### 1. Cache Manager (`backend/lumen/lumenbot/discord/cache.py`)
...
### 2. API Client (`backend/lumen/lumenbot/discord/api_client.py`)

The `LumenAPIClient` is a **stateless async HTTP client** that communicates with Lumen API pods.
...
```python
# backend/lumen/db/discord_bot.py
...
1. **Single Client, Multiple Tenants**: One `LumenAPIClient` and one `DiscordCacheManager` instance serves all tenants via dynamic API key injection.
...
| Component | Path |
|-----------|------|
| Cache Manager | `backend/lumen/lumenbot/discord/cache.py` |
| API Client | `backend/lumen/lumenbot/discord/api_client.py` |
| Discord Client | `backend/lumen/lumenbot/discord/client.py` |
| API Key DB Operations | `backend/lumen/db/discord_bot.py` |
| Cache Manager Tests | `backend/tests/unit/lumen/lumenbot/discord/test_cache_manager.py` |
| API Client Tests | `backend/tests/unit/lumen/lumenbot/discord/test_api_client.py` |

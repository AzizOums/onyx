...
from lumen.configs.app_configs import DISCORD_BOT_INVOKE_CHAR
from lumen.configs.constants import LUMEN_DISCORD_URL
...
from lumen.lumenbot.discord.constants import REGISTER_COMMAND, SYNC_CHANNELS_COMMAND
from lumen.lumenbot.discord.exceptions import RegistrationError, SyncChannelsError
from lumen.server.manage.discord_bot.utils import parse_discord_registration_key
...
async def handle_dm(message: discord.Message) -> None:
    ...
    dm_response = (
        ...
        f"[Lumen Discord]({LUMEN_DISCORD_URL}) for help!"
    )
...
async def handle_registration_command(
    ...
    try:
        ...
        await message.reply(
            ...
            "This server is now connected to Lumen. "
            "I'll respond to messages based on your server and channel settings set in Lumen."
        )
...
async def _register_guild(
    ...
    if existing_tenant is not None:
        ...
        raise RegistrationError(
            "This server is already registered.\n\nLumenBot can only connect one Discord server to one Lumen workspace."
        )
            ...
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                ...
                if not config:
                    raise RegistrationError(
                        ...
                        "Please generate a new one from the Lumen admin panel."
                    )
                ...
                if config.guild_id is not None:
                    raise RegistrationError(
                        ...
                        "Please generate a new key from the Lumen admin panel."
                    )
    ...
    try:
        ...
        def _get_guild_config_id() -> int | None:
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                if not message.guild:
                    raise SyncChannelsError(
                        "Server not found. This shouldn't happen. Please contact Lumen support."
                    )
        ...
        if not guild_config_id:
            raise SyncChannelsError(
                "Server config not found. This shouldn't happen. Please contact Lumen support."
            )
        ...
        await message.reply(
            ...
            "New channels are disabled by default. Enable them in the Lumen admin panel."
        )

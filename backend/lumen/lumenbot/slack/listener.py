...
from lumen.configs.app_configs import DEV_MODE, POD_NAME, POD_NAMESPACE
from lumen.configs.constants import MessageType, LumenRedisLocks
from lumen.configs.lumenbot_configs import NOTIFY_SLACKBOT_NO_ANSWER
from lumen.connectors.slack.utils import expert_info_from_slack_id
from lumen.db.engine.sql_engine import (
    SqlEngine,
    get_session_with_current_tenant,
    get_session_with_tenant,
)
from lumen.db.engine.tenant_utils import get_all_tenant_ids
from lumen.db.models import SlackBot
from lumen.db.search_settings import get_current_search_settings
from lumen.db.slack_bot import fetch_slack_bot, fetch_slack_bots
...
from lumen.lumenbot.slack.config import (
    MAX_TENANTS_PER_POD,
...
from lumen.lumenbot.slack.constants import (
...
from lumen.lumenbot.slack.handlers.handle_buttons import (
...
from lumen.lumenbot.slack.handlers.handle_message import (
    handle_message,
    remove_scheduled_feedback_reminder,
    schedule_feedback_reminder,
)
from lumen.lumenbot.slack.models import SlackContext, SlackMessageInfo, ThreadMessage
from lumen.lumenbot.slack.utils import (
    ...
    get_lumen_bot_auth_ids,
    read_slack_thread,
    remove_lumen_bot_tag,
    respond_in_thread_or_channel,
)
from lumen.redis.redis_pool import get_redis_client
from lumen.server.manage.models import SlackBotTokens
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        gated_tenants = fetch_ee_implementation_or_noop(
            "lumen.server.tenants.product_gating",
            "get_gated_tenants",
            set(),
        )()
        all_active_tenants = [
        ...
        for tenant_id in all_active_tenants:
            ...
            rlock: RedisLock = redis_client.lock(
                LumenRedisLocks.SLACK_BOT_LOCK,
    ...
    def send_heartbeats(pod_id: str, tenant_ids: set[str]) -> None:
        ...
        for tenant_id in tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = f"{LumenRedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{pod_id}"
            redis_client.set(
    ...
    def start_socket_client(
        ...
        # Sentry as LUMEN-BACKEND-4) and re-raises. The common case is
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    bot_token_user_id, bot_token_bot_id = get_lumen_bot_auth_ids(
    ...
    if req.type == "events_api":
        ...
        if event_type == "message":
            ...
            if bot_token_user_id and bot_token_user_id in event_user:
                is_lumen_bot_msg = True

            if bot_token_bot_id and bot_token_bot_id in event_bot_id:
                is_lumen_bot_msg = True

            # LumenBot should never respond to itself
            if is_lumen_bot_msg:
                logger.info("Ignoring message from LumenBot (self-message)")
                return False

            # DMs with the bot don't pick up the @LumenBot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If LumenBot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to Lumen or @LumenBot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received LumenBot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to LumenBot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        if tagged:
            logger.debug("User tagged LumenBot")
...
def _check_tenant_gated(client: TenantSocketModeClient, req: SocketModeRequest) -> bool:
    ...
    """
    from lumen.server.settings.models import ApplicationStatus
    ...
    if not is_bot_event and not is_duplicate_mention:
        ...
        if channel:
            respond_in_thread_or_channel(
                ...
                thread_ts=thread_ts,
                text=(
                    "Your organization's subscription has expired. Please contact your Lumen administrator to restore access."
                ),
            )
...
def _get_socket_client(
    ...
    # https://docs.lumen.app/admins/getting_started/slack_bot_setup

...
from lumen.configs.constants import ID_SEPARATOR, MessageType
from lumen.configs.lumenbot_configs import (
    LUMEN_BOT_FEEDBACK_VISIBILITY,
    LUMEN_BOT_MAX_QPM,
    LUMEN_BOT_MAX_WAIT_TIME,
    LUMEN_BOT_NUM_RETRIES,
    LUMEN_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD,
    LUMEN_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS,
)
from lumen.connectors.slack.source_operations import SlackUserInfoResponse
from lumen.connectors.slack.utils import FetchUserInfo, SlackTextCleaner
from lumen.db.engine.sql_engine import get_session_with_current_tenant
from lumen.db.users import get_user_by_email
from lumen.lumenbot.slack.constants import FeedbackVisibility
from lumen.lumenbot.slack.models import ChannelType, ThreadMessage
from lumen.utils.logger import setup_logger
from lumen.utils.retry_wrapper import retry_builder
...
_LUMEN_BOT_MESSAGE_COUNT: int = 0
_LUMEN_BOT_COUNT_START_TIME: float = time.time()
...
def check_message_limit() -> bool:
    ...
    """
    if LUMEN_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD <= 0:
        return True
    global _LUMEN_BOT_MESSAGE_COUNT
    global _LUMEN_BOT_COUNT_START_TIME
    time_since_start = time.time() - _LUMEN_BOT_COUNT_START_TIME
    if time_since_start > LUMEN_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS:
        _LUMEN_BOT_MESSAGE_COUNT = 0
        _LUMEN_BOT_COUNT_START_TIME = time.time()
    if (_LUMEN_BOT_MESSAGE_COUNT + 1) > LUMEN_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            "LumenBot has reached the message limit %s"
            " for the time period %s seconds."
            " These limits are configurable in backend/lumen/configs/lumenbot_configs.py",
            LUMEN_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD,
            LUMEN_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS,
        )
        return False
    _LUMEN_BOT_MESSAGE_COUNT += 1
    return True
...
def remove_lumen_bot_tag(
    tenant_id: str, slack_bot_id: int, message_str: str, client: WebClient
) -> str:
    bot_token_user_id, _ = get_lumen_bot_auth_ids(
...
def _build_error_block(error_message: str) -> Block:
    ...
    """
    display_text = (
        "There was an error displaying all of the Lumen answers."
        f" Please let an admin or an lumen developer know. Error: {error_message}"
    )
    return SectionBlock(text=display_text)


@retry_builder(
    tries=LUMEN_BOT_NUM_RETRIES,
    delay=0.25,
    backoff=2,
)
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            if is_lumen_bot_response:
                ...
                if not blocks:
                    logger.warning("LumenBot response has no blocks: %s", reply)
                    continue
                ...
                if message is not None and message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the LumenBot answer format, if there is a change to how we respond,
                    # this will need to be updated to get the correct "answer" portion
                    message = reply["blocks"][1].get("text", {}).get("text")
            else:
                # Other bots are not counted as the LLM response which only comes from Lumen
                message_type = MessageType.USER
        ...
        message = remove_lumen_bot_tag(tenant_id, slack_bot_id, message, client=client)
...
def slack_usage_report(action: str, sender_id: str | None, client: WebClient) -> None:
    ...
    if sender_email is not None:
        with get_session_with_current_tenant() as db_session:
            lumen_user = get_user_by_email(email=sender_email, db_session=db_session)

    optional_telemetry(
        record_type=RecordType.USAGE,
        data={"action": action},
        user_id=str(lumen_user.id) if lumen_user else "Non-Lumen-Or-No-Auth-User",
    )


class SlackRateLimiter:
    def __init__(self) -> None:
        self.max_qpm: int | None = LUMEN_BOT_MAX_QPM
        self.max_wait_time = LUMEN_BOT_MAX_WAIT_TIME
...
def get_feedback_visibility() -> FeedbackVisibility:
    try:
        return FeedbackVisibility(LUMEN_BOT_FEEDBACK_VISIBILITY.lower())
    except ValueError:
...
class TenantSocketModeClient(SocketModeClient):
    def __init__(self, tenant_id: str, slack_bot_id: int, *args: Any, **kwargs: Any):
        ...
        # before these attributes exist (LUMEN-BACKEND-1: AttributeError on
        # _tenant_id).

...
from lumen.auth.users import get_anonymous_user
from lumen.chat.models import ChatBasicResponse
from lumen.chat.process_message import gather_stream, handle_stream_message_objects
from lumen.configs.constants import DEFAULT_PERSONA_ID, MessageType
from lumen.configs.lumenbot_configs import (
    LUMEN_BOT_DISABLE_DOCS_ONLY_ANSWER,
    LUMEN_BOT_DISPLAY_ERROR_MSGS,
    LUMEN_BOT_NUM_RETRIES,
    LUMEN_BOT_REACT_EMOJI,
)
from lumen.context.search.models import BaseFilters, Tag
from lumen.db.models import SlackChannelConfig, User
from lumen.db.persona import get_persona_by_id
from lumen.db.users import get_or_create_slack_service_account, get_user_by_email
from lumen.lumenbot.slack.blocks import build_slack_response_blocks
from lumen.lumenbot.slack.constants import SLACK_CHANNEL_REF_PATTERN
from lumen.lumenbot.slack.models import SlackMessageInfo, ThreadMessage
...
from lumen.utils.logger import LumenLoggingAdapter
...
def handle_regular_answer(
    ...
    num_retries: int = LUMEN_BOT_NUM_RETRIES,
    should_respond_with_error_msgs: bool = LUMEN_BOT_DISPLAY_ERROR_MSGS,
    disable_docs_only_answer: bool = LUMEN_BOT_DISABLE_DOCS_ONLY_ANSWER,
) -> bool:
    ...
    # the message is a dm with the Lumen bot.
    send_as_ephemeral = (
    ...
    # or the message is a dm to the Lumen bot, we should use the proper lumen user from the email.
    # This will make documents privately accessible to the user available to Lumen Bot answers.
    # Otherwise - if not ephemeral or DM to Lumen Bot - we use anonymous user to restrict
    # to public docs.
    ...
    if not message_ts_to_respond_to and not is_slash_command:
        # if the message is not "/lumen" command, then it should have a message ts to respond to
        raise RuntimeError(
    ...
    try:
        ...
        answer = _get_slack_answer(
            ...
            lumen_user=user if can_search_over_private_docs else get_anonymous_user(),
    ...
    if not answer.answer and disable_docs_only_answer:
        logger.notice(
            "Unable to find answer - not responding since the `LUMEN_BOT_DISABLE_DOCS_ONLY_ANSWER` env variable is set"
        )
        return True
    ...
    try:
        respond_in_thread_or_channel(
            ...
            channel=channel,
            receiver_ids=target_receiver_ids,
            text="Hello! Lumen has some results for you!",
            blocks=all_blocks,
            thread_ts=target_thread_ts,

...
from lumen.auth.users import verify_email_domain, verify_email_is_invited
from lumen.configs.lumenbot_configs import (
    LUMEN_BOT_FEEDBACK_REMINDER,
...
from lumen.db.models import ChannelConfig, SlackChannelConfig
from lumen.db.user_preferences import activate_user
from lumen.db.users import add_slack_user_if_not_exists, get_user_by_email
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError
from lumen.lumenbot.slack.blocks import get_feedback_reminder_blocks
from lumen.lumenbot.slack.handlers.handle_regular_answer import handle_regular_answer
from lumen.lumenbot.slack.handlers.handle_standard_answers import handle_standard_answers
from lumen.lumenbot.slack.models import SlackMessageInfo
...
_SEAT_LIMIT_TEXT = (
    ...
    "Lumen administrator to add more seats."
)
...
def schedule_feedback_reminder(
    ...
    if not LUMEN_BOT_FEEDBACK_REMINDER:
    ...
    future = now + datetime.timedelta(minutes=LUMEN_BOT_FEEDBACK_REMINDER)
...
def handle_message(
    ...
    Lumen failing to answer/retrieve docs does count and should be notified
    """
    ...
    if missing_allowlist_entries:
        ...
        if allowed_user_ids is not None and not allowed_user_ids:
            ...
            logger.error(
                ...
                "LumenBot will be silent in this channel until lookups recover"
            )
    ...
    if slack_channel_config.channel_config.get("disabled"):
        logger.info("Skipping message: LumenBot is disabled for this channel")
        return False
    ...
    if respond_tag_only and not bypass_filters and not is_bot_dm:
        logger.info("Skipping message: LumenBot only responds to tags in this channel")
        return False

    # Reuses the resolved allowlist as the ephemeral response-visibility scope.
    send_to: list[str] | None = allowed_user_ids

    # If configured to respond to team members only, then cannot be used with a /LumenBot command
    ...
    if send_to and is_slash_command:
        if sender_id:
            respond_in_thread_or_channel(
                ...
                text="The LumenBot slash command is not enabled for this channel",
                thread_ts=None,
            )
        ...
        if message_info.email:
                ...
                try:
                    ...
                    return _deny(
                        ...
                        (
                            ...
                            "invited. Please ask your Lumen administrator for "
                            "an invite."
                            if e.error_code == LumenErrorCode.UNAUTHORIZED
                            else "We weren't able to respond because your email "
                            "address is not allowed in this workspace. Please "
                            "contact your Lumen administrator."
                        ),
                        ephemeral=True,
                    )
            ...
            def _slack_seat_enforcer(session: Session, seats_needed: int) -> None:
                ...
                if result is not None and not result.available:
                    raise LumenError(
                        LumenErrorCode.SEAT_LIMIT_EXCEEDED, result.error_message
                    )
            ...
            try:
                ...
                if e.error_code != LumenErrorCode.SEAT_LIMIT_EXCEEDED:
                    raise
                return _deny(

...
from lumen.chat.process_message import remove_answer_citations
from lumen.configs.constants import MessageType, SearchFeedbackType
from lumen.configs.lumenbot_configs import LUMEN_BOT_FOLLOWUP_EMOJI
...
from lumen.db.chat import get_chat_message, translate_db_message_to_chat_message_detail
from lumen.db.engine.sql_engine import get_session_with_current_tenant
from lumen.db.feedback import create_chat_message_feedback, create_doc_retrieval_feedback
from lumen.db.users import get_user_by_email
from lumen.lumenbot.slack.blocks import (
    build_follow_up_resolved_blocks,
    build_slack_response_blocks,
    get_document_feedback_blocks,
)
from lumen.lumenbot.slack.config import get_slack_channel_config_for_bot_and_channel
...
from lumen.server.query_and_chat.models import ChatMessageDetail
from lumen.server.query_and_chat.streaming_models import CitationInfo
...
def handle_publish_ephemeral_message_button(
    ...
    # Obtain lumen_user and chat_message information
    if not chat_message_id:
        raise ValueError("Missing chat_message_id in the payload")

    with get_session_with_current_tenant() as db_session:
        lumen_user = get_user_by_email(user_email, db_session)
        if not lumen_user:
            raise ValueError("Cannot determine lumen_user_id from email in payload")
        try:
            chat_message = get_chat_message(chat_message_id, lumen_user.id, db_session)
        except ValueError:
        ...
        lumen_bot_answer = ChatBasicResponse(
...
def handle_followup_button(
    ...
    update_emote_react(
        emoji=LUMEN_BOT_FOLLOWUP_EMOJI,
        channel=channel_id,

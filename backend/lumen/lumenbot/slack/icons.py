...
# Served by this deployment's own web server (web/public/slackbot-source-icons),
# so Slack renders source logos without calling out to a third-party host.
# Slack fetches the URL itself, so WEB_DOMAIN must be reachable from the internet.
_PUBLIC_SOURCE_IMAGE_BASE_URL = f"{WEB_DOMAIN.rstrip('/')}/slackbot-source-icons"

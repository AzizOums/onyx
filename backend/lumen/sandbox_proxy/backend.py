...
from lumen.sandbox_proxy.ca import CAStore
...
def build_ip_lookup() -> SandboxIPLookup:
    if SANDBOX_BACKEND is SandboxBackend.KUBERNETES:
        from lumen.sandbox_proxy.identity_k8s import K8sInformerLookup

        return K8sInformerLookup()
    if SANDBOX_BACKEND is SandboxBackend.DOCKER:
        from lumen.sandbox_proxy.identity_docker import DockerEventsLookup

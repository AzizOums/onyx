...
from lumen.sandbox_proxy.identity import SandboxIdentity, SandboxIPLookup
from lumen.server.features.build.configs import SANDBOX_NAMESPACE
from lumen.server.features.build.sandbox.kubernetes.k8s_client import build_core_v1_api
...
_SANDBOX_POD_SELECTOR = ",".join(
    [
        ...
        f"{LABEL_K8S_MANAGED_BY}={LABEL_K8S_MANAGED_BY_LUMEN}",
    ]
)
...
def _identity_from_pod(pod: client.V1Pod) -> SandboxIdentity | None:
    ...
    if labels.get(LABEL_K8S_MANAGED_BY) != LABEL_K8S_MANAGED_BY_LUMEN:
        return None

...
from lumen.sandbox_proxy.logging_utils import short_log_id
from lumen.sandbox_proxy.mcp_jsonrpc import McpRpcKind, classify_mcp_request
from lumen.sandbox_proxy.resolvers.mcp_matching import (
    ...
    def resolve(self, request: http.Request, ctx: InjectionContext) -> dict[str, str]:
        ...
        try:
            ...
            raise CredentialUnavailableError(
                str(e),
                sandbox_detail=(
                    "Multiple MCP servers in Lumen are configured with the same "
        ...
        if target is None:
            ...
            raise CredentialUnavailableError(
                ...
                sandbox_detail=(
                    "This host belongs to an MCP server configured in Lumen, but "
...
def _fix_instruction(admin_managed: bool, *, reconnect: bool) -> str:
    ...
    if admin_managed:
        return f"Ask a workspace admin to {verb} it on the MCP actions page in Lumen."

"""Lumen-API PAT credential resolver.

Claims requests bound for the Lumen API host (the host of ``LUMEN_SERVER_URL``)
...
from lumen.auth.constants import (
...
from lumen.sandbox_proxy.logging_utils import full_log_id, short_log_id
from lumen.server.features.build.configs import LUMEN_SERVER_URL
from lumen.server.features.build.db.sandbox import get_sandbox_by_id
from lumen.utils.logger import setup_logger

logger = setup_logger()


class LumenPatResolver(CredentialResolver):
    """Injects the sandbox's Lumen API PAT on requests to the configured API host."""

    def __init__(self) -> None:
        parsed = urlparse(LUMEN_SERVER_URL) if LUMEN_SERVER_URL else None
    ...
    def resolve(self, request: http.Request, ctx: InjectionContext) -> dict[str, str]:
        ...
        logger.debug(
            "lumen_pat_resolver.resolved sandbox=%s host=%s",

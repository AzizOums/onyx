...
from lumen.configs.constants import DocumentSource, LumenCallTypes
from lumen.configs.kg_configs import KG_METADATA_TRACKING_THRESHOLD
from lumen.db.engine.sql_engine import get_session_with_current_tenant
from lumen.db.entities import get_kg_entity_by_document
from lumen.db.entity_type import get_entity_types
from lumen.db.kg_config import KGConfigSettings
from lumen.db.models import Document, KGEntityType, KGRelationshipType
from lumen.db.tag import get_structured_tags_for_document
from lumen.kg.models import (
...
from lumen.kg.utils.formatting_utils import (
    extract_email,
    get_entity_type,
    kg_email_processing,
    make_entity_id,
...
from lumen.kg.vespa.vespa_interactions import get_document_vespa_contents
from lumen.llm.factory import get_default_llm
from lumen.llm.models import UserMessage
from lumen.llm.utils import llm_response_to_string
from lumen.prompts.kg_prompts import (
...
def kg_implied_extraction(
    ...
    document_is_from_call = document_entity_type.lower() in (
        call_type.value.lower() for call_type in LumenCallTypes
    )
...
def kg_classify_document(
    ...
    if entity_type not in (call_type.value for call_type in LumenCallTypes):
        return None
...
def kg_deep_extract_chunks(
    ...
    entity_type = get_entity_type(document_entity)
    is_call = entity_type in (call_type.value for call_type in LumenCallTypes)

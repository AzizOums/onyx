...
from lumen.skills.built_in import BUILT_IN_SKILLS
from lumen.skills.metadata import (
    parse_skill_document,
    parse_skill_md_frontmatter,
    serialize_skill_md,
    split_skill_md,
)
from lumen.skills.models import CustomSkillBundleContents, SkillBundleFile
...
def read_bundle_file(bundle_file: IO[bytes]) -> bytes:
    ...
    if len(data) > DEFAULT_TOTAL_MAX_BYTES:
        raise LumenError(
            LumenErrorCode.PAYLOAD_TOO_LARGE,
...
def build_skill_md(
    *,
    ...
    if not name:
        raise LumenError(LumenErrorCode.INVALID_INPUT, "Skill name cannot be empty.")
    if not description:
        raise LumenError(
...
def normalize_custom_bundle(
    ...
    *,
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.INVALID_INPUT, "bundle is not a valid zip")
    ...
    with ExitStack() as archives:
        ...
        if upload_bytes is not None:
            ...
            try:
                ...
                raise LumenError(
                    LumenErrorCode.INVALID_INPUT, "upload is not a valid zip"
                ) from exc
            ...
            if not upload_file_paths:
                raise LumenError(LumenErrorCode.INVALID_INPUT, "upload contains no files")
            if len(upload_file_paths) != len(set(upload_file_paths)):
                raise LumenError(
                    LumenErrorCode.INVALID_INPUT, "upload contains duplicate paths"
                )
            ...
            if normalized_remove_path == SKILL_MD_NAME:
                raise LumenError(
                    LumenErrorCode.INVALID_INPUT, "SKILL.md cannot be removed"
                )
        ...
        if remove_path is not None and not any(
            ...
            raise LumenError(LumenErrorCode.NOT_FOUND, "Skill file not found")

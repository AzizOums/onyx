...
from lumen.configs.constants import LUMEN_METADATA_FILENAME
from lumen.configs.llm_configs import get_image_extraction_and_analysis_enabled
from lumen.file_processing.file_types import (
    PRESENTATION_MIME_TYPE,
    WORD_PROCESSING_MIME_TYPE,
    LumenFileExtensions,
    LumenMimeTypes,
)
from lumen.file_processing.html_utils import parse_html_page_basic
from lumen.file_processing.pdf_image_utils import iter_pdf_extracted_images
from lumen.file_processing.unstructured import (
    get_unstructured_api_key,
    unstructured_to_text,
)
from lumen.utils.logger import setup_logger
from lumen.utils.process_isolation import IsolatedProcessError, run_in_isolated_process
...
def load_files_from_zip(
    ...
    with zipfile.ZipFile(zip_file_io, "r") as zip_file:
        for file_info in zip_file.infolist():
            ...
            ) or file_info.filename == LUMEN_METADATA_FILENAME:
                continue
...
def _extract_lumen_metadata(line: str) -> dict | None:
    """
    Example: first line has:
        <!-- LUMEN_METADATA={"title": "..."} -->
      or
        #LUMEN_METADATA={"title":"..."}
    """
    html_comment_pattern = r"<!--\s*LUMEN_METADATA=\{(.*?)\}\s*-->"
    hashtag_pattern = r"#LUMEN_METADATA=\{(.*?)\}"
...
def read_text_file(
    ...
    encoding: str = "utf-8",
    errors: str = "replace",
    ignore_lumen_metadata: bool = True,
) -> tuple[str, dict]:
    """
    For plain text files. Optionally extracts Lumen metadata from the first line.
    """
    metadata = {}
    file_content_raw = ""
    for ind, line in enumerate(file):
        ...
        if ind == 0 and not ignore_lumen_metadata:
            potential_meta = _extract_lumen_metadata(line)
...
def read_pdf_file(
    ...
    try:
        ...
        if pdf_reader.is_encrypted:
            ...
            # See https://github.com/lumen-dot-app/lumen/issues/9754
...
def read_docx_file(
    ...
    try:
        ...
        text_content_raw, _ = read_text_file(
            file, encoding=encoding, ignore_lumen_metadata=False
        )
...
def extract_file_text(
    ...
    try:
        ...
        if extension in LumenFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
...
def _extract_text_and_images(
    ...
    if content_type in LumenMimeTypes.TEXT_MIME_TYPES:
    ...
    try:
        ...
        if extension in LumenFileExtensions.PLAIN_TEXT_EXTENSIONS:
            return extract_result_from_text_file(file)

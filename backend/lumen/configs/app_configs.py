...
from lumen.auth.schemas import AuthBackend
from lumen.cache.interface import CacheBackendType
from lumen.configs.constants import QueryHistoryType
...
from lumen.prompts.image_analysis import (
...
# This override allows self-hosting the MCP server with Lumen Cloud backend.
...
# comma-separated document set NAMES. When set, the Lumen Search UI is restricted to those sets
...
# 3. no queries
LUMEN_QUERY_HISTORY_TYPE = QueryHistoryType(
    (os.environ.get("LUMEN_QUERY_HISTORY_TYPE") or QueryHistoryType.NORMAL.value).lower()
)

# Visibility-only: hides the Query History page from the admin sidebar; the
# history APIs + recording stay on (unlike LUMEN_QUERY_HISTORY_TYPE=disabled).
...
# NOTE: if you are having problems accessing the Lumen web UI locally (especially
...
# restrict access to Lumen to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict Lumen
...
SAML_CONF_DIR = os.environ.get("SAML_CONF_DIR") or "/app/lumen/configs/saml_config"
...
_DEFAULT_MOBILE_REDIRECT_URIS = frozenset({"lumen://auth/callback"})
...
# If set, Lumen will listen to the `expires_at` returned by the identity
...
# of OpenSearch running for the relevant Lumen instance.
# NOTE: Now enabled on by default, unless the env indicates otherwise.
ENABLE_OPENSEARCH_INDEXING_FOR_LUMEN = (
    os.environ.get("ENABLE_OPENSEARCH_INDEXING_FOR_LUMEN", "true").lower() == "true"
)
...
ENABLE_OPENSEARCH_RETRIEVAL_FOR_LUMEN = (
    ENABLE_OPENSEARCH_INDEXING_FOR_LUMEN
    and os.environ.get("ENABLE_OPENSEARCH_RETRIEVAL_FOR_LUMEN", "").lower() == "true"
)
DISABLE_OPENSEARCH_MIGRATION_TASK = (
    os.environ.get("DISABLE_OPENSEARCH_MIGRATION_TASK", "").lower() == "true"
)
LUMEN_DISABLE_VESPA = os.environ.get("LUMEN_DISABLE_VESPA", "true").lower() == "true"
...
LUMEN_SEARCH_UI_USES_OPENSEARCH_KEYWORD_SEARCH = (
    os.environ.get("LUMEN_SEARCH_UI_USES_OPENSEARCH_KEYWORD_SEARCH", "").lower()
    == "true"
)
...
# Lumen addresses a tenant by schema (`schema_translate_map`). These settings let
# tenants additionally be spread across separate physical databases ("shards").
#
# LUMEN_DB_SHARDS is a JSON object of shard name -> connection overrides, e.g.
#   {"shard-b": {"host": "other.rds.amazonaws.com", "db": "danswer"}}
# Keys not supplied for a shard fall back to the POSTGRES_* values above.
#
# When unset, exactly one shard exists (named by LUMEN_DB_DEFAULT_SHARD, built
# from POSTGRES_*), which is the single-database behavior Lumen has always had.
LUMEN_DB_SHARDS_JSON = os.environ.get("LUMEN_DB_SHARDS", "").strip()
# Shard that hosts tenants with no explicit mapping.
LUMEN_DB_DEFAULT_SHARD = os.environ.get("LUMEN_DB_DEFAULT_SHARD") or "default"
...
LUMEN_DB_CATALOG_SHARD = os.environ.get("LUMEN_DB_CATALOG_SHARD") or LUMEN_DB_DEFAULT_SHARD
...
LUMEN_DB_NEW_TENANT_SHARD = (
    os.environ.get("LUMEN_DB_NEW_TENANT_SHARD") or LUMEN_DB_DEFAULT_SHARD
)
...
# filling up.
LUMEN_DB_SHARD_POOL_SIZE = (
    int(os.environ["LUMEN_DB_SHARD_POOL_SIZE"])
    if os.environ.get("LUMEN_DB_SHARD_POOL_SIZE")
    else None
)
LUMEN_DB_SHARD_POOL_OVERFLOW = (
    int(os.environ["LUMEN_DB_SHARD_POOL_OVERFLOW"])
    if os.environ.get("LUMEN_DB_SHARD_POOL_OVERFLOW")
    else None
)
...
if LUMEN_DB_SHARD_POOL_SIZE is not None and LUMEN_DB_SHARD_POOL_SIZE < 1:
    ...
    raise ValueError(
        f"LUMEN_DB_SHARD_POOL_SIZE must be at least 1, got {LUMEN_DB_SHARD_POOL_SIZE}"
    )
if LUMEN_DB_SHARD_POOL_OVERFLOW is not None and LUMEN_DB_SHARD_POOL_OVERFLOW < 0:
    # 0 is meaningful (no overflow past pool_size); negative means unlimited.
    raise ValueError(
        f"LUMEN_DB_SHARD_POOL_OVERFLOW must be zero or greater, got "
        f"{LUMEN_DB_SHARD_POOL_OVERFLOW}"
    )
...
LUMEN_DB_SHARD_OVERRIDES_JSON = os.environ.get("LUMEN_DB_SHARD_OVERRIDES", "").strip()
...
LUMEN_DB_SHARD_MAP_TTL_SECONDS = int(
    os.environ.get("LUMEN_DB_SHARD_MAP_TTL_SECONDS") or 60
)
...
LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS = float(
    os.environ.get("LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS") or 5
)

if LUMEN_DB_SHARD_MAP_TTL_SECONDS <= 0:
    raise ValueError(
        f"LUMEN_DB_SHARD_MAP_TTL_SECONDS must be positive, got "
        f"{LUMEN_DB_SHARD_MAP_TTL_SECONDS}"
    )
if (
    not math.isfinite(LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS)
    or LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS <= 0
):
    # A non-finite interval would stop the poller permanently, silently reducing
    # flip propagation to the TTL path.
    raise ValueError(
        f"LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS must be a positive finite number, "
        f"got {LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS}"
    )
...
# View the list here:
# https://github.com/lumen-dot-app/lumen/blob/main/backend/lumen/connectors/factory.py
...
# When the LumenWebCrawler (open_url tool) hits a 403 / Cloudflare challenge,
...
CONFLUENCE_USE_LUMEN_USERS_FOR_GROUP_SYNC = (
    os.environ.get("CONFLUENCE_USE_LUMEN_USERS_FOR_GROUP_SYNC", "").lower() == "true"
)
...
# multi-tenant deployments. See lumen/indexing/document_push.py.
...
# lumen/file_processing/pdf_image_utils.py.
#
...
# backend/lumen/document_index/vespa/app_config/schemas/danswer_chunk.sd.jinja.
...
# Logs Lumen only model interactions like prompts, responses, messages etc.
LOG_LUMEN_MODEL_INTERACTIONS = (
    os.environ.get("LOG_LUMEN_MODEL_INTERACTIONS", "").lower() == "true"
)
...
# Off by default: this build ships no telemetry endpoint of its own. Set
# DISABLE_TELEMETRY=false and TELEMETRY_ENDPOINT to send anonymous telemetry.
...
BRAINTRUST_PROJECT = os.environ.get("BRAINTRUST_PROJECT", "Lumen")
...
SCHEDULED_EVAL_PERMISSIONS_EMAIL = os.environ.get(
    "SCHEDULED_EVAL_PERMISSIONS_EMAIL", "roshan@lumen.app"
)
...
API_SERVER_THREADPOOL_SIZE = int(os.environ.get("LUMEN_API_THREADPOOL_SIZE") or "0")
...
# Empty by default: this build publishes no hosted model catalog. Point it at
# your own recommended-models.json to turn the periodic sync back on.
AUTO_LLM_CONFIG_URL = os.environ.get("AUTO_LLM_CONFIG_URL", "")
...
# founders@lumen.app OR message Chris Weaver or Yuhong Sun in the Lumen
...
S3_FILE_STORE_BUCKET_NAME = (
    os.environ.get("S3_FILE_STORE_BUCKET_NAME") or "lumen-file-store-bucket"
)
S3_FILE_STORE_PREFIX = os.environ.get("S3_FILE_STORE_PREFIX") or "lumen-files"
...
GCS_FILE_STORE_PREFIX = os.environ.get("GCS_FILE_STORE_PREFIX") or "lumen-files"
...
AZURE_FILE_STORE_PREFIX = os.environ.get("AZURE_FILE_STORE_PREFIX") or "lumen-files"
...
# These are Lumen-managed API keys provided to tenants by default
#####
...
# Auto-create Lumen-managed built-in external app rows (disabled, Lumen-owned creds)
...
# Lumen-owned OAuth credentials for built-in external apps (managed cloud)
...
STRIPE_PUBLISHABLE_KEY_URL = (
    "https://lumen-stripe-public.s3.amazonaws.com/publishable-key.txt"
)

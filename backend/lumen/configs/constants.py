...
LUMEN_DEFAULT_APPLICATION_NAME = "Lumen"
LUMEN_DISCORD_URL = "https://discord.gg/4NA5SbzrWb"
LUMEN_UTM_SOURCE = "lumen_app"
SLACK_USER_TOKEN_PREFIX = "xoxp-"
LUMEN_EMAILABLE_LOGO_MAX_DIM = 512
...
ANONYMOUS_USER_COOKIE_NAME = "lumen_anonymous_user"
...
LUMEN_METADATA_FILENAME = ".lumen_metadata.json"
...
DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN = "lumenapikey.ai"
...
KV_SETTINGS_KEY = "lumen_settings"
KV_CUSTOMER_UUID_KEY = "customer_uuid"
KV_INSTANCE_DOMAIN_KEY = "instance_domain"
KV_ENTERPRISE_SETTINGS_KEY = "lumen_enterprise_settings"
...
class DocumentSource(str, Enum):
    # Special case, document passed in via Lumen APIs without specifying a source type
...
class MessageType(str, Enum):
    ...
    USER_REMINDER = "user_reminder"  # Custom Lumen message type which is translated into a USER message when passed to the LLM
...
class MilestoneRecordType(str, Enum):
    ...
    CREATED_LUMEN_BOT = "created_lumen_bot"
    REQUESTED_CONNECTOR = "requested_connector"


class LumenCeleryQueues:
...
class LumenRedisConstants:
    ACTIVE_FENCES = "active_fences"


class LumenCeleryPriority(int, Enum):
    HIGHEST = 0
    HIGH = auto()
    MEDIUM = auto()
    LOW = auto()
...
LUMEN_CLOUD_CELERY_TASK_PREFIX = "cloud"

# the tenant id we use for system level redis operations
LUMEN_CLOUD_TENANT_ID = "cloud"

# the redis namespace for runtime variables
LUMEN_CLOUD_REDIS_RUNTIME = "runtime"
CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT = 600


class LumenCeleryTask:
    DEFAULT = "celery"

    CLOUD_BEAT_TASK_GENERATOR = f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_generate_beat_tasks"
    CLOUD_MONITOR_ALEMBIC = f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_monitor_alembic"
    CLOUD_MONITOR_CELERY_QUEUES = (
        f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_queues"
    )
    CLOUD_CHECK_AVAILABLE_TENANTS = (
        f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_check_available_tenants"
    )
    CLOUD_MONITOR_CELERY_PIDBOX = (
        f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_pidbox"
    )
...
LUMEN_CELERY_BEAT_HEARTBEAT_KEY = "lumen:celery:beat:heartbeat"

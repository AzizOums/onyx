...
from lumen.configs.constants import PUBLIC_DOC_PAT, DocumentSource, FileOrigin
from lumen.db.document import get_access_info_for_document, get_access_info_for_documents
...
from lumen.db.user_file import fetch_user_files_with_access_relationships
...
def get_access_for_document(
    ...
    versioned_get_access_for_document_fn = fetch_versioned_implementation(
        "lumen.access.access", "_get_access_for_document"
    )
...
def get_access_for_documents(
    ...
    versioned_get_access_for_documents_fn = fetch_versioned_implementation(
        "lumen.access.access", "_get_access_for_documents"
    )
...
def get_acl_for_user(user: User, db_session: Session | None = None) -> set[str]:
    versioned_acl_for_user_fn = fetch_versioned_implementation(
        "lumen.access.access", "_get_acl_for_user"
    )
...
def get_access_for_user_files(
    ...
    versioned_fn = fetch_versioned_implementation(
        "lumen.access.access", "get_access_for_user_files_impl"
    )
...
def build_access_for_user_files(
    ...
    versioned_fn = fetch_versioned_implementation(
        "lumen.access.access", "build_access_for_user_files_impl"
    )
...
def _documents_from_file_connector_config(
    ...
    `lumen/connectors/file/connector.py`). The `@>` lookup also can't use

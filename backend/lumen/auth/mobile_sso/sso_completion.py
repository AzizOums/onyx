...
from lumen.auth.mobile_sso.tokens import issue_session_credential
from lumen.configs.app_configs import MOBILE_ALLOWED_REDIRECT_URIS
...
def apply_mobile_state(
    ...
    ``LumenError(VALIDATION_ERROR)`` on a missing challenge / disallowed URI.
    """
...
async def complete_mobile_sso(
    ...
    Raises ``LumenError(VALIDATION_ERROR)`` otherwise. Notably mints the token but

...
from lumen.configs.constants import LUMEN_DEFAULT_APPLICATION_NAME, LUMEN_DISCORD_URL
...
from lumen.utils.url import add_url_params
...
def build_html_email(
    ...
    community_link_fragment = ""
    if application_name == LUMEN_DEFAULT_APPLICATION_NAME:
        community_link_fragment = f'<br>Have questions? Join our Discord community <a href="{LUMEN_DISCORD_URL}">here</a>.'
...
def send_email_with_sendgrid(
    ...
    from_email = Email(mail_from) if mail_from else Email("noreply@lumen.app")
...
def send_email_with_smtplib(
    ...
    msg["Message-ID"] = make_msgid(domain="lumen.app")
...
def send_subscription_cancellation_email(user_email: str) -> None:
    ...
    try:
        load_runtime_settings_fn = fetch_versioned_implementation(
            "lumen.server.enterprise_settings.store", "load_runtime_settings"
        )
    ...
    cta_link = "https://www.lumen.app/pricing"
    ...
    text_content = (
        ...
        "If you change your mind, visit https://www.lumen.app/pricing"
    )
    send_email(
        user_email,
        subject,

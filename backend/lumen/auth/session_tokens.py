...
stashed in a request-scoped ContextVar and turned into a reasoned ``LumenError``
...
from lumen.auth.constants import API_KEY_PREFIX, DEPRECATED_API_KEY_PREFIX, PAT_PREFIX
...
_REASON_TO_ERROR_CODE = {
    SessionRejectionReason.EXPIRED: LumenErrorCode.SESSION_EXPIRED,
    SessionRejectionReason.TERMINATED: LumenErrorCode.SESSION_TERMINATED,
    SessionRejectionReason.NOT_FOUND: LumenErrorCode.SESSION_UNRECOGNIZED,
    SessionRejectionReason.MALFORMED: LumenErrorCode.SESSION_UNRECOGNIZED,
}
...
def build_session_rejection_error() -> LumenError | None:
    """
    Turns the request's recorded rejection (if any) into a reasoned
    ``LumenError``, logging the diagnostic WARNING.
    """

...
from lumen.natural_language_processing.exceptions import (
    CohereBillingLimitError,
    ModelServerRateLimitError,
)
from lumen.natural_language_processing.utils import get_tokenizer, tokenizer_trim_content
...
from lumen.utils.search_nlp_models_utils import pass_aws_key
...
WARM_UP_STRINGS = [
    ...
    "https://docs.lumen.app/deployment/getting_started/quickstart",
]
...
class CloudEmbedding:
    ...
    async def _embed_cohere(
        ...
        for text_batch in batch_list(texts, _COHERE_MAX_INPUT_LEN):
            # Does not use the same tokenizer as the Lumen API server but it's approximately the same
...
class EmbeddingModel:
    ...
    def _make_model_server_request(
        ...
        def _make_request() -> Response:
            headers = {}
            if tenant_id:
                headers["X-Lumen-Tenant-ID"] = tenant_id

            if request_id:
                headers["X-Lumen-Request-ID"] = request_id

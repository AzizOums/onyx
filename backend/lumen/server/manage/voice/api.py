...
def _fetch_provider_for_stored_secret(
    ...
    if provider is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Voice provider not found.")
...
async def upsert_voice_provider_endpoint(
    ...
    try:
        ...
        db_session.rollback()
        raise LumenError(LumenErrorCode.VALIDATION_ERROR, str(e)) from e
...
async def get_provider_voices(
    ...
    if not provider_db.api_key:
        raise LumenError(
            LumenErrorCode.VALIDATION_ERROR, "Provider has no API key configured."
        )

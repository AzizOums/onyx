...
from lumen.db.enums import Permission, SSOProviderType
...
from lumen.server.manage.get_state import invalidate_sso_provider_options_cache
from lumen.server.manage.sso.models import (
...
from lumen.utils.encryption import reject_masked_credentials, restore_masked_credentials
...
def _sync_login_domain_routing(db_session: Session) -> None:
    ...
    try:
        ...
        fetch_ee_implementation_or_noop(
            "lumen.db.tenant_sso_domain", "claim_email_domains", None
...
def _list_login_domains(tenant_id: str) -> list[Any]:
    return fetch_ee_implementation_or_noop(
        "lumen.db.tenant_sso_domain", "list_login_domains", lambda _tenant_id: []
    )(tenant_id)
...
def _build_statuses(
    ...
    verification_record = fetch_ee_implementation_or_noop(
        "lumen.auth.sso_domain_verification",
        "verification_record",
...
async def verify_sso_domain_via_dns_endpoint(
    ...
    if not MULTI_TENANT:
        raise LumenError(LumenErrorCode.SINGLE_TENANT_ONLY)

...
past the limit. When Redis is unavailable (e.g. Lumen Lite deployments
...
def _run_atomic(redis_client: TenantRedisClient, buckets: list[_Bucket]) -> None:
    """Run the check+increment Lua script. Raise LumenError on rejection.
    ...
    invite flow usable on Lumen Lite deployments (Redis is opt-in there)
...
def enforce_invite_rate_limit(
    ...
    Raises LumenError(RATE_LIMITED) without recording if any tier would be

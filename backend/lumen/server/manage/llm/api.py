...
from lumen.auth.permissions import has_global_permission, require_permission
from lumen.auth.users import current_chat_accessible_user
from lumen.db.engine.sql_engine import get_session
from lumen.db.enums import LLMModelFlowType, Permission
...
from lumen.db.persona import user_can_access_persona
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError
from lumen.llm.api_surfaces import SURFACE_SELECTION_CONFIG_KEYS
from lumen.llm.constants import (
...
from lumen.llm.well_known_providers.auto_update_service import (
...
from lumen.server.manage.llm.provider_cache import (
    cache_provider_listing,
    get_cached_provider_listing,
    invalidate_provider_listing_cache,
)
from lumen.server.manage.llm.utils import (
    ModelMetadata,
...
from lumen.utils.encryption import mask_string as mask_with_ellipsis
...
def _validate_llm_provider_change(
    ...
    Raises:
        LumenError: If api_base or custom_config changed without changing API key
    """
...
def fetch_llm_provider_options(
    ...
    raise LumenError(LumenErrorCode.NOT_FOUND, f"Provider {provider_name} not found")
...
def test_llm_configuration(
    ...
    if error_msg:
        raise LumenError(LumenErrorCode.VALIDATION_ERROR, error_msg)
...
def test_default_provider(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.VALIDATION_ERROR, "No LLM Provider setup")

    error = test_llm(llm)
    if error:
        raise LumenError(LumenErrorCode.VALIDATION_ERROR, str(error))
...
def get_llm_provider(
    ...
    if llm_provider_model is None:
        raise LumenError(
            LumenErrorCode.NOT_FOUND, f"LLM provider {provider_id} does not exist"
        )
...
def put_llm_provider(
    ...
    if existing_provider and is_creation:
        raise LumenError(
            LumenErrorCode.DUPLICATE_RESOURCE,
    ...
    try:
        ...
        if transitioning_to_auto_mode:
            from lumen.db.llm import sync_auto_mode_models
        ...
        raise LumenError(LumenErrorCode.VALIDATION_ERROR, str(e))
    finally:
...
def delete_llm_provider(
    ...
    if not force:
        ...
        if model and model.llm_provider_id == provider_id:
            raise LumenError(
                LumenErrorCode.RESOURCE_IN_USE,
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.NOT_FOUND, str(e))
...
def list_llm_providers_for_persona(
    ...
    persona = fetch_persona_with_groups(db_session, persona_id)
    if not persona:
        raise LumenError(LumenErrorCode.PERSONA_NOT_FOUND, "Persona not found")
...
def get_bedrock_available_models(
    ...
    try:
        ...
        try:
            ...
            raise LumenError(
                LumenErrorCode.CREDENTIAL_INVALID,
...
def _get_ollama_available_model_names(api_base: str) -> set[str]:
    ...
    URL, so a server Lumen can't route to is a client misconfiguration.
    """
    ...
    try:
        ...
        raise LumenError(
            ...
            f"is correct and reachable from Lumen ({type(e).__name__}).",
        )
    except Exception as e:
        raise LumenError(
            LumenErrorCode.BAD_GATEWAY,
...
def _get_openai_compatible_models_response(
    ...
    try:
        ...
        raise LumenError(
            ...
            f"correct and reachable from Lumen ({type(e).__name__}).",
        )
    except ValueError as e:
        logger.warning(
...
def list_modelsdev_providers(
    ...
    if not modelsdev_enabled():
        raise LumenError(LumenErrorCode.NOT_FOUND, "models.dev integration is disabled")
...
def list_modelsdev_provider_models(
    ...
    if provider is None:
        raise LumenError(
            LumenErrorCode.NOT_FOUND, f"Unknown models.dev provider: {provider_id}"
        )
    models = []

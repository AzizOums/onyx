...
from lumen.configs.app_configs import DISCORD_BOT_TOKEN
...
from lumen.server.manage.discord_bot.models import (
...
def delete_bot_config_endpoint(
    ...
    if not deleted:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Bot config not found")
...
def delete_service_api_key_endpoint(
    ...
    Discord bot to authenticate with the Lumen API. The key is also automatically
    deleted when:
    ...
    if not deleted:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Service API key not found")
    db_session.commit()
    return {"deleted": True}
...
def get_guild_config(
    ...
    if not config:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Guild config not found")
...
def list_channel_configs(
    ...
    if not guild_config.guild_id:
        raise LumenError(LumenErrorCode.INVALID_INPUT, "Guild not yet registered")
...
def update_channel_request(
    ...
    if not config:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Channel config not found")

...
from lumen.auth.anonymous_user import fetch_anonymous_user_info
from lumen.auth.email_utils import send_user_email_invite
from lumen.auth.invited_users import (
    get_invited_users,
    remove_user_from_invited_users,
    write_invited_users,
)
from lumen.auth.permissions import get_effective_permissions, require_permission
from lumen.auth.scoped_permissions import get_scoped_groups
...
from lumen.db.api_key import is_api_key_email_address
from lumen.db.auth import get_live_users_count
from lumen.db.engine.sql_engine import get_session, get_session_with_shared_schema
from lumen.db.enums import AccountType, Permission, UserFileStatus
from lumen.db.models import User, UserFile
from lumen.db.pinned_personas import set_pinned_personas
from lumen.db.tenant_invite_counter import release_trial_invites, reserve_trial_invites
from lumen.db.user_preferences import (
    activate_user,
    deactivate_user,
...
from lumen.llm.models import ReasoningEffort, parse_user_selectable_reasoning_effort
from lumen.redis.redis_pool import get_raw_redis_client, get_redis_client
from lumen.server.documents.models import PaginatedReturn
from lumen.server.features.projects.models import UserFileSnapshot
from lumen.server.manage.invite_rate_limit import (
    enforce_invite_rate_limit,
    enforce_remove_invited_rate_limit,
)
from lumen.server.manage.models import (
    AllUsersResponse,
    AutoScrollRequest,
    BulkInviteResponse,
...
from lumen.server.models import (
    FullUserSnapshot,
    InvitedUserSnapshot,
    MinimalUserSnapshot,
    UserGroupInfo,
...
from lumen.server.usage_limits import is_tenant_on_trial_fn
...
from lumen.utils.csv_utils import sanitize_csv_cell
...
def set_user_admin_access_endpoint(
    ...
    if not target:
        raise LumenError(LumenErrorCode.NOT_FOUND, "User not found")
...
async def test_upsert_user(
    ...
    user = await fetch_ee_implementation_or_noop(
        "lumen.server.saml", "upsert_saml_user", None
    )(email=request.email)
    return (
...
def list_accepted_users(
    ...
    try:
        from lumen.db.models import ScimUserMapping
...
def bulk_invite_users(
        ...
        if num_new_invites > 0:
            with get_session_with_shared_schema() as shared_session:
                ...
                if new_total > NUM_FREE_TRIAL_USER_INVITES:
                    ...
                    raise LumenError(
                        LumenErrorCode.TRIAL_INVITE_LIMIT_EXCEEDED,
    ...
    if MULTI_TENANT:
        try:
            fetch_ee_implementation_or_noop(
                "lumen.db.user_tenant_mapping", "add_users_to_tenant", None
    ...
    if MULTI_TENANT and not DEV_MODE:
        ...
        try:
            ...
            fetch_ee_implementation_or_noop(
                "lumen.server.tenants.billing", "register_tenant_users", None
            ...
            try:
                ...
                fetch_ee_implementation_or_noop(
                    "lumen.db.user_tenant_mapping", "remove_users_from_tenant", None
                )(new_invited_emails, tenant_id)
            finally:
        ...
        if MULTI_TENANT:
            if team_name != tenant_id:
                user_count = fetch_ee_implementation_or_noop(
                    "lumen.db.user_tenant_mapping", "get_tenant_count", None
                )(team_name)
            ...
            tenant_invitation = fetch_ee_implementation_or_noop(
                "lumen.db.user_tenant_mapping", "get_tenant_invitation", None
            )(user.email)

    super_users_list = cast(
        list[str],
        fetch_versioned_implementation_with_fallback(
            "lumen.configs.app_configs",
            "SUPER_USERS",
            [],
        ),
    )
...
class ReasoningEffortDefaultRequest(BaseModel):
    ...
    def _validate_reasoning_effort(cls, value: Any) -> Any:
        ...
        try:
            ...
            raise LumenError(LumenErrorCode.BAD_REQUEST, str(e))

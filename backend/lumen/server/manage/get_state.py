...
from lumen.auth.users import anonymous_user_enabled, user_needs_to_be_verified
from lumen.configs.app_configs import OAUTH_ENABLED
from lumen.configs.constants import (
    DEV_VERSION_PATTERN,
    PUBLIC_API_TAGS,
    STABLE_VERSION_PATTERN,
)
from lumen.db.auth import get_user_count
from lumen.db.engine.sql_engine import get_session_with_shared_schema
from lumen.db.sso_provider import fetch_sso_providers, sso_authorize_path
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError, lumen_error_to_json_response
...
def get_versions() -> AllVersions:
    """
    Fetches the latest stable and beta versions of Lumen Docker images.
    ...
    """
    # Fetch the latest tags from DockerHub for each Lumen component
    dockerhub_repos = [
        "lumendotapp/lumen-model-server",
        "lumendotapp/lumen-backend",
        "lumendotapp/lumen-web-server",
    ]

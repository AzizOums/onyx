...
from lumen.configs.app_configs import GENERATIVE_MODEL_ACCESS_CHECK_FREQ
...
from lumen.llm.utils import test_llm
from lumen.server.documents.models import (
    ConnectorCredentialPairIdentifier,
    PaginatedReturn,
)
from lumen.server.manage.models import BoostDoc, BoostUpdateRequest, HiddenUpdateRequest
...
def validate_existing_genai_api_key(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.NOT_FOUND, "LLM not setup")

    error = test_llm(llm)
    if error:
        raise LumenError(LumenErrorCode.VALIDATION_ERROR, error)
...
def create_deletion_attempt_for_connector_id(
    ...
    if cc_pair is None:
        ...
        logger.error(error)
        raise LumenError(LumenErrorCode.CONNECTOR_NOT_FOUND, error)
    ...
    client_app.send_task(
        LumenCeleryTask.CHECK_FOR_CONNECTOR_DELETION,

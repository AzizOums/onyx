...
from lumen.background.celery.tasks.index_reclaim.tasks import enqueue_index_reclaim
from lumen.background.celery.tasks.port.tasks import (
    PortResumeResult,
...
from lumen.db.enums import IndexReclaimStatus, Permission, SwitchoverType
from lumen.db.index_attempt import create_synthetic_seed_attempt, expire_index_attempts
...
from lumen.db.models import IndexModelStatus, SearchSettings, User
...
from lumen.document_index.factory import (
...
from lumen.server.manage.embedding.models import SearchSettingsDeleteRequest
from lumen.server.manage.models import FullModelVersionResponse
from lumen.server.models import IdReturn
...
def set_new_search_settings(
    ...
    if MULTI_TENANT and search_settings_new.enable_contextual_rag:
        raise HTTPException(
            ...
            detail="Contextual RAG disabled in Lumen Cloud",
        )
...
def resume_paused_port(
    ...
    target = _active_port_settings(db_session)
    if target is None:
        raise LumenError(LumenErrorCode.CONFLICT, "No reindex port is currently active.")
...
def update_saved_search_settings(
    ...
    if MULTI_TENANT and search_settings.enable_contextual_rag:
        raise LumenError(
            LumenErrorCode.INVALID_INPUT,
            "Contextual RAG disabled in Lumen Cloud",
        )
...
def validate_contextual_rag_model(
    ...
    if model_configuration_id is None:
        ...
        return
    from lumen.db.models import ModelConfiguration

...
from lumen.configs import app_configs as _cfg
from lumen.configs.constants import KV_PASSWORD_AUTH_ENABLED_KEY, LumenRedisLocks
from lumen.db.engine.sql_engine import get_session_with_current_tenant
from lumen.db.enums import IncognitoRecordMode
from lumen.db.security_settings import load_overrides as _db_load_overrides
from lumen.db.security_settings import upsert_overrides as _db_upsert_overrides
from lumen.db.sso_provider import fetch_sso_providers
...
def security_settings_write_lock() -> Iterator[None]:
    ...
    state. Raises ``LumenError(CONFLICT)`` if a competing writer holds it past the wait
    window.
    """
    cache = get_cache_backend()
    lock = cache.lock(
        LumenRedisLocks.SECURITY_SETTINGS, timeout=_WRITE_LOCK_LEASE_SECONDS
    )
...
def apply_patch(
    ...
    Raises ``LumenError(CONFLICT)`` if a competing writer holds the lock past
    the wait window, and ``LumenError(INVALID_INPUT)`` if the merged effective

...
from lumen.cache.factory import get_shared_cache_backend
from lumen.configs.app_configs import INSTANCE_TYPE
from lumen.configs.constants import LumenRedisLocks
from lumen.db.release_notes import create_release_notifications_for_versions
from lumen.server.features.release_notes.constants import (
...
from lumen.server.features.release_notes.models import ReleaseNoteEntry
        ...
        if is_valid_version(version):
            all_entries.append(
                ReleaseNoteEntry(
                    version=version,
                    date=date,
                    title=f"Lumen {version} is available!",
                )
            )
...
def ensure_release_notes_fresh_and_notify(db_session: Session) -> None:
    ...
    cache = get_shared_cache_backend()
    lock = cache.lock(
        LumenRedisLocks.RELEASE_NOTES_FETCH_LOCK,
        timeout=90,
    )
    ...
    if not GITHUB_CHANGELOG_RAW_URL:
        logger.debug("No changelog source configured, skipping release notes fetch.")
        return

    try:
        logger.debug("Checking the configured source for release notes updates.")

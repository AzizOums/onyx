...
from lumen.auth.oauth_token_manager import ensure_offline_access_auth_params
from lumen.cache.interface import CacheLockAcquisitionError
from lumen.cache.locks import cache_shared_lock
...
from lumen.db.models import MCPConnectionConfig, MCPServer
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError
from lumen.oauth.authorization_attempt import generate_authorization_state
...
from lumen.server.features.mcp.ssrf import (
    mcp_ssrf_httpx_client_factory,
)
from lumen.utils.logger import setup_logger
from lumen.utils.threadpool_concurrency import run_async_sync_no_cancel
from lumen.utils.url import SSRFException
from shared_configs.contextvars import LUMEN_REQUEST_ID_CONTEXTVAR, get_current_tenant_id
...
class MCPReauthenticationRequired(OAuthFlowError):
    ...
    def __init__(self) -> None:
        super().__init__("Please reconnect to the server through Lumen.")
...
class LumenTokenStorage(TokenStorage):
    """
    ...
    def _ensure_connection_config(
        ...
        if config is None:
            raise LumenError(LumenErrorCode.NOT_FOUND, "Connection config not found")
        return config
            ...
            logger.info(
                ...
                extra={
                    **self.refresh_log_context,
                    "request_id": LUMEN_REQUEST_ID_CONTEXTVAR.get(),
...
class LumenOAuthClientProvider(OAuthClientProvider):
    ...
    async def complete_authorization_code_exchange(
        ...
        *,
        ...
        if isinstance(storage, LumenTokenStorage):
    ...
    async def _persist_refresh_tokens(self, tokens: OAuthToken) -> bool:
        storage = self.context.storage
        if not isinstance(storage, LumenTokenStorage):
            await storage.set_tokens(tokens)
            return True
    ...
    async def _handle_refresh_response(self, response: httpx.Response) -> bool:
        ...
        if response.status_code != 200:
            ...
            storage = self.context.storage
            if oauth_error == "invalid_grant" and isinstance(storage, LumenTokenStorage):
...
def make_oauth_provider(
    ...
    *,
    ...
    provider = LumenOAuthClientProvider(
        refresh_log_context=refresh_log_context,
        server_url=mcp_server.server_url,
        client_metadata=OAuthClientMetadata(
            client_name=f"Lumen - {mcp_server.name}",

...
from lumen.auth.oauth_token_manager import validate_oauth_endpoint_url
from lumen.auth.permission_projection import mcp_server_permissions, tool_permissions
from lumen.auth.permissions import (
    get_effective_permissions,
    has_permission,
    require_permission,
)
from lumen.auth.scoped_permissions import assert_within_scope, get_scoped_groups
...
from lumen.db.models import MCPConnectionConfig, Tool, User
from lumen.db.models import MCPServer as DbMCPServer
from lumen.db.tools import (
...
from lumen.server.features.mcp.client import (
    discover_mcp_tools,
    log_exception_group,
)
from lumen.server.features.mcp.client_metadata import (
...
from lumen.server.features.mcp.oauth_flow import (
...
from lumen.server.features.mcp.ssrf import validate_mcp_outbound_url
from lumen.server.features.tool.models import ToolSnapshot
from lumen.utils.encryption import (
    is_masked_credential,
    mask_string,
    reject_masked_credentials,
)
from lumen.utils.logger import setup_logger
from lumen.utils.url import BLOCKED_HOSTNAMES, SSRFException
...
def _validate_mcp_server_url(
    ...
    """Store-time SSRF guard for a curator-supplied URL; raises ``LumenError`` as
...
async def _connect_oauth(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.NOT_FOUND, "MCP server not found") from error
    ...
    if auth_template is not None and auth_template.required_fields:
        ...
        if missing_fields:
            raise LumenError(
                LumenErrorCode.MISSING_REQUIRED_FIELD,
...
async def process_oauth_callback(
    ...
    user_id = str(user.id)
    if not state:
        raise LumenError(LumenErrorCode.INVALID_INPUT, "Missing state parameter")
    ...
    if not code:
        raise LumenError(LumenErrorCode.INVALID_INPUT, "Missing code parameter")
    try:
...
def save_user_credentials(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.NOT_FOUND, "MCP server not found")
...
def get_mcp_server_tools_snapshots(
    ...
    if source == ToolSnapshotSource.MCP:
        ...
        try:
            ...
            if isinstance(e, (HTTPException, LumenError)):
...
def _list_mcp_tools_by_id(
    ...
    if not credentials.can_authenticate():
        raise LumenError(
            LumenErrorCode.UNAUTHENTICATED,
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.UNAUTHENTICATED, str(error)) from error
    logger.info(
...
def _apply_mcp_server_access(
    *,
    ...
    fetch_versioned_implementation("lumen.db.mcp", "make_mcp_server_private")(
        server_id=mcp_server.id,
...
def upsert_mcp_server(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.INVALID_INPUT, str(e)) from e
    except Exception as e:

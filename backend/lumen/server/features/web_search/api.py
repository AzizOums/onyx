...
from lumen.configs.constants import PUBLIC_API_TAGS
...
from lumen.server.manage.web_search.models import (
    WebContentProviderView,
    WebSearchProviderView,
)
from lumen.tools.models import LlmOpenUrlResult, LlmWebSearchResult
...
from lumen.tools.tool_implementations.open_url.utils import (
...
from lumen.tools.tool_implementations.web_search.providers import (
    build_content_provider_from_config,
    build_search_provider_from_config,
)
from lumen.tools.tool_implementations.web_search.utils import (
...
def _get_active_search_provider(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.INVALID_INPUT, str(exc)) from exc
...
def _get_active_content_provider(
    ...
    if provider_model is None:
        ...
        # NOTE: the LumenWebCrawler is not stored in the content provider table,
        # so we need to return it directly.

        return None, LumenWebCrawler(
...
def _open_urls(
    ...
    # SSRF protection is handled inside the content provider (LumenWebCrawler)
    ...
    provider_type = (
        ...
        if provider_view
        else WebContentProviderType.LUMEN_WEB_CRAWLER
    )

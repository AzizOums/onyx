...
from lumen.external_apps.models import ActionPolicyView
from lumen.server.features.build.connect_app import ConnectAppDecision
...
class UpdateExternalAppRequest(BaseModel):
    ...
    This is the single update path for all apps. For Lumen-managed built-ins
    (cloud) the gateway-config fields (``upstream_url_patterns``,
    ``auth_template``, ``organization_credentials``) are Lumen-owned and ignored —
...
class ExternalAppAdminResponse(BaseModel):
    ...
    # Lumen-managed built-in (cloud): creds/config Lumen-owned and blanked above;
    # admin may only set availability and policies. UI hides the rest.
    is_lumen_managed: bool = False

...
from lumen.db.models import ExternalApp, ExternalAppUserCredential, User
from lumen.db.skill import affected_user_ids_for_skill
from lumen.db.utils import UNSET, none_as_unset
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError
from lumen.external_apps.models import BuiltInExternalAppDescriptor
from lumen.external_apps.providers.base import OAuthExternalAppProvider
from lumen.external_apps.providers.registry import (
    action_policy_views,
    fetch_available_built_in_apps,
    get_lumen_managed_provider,
...
from lumen.server.features.build.db.build_session import get_build_session
...
from lumen.skills.push import push_skill_to_affected_sandboxes, push_skills_for_users
...
def _to_admin_response(
    ...
    *,
    ...
    managed = MULTI_TENANT and get_lumen_managed_provider(app.app_type) is not None
    return ExternalAppAdminResponse(
        id=app.id,
        name=app.name,
        app_type=app.app_type,
        # Managed built-ins: hide Lumen-owned config/creds. Else mask secrets — the
...
def create_built_in_external_app(
    ...
    Lumen-managed built-ins are Lumen-provisioned and can't be created here.
    """
    ...
    if MULTI_TENANT and get_lumen_managed_provider(request.app_type) is not None:
        raise LumenError(
            LumenErrorCode.INVALID_INPUT,
            "Built-in apps are provided by Lumen; use PATCH /admin/apps/{id} to "
            "set action policies.",
        )
...
def update_external_app_admin(
    ...
    untouched. For Lumen-managed built-ins (cloud) the gateway-config fields
    are Lumen-owned and ignored — only ``enabled`` and ``action_policies`` apply.
    """
    ...
    app = update_external_app(
        ...
        # Gateway config is Lumen-owned for managed built-ins; leave it untouched.
...
def create_custom_external_app(
    ...
    if not request.name.strip():
        raise LumenError(LumenErrorCode.INVALID_INPUT, "name is required.")
...
def delete_external_app_admin(
    ...
    app = _get_app_or_404(db_session, external_app_id)
    if MULTI_TENANT and get_lumen_managed_provider(app.app_type) is not None:
        raise LumenError(
            LumenErrorCode.INVALID_INPUT,
            "Built-in apps are provided by Lumen and cannot be deleted.",
        )
    affected: set[UUID] = set()
...
def resolve_connect_app_request(
    ...
    if get_build_session(UUID(pending.build_session_id), user.id, db_session) is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Connect request not found.")
    sandbox = get_sandbox_by_user_id(db_session, user.id)
    if sandbox is None or sandbox.status != SandboxStatus.RUNNING:
        raise LumenError(LumenErrorCode.SERVICE_UNAVAILABLE, "Sandbox is not running.")

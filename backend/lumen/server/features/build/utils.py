...
from lumen.feature_flags.factory import get_default_feature_flag_provider
...
# PostHog feature flag key for enabling Lumen Craft (cloud rollout control)
# Flag logic: True = enabled, False/null/not found = disabled
LUMEN_CRAFT_ENABLED_FLAG = "lumen-craft-enabled"
...
OPENCODE_DISABLED_TOOLS_FLAG = "lumen-craft-opencode-disabled-tools"
...
def is_craft_available_for_deployment(user: User) -> bool:
    """
    Check whether Lumen Craft (Build Mode) is available at the deployment level.

    Flag logic for "lumen-craft-enabled":
    - Flag = True → enabled (Lumen Craft is available)
    - Flag = False → disabled (Lumen Craft is not available)
    - Flag = null/not found → disabled (Lumen Craft is not available)
    ...
    is_enabled = feature_flag_provider.feature_enabled_for_user_tenant(
        LUMEN_CRAFT_ENABLED_FLAG,
        user,
        get_current_tenant_id(),
    )

    if is_enabled:
        logger.debug("Lumen Craft enabled via PostHog feature flag")
        return True
    else:
        logger.debug("Lumen Craft disabled via PostHog feature flag")
        return False
...
def is_craft_enabled_for_user(
    ...
    """
    Check if Lumen Craft (Build Mode) is enabled for the user: the deployment
...
def ensure_build_mode_intro_notification(user: User, db_session: Session) -> None:
    ...
    """
    # PostHog feature flag check - only show notification if Lumen Craft is enabled
    ...
    create_notification(
        ...
        title="Introducing Lumen Craft",
        description="Unleash Lumen to create dashboards, slides, documents, and more with your connected data.",

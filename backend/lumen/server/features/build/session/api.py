...
from lumen.db.models import BuildMessage, Sandbox, User
from lumen.db.scheduled_task import get_scheduled_run_context
...
from lumen.server.features.build.configs import SSE_KEEPALIVE_INTERVAL
...
from lumen.server.features.build.session.session_ready import ensure_session_ready
from lumen.server.features.build.session.streaming import SSE_KEEPALIVE
from lumen.server.features.build.timeouts import POLL_INTERVAL_SECONDS
from lumen.server.features.build.utils import sanitize_filename, validate_file
...
def create_session(
    ...
    try:
        ...
        db_session.rollback()
        raise LumenError(LumenErrorCode.SERVICE_UNAVAILABLE, str(e)) from e
    except SandboxProvisioningInProgressError as e:
        # A live attempt (another replica/request) owns the sandbox; the
        # frontend retries.
        db_session.rollback()
        raise LumenError(LumenErrorCode.CONFLICT, str(e)) from e
    except LumenError:
...
def get_session_scheduled_run_context(
    ...
    if context is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Session has no scheduled-run context")
...
def get_session_scheduled_run_events(
    ...
    if context["status"] != ScheduledTaskRunStatus.RUNNING:
        raise LumenError(LumenErrorCode.CONFLICT, "Scheduled run is not running")

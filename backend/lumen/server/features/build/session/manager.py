...
from lumen.server.features.build.sandbox.serve_transport import PromptSlot
...
from lumen.server.features.build.session import streaming as _streaming
from lumen.server.features.build.session.errors import (
    StaleProvisioningAttemptError,
    UploadLimitExceededError,
)
from lumen.server.features.build.session.interrupt_signal import request_interrupt
from lumen.server.features.build.session.llm_config import (
    AgentSelection,
    GatewaySelection,
    build_lumen_gateway_config,
    parse_agent_selection,
)
from lumen.server.features.build.session.md_to_docx import markdown_to_docx_bytes
from lumen.server.features.build.session.naming import generate_session_name
...
from lumen.server.metrics.craft_sandbox import SandboxReadyOutcome
...
class SessionManager:
    ...
    def build_llm_configs(
        ...
        gateway_config = build_lumen_gateway_config(
    ...
    def reload_session_skills(self, session_id: UUID, user: User) -> bool:
        ...
        if session is None:
            raise LumenError(LumenErrorCode.SESSION_NOT_FOUND, "Session not found")
    ...
    def create_session(
        ...
        Raises:
            ...
            LumenError: If no LLM provider is accessible
    ...
    def _check_nextjs_ready(
        ...
        try:
            ...
            probe_url = (
                ...
                "/_next/static/lumen-ready-probe.js"
            )

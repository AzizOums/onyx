...
1. Public internet over HTTPS (the bridge has default internet egress; block at
   ...
   IMDS).
2. The Lumen API via the complete ``LUMEN_SERVER_URL`` API base. The Craft
...
from lumen.configs.app_configs import DEV_MODE
...
from lumen.server.features.build.sandbox.base import (
    BUN_CACHE_DIR,
    BUN_IMAGE_CACHE_DIR,
    SandboxManager,
)
from lumen.server.features.build.sandbox.docker.dev_mode_serve import (
    opencode_serve_port_bindings,
    published_opencode_serve_base_url,
)
from lumen.server.features.build.sandbox.docker.internal.exec_helpers import (
    ExecError,
    ExecResult,
    run_in_container,
...
from lumen.server.features.build.sandbox.labels import (
    LABEL_K8S_MANAGED_BY,
    LABEL_K8S_MANAGED_BY_LUMEN,
...
from lumen.server.features.build.sandbox.nextjs_dev import (
    allowed_dev_origins,
    build_webapp_restore_script,
)
from lumen.server.features.build.sandbox.serve_transport import ServeConnectionInfo
from lumen.server.features.build.sandbox.session_workspace import (
    MANAGED_SKILLS_PATH,
...
from lumen.server.features.build.sandbox.snapshot_manager import SnapshotManager
from lumen.server.features.build.sandbox.util.agent_instructions import (
    ATTACHMENTS_SECTION_CONTENT,
    generate_agent_instructions,
)
from lumen.server.features.build.sandbox.util.api_url_check import (
    validate_sandbox_api_url,
)
from lumen.server.features.build.sandbox.util.opencode_config import (
    build_opencode_base_config,
    build_provider_opencode_config,
)
from lumen.server.features.build.timeouts import (
    BULK_TRANSFER_TIMEOUT_SECONDS,
    POLL_INTERVAL_SECONDS,
    PROVISION_DEADLINE_SECONDS,
)
from lumen.server.features.build.utils import get_opencode_disabled_tools
...
LABEL_COMPONENT = "lumen.app/component"
LABEL_COMPONENT_VALUE = "craft-sandbox"
LABEL_USER_ID = "lumen.app/user-id"
...
def _looks_like_internal_compose_host(url: str) -> bool:
    ...
    Used to warn deployers that pointed LUMEN_SERVER_URL at a service available
    only on Compose's default network. Sandboxes can resolve the dedicated
    ``lumen-craft-api`` alias, but not these unrelated service names.
    """
    if not url:
        return False
    lowered = url.lower()
...
def build_sandbox_labels(
    ...
    Desktop groups sandbox containers under the same "lumen" stack header as
    ...
    labels: dict[str, str] = {
        ...
        LABEL_K8S_MANAGED_BY: LABEL_K8S_MANAGED_BY_LUMEN,
    }
...
def _proxy_env_vars(
    *,
    ...
    (``lumen.sandboxProxyEnv``); here they're layered on the docker env dict.
...
def build_container_create_kwargs(
    *,
    ...
    - **Env is a fixed allowlist**: LUMEN_PAT, LUMEN_SERVER_URL,
      LUMEN_API_PREFIX, ``OPENCODE_SERVER_PASSWORD``, and
    ...
    - **Single network**: joins only the caller-supplied ``network`` (the
      dedicated ``lumen_craft_sandbox`` bridge). Does NOT join compose's default
    ...
    - Env layered with ``HTTPS_PROXY`` / SDK CA vars + the ``firewall-init.sh``
      ...
      are layered on top.
    - ``LUMEN_PAT`` is replaced with ``SANDBOX_PROXY_INJECTED_PLACEHOLDER``;
    ...
    ``LUMEN_SERVER_URL`` is the complete API base used by lumen-cli inside the
    sandbox. The default ``lumen-craft-api`` alias is attached to the sandbox
    ...
    if _looks_like_internal_compose_host(api_server_url):
        logger.warning(
            "LUMEN_SERVER_URL=%s looks like an internal compose hostname. Sandboxes only "
            "join the craft bridge network, so default-network DNS will fail. Use the "
            "http://lumen-craft-api:8080 bridge alias or a public API base such as "
            "https://lumen.your-org.com/api.",
            api_server_url,
        )
    env: dict[str, str] = {
        "LUMEN_PAT": lumen_pat,
        "LUMEN_SERVER_URL": api_server_url,
        ...
        "LUMEN_WEBAPP_ALLOWED_DEV_ORIGINS": allowed_dev_origins(),
    }
...
class DockerSandboxManager(SandboxManager):
    ...
    def provision(
        ...
        if not lumen_pat:
            raise ValueError("lumen_pat is required for Docker sandbox provisioning.")
        if not LUMEN_SERVER_URL:
            raise ValueError(
                "LUMEN_SERVER_URL must be set for Docker sandbox provisioning."
            )
        validate_sandbox_api_url(LUMEN_SERVER_URL)
        ...
        if container is None:
            ...
            container_lumen_pat = (
                SANDBOX_PROXY_INJECTED_PLACEHOLDER if SANDBOX_PROXY_HOST else lumen_pat
            )
    ...
    def _create_sandbox_container(
        ...
        *,
        ...
        create_kwargs = build_container_create_kwargs(
            ...
            image=self._image,
            lumen_pat=lumen_pat,
            api_server_url=LUMEN_SERVER_URL,

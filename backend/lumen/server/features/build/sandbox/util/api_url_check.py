"""Provision-time validation of LUMEN_SERVER_URL.

LUMEN_SERVER_URL is the full base URL a client uses, any API path prefix
included. The classic misconfiguration is a public URL without its /api
prefix, which fails far from the cause: every LLM-gateway and lumen-cli call
...
from lumen.server.features.build.timeouts import CONNECT_TIMEOUT_SECONDS
...
def validate_sandbox_api_url(api_server_url: str) -> None:
    ...
    if response is None:
        logger.warning(
            "Could not probe %s/health to validate LUMEN_SERVER_URL; "
    ...
    if prefixed_response is not None and _is_health_response(prefixed_response):
        raise LumenError(
            ...
            f"LUMEN_SERVER_URL={api_server_url!r} is missing its API path "
            f"prefix: {base}/health does not serve the API health payload "
            f"(HTTP {response.status_code}) while {prefixed}/health does. "
            f"Set LUMEN_SERVER_URL to {prefixed!r} — the full base URL a "
            "sandbox client uses.",
        )
    logger.warning(
        "LUMEN_SERVER_URL=%s: %s/health did not serve the API health "

...
# $NS (default lumen-sandboxes) running `sleep` so we measure only the platform
...
#   bench-sandbox-spinup.sh lumendotapp/sandbox:dev lumendotapp/sandbox:candidate
#
#   # Override defaults
#   REPS=5 NS=lumen-sandboxes \
#     bench-sandbox-spinup.sh lumendotapp/sandbox:dev
#
# Requires: docker, kind, kubectl, python3. Cluster context must be
# kind-lumen-dev — see docs/craft/dev/local-kubernetes.md for the local setup.
# Full context + recorded numbers: docs/craft/sandbox/image-and-spinup.md.
set -euo pipefail

KIND_CLUSTER="${KIND_CLUSTER:-lumen-dev}"
KIND_NODE="${KIND_NODE:-${KIND_CLUSTER}-control-plane}"
NS="${NS:-lumen-sandboxes}"
REPS="${REPS:-3}"

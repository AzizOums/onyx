...
from lumen.cache.interface import CACHE_TRANSIENT_ERRORS
...
from lumen.server.features.build.sandbox.kubernetes.k8s_client import load_kube_config
from lumen.server.features.build.sandbox.kubernetes.sidecar_client import (
    SidecarClient,
    SidecarRequestError,
    SidecarStatusError,
    get_push_key_pair,
...
class KubernetesSandboxManager(SandboxManager):
    ...
    def __init__(self) -> None:
        ...
        build_dir = Path(__file__).parent.parent.parent  # /lumen/server/features/build/
    ...
    def _overlay_dynamic_fields(self, spec: client.V1PodSpec, sandbox_id: str) -> None:
        ...
        sandbox_container.env = list(sandbox_container.env or []) + [
            ...
            client.V1EnvVar(
                name="LUMEN_WEBAPP_ALLOWED_DEV_ORIGINS",
    ...
    def provision(
        ...
        Args:
            ...
            lumen_pat: Required by the interface and the Docker backend; on K8s
        ...
        if not lumen_pat:
            raise ValueError("lumen_pat is required for Kubernetes sandbox provisioning")
        if not LUMEN_SERVER_URL:
            raise ValueError(
                "LUMEN_SERVER_URL must be set for Kubernetes sandbox provisioning"
            )
...
"""
    ...
    def restore_snapshot(
        ...
        Steps:
        1. Read the snapshot from Lumen FileStore in the api-server

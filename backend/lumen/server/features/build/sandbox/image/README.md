...
This directory contains the Dockerfile and resources for building the Lumen Craft sandbox container image.
...
Built-in skill sources are **not** here — they live in `backend/lumen/skills/builtin/`
...
image, e.g. `lumendotapp/sandbox:v4.1.2` for app tag `v4.1.2`.
...
`lumendotapp/sandbox:dev` tag for new builds. Local development can still build
`lumendotapp/sandbox:dev` and load it directly into kind.
...
```bash
cd backend/lumen/server/features/build/sandbox/image
docker build --platform linux/amd64 -t lumendotapp/sandbox:dev .
```
...
- Docker compose: `lumendotapp/sandbox:${IMAGE_TAG}`.
- Helm: `lumendotapp/sandbox:${global.version}`.
...
- **lumen-cli**: `/usr/local/bin/lumen-cli` — Lumen CLI for search
- **Snapshot sidecar daemon**: Packages and restores session files; durable storage is handled by the api_server through the Lumen FileStore
...
```bash
curl -s "https://hub.docker.com/v2/repositories/lumendotapp/sandbox/tags" | jq '.results[].name'
```

### Check what image a pod is using

```bash
kubectl get pod <pod-name> -n lumen-sandboxes -o jsonpath='{.spec.containers[?(@.name=="sandbox")].image}'
```

...
const BUDGET_FILE = ".lumen-turn-budget.json";

const NOTICE_INTERVAL_MS = 60_000;

function convergeNotice(minutesLeft: number): string {
  return (
    `[Lumen turn budget] About ${minutesLeft} minute(s) of turn budget left. ` +
...
const FINISH_NOW_NOTICE =
  "[Lumen turn budget] Finish now: write any pending outputs to disk and " +

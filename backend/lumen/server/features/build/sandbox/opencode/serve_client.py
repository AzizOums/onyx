"""HTTP client for ``opencode serve`` — the only transport Lumen Craft
...
Sandbox-event types come from :mod:`lumen.server.features.build.sandbox.event_schema`.
"""
...
from lumen.server.features.build.packets import (
    CompactionPacket,
    ContextUsagePacket,
...
from lumen.server.features.build.sandbox.opencode.event_bus import (
    BUS_CLOSED_SENTINEL,
    PodEventBus,
    _Subscription,
)
...
# Lumen schema: "pending" | "in_progress" | "completed" | "failed"
...
# array on tool parts — only ``state.{input,output,metadata}``. Lumen's
...
class OpencodeServeClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._client_info = client_info or {
            "name": "lumen-opencode-serve-client",
            "version": "1.0.0",
        }
    ...
    def delete_session(self, opencode_session_id: str, *, directory: str) -> bool:
        ...
        Product deletion is owned by Lumen's BuildSession row. This cleanup is
        opportunistic: failure should not block deleting the Lumen session.
        """
        try:
            r = self._request(
                "DELETE",

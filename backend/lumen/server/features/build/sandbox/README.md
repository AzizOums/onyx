# Lumen Sandbox System

This directory contains the implementation of Lumen's sandbox system for running OpenCode agents in isolated environments.
...
1. **Kubernetes Mode** (`SANDBOX_BACKEND=kubernetes`) — default
   ...
   - Used by Lumen's Helm chart / cloud deployment
...
2. **Docker Mode** (`SANDBOX_BACKEND=docker`)
   ...
   - Sandboxes join only the dedicated `lumen_craft_sandbox` bridge — `postgres` / `redis` / `minio` / model servers are not reachable by compose DNS
...
| `Service` + DNS for Next.js preview   | Container IP on `lumen_craft_sandbox` bridge, proxied        |
...
`api_server` and `background` mount the host Docker socket so they can drive sandbox containers. Anything that can talk to that socket is effectively root on the host — only enable Craft on hosts you fully control. Sandbox containers themselves run unprivileged: `--security-opt no-new-privileges`, `--cap-drop ALL`, `user=1000:1000`, no Docker socket, and a fixed env allowlist (`LUMEN_PAT` + `LUMEN_SERVER_URL` + `LUMEN_API_PREFIX`).

`LUMEN_SERVER_URL` is the complete API base URL used by both the host-side
manager and the sandbox client. The Craft Compose overlay defaults it to
`http://lumen-craft-api:8080`, a private alias on the sandbox bridge. Public
reverse-proxy overrides must include their API path prefix, for example
`https://lumen.your-org.example/api`.
...
Deploy the normal Lumen application images with Craft enabled. The sandbox image
is selected from the same application version by default, so app tag `vX.Y.Z`
uses `lumendotapp/sandbox:vX.Y.Z`.
...
- **Disabled tools**: Resolved deployment-wide (not per-user) via the
  `lumen-craft-opencode-disabled-tools` PostHog multivariate flag, keyed on
...
- Built-in skill sources live in `backend/lumen/skills/builtin/` (pushed at session setup, not baked in)
...
# lumen-craft-opencode-disabled-tools flag's variant is unset/unrecognized.
...
SANDBOX_NAMESPACE=lumen-sandboxes          # Default: lumen-sandboxes

# Helm defaults the sandbox image to lumendotapp/sandbox:${global.version}.
# SANDBOX_CONTAINER_IMAGE is an internal override.

# Snapshots use the normal Lumen FileStore configuration
FILE_STORE_BACKEND=s3|gcs|postgres
S3_FILE_STORE_BUCKET_NAME=lumen-file-store # when FILE_STORE_BACKEND=s3
...
# http://lumen-craft-api:8080 alias; override only to route through a public
# reverse proxy.
LUMEN_SERVER_URL=https://lumen.your-org.example/api
...
# `docker network create lumen_craft_sandbox` manually). Sandboxes join *only*
# this network — compose services are not reachable by DNS from inside.
SANDBOX_DOCKER_NETWORK=lumen_craft_sandbox       # Default: lumen_craft_sandbox

# Prefix for per-sandbox named volumes (mounted at /workspace/sessions).
SANDBOX_DOCKER_VOLUME_PREFIX=lumen-sandbox       # Default: lumen-sandbox
...
- Confirm the dedicated bridge exists: `docker network inspect lumen_craft_sandbox` (created by `install.sh --include-craft`, or run `docker network create lumen_craft_sandbox` manually)
- Check sandbox logs: `docker logs sandbox-<id8>`
- Confirm `LUMEN_SERVER_URL` resolves from the sandbox bridge and includes any
  API path prefix. The Compose default is `http://lumen-craft-api:8080`.
...
- Check pod logs: `kubectl logs -n lumen-sandboxes sandbox-{sandbox-id}`
- Check sidecar logs: `kubectl logs -n lumen-sandboxes sandbox-{sandbox-id} -c sidecar`
...
- Check container logs: `kubectl logs -n lumen-sandboxes sandbox-{sandbox-id}`
- Verify `bun install` succeeded (check entrypoint.sh logs)
- Check that web template was copied: `kubectl exec -n lumen-sandboxes sandbox-{sandbox-id} -- ls /workspace/outputs/web`
...
- **Docker containers** run with `--security-opt no-new-privileges`, `--cap-drop ALL`, `user=1000:1000`, no Docker socket, and a fixed env allowlist (`LUMEN_PAT` + `LUMEN_SERVER_URL` + `LUMEN_API_PREFIX`)
- **Docker network isolation** is enforced by joining only the dedicated `lumen_craft_sandbox` bridge — compose's default network (postgres/redis/minio/model servers) is unreachable by DNS from inside a sandbox
...
- Snapshots are stored through the normal Lumen `FileStore` and isolated by tenant-scoped snapshot paths
...
The lightweight Next.js template (`backend/lumen/server/features/build/sandbox/image/templates/outputs/web/`) includes:
...
This template provides a modern development environment without the complexity of the full Lumen application, allowing agents to build custom UIs quickly.

...
from lumen.server.features.build.sandbox.base import BUN_CACHE_DIR, BUN_IMAGE_CACHE_DIR
...
{install_check}
export LUMEN_WEBAPP_PORT={nextjs_port}
export LUMEN_WEBAPP_BASE_PATH="/api/build/sessions/$(basename {session_path})/webapp"
export LUMEN_WEBAPP_ALLOWED_DEV_ORIGINS="{allowed_dev_origins()}"
if grep -q "WEBAPP_ASSET_PREFIX" next.config.ts 2>/dev/null; then
    cat > next.config.ts <<'EOF'
{template_next_config}
EOF
fi
# The template's `dev` script resolves LUMEN_WEBAPP_PORT itself; a second -p
...
PORT_FLAG=""
if ! grep -q "LUMEN_WEBAPP_PORT" package.json 2>/dev/null; then
...
WEBAPP_AUTOSTART_SENTINEL = "LUMEN_WEBAPP_AUTOSTART"
WEBAPP_ABSENT_SENTINEL = "LUMEN_WEBAPP_ABSENT"
...
def build_webapp_script_write_snippet(session_path: str, nextjs_port: int) -> str:
    ...
    directly from ``lumen.server.features.build.sandbox.nextjs_dev``.
    """

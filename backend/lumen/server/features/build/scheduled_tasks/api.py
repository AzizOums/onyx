...
Thin HTTP layer over ``lumen.db.scheduled_task``. Every handler:

1. Validates request payload via Pydantic.
2. Calls a DB op (which enforces ownership + raises ``LumenError``).
...
``backend/lumen/server/features/build/api.py``), which provides the
``require_lumen_craft_enabled`` gate.
"""
...
from lumen.background.celery.versioned_apps.client import app as celery_app
from lumen.configs.constants import LumenCeleryPriority, LumenCeleryQueues, LumenCeleryTask
...
from lumen.db.external_app import get_external_apps
from lumen.db.mcp import get_craft_enabled_mcp_servers
from lumen.db.models import ScheduledTask, ScheduledTaskRun, User
...
from lumen.server.features.build.scheduled_tasks.schedule import (
...
from lumen.server.features.build.timeouts import QUEUE_RESIDENCY_SECONDS
...
def _enqueue_executor(run_id: UUID) -> None:
    ...
    celery_app.send_task(
        LumenCeleryTask.SCHEDULED_TASKS_RUN,
        kwargs={"run_id": str(run_id), "tenant_id": tenant_id},
        queue=LumenCeleryQueues.SCHEDULED_TASKS,
...
def list_task_runs(
    ...
    Raises ``LumenError(NOT_FOUND)`` when the task is missing, soft-deleted,

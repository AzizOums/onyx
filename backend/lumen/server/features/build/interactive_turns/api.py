...
from lumen.server.features.build.interactive_turns.executor import (
    start_interactive_turn_runner,
)
from lumen.server.features.build.interactive_turns.models import InteractiveTurnResponse
from lumen.server.features.build.interactive_turns.state import (
    TURN_STATUS_FAILED,
    get_active_turn,
    get_turn,
)
...
def get_interactive_turn_events(
    ...
    if turn is None or turn.turn_id != turn_id:
        ...
        if (
            ...
            raise LumenError(LumenErrorCode.CONFLICT, "Interactive turn is not running")

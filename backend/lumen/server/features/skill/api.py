...
from lumen.db.engine.sql_engine import get_session, get_session_with_tenant
...
from lumen.external_apps.token_refresh import ensure_fresh_credentials
...
from lumen.server.features.skill.response_helpers import (
...
from lumen.skills.content import read_custom_skill_bundle_bytes
from lumen.skills.ingest import (
    delete_bundle_blob,
    ingested_skill_bundle,
    save_skill_bundle_bytes,
)
from lumen.skills.ingest_from_github import fetch_github_skill_bundles
from lumen.skills.metadata import parse_skill_document
...
def fetch_skill_for_current_user(
    ...
    if skill is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Skill not found")
    return skill_response_for_user(skill, user, db_session)
...
def create_custom_skill(
    ...
    with ingested_skill_bundle(
        ...
        if auto_enable and not enable_new_skill_if_name_available__no_commit(
            ...
            raise LumenError(
                LumenErrorCode.SKILL_NAME_CONFLICT,
...
def create_custom_skill_from_editor(
    ...
    if external_app_id is not None:
        ...
        if get_external_app_by_id(db_session, external_app_id) is None:
            raise LumenError(LumenErrorCode.NOT_FOUND, "External app not found.")
...
def inspect_custom_skill_bundle_upload(
    ...
    if not upload.filename:
        raise LumenError(LumenErrorCode.INVALID_INPUT, "upload is missing a filename")
...
def remove_current_user_skill_file(
    ...
    if skill is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Skill not found")
    if not path:
        raise LumenError(LumenErrorCode.INVALID_INPUT, "Skill file path cannot be empty")
...
def transfer_current_user_skill_ownership(
    ...
    if target is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "New owner not found.")

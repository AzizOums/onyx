...
from lumen.configs.constants import PUBLIC_API_TAGS, FileOrigin, MilestoneRecordType
from lumen.db.engine.sql_engine import get_session
from lumen.db.enums import Permission, PermissionAuthority, PersonaSharePermission
...
from lumen.db.users import get_active_admin_count
...
from lumen.server.manage.llm.api import get_valid_model_configuration_ids_for_persona
from lumen.server.manage.llm.provider_cache import invalidate_provider_listing_cache
from lumen.server.models import DisplayPriorityRequest
...
def update_persona(
    ...
    # (see lumen/server/manage/llm/provider_cache.py), so edits must drop it.
...
def transfer_persona_ownership_endpoint(
    ...
    versioned_transfer = fetch_versioned_implementation(
        "lumen.db.persona", "transfer_persona_ownership"
    )
    try:
        versioned_transfer(
            persona_id=persona_id,
...
def delete_persona(
    ...
    try:
        ...
        try:
            ...
            raise LumenError(
                LumenErrorCode.PERSONA_NOT_FOUND,
...
def get_persona_avatar(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.NOT_FOUND, "Avatar not found")

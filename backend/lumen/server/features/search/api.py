...
Intended for programmatic consumers (lumen-cli, Craft sandbox, integrations).

Not the same as the Lumen Search UI backend at /api/search/send-search-message
(ee/lumen/server/query_and_chat/search_backend.py), which calls search_pipeline()
...
from lumen.chat.emitter import NullEmitter
from lumen.configs.constants import PUBLIC_API_TAGS, MessageType
from lumen.context.search.models import BaseFilters, PersonaSearchInfo, TimeRange
...
from lumen.llm.factory import get_default_llm, get_llm_for_persona, llm_from_provider
from lumen.server.features.search.models import (
    SearchRequest,
    SearchResponse,
    SearchResult,
)
from lumen.server.manage.llm.models import LLMProviderView
...
from lumen.tools.models import ChatMinimalTextMessage, SearchToolOverrideKwargs
...
def search(
    ...
    """
    Search the Lumen index and get back ranked document sections.
    ...
    if request.persona_id is not None:
        try:
            ...
            raise LumenError(LumenErrorCode.PERSONA_NOT_FOUND)
    ...
    if request.provider:
        ...
        if not can_user_access_llm_provider(
            ...
            raise LumenError(LumenErrorCode.UNAUTHORIZED)

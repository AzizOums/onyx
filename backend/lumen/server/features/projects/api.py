...
from lumen.chat.incognito_context import incognito_session_torn_down
...
from lumen.db.enums import Permission, UserFileStatus
from lumen.db.incognito import mark_incognito_user_files_deleting
from lumen.db.models import ChatSession, Project__UserFile, User, UserFile, UserProject
from lumen.db.persona import get_personas_by_ids
from lumen.db.projects import (
...
def _trigger_user_file_project_sync(
    ...
    if DISABLE_VECTOR_DB and background_tasks is not None:
        from lumen.background.task_utils import drain_project_sync_loop
...
def get_files_in_project(
    ...
    if not check_project_ownership(project_id, user_id, db_session):
        raise LumenError(LumenErrorCode.NOT_FOUND, "Project not found")
...
def delete_user_file(
    ...
    if DISABLE_VECTOR_DB:
        from lumen.background.task_utils import drain_delete_loop
        ...
        task = client_app.send_task(
            LumenCeleryTask.DELETE_SINGLE_USER_FILE,
            kwargs={"user_file_id": str(user_file.id), "tenant_id": tenant_id},
            queue=LumenCeleryQueues.USER_FILE_DELETE,

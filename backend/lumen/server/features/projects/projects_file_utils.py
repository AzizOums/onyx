...
from lumen.db.llm import fetch_default_llm_model
...
from lumen.file_processing.password_validation import is_file_password_protected
from lumen.file_processing.pdf_image_utils import count_pdf_embedded_images
from lumen.natural_language_processing.utils import count_tokens, get_tokenizer
...
def _skip_token_threshold(extension: str) -> bool:
    ...
    return extension.lower() in LumenFileExtensions.TABULAR_EXTENSIONS
...
def categorize_uploaded_files(
    ...
    for upload in files:
        try:
            ...
            if (
                extension in LumenFileExtensions.AUDIO_EXTENSIONS
                or extension in LumenFileExtensions.VIDEO_EXTENSIONS
            ):
                if (

...
from lumen.image_gen.generation import (
    GeneratedImageData,
    ensure_image_generation_configured,
    generate_images_with_default_config,
)
from lumen.image_gen.interfaces import ImageShape, ReferenceImage
from lumen.utils.b64 import get_image_type, get_image_type_from_bytes
...
def _map_generation_error(error: BaseException) -> LumenError:
    if isinstance(error, ImageGenerationNotConfiguredError):
        return LumenError(LumenErrorCode.NOT_FOUND, str(error))
    if isinstance(error, ValueError):
        return LumenError(LumenErrorCode.INVALID_INPUT, str(error))
    logger.error("Image generation failed", exc_info=error)
    return LumenError(
        LumenErrorCode.LLM_PROVIDER_ERROR,
...
def _error_envelope(error: LumenError) -> bytes:
    return json.dumps(
...
def _keepalive_stream(run: _GenerationRun) -> Iterator[bytes]:
    ...
    while True:
        ...
        if remaining <= 0:
            ...
            yield _error_envelope(
                LumenError(LumenErrorCode.GATEWAY_TIMEOUT, "Image generation timed out.")
            )
            return
    ...
    if run.images is None:
        yield _error_envelope(
            LumenError(LumenErrorCode.LLM_PROVIDER_ERROR, "Image generation failed.")
        )
        return
...
def generate_image(
    ...
    if not request.prompt.strip():
        raise LumenError(LumenErrorCode.INVALID_INPUT, "prompt must not be empty")

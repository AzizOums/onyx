...
from lumen.configs.app_configs import APP_API_PREFIX
...
PUBLIC_ENDPOINT_SPECS = [
    ...
    ("/auth/type", {"GET"}),
    # just gets the version of Lumen (e.g. 0.3.11)
    ("/version", {"GET"}),
    # Gets stable and beta versions for Lumen docker images
    ("/versions", {"GET"}),
...
def check_router_auth(
    ...
    control_plane_dep = fetch_ee_implementation_or_noop(
        "lumen.server.tenants.access", "control_plane_dep"
    )
    current_cloud_superuser = fetch_ee_implementation_or_noop(
        "lumen.auth.users", "current_cloud_superuser"
    )
    verify_scim_token = fetch_ee_implementation_or_noop(
        "lumen.server.scim.auth", "verify_scim_token"
    )

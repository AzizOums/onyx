...
from lumen.db.usage import UsageLimitExceededError, UsageType, check_usage_limit
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError
from lumen.server.tenant_usage_limits import (
...
# Collect all Lumen-managed default API keys for comparison
_LUMEN_MANAGED_API_KEYS: set[str] = set()
for key in [
    OPENAI_DEFAULT_API_KEY,
    ANTHROPIC_DEFAULT_API_KEY,
    COHERE_DEFAULT_API_KEY,
    ...
    if key:
        _LUMEN_MANAGED_API_KEYS.add(key)


def is_lumen_managed_api_key(api_key: str | None) -> bool:
    """Check if the given API key is one of Lumen's managed default keys."""
    return bool(api_key) and api_key in _LUMEN_MANAGED_API_KEYS
...
def is_tenant_on_trial_fn(tenant_id: str) -> bool:
    ...
    fn: Callable[[str], bool] = fetch_versioned_implementation(
        "lumen.server.usage_limits", "is_tenant_on_trial"
    )
    return fn(tenant_id)
...
def _get_tenant_override(tenant_id: str, field_name: str) -> int | None:
    ...
    try:
        ...
        get_overrides_fn = fetch_versioned_implementation(
            "lumen.server.tenant_usage_limits", "get_tenant_usage_limit_overrides"
        )
...
def check_llm_cost_limit_for_provider(
    ...
    """
    Check if the LLM cost limit would be exceeded for a provider using Lumen-managed keys.

    Only enforces limits when the provider uses Lumen-managed API keys.
    ...
    # Only enforce limits for Lumen-managed API keys
    if not is_lumen_managed_api_key(llm_provider_api_key):
        return
...
def check_usage_and_raise(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.RATE_LIMITED, detail)

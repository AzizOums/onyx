...
from lumen.db.document_set import set_document_set_group_membership__no_commit
from lumen.db.persona import update_persona_access
from lumen.db.user_group import assert_group_config_is_editable
...
from lumen.db.user_group_crud import delete_user_group as db_delete_user_group
from lumen.server.user_group.models import (
...
from lumen.auth.permission_projection import user_group_permissions
...
from lumen.configs.constants import PUBLIC_API_TAGS, LumenCeleryPriority, LumenCeleryTask
...
from lumen.db.enums import Permission, PermissionAuthority
from lumen.db.models import User
from lumen.db.persona import fetch_persona_by_id_for_user, get_personas_by_ids
...
def get_user_group(
    ...
    if not has_global_permission(user, Permission.READ_USER_GROUPS) and not can_manage:
        raise LumenError(LumenErrorCode.NOT_FOUND, "User group not found")
...
def rename_user_group_endpoint(
    ...
    try:
        ...
        if "not found" in msg.lower():
            raise LumenError(LumenErrorCode.NOT_FOUND, msg)
        raise LumenError(LumenErrorCode.CONFLICT, msg)
...
def update_group_document_sets(
    ...
    if missing:
        raise LumenError(
            LumenErrorCode.DOCUMENT_SET_NOT_FOUND,
    ...
    if changed and not DISABLE_VECTOR_DB:
        client_app.send_task(
            LumenCeleryTask.CHECK_FOR_VESPA_SYNC_TASK,

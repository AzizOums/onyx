...
from lumen.configs.app_configs import (
    ...
    LUMEN_QUERY_HISTORY_TYPE,
    SHOW_EXTRA_CONNECTORS,
)
from lumen.configs.constants import KV_SETTINGS_KEY, LumenRedisLocks
...
def load_settings(raise_on_error: bool = False) -> Settings:
    ...
    try:
        ...
        if value is not None:
            ...
            cache.set(LumenRedisLocks.ANONYMOUS_USER_ENABLED, "0", ex=SETTINGS_TTL)
    except Exception as e:
    ...
    if settings.query_history_type is None:
        settings.query_history_type = LUMEN_QUERY_HISTORY_TYPE

    if DISABLE_USER_KNOWLEDGE:
        settings.user_knowledge_enabled = False

    settings.show_extra_connectors = SHOW_EXTRA_CONNECTORS
    settings.opensearch_indexing_enabled = ENABLE_OPENSEARCH_INDEXING_FOR_LUMEN
...
def store_settings(settings: Settings) -> None:
    ...
    if settings.anonymous_user_enabled is not None:
        cache.set(
            LumenRedisLocks.ANONYMOUS_USER_ENABLED,

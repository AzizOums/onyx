...
from lumen.connectors.exceptions import ValidationError
from lumen.connectors.factory import identify_connector_class, validate_ccpair_for_user
...
from lumen.db.document import get_document_counts_for_cc_pairs, get_documents_for_cc_pair
...
from lumen.db.index_attempt_metrics import get_stage_metrics_for_attempt
from lumen.db.indexing_coordination import IndexingCoordination
from lumen.db.models import ConnectorCredentialPair, IndexAttempt, User
...
from lumen.redis.redis_connector_utils import get_deletion_attempt_snapshot
...
def get_index_attempt_stage_metrics(
    ...
    if index_attempt is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Index attempt not found")
...
def _get_cc_pair_source_or_raise(
    ...
    Raises ``LumenError(NOT_FOUND)`` if the cc-pair doesn't exist or the user
...
def get_cc_pair_permission_sync_attempts(
    ...
    source_requires_doc_sync = fetch_ee_implementation_or_noop(
        "lumen.external_permissions.sync_params",
...
def update_cc_pair_name(
    ...
    try:
        ...
        db_session.rollback()
        raise LumenError(
            LumenErrorCode.CONFLICT, f"Could not rename cc-pair {cc_pair_id}."
        )
...
def associate_credential_to_connector(
    ...
    try:
        ...
        if not response.success:
            ...
            raise LumenError(LumenErrorCode.CONFLICT, response.message)
        ...
        raise LumenError(LumenErrorCode.INTERNAL_ERROR, "Unexpected error")

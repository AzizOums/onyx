...
from lumen.auth.email_utils import send_email
from lumen.auth.permission_projection import cc_pair_permissions
from lumen.auth.permissions import (
    get_effective_permissions,
    has_global_permission,
    require_permission,
)
from lumen.auth.scoped_permissions import assert_within_scope
from lumen.auth.users import current_chat_accessible_user
from lumen.background.celery.tasks.pruning.tasks import try_creating_prune_generator_task
...
from lumen.connectors.google_utils.google_auth import get_google_oauth_creds
from lumen.connectors.google_utils.google_kv import (
...
from lumen.db.connector import (
    create_connector,
    delete_connector,
...
from lumen.db.credentials import create_credential, fetch_credential_by_id_for_user
from lumen.db.deletion_attempt import check_deletion_attempt_is_allowed
from lumen.db.document import get_document_counts_for_all_cc_pairs
...
from lumen.db.federated import fetch_all_federated_connectors_parallel
...
from lumen.file_store.file_store import (
...
from lumen.server.federated.models import FederatedConnectorStatus
from lumen.server.models import StatusResponse
from lumen.server.security.store import get_security_settings
from lumen.server.utils_vector_db import require_vector_db
...
from lumen.utils.threadpool_concurrency import (
    CallableProtocol,
...
def save_zip_metadata_to_file_store(
    ...
    """
    Extract .lumen_metadata.json from zip and save to file store.
    Returns the file_id or None if no metadata file exists.
    """
    try:
        metadata_file_info = zf.getinfo(LUMEN_METADATA_FILENAME)
        with zf.open(metadata_file_info, "r") as metadata_file:
            ...
            try:
                ...
                logger.warning("Unable to load %s: %s", LUMEN_METADATA_FILENAME, e)
                raise HTTPException(
                    status_code=400,
                    detail=f"Unable to load {LUMEN_METADATA_FILENAME}: {e}",
                )

            # Save to file store
            file_id = file_store.save_file(
                content=BytesIO(metadata_bytes),
                display_name=LUMEN_METADATA_FILENAME,
                file_origin=FileOrigin.CONNECTOR_METADATA,
                file_type="application/json",
            )
            return file_id
    except KeyError:
        logger.info("No %s file", LUMEN_METADATA_FILENAME)
        return None
    ...
    try:
        ...
        if new_file_paths:
            ...
            client_app.send_task(
                LumenCeleryTask.CHECK_FOR_INDEXING,
...
def update_connector_from_model(
    ...
    if updated_connector is None:
        raise LumenError(
            LumenErrorCode.CONNECTOR_NOT_FOUND,
...
def submit_connector_request(
    ...
    Tracks via PostHog telemetry and sends email to hello@lumen.app.
    """
    tenant_id = get_current_tenant_id()
    ...
    if EMAIL_CONFIGURED:
        try:
            subject = "Lumen Craft Connector Request"
...
<li><strong>Tenant ID:</strong> {tenant_id}</li>
            ...
            send_email(
                user_email="hello@lumen.app",
                subject=subject,
                html_body=email_body_html,
                text_body=email_body_text,
            )
            logger.info(
                "Connector request email sent to hello@lumen.app for connector: %s",
                connector_name,
            )
        except Exception as e:

...
from lumen.connectors.interfaces import OAuthConnector
from lumen.db.credentials import create_credential
...
from lumen.oauth.authorization_attempt import (
...
from lumen.utils.subclasses import find_all_subclasses_in_package
from lumen.utils.url import sanitize_next_url
...
def _discover_oauth_connectors() -> dict[DocumentSource, type[OAuthConnector]]:
    ...
    oauth_connectors = find_all_subclasses_in_package(
        cast(type[OAuthConnector], OAuthConnector), "lumen.connectors"
    )
...
def _validate_additional_kwargs(
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.VALIDATION_ERROR, detail) from error
...
def _attempt_payload(
    ...
    if return_url.scheme or return_url.netloc:
        ...
        if return_origin != web_origin or (
            ...
            raise LumenError(
                LumenErrorCode.INVALID_INPUT,
                "OAuth return URL must use the Lumen application origin",
            )
...
def _get_enabled_oauth_connector(
    ...
    if connector_cls is None:
        raise LumenError(LumenErrorCode.INVALID_INPUT, f"Unknown OAuth source: {source}")
    if not connector_cls.oauth_enabled():
        raise LumenError(
            LumenErrorCode.INVALID_INPUT, f"OAuth is not configured for {source}"
        )
...
def oauth_callback(
    ...
    if connector_cls.supports_pkce and not attempt.payload.code_verifier:
        raise LumenError(LumenErrorCode.INVALID_INPUT, "Invalid OAuth state")

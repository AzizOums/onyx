...
from lumen.auth.permissions import Permission, has_permission, require_permission
...
from lumen.db.connector_credential_pair import verify_user_can_edit_all_cc_pairs
from lumen.db.engine.sql_engine import get_session
from lumen.db.enums import IndexingStatus, PermissionAuthority
...
def submit_targeted_reindex(
    ...
    try:
        ...
        client_app.send_task(
            LumenCeleryTask.TARGETED_REINDEX_TASK,
            kwargs={
...
def get_targeted_reindex_status(
    ...
    if job is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Job not found.")

...
from lumen.auth.login_claims_capture import capture_saml_login_claims
from lumen.auth.sso_web_error import redirect_sso_errors_to_web
from lumen.auth.users import UserManager, auth_backend, fastapi_users, get_user_manager
from lumen.configs.app_configs import REQUIRE_EMAIL_VERIFICATION, WEB_DOMAIN
from lumen.db.engine.sql_engine import get_session
from lumen.db.enums import SSOProviderType
from lumen.db.models import SSOProvider, User
...
def _reject_if_unsupported() -> None:
    ...
    issuer, and an IdP-initiated assertion carries no state Lumen signed. Neither
...
def _resolve_saml_provider(
    ...
    if provider is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "unknown SAML provider")
...
def _resolve_saml_provider_by_issuer(
    ...
    raise LumenError(LumenErrorCode.UNAUTHORIZED, "unrecognized SAML issuer")
...
def _extract_issuer_from_saml_response(encoded_response: str) -> str:
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.UNAUTHORIZED, "malformed SAML response")
    ...
    raise LumenError(LumenErrorCode.UNAUTHORIZED, "SAML response missing issuer")
...
def _raise_saml_access_denied(auth: OneLogin_Saml2_Auth, detail: str) -> NoReturn:
    ...
    raise LumenError(LumenErrorCode.UNAUTHORIZED, detail)
...
async def _process_saml_callback(
    ...
    if not isinstance(encoded_response, str) or not encoded_response:
        raise LumenError(LumenErrorCode.UNAUTHORIZED, "missing SAML response")

...
from lumen.server.manage.models import SSOProviderOption
from lumen.utils.client_ip import get_client_ip
...
async def _enforce_discovery_rate_limit(request: Request) -> None:
    ...
    # Lumen-lite, so the bound applies only on cloud, where it is the sole limit
...
def _resolve_workspace(email: str) -> str | None:
    ...
    tenant_id = fetch_ee_implementation_or_noop(
        "lumen.db.user_tenant_mapping", "lookup_tenant_id_for_login", None
    )(email)
    if tenant_id:
        return tenant_id

    return fetch_ee_implementation_or_noop(
        "lumen.db.tenant_sso_domain", "lookup_tenant_id_for_email_domain", None
    )(email)

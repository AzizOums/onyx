...
reads it back out of the OAuth state Lumen signed on the way out.
"""
...
from lumen.auth.oidc_client import VerifiedEmailOpenID, log_token_exchange_failure
from lumen.auth.sso_tenant_token import (
    SSO_TENANT_TOKEN_PARAM,
    decode_sso_tenant_token,
)
from lumen.auth.sso_url_guard import (
    UnsafeSSOUrl,
    validate_discovered_endpoints,
    validate_idp_url,
)
from lumen.auth.sso_web_error import delete_pkce_cookie, redirect_sso_errors_to_web
...
def _workspace_session(tenant_id: str | None) -> Generator[Session, None, None]:
    ...
    if not tenant_id:
        raise LumenError(LumenErrorCode.UNAUTHORIZED, _NO_WORKSPACE_DETAIL)
...
def _resolve_oidc_provider(
    ...
    if provider is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "unknown OIDC provider")
    ...
    try:
        ...
        raise LumenError(LumenErrorCode.NOT_FOUND, "unknown OIDC provider") from e

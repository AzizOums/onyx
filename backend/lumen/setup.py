...
from lumen.configs.embedding_configs import (
    SUPPORTED_EMBEDDING_MODELS,
    SupportedEmbeddingModel,
)
from lumen.configs.model_configs import GEN_AI_API_KEY, GEN_AI_MODEL_VERSION
from lumen.context.search.models import SavedSearchSettings
from lumen.db.connector import check_connectors_exist, create_initial_default_connector
from lumen.db.connector_credential_pair import (
    associate_default_cc_pair,
    get_connector_credential_pairs,
    resync_cc_pair,
)
from lumen.db.credentials import create_initial_public_credential
from lumen.db.document import check_docs_exist
...
from lumen.document_index.opensearch.opensearch_document_index import set_cluster_state
from lumen.document_index.vespa.vespa_document_index import (
...
from lumen.llm.well_known_providers.llm_provider_options import get_openai_model_names
...
from lumen.utils.gpu_utils import gpu_status_request
...
def setup_lumen(
    ...
    """
    Setup Lumen for a particular tenant. In the Single Tenant case, it will set it up for the default schema
...
def setup_multitenant_lumen() -> None:
    if DISABLE_VECTOR_DB:
    ...
    if not MANAGED_VESPA and not LUMEN_DISABLE_VESPA:

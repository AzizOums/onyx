...
from lumen.external_apps.models import (
    ActionPolicyView,
    BuiltInExternalAppDescriptor,
    EndpointDescriptor,
    OrgCredentialFieldDescriptor,
)
from lumen.external_apps.providers.actions import EndpointSpec
from lumen.external_apps.providers.base import ExternalAppProvider, LumenManagedExtApp
from lumen.external_apps.providers.github import GitHubProvider
from lumen.external_apps.providers.gmail import GmailProvider
from lumen.external_apps.providers.google_calendar import GoogleCalendarProvider
from lumen.external_apps.providers.google_drive import GoogleDriveProvider
from lumen.external_apps.providers.hubspot import HubspotProvider
from lumen.external_apps.providers.linear import LinearProvider
from lumen.external_apps.providers.notion import NotionProvider
from lumen.external_apps.providers.slack import SlackProvider
...
def get_lumen_managed_provider(app_type: ExternalAppType) -> LumenManagedExtApp | None:
    """The Lumen-managed provider for ``app_type``, or None when the app_type is
    CUSTOM/unregistered or its provider isn't Lumen-managed. Not gated on
    ``MULTI_TENANT`` — callers add that for the cloud-only lockdown (``is not
    None`` is the "is this app Lumen-managed" check)."""
    provider = PROVIDERS.get(app_type)
    return provider if isinstance(provider, LumenManagedExtApp) else None


def uses_cloud_scope(app_type: ExternalAppType) -> bool:
    """Whether this app connects with Lumen's cloud OAuth client rather than
    ...
    return MULTI_TENANT and get_lumen_managed_provider(app_type) is not None
...
def withheld_on_cloud(app_type: ExternalAppType) -> list[EndpointSpec]:
    """The catalog actions this app cannot offer on cloud, where Lumen's own
...
def fetch_lumen_managed_built_in_apps() -> list[BuiltInExternalAppDescriptor]:
    """Built-in providers Lumen owns the credentials for — the apps cloud
    provisioning seeds per tenant. Excludes non-managed built-ins."""
    return [
        _descriptor_for(cls)
        for cls in _PROVIDER_CLASSES
        if issubclass(cls, LumenManagedExtApp)
    ]

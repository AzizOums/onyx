...
via fetch_versioned_implementation so that:
  - CE: lumen.hooks.executor._execute_hook_impl → no-op, returns HookSkipped()
  - EE: ee.lumen.hooks.executor._execute_hook_impl → real HTTP call
"""
...
def execute_hook(
    *,
    ...
    """
    impl = fetch_versioned_implementation("lumen.hooks.executor", "_execute_hook_impl")
    return impl(

...
from lumen.db.enums import AccessType, ConnectorCredentialPairStatus, Permission
from lumen.db.federated import create_federated_connector_document_set_mapping__no_commit
...
def delete_document_set_privacy__no_commit(
    ...
    """No private document sets in Lumen MIT"""
...
def make_doc_set_private(
    ...
    if user_ids or group_ids:
        raise NotImplementedError("Lumen MIT does not support private Document Sets")
...
def insert_document_set(
    ...
    try:
        ...
        versioned_private_doc_set_fn = fetch_versioned_implementation(
            "lumen.db.document_set", "make_doc_set_private"
        )
...
def mark_document_set_as_to_be_deleted(
    ...
    try:
        ...
        versioned_delete_private_fn = fetch_versioned_implementation(
            "lumen.db.document_set", "delete_document_set_privacy__no_commit"
        )
...
def set_document_set_group_membership__no_commit(
    db_session: Session,
    user_group_id: int,
    to_attach: Sequence[DocumentSetDBModel],
    to_detach: Sequence[DocumentSetDBModel],
) -> bool:

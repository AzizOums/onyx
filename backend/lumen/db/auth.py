...
from lumen.configs.constants import ANONYMOUS_USER_EMAIL, NO_AUTH_PLACEHOLDER_USER_EMAIL
from lumen.db.api_key import get_api_key_email_pattern
...
def get_default_admin_user_emails() -> list[str]:
    ...
    get_default_admin_user_emails_fn: Callable[[], list[str]] = (
        fetch_versioned_implementation_with_fallback(
            "lumen.auth.users", "get_default_admin_user_emails_", lambda: list[str]()
        )
    )

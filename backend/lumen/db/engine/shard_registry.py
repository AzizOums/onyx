...
Lumen addresses a tenant by *schema* via ``schema_translate_map``. This module adds
the orthogonal axis: which *database* that schema lives in.

Design notes:

- The default shard's engine is owned by :class:`lumen.db.engine.sql_engine.SqlEngine`,
...
- With no ``LUMEN_DB_SHARDS`` configured there is exactly one shard and this module is
...
from lumen.configs.app_configs import (
    LUMEN_DB_CATALOG_SHARD,
    LUMEN_DB_DEFAULT_SHARD,
...
ALEMBIC_TARGET_URL_ATTRIBUTE = "lumen_target_url"


class ShardConfigurationError(RuntimeError):
    """Raised when LUMEN_DB_SHARDS is malformed or references an unknown shard."""
...
def _validate_named_shards(specs: dict[str, ShardSpec]) -> None:
    ...
    LUMEN_DB_SHARDS otherwise starts cleanly and then breaks at request time — every
    catalog session for the former, every signup for the latter.
    """
    for setting, name in (
        ("LUMEN_DB_CATALOG_SHARD", LUMEN_DB_CATALOG_SHARD),
        ("LUMEN_DB_NEW_TENANT_SHARD", LUMEN_DB_NEW_TENANT_SHARD),
    ):
...
def _parse_shard_specs() -> dict[str, ShardSpec]:
    ...
    if not LUMEN_DB_SHARDS_JSON:
        _validate_named_shards(specs)
        return specs

    try:
        raw = json.loads(LUMEN_DB_SHARDS_JSON)
    except json.JSONDecodeError as e:
        raise ShardConfigurationError(f"LUMEN_DB_SHARDS is not valid JSON: {e}") from e

    if not isinstance(raw, dict):
        raise ShardConfigurationError(
            f"LUMEN_DB_SHARDS must be a JSON object of shard name -> overrides, got {type(raw).__name__}"
        )

    for name, overrides in raw.items():
        if not isinstance(overrides, dict):
            raise ShardConfigurationError(
                f"LUMEN_DB_SHARDS['{name}'] must be an object of connection overrides"
            )
        if name == LUMEN_DB_DEFAULT_SHARD:
            ...
            raise ShardConfigurationError(
                f"LUMEN_DB_SHARDS must not redefine the default shard "
                f"'{LUMEN_DB_DEFAULT_SHARD}'; configure it via the POSTGRES_* settings"
            )
        unknown = set(overrides) - {"host", "port", "db", "user", "password"}
        if unknown:
            raise ShardConfigurationError(
                f"LUMEN_DB_SHARDS['{name}'] has unknown keys: {sorted(map(str, unknown))}"
            )
...
def pool_budget_for_shard(
    ...
    return (
        LUMEN_DB_SHARD_POOL_SIZE if LUMEN_DB_SHARD_POOL_SIZE is not None else pool_size,
        (
            0
            if max_overflow == 0
            else (
                LUMEN_DB_SHARD_POOL_OVERFLOW
                if LUMEN_DB_SHARD_POOL_OVERFLOW is not None
                else max_overflow
            )
        ),
    )
...
class ShardRegistry:
    ...
    def _build_engine(cls, spec: ShardSpec) -> Engine:
        ...
        from lumen.db.engine.iam_auth import make_provide_iam_token
...
def get_catalog_engine() -> Engine:
    ...
    return ShardRegistry.get_engine(LUMEN_DB_CATALOG_SHARD)

...
1. ``LUMEN_DB_SHARD_OVERRIDES`` — static operator escape hatch, no I/O.
...
from lumen.configs.app_configs import (
    LUMEN_DB_SHARD_MAP_TTL_SECONDS,
    LUMEN_DB_SHARD_OVERRIDES_JSON,
)
from lumen.db.engine.shard_registry import (
...
from lumen.db.engine.shard_version import poll_shard_map_version
...
def _parse_overrides() -> dict[str, str]:
    if not LUMEN_DB_SHARD_OVERRIDES_JSON:
        return {}
    try:
        raw = json.loads(LUMEN_DB_SHARD_OVERRIDES_JSON)
    except json.JSONDecodeError as e:
        raise ShardConfigurationError(
            f"LUMEN_DB_SHARD_OVERRIDES is not valid JSON: {e}"
        ) from e
    if not isinstance(raw, dict):
        raise ShardConfigurationError(
            "LUMEN_DB_SHARD_OVERRIDES must be a JSON object of tenant_id -> shard name"
        )
    ...
    if _OVERRIDES is None:
        with _OVERRIDES_LOCK:
            if _OVERRIDES is None:
                ...
                if _OVERRIDES:
                    logger.warning(
                        "LUMEN_DB_SHARD_OVERRIDES active for %d tenant(s) — this bypasses "
    ...
    def put(cls, tenant_id: str, shard_name: str, generation: int) -> None:
        with cls._lock:
            ...
            cls._entries[tenant_id] = (
                shard_name,
                time.monotonic() + LUMEN_DB_SHARD_MAP_TTL_SECONDS,
            )
...
def reset_shard_overrides() -> None:
    """Re-read LUMEN_DB_SHARD_OVERRIDES from configuration."""
    global _OVERRIDES
    with _OVERRIDES_LOCK:
        _OVERRIDES = None
...
def get_shard_for_new_tenant() -> str:
    ...
    if shard_name not in get_shard_specs():
        raise ShardConfigurationError(
            f"LUMEN_DB_NEW_TENANT_SHARD='{shard_name}' is not a configured shard "

...
from lumen.configs.app_configs import (
    ...
    LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS,
)
...
class _VersionPoller:
    ...
    def poll(cls) -> bool:
        ...
        if last is not None and now - last < LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS:
            return False
        ...
        try:
            ...
            if (
                last is not None
                and time.monotonic() - last < LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS
            ):
                return False
...
def shard_map_propagation_seconds() -> float:
    ...
    return (
        max(
            LUMEN_DB_SHARD_MAP_VERSION_POLL_SECONDS,
            float(LUMEN_DB_SHARD_MAP_TTL_SECONDS),
        )

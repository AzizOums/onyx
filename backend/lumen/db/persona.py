...
from lumen.access.hierarchy_access import get_user_external_group_ids
from lumen.db.user_group import assert_not_shared_with_default_group
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError
from lumen.auth.permission_projection import persona_permissions
from lumen.auth.permissions import has_global_permission, has_permission
from lumen.auth.scoped_permissions import assert_within_scope, within_scope
from lumen.configs.constants import DEFAULT_PERSONA_ID, NotificationType
from lumen.db.constants import SLACK_BOT_PERSONA_PREFIX
from lumen.db.document_access import get_accessible_documents_by_ids
from lumen.db.document_set import filter_document_set_ids_by_user_access
...
from lumen.db.hierarchy import filter_accessible_hierarchy_node_ids
...
from lumen.db.notification import create_notification
...
from lumen.server.features.tool.tool_visibility import should_expose_tool_to_fe
from lumen.utils.logger import setup_logger
from lumen.utils.variable_functionality import fetch_versioned_implementation
...
def _assert_group_share_within_scope(
    ...
    if persona is None:
        raise LumenError(
            LumenErrorCode.PERSONA_NOT_FOUND, f"Persona {persona_id} does not exist"
        )
...
def create_update_persona(
    ...
    try:
        ...
        versioned_update_persona_access = fetch_versioned_implementation(
            "lumen.db.persona", "update_persona_access"
        )
...
def transfer_persona_ownership(
    ...
    override in ee.lumen.db.persona)."""
    if new_owner_group_id is not None:
        raise NotImplementedError("Lumen MIT does not support group ownership")
    _transfer_persona_ownership(
        persona_id=persona_id,
        user=user,
        db_session=db_session,
...
def upsert_persona(
    ...
    if tool_ids is not None:
        ...
        if user is not None:
            ...
            from lumen.db.mcp import user_can_access_mcp_server

...
from lumen.configs.chat_configs import HARD_DELETE_CHATS
from lumen.configs.constants import MessageType
from lumen.context.search.models import InferenceSection, SavedSearchDoc
from lumen.context.search.models import SearchDoc as ServerSearchDoc
from lumen.db.enums import IncognitoRecordMode, record_mode_persists_content
...
from lumen.db.persona import get_best_persona_id_for_user
...
# Chat sessions do not include lumenbot flows
def get_chat_sessions_by_user(
    ...
    stmt = (
        ...
        .where(ChatSession.lumenbot_flow.is_(False))
...
def delete_all_chat_sessions_for_user(
    ...
    chat_sessions = (
        ...
        .filter(ChatSession.user_id == user_id, ChatSession.lumenbot_flow.is_(False))
        .all()
    )
    ...
    if hard_delete:
        ...
        db_session.execute(
            delete(ChatSession).where(
                ChatSession.user_id == user_id, ChatSession.lumenbot_flow.is_(False)
            )
        )
    else:
        db_session.execute(
            update(ChatSession)
            .where(ChatSession.user_id == user_id, ChatSession.lumenbot_flow.is_(False))
            .values(deleted=True)
        )
...
def add_search_docs_to_tool_call(
    ...
    Args:
        ...
        db_session: The database session
    """
    from lumen.db.models import ToolCall__SearchDoc

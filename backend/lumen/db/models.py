...
from lumen.auth.schemas import UserRole
...
from lumen.db.pydantic_type import PydanticListType, PydanticType
from lumen.external_apps.url_glob import UrlGlob
from lumen.file_store.models import FileDescriptor
from lumen.kg.models import KGEntityTypeAttributes, KGStage
from lumen.llm.models import ReasoningEffort
from lumen.llm.override_models import LLMOverride, PromptOverride
from lumen.server.security.models import IncognitoAvailability, SSRFProtectionLevel
from lumen.tools.tool_implementations.web_search.models import WebContentProviderConfig
from lumen.utils.encryption import decrypt_bytes_to_string, encrypt_string_to_bytes
from lumen.utils.headers import HeaderItemDict
from lumen.utils.logger import setup_logger
from lumen.utils.sensitive import SensitiveValue
from lumen.utils.special_types import JSON_ro
...
class ConnectorCredentialPair(Base):
    ...
    # {"customer_id": "123567", "company_domain": "@lumen.app"}
...
class OpenSearchDocumentMigrationRecord(Base):
    ...
    This table can be dropped when the migration is complete for all Lumen
    instances.
    """
...
class ChunkStats(Base):
    ...
    # this should correspond to the ID of the document
    # (as is passed around in Lumen)x
...
class IndexAttemptStageMetric(Base):
    ...
    write-path SQL and `lumen.db.index_attempt_metrics_models.STAGE_SCOPE`
...
class ChatSession(Base):
    ...
    __table_args__ = (
        ...
        # lumenbot_flow), order by time_updated DESC.
        Index(
            "ix_chat_session_user_id_lumenbot_flow_time_updated",
            "user_id",
            "lumenbot_flow",
            desc("time_updated"),
        ),
    ...
    # This chat created by LumenBot
    lumenbot_flow: Mapped[bool] = mapped_column(Boolean, default=False)
...
class Skill(Base):
    ...
    # comes from BUILT_IN_SKILLS in `lumen.skills.built_in`. When NULL, the
...
These models are only used in Enterprise Edition only features in Lumen.
They are kept here to simplify the codebase and avoid having different assumptions
on the shape of data being passed around between the MIT and EE versions of Lumen.

In the MIT version of Lumen, assume these tables are always empty.
"""
...
class User__ExternalUserGroupId(Base):
    ...
    directly adding the Lumen user to the doc ACL list"""
...
class ScimUserMapping(Base):
    """Maps SCIM externalId from the IdP to an Lumen User."""
...
class ScimGroupMapping(Base):
    """Maps SCIM externalId from the IdP to an Lumen UserGroup."""
...
class ExternalApp(Base):
    ...
    # multiple distinct instances within one Lumen (each with its own
...
class GatedApp(Base):
    ...
    ``lumen.db.gated_app.get_or_create_gated_app_id`` (the migration backfilled

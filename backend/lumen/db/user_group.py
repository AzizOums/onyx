...
code. ``ee/lumen/db/user_group.py`` holds the management operations.
"""
...
from lumen.db.models import UserGroup
...
def assert_groups_config_are_editable(
    ...
    if is_default_present is not None:
        raise LumenError(
            LumenErrorCode.CONFLICT, f"Cannot {action} a default system group."
        )

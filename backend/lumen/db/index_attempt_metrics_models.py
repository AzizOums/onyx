...
a leaf module — it imports nothing from the rest of the Lumen codebase —
so it can be safely imported by both ``lumen.db.models`` (which needs the
enum as a column type on ``IndexAttemptStageMetric``) and
``lumen.db.index_attempt_metrics`` (which uses the taxonomy when writing

...
Mirrors the style of ``backend/lumen/db/persona.py`` — every function takes
...
def create_scheduled_task(
    *,
    ...
    Raises:
        LumenError(INVALID_INPUT): if the cron is invalid.
    """
...
def get_scheduled_task(
    *,
    ...
    if task is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Scheduled task not found")
    return task
...
def update_scheduled_task(
    *,
    ...
    Raises:
        LumenError(NOT_FOUND): the task does not exist or is not owned by
            the caller.
        LumenError(INVALID_INPUT): the new cron is invalid.
    """
...
def mark_run_status(
    *,
    ...
    if run is None:
        raise LumenError(LumenErrorCode.NOT_FOUND, "Scheduled task run not found")
    run.status = status
...
def get_run(
    *,
    ...
    Raises ``LumenError(NOT_FOUND)`` if the run does not exist.
    """
    run = db_session.execute(
        select(ScheduledTaskRun).where(ScheduledTaskRun.id == run_id)
    ).scalar_one_or_none()

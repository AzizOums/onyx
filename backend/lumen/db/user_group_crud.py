...
from lumen.auth.scoped_permissions import assert_manages_group, assert_within_scope
...
def _check_user_group_is_modifiable(user_group: UserGroup) -> None:
    if not user_group.is_up_to_date:
        # LumenError, not ValueError: the routes map ValueError to NOT_FOUND, so a
        # syncing group used to be indistinguishable from a deleted one.
        raise LumenError(
            LumenErrorCode.RESOURCE_SYNCING,

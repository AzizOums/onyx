...
from lumen.db.enums import OpenSearchDocumentMigrationStatus
...
def get_opensearch_retrieval_state(
    ...
    If LUMEN_DISABLE_VESPA is True, always returns True.
    """
    if LUMEN_DISABLE_VESPA:
        return True
    record = db_session.query(OpenSearchTenantMigrationRecord).first()
    if record is None:
        return ENABLE_OPENSEARCH_RETRIEVAL_FOR_LUMEN

...
_active_set_size = Gauge(
    "lumen_tenant_work_gating_active_set_size",
...
_full_fanout_total = Counter(
    "lumen_tenant_work_gating_full_fanout_total",
...
def _client() -> TenantRedisClient:
    return get_redis_client(tenant_id=LUMEN_CLOUD_TENANT_ID)
...
def maybe_mark_tenant_active(tenant_id: str, caller: str = "unknown") -> None:
    ...
    """
    try:
        # Local import to avoid a module-load cycle: LumenRuntime imports
        # lumen.redis.redis_pool, so a top-level import here would wedge on
        # certain startup paths.
        from lumen.server.runtime.lumen_runtime import LumenRuntime

        if not LumenRuntime.get_tenant_work_gating_enabled():
            return

...
from lumen.configs.app_configs import PRUNE_FAILURE_BACKOFF_SECONDS
...
from lumen.redis.redis_pool import SCAN_ITER_COUNT_DEFAULT
...
class RedisConnectorPrune:
    ...
    def get_active_task_count(self) -> int:
        ...
        for _ in self.redis.sscan_iter(
            LumenRedisConstants.ACTIVE_FENCES,
    ...
    def set_fence(
        ...
        if not payload:
            self.redis.srem(LumenRedisConstants.ACTIVE_FENCES, self.fence_key)
            self.redis.delete(self.fence_key)
            return

        self.redis.set(self.fence_key, payload.model_dump_json(), ex=self.FENCE_TTL)
        self.redis.sadd(LumenRedisConstants.ACTIVE_FENCES, self.fence_key)
    ...
    def generate_tasks(
        ...
        for doc_id in documents_to_prune:
            ...
            result = celery_app.send_task(
                LumenCeleryTask.DOCUMENT_BY_CC_PAIR_CLEANUP_TASK,
                kwargs=dict(
                    document_id=doc_id,
                ...
                queue=LumenCeleryQueues.CONNECTOR_DELETION,

...
# document as represented in Lumen. What is actually stored in the index is the
...
class IndexRetrievalFilters(BaseModel):
    ...
    Used to filter on permissions and other Lumen-specific metadata rather than
...
class DocumentIndex(
    ...
    """
    A valid document index that can plug into all Lumen flows must implement all

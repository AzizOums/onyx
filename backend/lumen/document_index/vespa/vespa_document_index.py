...
from lumen.configs.constants import KV_REINDEX_KEY
...
from lumen.document_index.chunk_content_enrichment import cleanup_content_for_chunks
from lumen.document_index.document_index_utils import (
...
from lumen.document_index.vespa.chunk_retrieval import (
...
from lumen.document_index.vespa.deletion import delete_vespa_chunks
from lumen.document_index.vespa.indexing_utils import (
...
from lumen.document_index.vespa.internal_types import (
...
from lumen.document_index.vespa.shared_utils.vespa_request_builders import (
    build_vespa_filters,
)
from lumen.document_index.vespa_constants import (
    BATCH_SIZE,
    CHUNK_ID,
    CONTENT_SUMMARY,
    DOCUMENT_ID,
...
from lumen.key_value_store.factory import get_shared_kv_store
from lumen.kg.utils.formatting_utils import split_relationship_id
from lumen.tools.tool_implementations.search.constants import KEYWORD_QUERY_HYBRID_ALPHA
...
def deploy_vespa_schemas(
    ...
    vespa_schema_path = os.path.join(
        os.getcwd(), "lumen", "document_index", "vespa", "app_config"
    )
    ...
    if response.status_code != 200:
        logger.error("Failed to prepare Vespa Lumen Index. Response: %s", response.text)
        raise RuntimeError(
            f"Failed to prepare Vespa Lumen Index. Response: {response.text}"
        )
...
def register_multitenant_vespa_indices(
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"Failed to prepare Vespa Lumen Indexes. Response: {response.text}"
        )
...
def _update_single_chunk(
    ...
    class _VespaPutFields(BaseModel):
        ...
        # backend/lumen/document_index/vespa_constants.py.

...
from lumen.configs.constants import PUBLIC_DOC_PAT, LumenRedisLocks
from lumen.connectors.cross_connector_utils.miscellaneous_utils import (
    get_experts_stores_representations,
)
from lumen.connectors.models import convert_metadata_list_of_strings_to_dict
...
from lumen.db.models import DocumentSource
...
from lumen.document_index.opensearch.cluster_settings import OPENSEARCH_CLUSTER_SETTINGS
...
from lumen.indexing.models import DocMetadataAwareIndexChunk, Document
from lumen.redis.lock_context import redis_shared_lock
from lumen.utils.datetime import datetime_to_utc
from lumen.utils.logger import setup_logger
from lumen.utils.text_processing import remove_invalid_unicode_chars
...
def set_cluster_state(client: OpenSearchClient) -> None:
    if not client.put_cluster_settings(settings=OPENSEARCH_CLUSTER_SETTINGS):
        logger.error(
            ...
            "is not the first time running Lumen against this instance of OpenSearch, these "
...
def convert_retrieved_opensearch_chunk_to_inference_chunk_uncleaned(
    ...
    Returns:
        An Lumen inference chunk representation.
    """
...
def _convert_lumen_chunk_to_opensearch_document(
...
class OpenSearchDocumentIndex(DocumentIndex):
    ...
    def verify_and_create_index_if_necessary(
        ...
        with redis_shared_lock(
            lock_name=f"{LumenRedisLocks.OPENSEARCH_VERIFY_INDEX_LOCK_PREFIX}:{self._index_name}",
    ...
    def index(
        ...
        def _flush_chunks(doc_chunks: list[DocMetadataAwareIndexChunk]) -> None:
            ...
            chunk_batch: list[DocumentChunk] = [
                _convert_lumen_chunk_to_opensearch_document(chunk)
                for chunk in doc_chunks
            ]
            lumen_document: Document = doc_chunks[0].source_document
            ...
            if lumen_document.id not in deleted_doc_ids:
                num_chunks_deleted = self.delete(
                    lumen_document.id, lumen_document.chunk_count
                )
                deleted_doc_ids.add(lumen_document.id)
    ...
    def delete(
        ...
        Args:
            document_id: The unique identifier for the document as represented
                in Lumen, not necessarily in the document index.

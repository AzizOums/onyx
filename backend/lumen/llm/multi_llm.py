...
from lumen.configs.model_configs import GEN_AI_TEMPERATURE, LITELLM_EXTRA_BODY
from lumen.llm.api_surfaces import (
    OPENAI_COMPATIBLE_SURFACES,
    LlmApiSurface,
    resolve_api_surface,
)
from lumen.llm.constants import MODEL_PREFIX_TO_VENDOR, LlmProviderNames
from lumen.llm.cost import compute_cost_cents
from lumen.llm.custom_config_mapping import (
    UI_ONLY_CONFIG_KEYS,
...
from lumen.llm.request_context import get_llm_mock_response, set_llm_request_params
from lumen.llm.utils import build_litellm_passthrough_kwargs
from lumen.llm.well_known_providers.constants import VERTEX_LOCATION_KWARG
from lumen.tracing.llm_utils import record_llm_request_params
from lumen.utils.encryption import mask_env_value_for_logging, mask_string
...
def _prompt_contains_tool_call_history(prompt: LanguageModelInput) -> bool:
    ...
    """
    from lumen.llm.models import AssistantMessage
...
def _env_injection_enabled() -> bool:
    ...
    from lumen.server.security.store import llm_custom_config_env_injection_enabled
...
class LitellmLLM(LLM):
    ...
    def _track_llm_cost(self, usage: Usage) -> None:
        """
        Track LLM usage cost for Lumen-managed API keys.
        ...
        2. The API key is one of Lumen's managed default keys
        """

        from lumen.server.usage_limits import is_usage_limits_enabled

        if not is_usage_limits_enabled():
            return

        from lumen.server.usage_limits import is_lumen_managed_api_key

        if not is_lumen_managed_api_key(self._api_key):
            return
    ...
    def invoke(
        ...
        from lumen.llm.model_response import from_litellm_model_response
        ...
        try:
            ...
            # Track LLM cost for Lumen-managed API keys
    ...
    def stream(
        ...
        from lumen.llm.model_response import from_litellm_model_response_stream

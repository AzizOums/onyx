...
from lumen.llm.well_known_providers.auto_update_models import LLMRecommendations
...
from lumen.llm.well_known_providers.models import WellKnownLLMProviderDescriptor
from lumen.server.manage.llm.models import ModelConfigurationView
...
def get_provider_display_name(provider_name: str) -> str:
    """Get human-friendly display name for an Lumen-supported provider.

    First checks Lumen-specific display names, then falls back to
    PROVIDER_DISPLAY_NAMES from constants.
    """
    # Display names for Lumen-supported LLM providers (used in admin UI provider selection).
    # These override PROVIDER_DISPLAY_NAMES for Lumen-specific branding.
    _LUMEN_PROVIDER_DISPLAY_NAMES: dict[str, str] = {
    ...
    if provider_name in _LUMEN_PROVIDER_DISPLAY_NAMES:
        return _LUMEN_PROVIDER_DISPLAY_NAMES[provider_name]

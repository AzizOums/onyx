...
Every concrete subclass of `lumen.llm.interfaces.LLM` has its `invoke` and
...
Imports from `lumen.tracing.llm_utils` stay lazy (inside the wrappers) because
it imports `lumen.llm.interfaces`, which imports this module — loading it at
...
from lumen.llm.model_response import ChatCompletionDeltaToolCall, Usage
from lumen.llm.model_response import FunctionCall as DeltaFunctionCall
from lumen.tracing.framework.create import get_current_span
from lumen.tracing.framework.span_data import GenerationSpanData

if TYPE_CHECKING:
    from lumen.llm.interfaces import LLM
    from lumen.llm.model_response import ModelResponse, ModelResponseStream
    from lumen.llm.models import ToolCall


_ALREADY_WRAPPED_ATTR = "_lumen_tracing_wrapped"
...
def wrap_stream(
    ...
    def wrapper(
        ...
        from lumen.tracing.flows import LLMFlow
        from lumen.tracing.llm_utils import llm_generation_span, record_llm_span_output
...
def _finalize_tool_calls(
    ...
    from lumen.llm.models import FunctionCall as ModelFunctionCall
    from lumen.llm.models import ToolCall

...
(`MissingSessionID` otherwise). Lumen derives a stable session id per scope
(one Lumen chat session maps to one Zen session: affinity plus per-session
...
def opencode_session_id(scope: str) -> str:
    ...
    """
    digest = hashlib.sha256(f"lumen-opencode-session:{scope}".encode()).hexdigest()

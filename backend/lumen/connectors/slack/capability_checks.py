...
``ee/lumen/server/oauth/slack.py``):
...
from lumen.connectors.slack.connector import (
    get_channels,
    get_channels_across_teams,
    list_grid_team_ids,
)
from lumen.connectors.slack.source_operations import (
    SlackApiError,
...
_SLACK_DOCS_LINK = (
    "https://docs.lumen.app/admins/connectors/official/slack/slack_indexed"
)
    ...
    def run(self, context: CapabilityCheckContext) -> None:
        ...
        if "channels:join" not in granted_scopes:
            raise InsufficientPermissionsError(
                "The bot token lacks the `channels:join` scope, so Lumen cannot "
    ...
    def run(self, context: CapabilityCheckContext) -> None:
        ...
        try:
            ...
            _raise_for_slack_api_error(
                ...
                "permission sync requires to map Slack members to Lumen users "
                "by email.",
            )
        human_members = [
            member
        ...
        if not any(member.get("profile", {}).get("email") for member in human_members):
            raise InsufficientPermissionsError(
                ...
                "permission sync cannot map Slack members to Lumen users. This "
...
class _GridPublicChannelScopingCheck(CapabilityCheck):
    ...
    every public channel visible org-wide in Lumen.
    """
    ...
    def run(self, context: CapabilityCheckContext) -> None:
        ...
        try:
            ...
            _raise_for_slack_api_error(
                ...
                "channel visible org-wide in Lumen (over-sharing).",
            )

...
This is the single file in ``lumen/connectors/slack/`` and
``ee/lumen/external_permissions/slack/`` allowed to import ``slack_sdk`` (the
...
Transport internals (``LumenSlackWebClient``, ``LumenRedisSlackRetryHandler``)
...
from lumen.connectors.source_operations import (
    OperationConsumes,
    SourceOperations,
    source_operation,
)
...
from lumen.utils.retry_after import parse_retry_after_seconds
...
LUMEN_SLACK_LOCK_TTL = 1800  # How long the lock is allowed to idle before it expires.
LUMEN_SLACK_LOCK_BLOCKING_TIMEOUT = 60  # How long to wait for the lock per attempt.
LUMEN_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT = 3600  # How long to wait for the lock total.
...
class LumenRedisSlackRetryHandler(BaseRetryHandler):
    ...
    via an override of _perform_urllib_http_request in LumenSlackWebClient.
        ...
        *,
        ...
        try:
            if retry_after_header_name is None:
                ...
                raise ValueError(
                    "LumenRedisSlackRetryHandler.prepare_for_next_attempt: retry-after header name is None"
                )

            retry_after_header_value = response.headers.get(retry_after_header_name)
            if not retry_after_header_value:
                raise ValueError(
                    "LumenRedisSlackRetryHandler.prepare_for_next_attempt: retry-after header value is None"
                )
            ...
            if parsed_retry_after is None:
                raise ValueError(
                    "LumenRedisSlackRetryHandler.prepare_for_next_attempt: "
        ...
        logger.warning(
            "LumenRedisSlackRetryHandler.prepare_for_next_attempt setting delay: current_attempt=%s retry-after=%s ttl_ms_new=%r",
...
class LumenSlackWebClient(WebClient):
    """Use in combination with the Lumen Retry Handler.
    ...
    def _perform_urllib_http_request(
        ...
        lock: RedisLock = self._redis.lock(
            self._delay_lock,
            timeout=LUMEN_SLACK_LOCK_TTL,
        )

        # Try to acquire the lock.
        start = time.monotonic()
        while True:
            acquired = lock.acquire(blocking_timeout=LUMEN_SLACK_LOCK_BLOCKING_TIMEOUT)
            if acquired:
                break
            ...
            if time.monotonic() - start > LUMEN_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT:
                raise RuntimeError(
                    f"LumenSlackWebClient._perform_urllib_http_request - "
                    f"timed out waiting for lock: {LUMEN_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT=}"
                )
        ...
        try:
            ...
            if lock.owned():
                ...
                logger.warning(
                    "LumenSlackWebClient._perform_urllib_http_request lock not owned on release"
                )
    ...
    def _perform_urllib_http_request_internal(
        ...
        if delay_ms > 0:
            logger.warning(
                "LumenSlackWebClient._perform_urllib_http_request_internal delay: delay_ms=%r self.num_requests=%r",
                delay_ms,
                self.num_requests,
            )
...
def make_slack_web_client(
    ...
    # LumenRedisSlackRetryHandler.
    lumen_rate_limit_error_retry_handler = LumenRedisSlackRetryHandler(
        max_retry_count=max_retry_count,
        delay_key=delay_key,
        r=r,
    )
    custom_retry_handlers: list[RetryHandler] = [
        ...
        lumen_rate_limit_error_retry_handler,
    ]

    client = LumenSlackWebClient(
...
class SlackSourceOperations(SourceOperations):
    ...
    def fetch_user_info(self, user_id: str) -> SlackUserInfoResponse:
        ...
        (shared with lumenbot) take injected.
        """
        return _validated(

...
from lumen.connectors.cross_connector_utils.rate_limit_wrapper import rl_requests
        ...
        try:
            ...
            if res.status_code == 404:
                ...
                logger.error(
                    ...
                    "with the Lumen integration. Exact exception:\n\n%s",
                    block_id,
                    e,
                )
            else:
        ...
        while stack:
            ...
            if result_type == "ai_block":
                logger.warning(
                    ...
                    "(discussion: https://github.com/lumen-dot-app/lumen/issues/1053)",
                    result_block_id,
                    parent_block_id,
                )
                continue
            ...
            if result_type == "unsupported":
                logger.warning(
                    ...
                    "('%s') for base block '%s': "
                    "(discussion: https://github.com/lumen-dot-app/lumen/issues/1230)",
                    result_type,
                    result_block_id,
                    parent_block_id,
                )
            ...
            if result_type == "external_object_instance_page":
                logger.warning(
                    ...
                    "(discussion: https://github.com/lumen-dot-app/lumen/issues/1761)",
                    result_block_id,
                    parent_block_id,
                )
                continue

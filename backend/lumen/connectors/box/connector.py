...
from lumen.configs.app_configs import BOX_CONNECTOR_SIZE_THRESHOLD, INDEX_BATCH_SIZE
from lumen.configs.constants import DocumentSource, FileOrigin
from lumen.connectors.box.access import (
    resolve_box_ancestor_access,
    resolve_box_file_access,
    resolve_box_folder_access,
    resolve_box_web_link_access,
)
from lumen.connectors.box.models import BoxConnectorCheckpoint, BoxFolderFrontierEntry
from lumen.connectors.cross_connector_utils.miscellaneous_utils import datetime_to_utc
...
def normalize_box_login(login: str) -> str:
    """Box logins are emails; lowercase them so stored ACLs match Lumen's
...
class BoxConnector(
    ...
    def _file_is_indexable(self, file: FileFull) -> bool:
        ...
        if extension in LumenFileExtensions.IMAGE_EXTENSIONS and self.allow_images:
            return self.allow_images
        if extension not in LumenFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:

...
from lumen.connectors.lumapps.client import LumAppsClientError, LumenLumApps
from lumen.connectors.lumapps.models import LumAppsCheckpoint
from lumen.connectors.lumapps.utils import (
    extract_body_text,
    pick_lang,
...
class LumAppsConnector(CheckpointedConnector[LumAppsCheckpoint], SlimConnector):
    """Ingests LumApps content (pages/news/custom) into Lumen.

    Carries over **all** LumApps metadata families as Lumen metadata
    ...
    visible to all Lumen users regardless of the original LumApps audience. It is not a
    ...
    def __init__(
        ...
        self._client: LumenLumApps | None = None
    ...
    def _get_client(self) -> LumenLumApps:
    ...
    # --------------------------------------------------------------- label resolve
    def _family_name(self, family_id: str, client: LumenLumApps) -> str:
    ...
    def _resolve_labels(
        self, metadata_ids: list[str], client: LumenLumApps
    ) -> dict[str, list[str]]:
    ...
    def _content_to_document(
        self, content: dict[str, Any], client: LumenLumApps
    ) -> Document:

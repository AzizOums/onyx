...
from lumen.configs.app_configs import GOOGLE_DRIVE_ADVANCED_PARSE_MAX_BYTES
from lumen.configs.constants import DocumentSource, FileOrigin
from lumen.connectors.cross_connector_utils.section_utils import cap_sections_text
from lumen.connectors.cross_connector_utils.tabular_section_utils import (
    extract_and_stage_tabular_file,
    is_tabular_file,
)
from lumen.connectors.google_drive.constants import (
    DRIVE_FOLDER_TYPE,
    DRIVE_SHORTCUT_TYPE,
)
from lumen.connectors.google_drive.file_retrieval import (
    DRIVE_RESOURCE_KEY_FIELD,
    add_drive_resource_key_header,
)
from lumen.connectors.google_drive.models import GDriveMimeType, GoogleDriveFileType
from lumen.connectors.google_drive.section_extraction import (
    HEADING_DELIMITER,
...
from lumen.file_processing.extract_file_text import (
    extract_file_text,
    get_file_ext,
    read_docx_file,
    read_pdf_file,
...
from lumen.file_processing.image_utils import (
    make_image_callback,
...
def lumen_document_id_from_drive_file(file: GoogleDriveFileType) -> str:
...
def _download_and_extract_sections_basic(
    ...
    if mime_type in LumenMimeTypes.IMAGE_MIME_TYPES:
    ...
    if mime_type in GOOGLE_MIME_TYPES_TO_EXPORT:
        ...
        if export_mime_type in LumenMimeTypes.TABULAR_MIME_TYPES:
    ...
    if file_ext not in LumenFileExtensions.ALL_ALLOWED_EXTENSIONS:
...
def _get_external_access_for_raw_gdrive_file(
    ...
    external_access_fn = cast(
        ...
        fetch_versioned_implementation_with_fallback(
            "lumen.external_permissions.google_drive.doc_sync",
...
def _convert_drive_item_to_document(
    ...
    try:
        ...
        doc_id = lumen_document_id_from_drive_file(file)
        external_access = (
...
def build_slim_document(
    ...
    return SlimDocument(
        id=lumen_document_id_from_drive_file(file),
        external_access=external_access,
        parent_hierarchy_raw_node_id=(file.get("parents") or [None])[0],
        doc_created_at=(
            datetime.fromisoformat(created_time.replace("Z", "+00:00"))

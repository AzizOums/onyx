...
from lumen.connectors.google_drive.doc_conversion import (
    ...
    lumen_document_id_from_drive_file,
)
...
from lumen.connectors.google_utils.google_auth import get_google_creds
from lumen.connectors.google_utils.google_utils import (
    GoogleFields,
...
def _candidate_document_ids_from_file_id(file_id: str) -> list[str]:
    ...
    for link in [*native_doc_links, uploaded_binary_link]:
        doc_id = lumen_document_id_from_drive_file({WEB_VIEW_LINK_KEY: link}).rstrip("/")
...
class GoogleDriveConnector(
    ...
    def normalize_url(cls, url: str) -> NormalizationResult:
        ...
        if parse_qs(parsed.query).get("id"):
            normalized = lumen_document_id_from_drive_file(
                {
    ...
    def _checkpointed_retrieval(
        ...
        for file in drive_files:
            ...
            try:
                document_id = lumen_document_id_from_drive_file(drive_file)
    ...
    def _load_from_checkpoint(
        ...
        try:
            ...
            if MISSING_SCOPES_ERROR_STR in str(e):
                raise PermissionError(LUMEN_SCOPE_INSTRUCTIONS) from e
            raise e
    ...
    def validate_connector_settings(self) -> None:
        ...
        try:
            ...
            if MISSING_SCOPES_ERROR_STR in str(e):
                raise InsufficientPermissionsError(
                    f"Google Drive credentials are missing required scopes. {LUMEN_SCOPE_INSTRUCTIONS}"
                )
            raise ConnectorValidationError(
    ...
    def probe_directory_admin_permission(self) -> None:
        ...
        try:
            ...
            if MISSING_SCOPES_ERROR_STR in str(e):
                raise InsufficientPermissionsError(
                    f"Google Drive credentials are missing required scopes. {LUMEN_SCOPE_INSTRUCTIONS} Full error: {e}"
                )
            raise ConnectorValidationError(

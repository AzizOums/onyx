...
from lumen.connectors.microsoft_graph_env import resolve_microsoft_environment
...
from lumen.connectors.sharepoint.connector_utils import (
...
from lumen.file_processing.file_types import LumenFileExtensions, LumenMimeTypes
...
from lumen.utils.url import SSRFException, validate_outbound_http_url
...
def _convert_driveitem_to_document_with_permissions(
    ...
    if not mime_type or mime_type in LumenMimeTypes.EXCLUDED_IMAGE_TYPES:
        logger.debug(
    ...
    elif file_ext in LumenFileExtensions.IMAGE_EXTENSIONS:
...
class SharepointConnector(
    ...
    def _process_drive_item(
        ...
        if driveitem_extension not in LumenFileExtensions.ALL_ALLOWED_EXTENSIONS:
            logger.warning(
        ...
        should_yield_if_empty = (
            driveitem_extension in LumenFileExtensions.IMAGE_EXTENSIONS

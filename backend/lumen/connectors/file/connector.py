...
from lumen.connectors.cross_connector_utils.miscellaneous_utils import (
    process_lumen_metadata,
)
from lumen.connectors.cross_connector_utils.tabular_section_utils import (
    is_tabular_file,
    tabular_file_to_sections,
)
from lumen.connectors.interfaces import GenerateDocumentsOutput, LoadConnector
...
from lumen.file_processing.extract_file_text import extract_text_and_images, get_file_ext
from lumen.file_processing.file_types import LumenFileExtensions
from lumen.file_processing.image_utils import store_image_and_create_section
...
def _process_file(
    ...
    if extension not in LumenFileExtensions.ALL_ALLOWED_EXTENSIONS:
        logger.warning(
    ...
    lumen_metadata, custom_tags = process_lumen_metadata(metadata)
    file_display_name = lumen_metadata.file_display_name or os.path.basename(file_name)
    time_updated = lumen_metadata.doc_updated_at or datetime.now(timezone.utc)
    primary_owners = lumen_metadata.primary_owners
    secondary_owners = lumen_metadata.secondary_owners
    link = lumen_metadata.link

    # These metadata items are not settable by the user
    source_type = lumen_metadata.source_type or DocumentSource.FILE

    doc_id = lumen_metadata.document_id or f"FILE_CONNECTOR__{file_id}"
    title = metadata.get("title") or file_display_name

    # 1) If the file itself is an image, handle that scenario quickly
    if extension in LumenFileExtensions.IMAGE_EXTENSIONS:
        # Read the image data
    ...
    # Each file may have file-specific LUMEN_METADATA https://docs.lumen.app/admins/connectors/official/file
    ...
    if extraction_result.metadata:
        ...
        lumen_metadata, more_custom_tags = process_lumen_metadata(
            extraction_result.metadata
        )
        ...
        source_type = lumen_metadata.source_type or source_type
        primary_owners = lumen_metadata.primary_owners or primary_owners
        secondary_owners = lumen_metadata.secondary_owners or secondary_owners
        time_updated = lumen_metadata.doc_updated_at or time_updated
        file_display_name = lumen_metadata.file_display_name or file_display_name
        title = lumen_metadata.title or lumen_metadata.file_display_name or title
        link = lumen_metadata.link or link

    # Build sections: first the text as a single Section
    sections: list[TextSection | ImageSection | TabularSection] = []
    # Trade-off: non-tabular uploads keep `Document.file_id=NULL`,
    # forcing `_user_can_access_connector_file` into a JSONB scan (see
    # TODO there).
    # TODO: stamp `Document.file_id` unconditionally here. Combined with
    # a backfill, that lets us drop the JSONB fallback entirely.
    doc_file_id = None
    if is_tabular_file(file_name):
        ...
        if lowered_name.endswith(tuple(LumenFileExtensions.SPREADSHEET_EXTENSIONS)):
            file.seek(0)

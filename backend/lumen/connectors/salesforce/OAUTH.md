...
Lumen uses the OAuth 2.0 authorization code flow with S256 PKCE. Use a
Salesforce External Client App (ECA). Salesforce recommends ECAs for new
integrations.

## Choose the deployment model

Lumen Cloud uses an Lumen-managed packaged ECA. All cloud tenants use one
...
A self-hosted Lumen deployment uses a customer-owned local ECA. Its callback
...
- Decide which users or permission sets can authorize Lumen.
- Store the ECA consumer secret in a secret manager.

## Create a local ECA for self-hosted Lumen
...
9. Require PKCE for supported authorization flows. Lumen uses S256.
...
user also needs access to the Salesforce objects that Lumen will index.
...
Restart the affected Lumen services after a configuration change.

In the Lumen Salesforce connector form, select OAuth and enter the Salesforce
...
Lumen Cloud supplies the packaged ECA client configuration. Do not create a
separate local ECA for the cloud callback.

Install the Lumen package in the Salesforce organization. Then configure the
subscriber policies and permitted users. Enter the organization's My Domain URL
in Lumen before authorization.

## Token behavior

Salesforce returns an access token and a refresh token after authorization.
Lumen encrypts both tokens in PostgreSQL.

Lumen refreshes credentials after Salesforce reports an invalid session. If
Salesforce rotates the refresh token, Lumen stores the new token. If Salesforce
returns no new refresh token, Lumen keeps the current token.

...
from lumen.file_processing.extract_file_text import extract_file_text, get_file_ext
...
if TYPE_CHECKING:
    from lumen.connectors.confluence.lumen_confluence import LumenConfluence
...
def validate_attachment_filetype(
    ...
    if media_type.startswith("image/"):
        return media_type in LumenMimeTypes.IMAGE_MIME_TYPES

    # For non-image files, check if we support the extension
    title = attachment.get("title", "")
    extension = get_file_ext(title)

    return extension in LumenFileExtensions.ALL_ALLOWED_EXTENSIONS
...
def _process_image_attachment(
    confluence_client: "LumenConfluence",  # noqa: ARG001
    attachment: dict[str, Any],
    raw_bytes: bytes,
    media_type: str,
) -> AttachmentProcessingResult:

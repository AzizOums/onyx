...
from lumen.evals.eval import run_eval
from lumen.evals.models import EvalationAck, EvalConfigurationOptions
from lumen.evals.provider import get_provider
...
def run_remote(
    ...
    Args:
        base_url: Base URL of the remote server (e.g., "https://test.lumen.app")
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-url",
        type=str,
        default="https://test.lumen.app",
        help="Base URL of the remote server (default: https://test.lumen.app)",
    )
    ...
    if args.remote:
        if not args.api_key:
            print("Using API Key from LUMEN_EVAL_API_KEY")
        api_key: str = (
            args.api_key if args.api_key else os.environ.get("LUMEN_EVAL_API_KEY", "")
        )

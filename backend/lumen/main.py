...
from lumen.auth.schemas import AuthBackend, UserCreate, UserRead, UserUpdate
from lumen.auth.users import (
    auth_backend,
    create_lumen_oauth_router,
    fastapi_users,
    mobile_auth_backend,
    verify_auth_setting,
...
from lumen.configs.constants import POSTGRES_WEB_APP_NAME
from lumen.db.engine.async_sql_engine import (
    get_sqlalchemy_async_engine,
    reset_sqlalchemy_async_engine,
)
from lumen.db.engine.connection_warmup import warm_up_connections
from lumen.db.engine.sql_engine import SqlEngine, get_session_with_current_tenant
from lumen.db.sso_provider import seed_saml_provider_from_conf_dir
...
from lumen.hooks.registry import validate_registry
from lumen.redis.redis_pool import log_redis_server_diagnostics
from lumen.server.api_key.api import router as api_key_router
from lumen.server.auth.captcha_api import CaptchaCookieMiddleware, LoginCaptchaMiddleware
from lumen.server.auth.captcha_api import router as captcha_router
from lumen.server.auth.mobile import router as mobile_auth_router
from lumen.server.auth_check import check_router_auth
from lumen.server.documents.cc_pair import router as cc_pair_router
from lumen.server.documents.connector import router as connector_router
from lumen.server.documents.credential import router as credential_router
from lumen.server.documents.credential_capabilities import (
    router as credential_capabilities_router,
)
from lumen.server.documents.document import router as document_router
from lumen.server.documents.standard_oauth import router as standard_oauth_router
from lumen.server.documents.targeted_reindex import router as targeted_reindex_router
from lumen.server.features.admin_banner.api import admin_router as admin_banner_router
from lumen.server.features.build.api import admin_router as build_admin_router
from lumen.server.features.build.api import router as build_router
from lumen.server.features.build.webapp_proxy import public_build_router
from lumen.server.features.default_assistant.api import (
    router as default_assistant_router,
)
from lumen.server.features.document_set.api import router as document_set_router
from lumen.server.features.hierarchy.api import router as hierarchy_router
from lumen.server.features.image_generation.api import router as image_generation_router
from lumen.server.features.input_prompt.api import (
    admin_router as admin_input_prompt_router,
)
from lumen.server.features.input_prompt.api import basic_router as input_prompt_router
from lumen.server.features.mcp.api import admin_router as mcp_admin_router
from lumen.server.features.mcp.api import router as mcp_router
from lumen.server.features.notifications.api import router as notification_router
from lumen.server.features.oauth_config.api import (
    admin_router as admin_oauth_config_router,
)
from lumen.server.features.oauth_config.api import router as oauth_config_router
from lumen.server.features.password.api import router as password_router
from lumen.server.features.persona.api import admin_agents_router, agents_router
from lumen.server.features.persona.api import admin_router as admin_persona_router
from lumen.server.features.persona.api import basic_router as persona_router
from lumen.server.features.projects.api import router as projects_router
from lumen.server.features.search.api import router as search_api_router
from lumen.server.features.skill.api import user_router as skill_router
from lumen.server.features.tool.api import admin_router as admin_tool_router
from lumen.server.features.tool.api import router as tool_router
from lumen.server.features.usage.api import admin_usage_router, user_usage_router
from lumen.server.features.usage.api import router as cost_override_router
from lumen.server.features.user_oauth_token.api import router as user_oauth_token_router
from lumen.server.features.web_search.api import router as web_search_router
from lumen.server.federated.api import router as federated_router
from lumen.server.kg.api import admin_router as kg_admin_router
from lumen.server.enterprise_settings.api import (
    admin_router as enterprise_settings_admin_router,
)
from lumen.server.enterprise_settings.api import (
    basic_router as enterprise_settings_router,
)
from lumen.server.manage.administrative import router as admin_router
from lumen.server.user_group.api import router as user_group_router
from lumen.server.manage.discord_bot.api import router as discord_bot_router
from lumen.server.manage.embedding.api import admin_router as embedding_admin_router
from lumen.server.manage.embedding.api import basic_router as embedding_router
from lumen.server.manage.get_state import router as state_router
from lumen.server.manage.image_generation.api import (
    admin_router as image_generation_admin_router,
)
from lumen.server.manage.llm.api import admin_router as llm_admin_router
from lumen.server.manage.llm.api import basic_router as llm_router
from lumen.server.manage.oauth_test import router as oauth_test_admin_router
from lumen.server.manage.opensearch_migration.api import (
    admin_router as opensearch_migration_admin_router,
)
from lumen.server.manage.search_settings import router as search_settings_router
from lumen.server.manage.slack_bot import router as slack_bot_management_router
from lumen.server.manage.sso.api import admin_router as sso_admin_router
from lumen.server.manage.tracing.api import admin_router as tracing_admin_router
from lumen.server.manage.users import router as user_router
from lumen.server.manage.voice.api import admin_router as voice_admin_router
from lumen.server.manage.voice.user_api import router as voice_router
from lumen.server.manage.voice.websocket_api import router as voice_websocket_router
from lumen.server.manage.web_search.api import admin_router as web_search_admin_router
from lumen.server.metrics.connector_state_metrics import register_connector_state_metrics
from lumen.server.metrics.postgres_connection_pool import (
    setup_postgres_connection_pool_metrics,
)
from lumen.server.metrics.prometheus_setup import setup_prometheus_metrics
from lumen.server.middleware.latency_logging import add_latency_logging_middleware
from lumen.server.middleware.rate_limiting import (
    RATE_LIMITING_ENABLED,
    close_auth_limiter,
    get_auth_rate_limiters,
    setup_auth_limiter,
)
from lumen.server.oidc_multi import router as oidc_multi_router
from lumen.server.lumen_api.ingestion import router as lumen_api_router
from lumen.server.pat.api import router as pat_router
from lumen.server.query_and_chat.chat_backend import router as chat_router
from lumen.server.query_and_chat.query_backend import admin_router as admin_query_router
from lumen.server.query_and_chat.query_backend import basic_router as query_router
from lumen.server.saml_multi import router as saml_multi_router
from lumen.server.security.api import admin_router as security_admin_router
from lumen.server.security.store import seed_jwt_settings_from_env
from lumen.server.settings.api import admin_router as settings_admin_router
from lumen.server.settings.api import basic_router as settings_router
from lumen.server.sso_discovery import router as sso_discovery_router
from lumen.server.utils import BasicAuthenticationError
from lumen.setup import setup_multitenant_lumen, setup_lumen
from lumen.tracing.setup import setup_tracing
from lumen.utils.client_ip import ClientIPMiddleware
...
from lumen.utils.telemetry import RecordType, get_or_generate_uuid, optional_telemetry
...
def validation_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    content = {
        ...
        "message": exc_str,
        "data": None,
        **LumenErrorCode.VALIDATION_ERROR.detail(exc_str),
    }
...
def value_error_handler(_: Request, exc: Exception) -> JSONResponse:
    ...
    # are added so a bare ValueError reads like any other Lumen error.
    return JSONResponse(
        status_code=400,
        content={
            "message": str(exc),
            **LumenErrorCode.BAD_REQUEST.detail(str(exc)),
        },
    )
...
def validate_no_vector_db_settings() -> None:
    ...
    from lumen.server.features.build.configs import ENABLE_CRAFT

    if ENABLE_CRAFT:
        raise RuntimeError(
            "DISABLE_VECTOR_DB cannot be used with ENABLE_CRAFT. "
            "Lumen Craft requires background workers for sandbox lifecycle "
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:  # noqa: ARG001
    ...
    fetch_ee_implementation_or_noop(
        "lumen.server.metrics.license_metrics", "register_license_metrics"
    )()
    ...
    if not MULTI_TENANT:
        ...
        with get_session_with_current_tenant() as db_session:
            setup_lumen(db_session, POSTGRES_DEFAULT_SCHEMA)
    ...
    from lumen.tracing.setup import shutdown_tracing

    shutdown_tracing()

    if DISABLE_VECTOR_DB:
        from lumen.background.periodic_poller import stop_periodic_poller
...
def log_http_error(request: Request, exc: Exception) -> JSONResponse:
    ...
    return JSONResponse(
        ...
        content={
            "error_code": LumenErrorCode.for_status(status_code).code,
            "detail": detail,
        },
    )
...
def get_application(lifespan_override: Lifespan | None = None) -> FastAPI:
    application = FastAPI(
        title="Lumen Backend",
        version=__version__,
        description="Lumen API for AI-powered chat with search, document indexing, agents, actions, and more",
        servers=[
            {"url": f"{WEB_DOMAIN.rstrip('/')}/api", "description": "Lumen API Server"}
        ],
    ...
    register_lumen_exception_handlers(application)
    ...
    include_router_with_global_prefix_prepended(application, lumen_api_router)
    ...
    add_lumen_request_id_middleware(application, "API", logger)
...
# app is exportable
set_is_ee_based_on_env_variable()
app = fetch_versioned_implementation(module="lumen.main", attribute="get_application")


if __name__ == "__main__":
    logger.notice(
        "Starting Lumen Backend version %s on http://%s:%s/",
        __version__,
        APP_HOST,
        str(APP_PORT),
    )

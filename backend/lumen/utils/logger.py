...
from lumen.utils.tenant import get_tenant_id_short_string
...
class LumenRequestIDFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        from shared_configs.contextvars import LUMEN_REQUEST_ID_CONTEXTVAR

        record.request_id = LUMEN_REQUEST_ID_CONTEXTVAR.get() or "-"
        return True


class LumenLoggingAdapter(logging.LoggerAdapter):
    def process(
        ...
        fastapi_request_id = LUMEN_REQUEST_ID_CONTEXTVAR.get()
...
def _add_file_handlers(logger: logging.Logger, formatter: logging.Formatter) -> None:
    ...
    for level in ["debug", "info", "notice"]:
        file_name = (
            f"/var/log/lumen/{LOG_FILE_NAME}_{level}.log"
            if is_containerized
        ...
        try:
            ...
            # container whose mounted /var/log/lumen volume is owned by another UID.
...
def setup_logger(
    ...
    if logger.handlers:
        return LumenLoggingAdapter(logger, extra=extra)
...
def setup_uvicorn_logger(
    ...
    uvicorn_logger.addFilter(LumenRequestIDFilter())

...
from shared_configs.contextvars import (
    ...
    LUMEN_REQUEST_ID_CONTEXTVAR,
)


def add_lumen_tenant_id_middleware(
    app: FastAPI,
    ...
    async def set_tenant_id(
        ...
        lumen_tenant_id = request.headers.get("X-Lumen-Tenant-ID")
        if lumen_tenant_id:
            CURRENT_TENANT_ID_CONTEXTVAR.set(lumen_tenant_id)
        return await call_next(request)


def add_lumen_request_id_middleware(
    app: FastAPI,
    prefix: str,
    logger: logging.LoggerAdapter,  # noqa: ARG001
) -> None:
    ...
    async def set_request_id(
        ...
        lumen_request_id = request.headers.get("X-Lumen-Request-ID")
        if not lumen_request_id:
            lumen_request_id = make_randomized_lumen_request_id(prefix)

        LUMEN_REQUEST_ID_CONTEXTVAR.set(lumen_request_id)
        return await call_next(request)


def make_randomized_lumen_request_id(prefix: str) -> str:
    """generates a randomized request id"""

    hash_input = str(uuid.uuid4())
    return _make_lumen_request_id(prefix, hash_input)


def make_structured_lumen_request_id(prefix: str, request_url: str) -> str:
    """Not used yet, but could be in the future!"""
    hash_input = f"{request_url}:{datetime.now(timezone.utc)}"
    return _make_lumen_request_id(prefix, hash_input)


def _make_lumen_request_id(prefix: str, hash_input: str) -> str:
    ...
    lumen_request_id = f"{prefix}:{hash_str}"
    return lumen_request_id
...
def add_endpoint_context_middleware(app: FastAPI) -> None:
    ...
    Used by ``lumen_db_connections_held_by_endpoint`` and
    ``lumen_db_connection_hold_seconds`` in the pool event listeners.

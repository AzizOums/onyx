...
access-control change, credential access) on the ``lumen.audit`` logger tree.
...
AUDIT_LOGGER_ROOT = "lumen.audit"
AUDIT_HANDLER_NAME = "lumen_audit_stdout"

# Diagnostics logger for the subsystem's own failures. Deliberately NOT under
# ``lumen.audit`` so internal warnings never land in the parsed audit stream.
...
def _safe_get_request_id() -> str | None:
    try:
        ...
        return LUMEN_REQUEST_ID_CONTEXTVAR.get()
    except Exception:
        return None
...
def _safe_get_client_ip() -> str | None:
    try:
        from lumen.utils.client_ip import current_client_ip
...
def _configure_audit_logging() -> None:
    """Own an INFO stdout handler on ``lumen.audit`` so events reach stdout

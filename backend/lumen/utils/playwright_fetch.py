...
logic that was originally embedded in `lumen.connectors.web.connector`.

Two consumers:
- `WebConnector` (long-lived `BrowserContext` reused across many pages
  in a single crawl) uses `start_playwright()` directly.
- `LumenWebCrawler` (lazy fallback when its `requests`-based fetch hits
...
def start_playwright() -> tuple[Playwright, BrowserContext]:
    ...
    LumenWebCrawler fallback. Caller owns lifecycle and must call
...
def looks_like_cloudflare_challenge(html: str) -> bool:
    ...
    Used by callers (e.g. `LumenWebCrawler`) to distinguish "we got real

...
from lumen.configs.app_configs import DISABLE_TELEMETRY, ENTERPRISE_EDITION_ENABLED
from lumen.configs.app_configs import TELEMETRY_ENDPOINT
from lumen.configs.constants import (
    KV_CUSTOMER_UUID_KEY,
    KV_INSTANCE_DOMAIN_KEY,
    MilestoneRecordType,
)
from lumen.db.encrypted_kv_store import load_encrypted_kv, upsert_encrypted_kv
from lumen.db.engine.sql_engine import get_session_with_current_tenant
from lumen.db.models import User
from lumen.key_value_store.interface import KvKeyNotFoundError, unwrap_str
...
# No endpoint ships with this build; telemetry stays off unless TELEMETRY_ENDPOINT
# is set (and DISABLE_TELEMETRY=false).
...
def optional_telemetry(
    ...
    if DISABLE_TELEMETRY or not _TELEMETRY_ENDPOINT:
    ...
    try:
        ...
        # Should never interfere with normal functions of Lumen
        pass
...
def mt_cloud_telemetry(
    ...
    # This is only for Lumen MT Cloud, this code should also never be hit, no reason for any orgs to
    # be running the Multi Tenant version of Lumen.
    fetch_versioned_implementation_with_fallback(
        module="lumen.utils.telemetry",
...
def _get_tenant_id_for_user_identify(user_email: str) -> str | None:
    try:
        return fetch_versioned_implementation_with_fallback(
            module="lumen.db.user_tenant_mapping",

...
def _run_auto_llm_update() -> None:
    from lumen.configs.app_configs import AUTO_LLM_CONFIG_URL
...
def _run_cache_cleanup() -> None:
    from lumen.cache.postgres_backend import cleanup_expired_cache_entries
...
def _run_license_reclaim() -> None:
    ...
    fetch_versioned_implementation(
        "lumen.background.celery.tasks.license_reclaim.tasks",
...
def _run_scheduled_eval() -> None:
    ...
    from lumen.evals.models import EvalConfigurationOptions
...
def _run_drain_loops(tenant_id: str) -> None:
    from lumen.background.task_utils import (
        drain_delete_loop,

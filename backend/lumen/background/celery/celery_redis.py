...
from lumen.background.celery.configs.base import CELERY_SEPARATOR
from lumen.configs.app_configs import REDIS_HEALTH_CHECK_INTERVAL
from lumen.configs.constants import REDIS_SOCKET_KEEPALIVE_OPTIONS, LumenCeleryPriority
...
def celery_get_queue_length(queue: str, r: Redis) -> int:
    ...
    total_length = 0
    for i in range(len(LumenCeleryPriority)):
        queue_name = queue
        if i > 0:
...
def celery_find_task(task_id: str, queue: str, r: Redis) -> int:
    ...
    """
    for priority in range(len(LumenCeleryPriority)):

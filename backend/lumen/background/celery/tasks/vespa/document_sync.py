...
from lumen.configs.app_configs import DB_YIELD_PER_DEFAULT
...
from lumen.db.port_attempt import any_future_port_in_progress
...
def set_document_sync_fence(r: TenantRedisClient, payload: int | None) -> None:
    ...
    if payload is None:
        r.srem(LumenRedisConstants.ACTIVE_FENCES, DOCUMENT_SYNC_FENCE_KEY)
        r.delete(DOCUMENT_SYNC_FENCE_KEY)
        return

    r.set(DOCUMENT_SYNC_FENCE_KEY, payload, ex=FENCE_TTL)
    r.sadd(LumenRedisConstants.ACTIVE_FENCES, DOCUMENT_SYNC_FENCE_KEY)
...
def generate_document_sync_tasks(
    ...
    for doc_id, is_secondary_pending in db_session.execute(stmt).yield_per(
        ...
        if is_secondary_pending:
            priority = (
                LumenCeleryPriority.LOW if port_running else LumenCeleryPriority.HIGH
            )
        ...
        celery_app.send_task(
            LumenCeleryTask.DOCUMENT_INDEX_METADATA_SYNC_TASK,
            kwargs=dict(document_id=doc_id, tenant_id=tenant_id),
            queue=LumenCeleryQueues.VESPA_METADATA_SYNC,

...
from lumen.background.celery.apps.app_base import task_logger
from lumen.background.celery.celery_redis import (
    celery_get_broker_client,
    celery_get_queue_length,
    celery_get_unacked_task_ids,
)
from lumen.background.celery.memory_monitoring import emit_process_memory
from lumen.configs.app_configs import DISABLE_TELEMETRY
...
from lumen.db.enums import IndexingStatus, SyncStatus, SyncType
...
from lumen.db.search_settings import get_active_search_settings_list
from lumen.redis.redis_pool import (
    get_redis_client,
    get_shared_redis_client,
    redis_lock_dump,
)
from lumen.redis.tenant_redis_client import TenantRedisClient
from lumen.utils.platform_utils import is_running_in_container, is_running_in_kubernetes
from lumen.utils.telemetry import RecordType, optional_telemetry
...
def _collect_queue_metrics(redis_celery: Redis) -> list[Metric]:
    ...
    metrics = []
    queue_mappings = {
        "celery_queue_length": LumenCeleryQueues.PRIMARY,
        "docprocessing_queue_length": LumenCeleryQueues.DOCPROCESSING,
        "port_queue_length": LumenCeleryQueues.PORT,
        "docfetching_queue_length": LumenCeleryQueues.CONNECTOR_DOC_FETCHING,
        "sync_queue_length": LumenCeleryQueues.VESPA_METADATA_SYNC,
        "deletion_queue_length": LumenCeleryQueues.CONNECTOR_DELETION,
        "pruning_queue_length": LumenCeleryQueues.CONNECTOR_PRUNING,
        "permissions_sync_queue_length": LumenCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        "external_group_sync_queue_length": LumenCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
        "permissions_upsert_queue_length": LumenCeleryQueues.DOC_PERMISSIONS_UPSERT,
        "hierarchy_fetching_queue_length": LumenCeleryQueues.CONNECTOR_HIERARCHY_FETCHING,
        "llm_model_update_queue_length": LumenCeleryQueues.LLM_MODEL_UPDATE,
        "checkpoint_cleanup_queue_length": LumenCeleryQueues.CHECKPOINT_CLEANUP,
        "index_attempt_cleanup_queue_length": LumenCeleryQueues.INDEX_ATTEMPT_CLEANUP,
        "index_reclaim_queue_length": LumenCeleryQueues.INDEX_RECLAIM,
        "chat_ttl_deletion_queue_length": LumenCeleryQueues.CHAT_TTL_DELETION,
        "csv_generation_queue_length": LumenCeleryQueues.CSV_GENERATION,
        "capability_checks_queue_length": LumenCeleryQueues.CAPABILITY_CHECKS,
        "user_file_processing_queue_length": LumenCeleryQueues.USER_FILE_PROCESSING,
        "user_file_project_sync_queue_length": LumenCeleryQueues.USER_FILE_PROJECT_SYNC,
        "user_file_delete_queue_length": LumenCeleryQueues.USER_FILE_DELETE,
        "user_file_port_queue_length": LumenCeleryQueues.USER_FILE_PORT,
        "monitoring_queue_length": LumenCeleryQueues.MONITORING,
        "sandbox_queue_length": LumenCeleryQueues.SANDBOX,
        "opensearch_migration_queue_length": LumenCeleryQueues.OPENSEARCH_MIGRATION,
    }
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.MONITOR_BACKGROUND_PROCESSES,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=LumenCeleryQueues.MONITORING,
    bind=True,
)
def monitor_background_processes(self: Task, *, tenant_id: str) -> None:
    ...
    lock_monitoring: RedisLock = r.lock(
        LumenRedisLocks.MONITOR_BACKGROUND_PROCESSES_LOCK,
...
@shared_task(
    name=LumenCeleryTask.CLOUD_MONITOR_ALEMBIC,
)
def cloud_check_alembic() -> bool | None:
    ...
    redis_client = get_redis_client(tenant_id=LUMEN_CLOUD_TENANT_ID)

    lock_beat: RedisLock = redis_client.lock(
        LumenRedisLocks.CLOUD_CHECK_ALEMBIC_BEAT_LOCK,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CLOUD_MONITOR_CELERY_QUEUES, ignore_result=True, bind=True
)
...
@shared_task(name=LumenCeleryTask.MONITOR_CELERY_QUEUES, ignore_result=True, bind=True)  # ty: ignore[invalid-argument-type]
...
def monitor_celery_queues_helper(
    ...
    n_celery = celery_get_queue_length(LumenCeleryQueues.PRIMARY, r_celery)
    n_docfetching = celery_get_queue_length(
        LumenCeleryQueues.CONNECTOR_DOC_FETCHING, r_celery
    )
    n_docprocessing = celery_get_queue_length(LumenCeleryQueues.DOCPROCESSING, r_celery)
    n_port = celery_get_queue_length(LumenCeleryQueues.PORT, r_celery)

    n_user_file_processing = celery_get_queue_length(
        LumenCeleryQueues.USER_FILE_PROCESSING, r_celery
    )
    n_user_file_project_sync = celery_get_queue_length(
        LumenCeleryQueues.USER_FILE_PROJECT_SYNC, r_celery
    )
    n_user_file_delete = celery_get_queue_length(
        LumenCeleryQueues.USER_FILE_DELETE, r_celery
    )
    n_user_file_port = celery_get_queue_length(
        LumenCeleryQueues.USER_FILE_PORT, r_celery
    )
    n_sync = celery_get_queue_length(LumenCeleryQueues.VESPA_METADATA_SYNC, r_celery)
    n_deletion = celery_get_queue_length(LumenCeleryQueues.CONNECTOR_DELETION, r_celery)
    n_pruning = celery_get_queue_length(LumenCeleryQueues.CONNECTOR_PRUNING, r_celery)
    n_permissions_sync = celery_get_queue_length(
        LumenCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC, r_celery
    )
    n_external_group_sync = celery_get_queue_length(
        LumenCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC, r_celery
    )
    n_permissions_upsert = celery_get_queue_length(
        LumenCeleryQueues.DOC_PERMISSIONS_UPSERT, r_celery
    )
    n_hierarchy_fetching = celery_get_queue_length(
        LumenCeleryQueues.CONNECTOR_HIERARCHY_FETCHING, r_celery
    )
    n_llm_model_update = celery_get_queue_length(
        LumenCeleryQueues.LLM_MODEL_UPDATE, r_celery
    )
    n_checkpoint_cleanup = celery_get_queue_length(
        LumenCeleryQueues.CHECKPOINT_CLEANUP, r_celery
    )
    n_index_attempt_cleanup = celery_get_queue_length(
        LumenCeleryQueues.INDEX_ATTEMPT_CLEANUP, r_celery
    )
    n_index_reclaim = celery_get_queue_length(LumenCeleryQueues.INDEX_RECLAIM, r_celery)
    n_csv_generation = celery_get_queue_length(
        LumenCeleryQueues.CSV_GENERATION, r_celery
    )
    n_capability_checks = celery_get_queue_length(
        LumenCeleryQueues.CAPABILITY_CHECKS, r_celery
    )
    n_monitoring = celery_get_queue_length(LumenCeleryQueues.MONITORING, r_celery)
    n_sandbox = celery_get_queue_length(LumenCeleryQueues.SANDBOX, r_celery)
    n_opensearch_migration = celery_get_queue_length(
        LumenCeleryQueues.OPENSEARCH_MIGRATION, r_celery
    )
    ...
    n_docprocessing_prefetched = celery_get_unacked_task_ids(
        LumenCeleryQueues.DOCPROCESSING, r_celery
    )
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.MONITOR_PROCESS_MEMORY,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=LumenCeleryQueues.MONITORING,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CLOUD_MONITOR_CELERY_PIDBOX, ignore_result=True, bind=True
)
...
@shared_task(
    name=LumenCeleryTask.EMIT_VERSION_TELEMETRY,
    ignore_result=True,
    queue=LumenCeleryQueues.MONITORING,
)

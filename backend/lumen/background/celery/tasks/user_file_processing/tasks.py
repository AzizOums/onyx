...
from lumen.access.access import build_access_for_user_files
...
from lumen.background.task_utils import send_user_file_delete_task
...
from lumen.connectors.file.connector import LocalFileConnector
...
from lumen.db.models import SearchSettings, UserFile
from lumen.db.port_attempt import port_backfill_has_pending_work
from lumen.db.port_orphan_candidate import record_port_orphan_candidates_for_user_file
...
def _user_file_lock_key(user_file_id: str | UUID) -> str:
    return f"{LumenRedisLocks.USER_FILE_PROCESSING_LOCK_PREFIX}:{user_file_id}"
...
def _user_file_queued_key(user_file_id: str | UUID) -> str:
    ...
    in flight.
    """
    return f"{LumenRedisLocks.USER_FILE_QUEUED_PREFIX}:{user_file_id}"


def user_file_project_sync_lock_key(user_file_id: str | UUID) -> str:
    return f"{LumenRedisLocks.USER_FILE_PROJECT_SYNC_LOCK_PREFIX}:{user_file_id}"


def _user_file_project_sync_queued_key(user_file_id: str | UUID) -> str:
    return f"{LumenRedisLocks.USER_FILE_PROJECT_SYNC_QUEUED_PREFIX}:{user_file_id}"


def _user_file_delete_lock_key(user_file_id: str | UUID) -> str:
    return f"{LumenRedisLocks.USER_FILE_DELETE_LOCK_PREFIX}:{user_file_id}"
...
def _user_file_delete_queued_key(user_file_id: str | UUID) -> str:
    ...
    in flight.
    """
    return f"{LumenRedisLocks.USER_FILE_DELETE_QUEUED_PREFIX}:{user_file_id}"


def get_user_file_project_sync_queue_depth(celery_app: Celery) -> int:
    redis_celery = celery_get_broker_client(celery_app)
    return celery_get_queue_length(
        LumenCeleryQueues.USER_FILE_PROJECT_SYNC, redis_celery
    )
...
def enqueue_user_file_project_sync_task(
    *,
    ...
    tenant_id: str,
    priority: LumenCeleryPriority = LumenCeleryPriority.HIGH,
) -> bool:
    ...
    try:
        celery_app.send_task(
            LumenCeleryTask.PROCESS_SINGLE_USER_FILE_PROJECT_SYNC,
            kwargs={"user_file_id": str(user_file_id), "tenant_id": tenant_id},
            queue=LumenCeleryQueues.USER_FILE_PROJECT_SYNC,
            priority=priority,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_USER_FILE_PROCESSING,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_user_file_processing(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        LumenRedisLocks.USER_FILE_PROCESSING_BEAT_LOCK,
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for user_file_id in user_file_ids:
                ...
                try:
                    self.app.send_task(
                        LumenCeleryTask.PROCESS_SINGLE_USER_FILE,
                        kwargs={
                            "user_file_id": str(user_file_id),
                            "tenant_id": tenant_id,
                        },
                        queue=LumenCeleryQueues.USER_FILE_PROCESSING,
                        priority=LumenCeleryPriority.HIGH,
...
def _process_user_file_without_vector_db(
    ...
    """
    from lumen.llm.factory import get_default_llm, get_llm_tokenizer_encode_func
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.PROCESS_SINGLE_USER_FILE,
    bind=True,
    ignore_result=True,
)
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_USER_FILE_DELETE,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_for_user_file_delete(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        LumenRedisLocks.USER_FILE_DELETE_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    if not lock.acquire(blocking=False):
        return None
    ...
    try:
        ...
        r_celery = celery_get_broker_client(self.app)
        queue_len = celery_get_queue_length(LumenCeleryQueues.USER_FILE_DELETE, r_celery)
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.DELETE_SINGLE_USER_FILE,
    bind=True,
    ignore_result=True,
)
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_USER_FILE_PROJECT_SYNC,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_for_user_file_project_sync(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        LumenRedisLocks.USER_FILE_PROJECT_SYNC_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.PROCESS_SINGLE_USER_FILE_PROJECT_SYNC,
    bind=True,
    ignore_result=True,
)
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_INCOGNITO_FILE_CLEANUP,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_for_incognito_file_cleanup(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        LumenRedisLocks.INCOGNITO_FILE_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    if not lock.acquire(blocking=False):
        return

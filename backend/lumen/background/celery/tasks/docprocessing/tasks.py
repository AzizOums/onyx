...
from lumen.background.celery.celery_utils import httpx_init_vespa_pool
from lumen.background.celery.memory_monitoring import emit_process_memory
from lumen.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from lumen.background.celery.tasks.docfetching.task_creation_utils import (
    try_creating_docfetching_task,
)
from lumen.background.celery.tasks.docprocessing.heartbeat import (
    start_heartbeat,
    stop_heartbeat,
)
from lumen.background.celery.tasks.docprocessing.targeted_reindex_task import (  # noqa: F401  # registers @shared_task with celery
    targeted_reindex_task,
)
from lumen.background.celery.tasks.docprocessing.utils import (
    IndexingCallback,
    is_in_repeated_error_state,
    should_index,
)
from lumen.background.celery.tasks.models import DocProcessingContext
from lumen.background.indexing.checkpointing_utils import (
    cleanup_checkpoint,
...
from lumen.connectors.models import ConnectorFailure, Document, IndexAttemptMetadata
from lumen.db.connector import mark_ccpair_with_indexing_trigger
...
from lumen.db.indexing_coordination import CoordinationStatus, IndexingCoordination
from lumen.db.models import IndexAttempt, SearchSettings
from lumen.db.search_settings import (
    get_current_search_settings,
    get_secondary_search_settings,
)
from lumen.db.swap_index import check_and_perform_index_swap
from lumen.document_index.factory import get_all_document_indices
from lumen.error_handling.exceptions import LumenError
from lumen.file_store.document_batch_storage import (
    DocumentBatchStorage,
    get_document_batch_storage,
)
from lumen.file_store.staging import cleanup_staged_files_for_attempt
from lumen.httpx.httpx_pool import HttpxPool
from lumen.indexing.adapters.document_indexing_adapter import (
    DocumentIndexingBatchAdapter,
)
from lumen.indexing.embedder import DefaultIndexingEmbedder
from lumen.indexing.indexing_pipeline import run_indexing_pipeline
from lumen.indexing.persistent_indexing import (
    build_generic_connector_failure,
    record_generic_failure,
)
from lumen.natural_language_processing.search_nlp_models import (
    EmbeddingModel,
    warm_up_bi_encoder,
)
from lumen.redis.redis_connector import RedisConnector
from lumen.redis.redis_docprocessing import RedisDocprocessing
...
from lumen.redis.redis_tenant_work_gating import maybe_mark_tenant_active
from lumen.redis.redis_utils import is_fence
from lumen.redis.tenant_redis_client import TenantRedisClient
from lumen.server.metrics.connector_health_metrics import (
...
from lumen.utils.middleware import make_randomized_lumen_request_id
from lumen.utils.telemetry import RecordType, mt_cloud_telemetry, optional_telemetry
...
def _get_fence_validation_block_expiration() -> int:
    ...
    try:
        beat_multiplier = LumenRuntime.get_beat_multiplier()
    except Exception:
    ...
    with get_session_with_current_tenant() as db_session:
        ...
        if stale_not_started:
            ...
            queued_ids = celery_get_queued_task_ids(
                LumenCeleryQueues.CONNECTOR_DOC_FETCHING, redis_celery
            )
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_INDEXING,
    soft_time_limit=300,
    bind=True,
)
def check_for_indexing(self: Task, *, tenant_id: str) -> int | None:
    ...
    the docfetching task (LumenCeleryTask.CONNECTOR_DOC_FETCHING_TASK).
    ...
    lock_beat: RedisLock = redis_client.lock(
        LumenRedisLocks.CHECK_INDEXING_BEAT_LOCK,
    ...
    try:
        ...
        if not redis_client.exists(LumenRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE):
            ...
            for key_bytes in redis_client_replica.scan_iter(
                ...
                if is_fence(key_bytes) and not redis_client.sismember(
                    LumenRedisConstants.ACTIVE_FENCES, key_bytes
                ):
                    logger.warning("Adding %s to the lookup table.", key_bytes)
                    redis_client.sadd(LumenRedisConstants.ACTIVE_FENCES, key_bytes)

            redis_client.set(
                LumenRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE,
                1,
                ex=LumenRuntime.get_build_fence_lookup_table_interval(),
            )
        ...
        lock_beat.reacquire()
        # we want to run this less frequently than the overall task
        if not redis_client.exists(LumenRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(
                LumenRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES,
                1,
...
# primary
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_checkpoint_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        LumenRedisLocks.CHECK_CHECKPOINT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for attempt in old_attempts:
                ...
                self.app.send_task(
                    LumenCeleryTask.CLEANUP_CHECKPOINT,
                    kwargs={
                        "index_attempt_id": attempt.id,
                        "tenant_id": tenant_id,
                    },
                    queue=LumenCeleryQueues.CHECKPOINT_CLEANUP,
                    priority=LumenCeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CLEANUP_CHECKPOINT,
    bind=True,
)
...
# primary
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_INDEX_ATTEMPT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_index_attempt_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        LumenRedisLocks.CHECK_INDEX_ATTEMPT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for i in range(0, len(old_attempt_ids), batch_size):
                ...
                self.app.send_task(
                    LumenCeleryTask.CLEANUP_INDEX_ATTEMPT,
                    kwargs={
                        "index_attempt_ids": batch,
                        "tenant_id": tenant_id,
                    },
                    queue=LumenCeleryQueues.INDEX_ATTEMPT_CLEANUP,
                    priority=LumenCeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CLEANUP_INDEX_ATTEMPT,
    bind=True,
)
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.DOCPROCESSING_TASK,
    bind=True,
)
...
def _check_chunk_usage_limit(tenant_id: str) -> None:
    ...
    from lumen.db.usage import UsageType
    from lumen.server.usage_limits import check_usage_and_raise
...
def _docprocessing_task(
    ...
    # check_usage_and_raise raises LumenError; hitting a trial/paid usage limit
    ...
    # would ship LUMEN-BACKEND-H6ED to Sentry on every queued batch of an
    # over-limit tenant).
    if USAGE_LIMITS_ENABLED:
        try:
            ...
            with get_session_with_current_tenant() as db_session:
                from lumen.db.index_attempt import mark_attempt_failed
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            index_attempt_metadata = IndexAttemptMetadata(
                ...
                request_id=make_randomized_lumen_request_id("DIP"),
        ...
        if USAGE_LIMITS_ENABLED and index_pipeline_result.total_chunks > 0:
            try:
                from lumen.db.usage import UsageType, increment_usage

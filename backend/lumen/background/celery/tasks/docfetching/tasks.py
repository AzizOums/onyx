...
from lumen.background.celery.memory_monitoring import (
    emit_process_memory,
    start_memory_observer,
    stop_memory_observer,
)
from lumen.background.celery.tasks.docfetching.worker_shutdown import (
...
from lumen.background.celery.tasks.docprocessing.tasks import ConnectorIndexingLogBuilder
from lumen.background.celery.tasks.docprocessing.utils import IndexingCallback
from lumen.background.celery.tasks.models import (
    DocProcessingContext,
    IndexingWatchdogTerminalStatus,
    SimpleJobResult,
)
from lumen.background.indexing.job_client import (
    SimpleJob,
    SimpleJobClient,
    SimpleJobException,
)
from lumen.background.indexing.run_docfetching import run_docfetching_entrypoint
from lumen.configs.app_configs import INDEXING_WORKER_MEMORY_LIMIT_MB
...
from lumen.db.connector_credential_pair import get_connector_credential_pair_from_id
...
from lumen.server.metrics.connector_health_metrics import on_index_attempt_status_change
from lumen.utils.logger import setup_logger
from lumen.utils.os_reaper import reap_children_before_exit
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CONNECTOR_DOC_FETCHING_TASK,
    bind=True,
    acks_late=False,
    track_started=True,
)
def docfetching_proxy_task(
    ...
    1)  determines parameters of the indexing attempt (which connector indexing function to run,
        ...
        connectors are responsible for reading data from an outside source and converting it to Lumen documents.
        At the moment these two steps (reading external data and converting to an Lumen document)

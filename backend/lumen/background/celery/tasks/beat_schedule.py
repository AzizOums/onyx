...
from lumen.configs.constants import (
    LUMEN_CLOUD_CELERY_TASK_PREFIX,
    LumenCeleryPriority,
    LumenCeleryQueues,
    LumenCeleryTask,
)
from lumen.server.features.build.configs import SANDBOX_IDLE_CLEANUP_INTERVAL_SECONDS
...
beat_task_templates: list[dict] = [
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_USER_FILE_PROCESSING,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_INCOGNITO_FILE_CLEANUP,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_USER_FILE_PROJECT_SYNC,
        "schedule": timedelta(seconds=20),
        "options": {
            "priority": LumenCeleryPriority.MEDIUM,
            "expires": BEAT_EXPIRES_DEFAULT,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_USER_FILE_DELETE,
        "schedule": timedelta(seconds=20),
        "options": {
            "priority": LumenCeleryPriority.MEDIUM,
            "expires": BEAT_EXPIRES_DEFAULT,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_INDEXING,
    ...
    {
        "name": "check-for-port",
        "task": LumenCeleryTask.CHECK_FOR_PORT,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_OLD_INDEX_RECLAIM,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_STALE_CAPABILITY_RUNS,
        "schedule": timedelta(minutes=10),
        "options": {
            "priority": LumenCeleryPriority.LOW,
            "expires": BEAT_EXPIRES_DEFAULT,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_INDEX_ATTEMPT_CLEANUP,
        "schedule": timedelta(minutes=30),
        "options": {
            "priority": LumenCeleryPriority.MEDIUM,
            "expires": BEAT_EXPIRES_DEFAULT,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_CONNECTOR_DELETION,
        "schedule": timedelta(seconds=20),
        "options": {
            "priority": LumenCeleryPriority.MEDIUM,
            "expires": BEAT_EXPIRES_DEFAULT,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_VESPA_SYNC_TASK,
        "schedule": timedelta(seconds=20),
        "options": {
            "priority": LumenCeleryPriority.MEDIUM,
            "expires": BEAT_EXPIRES_DEFAULT,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_PRUNING,
        "schedule": timedelta(seconds=20),
        "options": {
            "priority": LumenCeleryPriority.MEDIUM,
            "expires": BEAT_EXPIRES_DEFAULT,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.CHECK_FOR_HIERARCHY_FETCHING,
    ...
    },
    {
        ...
        "task": LumenCeleryTask.MONITOR_BACKGROUND_PROCESSES,
    ...
    # dispatcher there would let a saturated executor pool stall dispatch.
    {
        ...
        "task": LumenCeleryTask.SCHEDULED_TASKS_DISPATCH_DUE,
        ...
        "options": {
            ...
            "expires": 60,
            "queue": LumenCeleryQueues.PRIMARY,
        },
    },
    {
        "name": "cleanup-stuck-scheduled-runs",
        "task": LumenCeleryTask.SCHEDULED_TASKS_CLEANUP_STUCK,
        "schedule": timedelta(hours=1),
        "options": {
            "priority": LumenCeleryPriority.LOW,
            "expires": 60 * 60,
    ...
    # Sandbox sweep: background-snapshot changed sessions, sleep idle sandboxes.
    {
        ...
        "task": LumenCeleryTask.CLEANUP_IDLE_SANDBOXES,
        ...
        "options": {
            "priority": LumenCeleryPriority.LOW,
            "expires": BEAT_EXPIRES_DEFAULT,
            "queue": LumenCeleryQueues.SANDBOX,
            "work_gated": True,
        },
    },
]
    ...
    beat_task_templates.append(
        {
            ...
            "task": LumenCeleryTask.CHECK_FOR_AUTO_LLM_UPDATE,
    ...
    beat_task_templates.append(
        {
            ...
            "task": LumenCeleryTask.SCHEDULED_EVAL_TASK,
...
if (
    ...
    and not LUMEN_DISABLE_VESPA
):
    beat_task_templates.append(
        {
            "name": "migrate-chunks-from-vespa-to-opensearch",
            "task": LumenCeleryTask.MIGRATE_CHUNKS_FROM_VESPA_TO_OPENSEARCH_TASK,
            ...
            "options": {
                ...
                "expires": BEAT_EXPIRES_DEFAULT,
                "queue": LumenCeleryQueues.OPENSEARCH_MIGRATION,
            },
        }
    )
...
def make_cloud_generator_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    cloud_task["options"] = {}
    cloud_task["options"]["priority"] = LumenCeleryPriority.HIGHEST
    cloud_task["options"]["expires"] = BEAT_EXPIRES_DEFAULT

    # settings dependent on the original task
    cloud_task["name"] = f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_{task['name']}"
    cloud_task["task"] = LumenCeleryTask.CLOUD_BEAT_TASK_GENERATOR
    cloud_task["kwargs"] = {}
...
# the name attribute must start with LUMEN_CLOUD_CELERY_TASK_PREFIX = "cloud" to be seen
# by the DynamicTenantScheduler as system wide task and not a per tenant task
beat_cloud_tasks: list[dict] = [
    # cloud specific tasks
    {
        "name": f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_monitor-alembic",
        "task": LumenCeleryTask.CLOUD_MONITOR_ALEMBIC,
        "schedule": timedelta(hours=1),
        "options": {
            "queue": LumenCeleryQueues.MONITORING,
    ...
    {
        "name": f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_monitor-celery-queues",
        "task": LumenCeleryTask.CLOUD_MONITOR_CELERY_QUEUES,
        "schedule": timedelta(seconds=30),
        "options": {
            "queue": LumenCeleryQueues.MONITORING,
            "priority": LumenCeleryPriority.HIGH,
    ...
    {
        "name": f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_check-available-tenants",
        "task": LumenCeleryTask.CLOUD_CHECK_AVAILABLE_TENANTS,
    ...
    {
        "name": f"{LUMEN_CLOUD_CELERY_TASK_PREFIX}_monitor-celery-pidbox",
        "task": LumenCeleryTask.CLOUD_MONITOR_CELERY_PIDBOX,
        ...
        [
            {
                ...
                "task": LumenCeleryTask.MONITOR_CELERY_QUEUES,
            ...
            },
            {
                ...
                "task": LumenCeleryTask.MONITOR_PROCESS_MEMORY,
                "schedule": timedelta(minutes=5),
                "options": {
                    "priority": LumenCeleryPriority.LOW,
                    "expires": BEAT_EXPIRES_DEFAULT,
            ...
            },
            {
                ...
                "task": LumenCeleryTask.CELERY_BEAT_HEARTBEAT,
                "schedule": timedelta(minutes=1),
                "options": {
                    "priority": LumenCeleryPriority.HIGHEST,
                    "expires": BEAT_EXPIRES_DEFAULT,
                    "queue": LumenCeleryQueues.PRIMARY,
                },
            },
            # hourly tick; the task itself enforces a once-per-day cadence
            {
                ...
                "task": LumenCeleryTask.EMIT_VERSION_TELEMETRY,
                "schedule": timedelta(hours=1),
                "options": {
                    "priority": LumenCeleryPriority.LOW,
                    "expires": BEAT_EXPIRES_DEFAULT,

...
from lumen.db.models import PortAttempt, SearchSettings
...
from lumen.document_index.opensearch.port_copy import PortCopier
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.RUN_PORT_ATTEMPT,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.RUN_USER_FILE_PORT_ATTEMPT,
    soft_time_limit=_PORT_SOFT_TIME_LIMIT,
    time_limit=_PORT_TIME_LIMIT,
    bind=True,
)
...
def _enqueue_run_port_attempt(
    ...
    if scope == "user_file":
        task_name = LumenCeleryTask.RUN_USER_FILE_PORT_ATTEMPT
        queue = LumenCeleryQueues.USER_FILE_PORT
    else:
        task_name = LumenCeleryTask.RUN_PORT_ATTEMPT
        queue = LumenCeleryQueues.PORT
...
def run_check_for_port(tenant_id: str, celery_app: Celery) -> int | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        LumenRedisLocks.CHECK_PORT_BEAT_LOCK,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_PORT,
    soft_time_limit=300,
    bind=True,
)

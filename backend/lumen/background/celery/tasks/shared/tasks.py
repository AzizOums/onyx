...
from lumen.configs.constants import LUMEN_CELERY_BEAT_HEARTBEAT_KEY, LumenCeleryTask
...
from lumen.db.document_set import fetch_document_sets_for_document
...
from lumen.db.relationships import delete_document_references_from_kg
...
from lumen.document_index.interfaces_new import MetadataUpdateRequest
...
class LumenCeleryTaskCompletionStatus(str, Enum):
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.DOCUMENT_BY_CC_PAIR_CLEANUP_TASK,
...
def document_by_cc_pair_cleanup_task(
    ...
    if completion_status != LumenCeleryTaskCompletionStatus.SUCCEEDED:
        return False
...
@shared_task(name=LumenCeleryTask.CELERY_BEAT_HEARTBEAT, ignore_result=True, bind=True)  # ty: ignore[invalid-argument-type]
def celery_beat_heartbeat(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    r = get_redis_client()
    r.set(LUMEN_CELERY_BEAT_HEARTBEAT_KEY, 1, ex=600)

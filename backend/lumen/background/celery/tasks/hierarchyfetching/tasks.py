...
from lumen.connectors.factory import (
    ConnectorMissingException,
    identify_connector_class,
    instantiate_connector,
)
from lumen.connectors.interfaces import HierarchyConnector
from lumen.connectors.models import HierarchyNode as PydanticHierarchyNode
from lumen.db.connector import mark_cc_pair_as_hierarchy_fetched
...
def _try_creating_hierarchy_fetching_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            LumenCeleryTask.CONNECTOR_HIERARCHY_FETCHING_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair.id,
                tenant_id=tenant_id,
            ),
            queue=LumenCeleryQueues.CONNECTOR_HIERARCHY_FETCHING,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_HIERARCHY_FETCHING,
    soft_time_limit=300,
    bind=True,
)
def check_for_hierarchy_fetching(self: Task, *, tenant_id: str) -> int | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        LumenRedisLocks.CHECK_HIERARCHY_FETCHING_BEAT_LOCK,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CONNECTOR_HIERARCHY_FETCHING_TASK,

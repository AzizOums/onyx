...
from lumen.background.celery.tasks.docprocessing.utils import IndexingCallbackBase
from lumen.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING, JOB_TIMEOUT
...
from lumen.db.connector import mark_ccpair_as_pruned
...
from lumen.db.sync_record import insert_sync_record, update_sync_record_status
from lumen.db.tag import delete_orphan_tags_batched
from lumen.redis.redis_connector import RedisConnector
from lumen.redis.redis_connector_prune import (
    RedisConnectorPrune,
    RedisConnectorPrunePayload,
)
from lumen.redis.redis_hierarchy import (
...
from lumen.redis.redis_pool import get_redis_client, get_redis_replica_client
...
from lumen.server.metrics.pruning_metrics import observe_pruning_diff_duration
from lumen.server.runtime.lumen_runtime import LumenRuntime
from lumen.server.utils import make_short_id
from lumen.utils.logger import (
    LoggerContextVars,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_PRUNING,
    ignore_result=True,
...
def check_for_pruning(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        LumenRedisLocks.CHECK_PRUNE_BEAT_LOCK,
    ...
    try:
        ...
        if not r.exists(LumenRedisSignals.BLOCK_PRUNING):
            ...
            r.set(LumenRedisSignals.BLOCK_PRUNING, 1, ex=_get_pruning_block_expiration())

        # we want to run this less frequently than the overall task
        lock_beat.reacquire()
        if not r.exists(LumenRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES):
            ...
            r.set(
                LumenRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES,
                1,
        ...
        lock_beat.reacquire()
        keys = cast(set[Any], r_replica.smembers(LumenRedisConstants.ACTIVE_FENCES))
        for key in keys:
            key_bytes = cast(bytes, key)

            if not r.exists(key_bytes):
                r.srem(LumenRedisConstants.ACTIVE_FENCES, key_bytes)
                continue
...
def try_creating_prune_generator_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            LumenCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
            kwargs=dict(
            ...
            queue=LumenCeleryQueues.CONNECTOR_PRUNING,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_pruning_generator_task(
    ...
    lock: RedisLock = r.lock(
        LumenRedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.cc_pair_id}",
...
def validate_pruning_fences(
    ...
    queue_len = celery_get_queue_length(LumenCeleryQueues.CONNECTOR_DELETION, r_celery)
...
def validate_pruning_fence(
    ...
    found = celery_find_task(
        ...
        LumenCeleryQueues.CONNECTOR_PRUNING,
        r_celery,
    )
    if found:

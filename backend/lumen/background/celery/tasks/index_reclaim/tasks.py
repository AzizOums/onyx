...
def _reclaim_row_lock_key(search_settings_id: int) -> str:
    return f"{LumenRedisLocks.OLD_INDEX_RECLAIM_LOCK_PREFIX}:{search_settings_id}"
...
def enqueue_index_reclaim(
    ...
    celery_app.send_task(
        LumenCeleryTask.RUN_OLD_INDEX_RECLAIM,
        kwargs={"tenant_id": tenant_id, "search_settings_id": search_settings_id},
        queue=LumenCeleryQueues.INDEX_RECLAIM,
...
def run_check_for_old_index_reclaim(tenant_id: str, celery_app: Celery) -> int | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        LumenRedisLocks.CHECK_OLD_INDEX_RECLAIM_BEAT_LOCK,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.RUN_OLD_INDEX_RECLAIM,
...
@shared_task(  # ty: ignore[invalid-argument-type]
    name=LumenCeleryTask.CHECK_FOR_OLD_INDEX_RECLAIM,

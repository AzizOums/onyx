...
from lumen.background.celery.celery_utils import celery_is_worker_primary
from lumen.background.celery.tasks.vespa.document_sync import reset_document_sync
from lumen.configs.app_configs import CELERY_WORKER_PRIMARY_POOL_OVERFLOW
...
from lumen.db.index_attempt import get_index_attempt, mark_attempt_canceled
...
from lumen.redis.redis_connector_stop import RedisConnectorStop
...
celery_app.config_from_object("lumen.background.celery.configs.primary")
...
def on_worker_init(sender: Worker, **kwargs: Any) -> None:
    ...
    r.delete(LumenRedisLocks.PRIMARY_WORKER)
    ...
    lock: RedisLock = r.lock(
        LumenRedisLocks.PRIMARY_WORKER,
    ...
    r.delete(LumenRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK)

    r.delete(LumenRedisConstants.ACTIVE_FENCES)
...
celery_app.autodiscover_tasks(
    app_base.filter_task_modules(
        [
            ...
            "lumen.background.celery.tasks.index_reclaim",
            "lumen.background.celery.tasks.port",
            "lumen.background.celery.tasks.evals",
            "lumen.background.celery.tasks.hierarchyfetching",
            "lumen.background.celery.tasks.pruning",
            "lumen.background.celery.tasks.scheduled_tasks",
            "lumen.background.celery.tasks.shared",
            "lumen.background.celery.tasks.vespa",
            "lumen.background.celery.tasks.llm_model_update",
            "lumen.background.celery.tasks.user_file_processing",
            "lumen.background.celery.tasks.capability_checks",
        ]
    )
)

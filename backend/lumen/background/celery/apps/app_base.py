...
from lumen.background.celery.apps.task_formatters import (
    CeleryTaskColoredFormatter,
    CeleryTaskJsonFormatter,
    CeleryTaskPlainFormatter,
)
from lumen.background.celery.celery_utils import (
...
from lumen.configs.constants import LUMEN_CLOUD_CELERY_TASK_PREFIX, LumenRedisLocks
...
from lumen.document_index.vespa.shared_utils.utils import wait_for_vespa_with_timeout
from lumen.httpx.httpx_pool import HttpxPool
from lumen.redis.redis_connector import RedisConnector
from lumen.redis.redis_connector_delete import RedisConnectorDelete
from lumen.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSync
from lumen.redis.redis_connector_ext_group_sync import RedisConnectorExternalGroupSync
from lumen.redis.redis_connector_prune import RedisConnectorPrune
from lumen.redis.redis_document_set import RedisDocumentSet
from lumen.redis.redis_pool import get_redis_client
from lumen.redis.redis_usergroup import RedisUserGroup
...
def on_task_postrun(
    ...
    **kwds: Any,  # noqa: ARG001
    ...
    if task.name.startswith(LUMEN_CLOUD_CELERY_TASK_PREFIX):
...
def on_secondary_worker_init(sender: Any, **kwargs: Any) -> None:  # noqa: ARG001
    ...
    while True:
        if r.exists(LumenRedisLocks.PRIMARY_WORKER):
            break
...
class TenantContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if tenant_id:
            # Match the 8 character tenant abbreviation used in LumenLoggingAdapter
...
def wait_for_document_index_or_shutdown() -> None:
    ...
    if ENABLE_OPENSEARCH_INDEXING_FOR_LUMEN:
...
_VECTOR_DB_TASK_MODULES: set[str] = {
    "lumen.background.celery.tasks.connector_deletion",
    "lumen.background.celery.tasks.docprocessing",
    "lumen.background.celery.tasks.docfetching",
    "lumen.background.celery.tasks.pruning",
    "lumen.background.celery.tasks.vespa",
    "lumen.background.celery.tasks.opensearch_migration",
    "lumen.background.celery.tasks.doc_permission_syncing",
    "lumen.background.celery.tasks.hierarchyfetching",
    # EE modules that are vector-DB-dependent
    "ee.lumen.background.celery.tasks.doc_permission_syncing",
    "ee.lumen.background.celery.tasks.external_group_syncing",
}
# NOTE: "lumen.background.celery.tasks.shared" is intentionally NOT in the set

...
The Lumen MCP server allows LLMs to connect to your Lumen instance and access its knowledge base and search capabilities through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/).

With the Lumen MCP Server, you can search your knowledgebase and
give your LLMs web search.

All access controls are managed within the main Lumen application.

### Authentication

Provide an Lumen Personal Access Token or API Key in the `Authorization` header as a Bearer token.
...
The MCP server is built on [FastMCP](https://github.com/jlowin/fastmcp) and runs alongside the main Lumen API server:
...
```json
{
  "mcpServers": {
    "lumen": {
      "url": "https://[YOUR_LUMEN_DOMAIN]:8090/",
      "transport": "http",
      "headers": {
        "Authorization": "Bearer YOUR_LUMEN_TOKEN_HERE"
      }
    }
  }
}
...
Search the user's private knowledge base indexed in Lumen. Returns ranked documents with content snippets, scores, and metadata.

Pass `agent` with an agent name to run the search as that Lumen agent. The search then applies the agent's knowledge scope (document sets, attached documents, start date) and its configured model. An unresolvable name returns an error listing the agents available to the user, so no lookup call is needed first.
...
3. `agents`
Lists the Lumen agents accessible to the user (`id`, `name`, `description`). Use a returned `name` with the `agent` filter of `search_indexed_documents`.

## Local Development

### Running the MCP Server

The MCP Server automatically launches with the `Run All Lumen Services` task from the default launch.json.
...
4. Paste your Lumen bearer token
5. Click **Connect**
...
- `API_SERVER_URL_OVERRIDE_FOR_HTTP_REQUESTS`: Optional override URL. If set, takes precedence over the protocol/host variables. Used for self-hosting the MCP server with Lumen Cloud as the backend.
...
from lumen.configs.app_configs import MCP_SERVER_API_REQUEST_TIMEOUT_SECONDS
from lumen.configs.constants import DocumentSource
from lumen.mcp_server.api import mcp_server
from lumen.mcp_server.utils import (
    AgentEntry,
...
from lumen.server.features.search.models import (
    SearchRequest,
    SearchResponse,
    SearchResult,
)
from lumen.server.features.web_search.models import (
    OpenUrlsToolRequest,
...
from lumen.server.metrics.mcp_server import (
...
from lumen.utils.variable_functionality import build_api_server_url_for_http_requests
...
async def _post_model(
    ...
    """POST a Pydantic model as JSON to the Lumen backend."""
...
def _extract_error_detail(response: httpx.Response) -> str:
    ...
    The backend returns LumenError responses as
    ``{"error_code": "...", "detail": "..."}``.
    """
    try:
        body = response.json()
        ...
        logger.debug("Lumen MCP Server: error body was not JSON (%s)", exc)
...
async def search_indexed_documents(
    ...
    """
    Search the user's knowledge base indexed in Lumen.
    ...
    logger.info(
        "Lumen MCP Server: document search: query='%s', sources=%s, document_sets=%s, agent=%s",
        query,
        source_types,
        document_set_names,
        agent,
    ...
    try:
        ...
        if agent is not None:
            try:
                ...
                logger.error(
                    "Lumen MCP Server: Error fetching agents: %s", err, exc_info=True
                )
        ...
        if agent is None:
            try:
                ...
                logger.error(
                    "Lumen MCP Server: Error checking indexed sources: %s",
                    err,
                    exc_info=True,
                )
            ...
            if not sources:
                logger.info("Lumen MCP Server: No indexed sources available for tenant")
        ...
        if source_types is not None:
            ...
            for source_str in source_types:
                try:
                    ...
                    logger.warning(
                        "Lumen MCP Server: Invalid source type '%s' - skipping",
                        source_str,
                    )
        ...
        try:
            ...
            logger.warning(
                "Lumen MCP Server: invalid time_cutoff '%s' (%s); continuing without time filter",
                time_cutoff,
                err,
            )
            parsed_cutoff = None
        ...
        logger.info(
            "Lumen MCP Server: Internal search returned %s results", len(results)
        )
        return {"results": results}
    except Exception as err:
        logger.error("Lumen MCP Server: Document search error: %s", err, exc_info=True)
...
async def search_web(
    ...
    logger.info("Lumen MCP Server: Web search: query='%s', limit=%s", query, limit)
    ...
    try:
        ...
        logger.error("Lumen MCP Server: Web search error: %s", e, exc_info=True)
        return {
...
async def open_urls(
    ...
    logger.info("Lumen MCP Server: Open URL: fetching %s URLs", len(urls))
    ...
    try:
        ...
        logger.error("Lumen MCP Server: URL fetch error: %s", err, exc_info=True)

...
from lumen.configs.app_configs import MCP_SERVER_CORS_ORIGINS
...
from lumen.mcp_server.utils import shutdown_http_client
from lumen.server.metrics.prometheus_setup import (
...
# (python -m lumen.mcp_server_main, uvicorn lumen.mcp_server.api:mcp_app, etc.).
set_is_ee_based_on_env_variable()

logger.info("Creating Lumen MCP Server...")

mcp_server = FastMCP(
    name="Lumen MCP Server",
    version="1.0.0",
    auth=LumenTokenVerifier(),
)
...
from lumen.mcp_server.resources import indexed_sources  # noqa: E402, F401
from lumen.mcp_server.tools import search  # noqa: E402, F401
...
def create_mcp_fastapi_app() -> FastAPI:
    ...
    # Authentication is handled by FastMCP's LumenTokenVerifier (see auth.py)

"""Resources that expose metadata for the Lumen MCP server."""
...
from lumen.mcp_server.utils import get_indexed_sources, require_access_token
...
async def indexed_sources_resource() -> str:
    ...
    logger.info(
        "Lumen MCP Server: indexed_sources resource returning %s entries",
        len(sources),
    )

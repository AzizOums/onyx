...
The Lumen file store provides a unified interface for storing files and large binary objects. It supports four storage backends: S3-compatible storage (AWS S3, MinIO, Digital Ocean Spaces, etc.), Google Cloud Storage (GCS), Azure Blob Storage, and PostgreSQL Large Objects.
...
S3_FILE_STORE_BUCKET_NAME=your-bucket-name  # Defaults to 'lumen-file-store-bucket'
S3_FILE_STORE_PREFIX=lumen-files  # Optional, defaults to 'lumen-files'
...
GCS_FILE_STORE_PREFIX=lumen-files               # Optional, defaults to 'lumen-files'
...
AZURE_FILE_STORE_PREFIX=lumen-files                   # Optional, defaults to 'lumen-files'
...
The blob storage connector (`backend/lumen/connectors/blob/connector.py`) also supports native GCS authentication via the admin UI. When creating a Google Cloud Storage connector, three auth methods are available:

...
from lumen.configs.constants import DEV_VERSION_PATTERN, STABLE_VERSION_PATTERN
from lumen.main import fetch_versioned_implementation
...
def test_versions_endpoint(client: TestClient) -> None:
    ...
    assert STABLE_VERSION_PATTERN.match(stable["lumen"]), (
        f"Stable version {stable['lumen']} doesn't match pattern v(number).(number).(number)"
    )
    ...
    assert DEV_VERSION_PATTERN.match(dev["lumen"]), (
        f"Dev version {dev['lumen']} doesn't match pattern v(number).(number).(number)-beta.(number)"
    )

    # Verify migration configuration
    migration = data["migration"]
    assert "lumen" in migration
    assert "relational_db" in migration
    assert "index" in migration
    assert "nginx" in migration

    # Verify migration has expected values
    assert migration["lumen"] == "airgapped-intfloat-nomic-migration"
    ...
    assert stable["lumen"] != dev["lumen"], "Stable and dev versions should be different"

...
from lumen.external_apps import token_refresh as tr
...
def _patch_post(monkeypatch: pytest.MonkeyPatch, response: object) -> None:
    monkeypatch.setattr(
        "lumen.external_apps.providers.base.requests.post",
...
def test_refresh_missing_refresh_token_is_terminal(
    ...
    called = MagicMock()
    monkeypatch.setattr("lumen.external_apps.providers.base.requests.post", called)
...
def test_refresh_network_error_is_transient(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("lumen.external_apps.providers.base.requests.post", _boom)
    with pytest.raises(TokenRefreshTransientError):
        GoogleCalendarProvider().refresh_credentials({"refresh_token": "rt"}, "c", "s")
...
def _capturing_post(
    ...
    monkeypatch.setattr("lumen.external_apps.providers.base.requests.post", _post)
    return captured

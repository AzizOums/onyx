"""Unit tests for `LumenPatResolver`.

Coverage: the host claim rule (incl. inert when `LUMEN_SERVER_URL` is
...
is pinned in `tests/external_dependency_unit/sandbox_proxy/test_lumen_pat_resolver.py`.
"""
...
from lumen.sandbox_proxy.resolvers import lumen_pat
...
_API_URL = "https://api.lumen.example.com"
_API_HOST = "api.lumen.example.com"
...
def _patch_session(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(lumen_pat, "get_session_with_tenant", _noop_db_factory())
...
@pytest.fixture
def resolver(monkeypatch: pytest.MonkeyPatch) -> LumenPatResolver:
    # __init__ captures the API host, so the patch must precede construction.
    monkeypatch.setattr(lumen_pat, "LUMEN_SERVER_URL", _API_URL)
    return LumenPatResolver()


def test_claims_requires_matching_host_and_port(resolver: LumenPatResolver) -> None:
...
def test_claims_matches_explicit_port(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(lumen_pat, "LUMEN_SERVER_URL", "http://internal:8080")
...
def test_inert_when_api_url_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(lumen_pat, "LUMEN_SERVER_URL", "")
    assert (
        LumenPatResolver().claims(make_flow(host=_API_HOST, port=443).request, _ctx())
        is False
    )


def test_resolve_renders_pat_on_both_auth_headers(
    resolver: LumenPatResolver, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        lumen_pat,
        "get_sandbox_by_id",
        lambda *_: _FakeSandbox(_FakeEncrypted(value="lumen_pat_t.secret")),
    )
    headers = resolver.resolve(make_flow(host=_API_HOST).request, _ctx())
    assert headers == {
        API_KEY_HEADER_NAME: "Bearer lumen_pat_t.secret",
        API_KEY_HEADER_ALTERNATIVE_NAME: "Bearer lumen_pat_t.secret",
    }
...
def test_resolve_raises_when_sandbox_missing(
    ...
    monkeypatch.setattr(lumen_pat, "get_sandbox_by_id", lambda *_: None)
...
def test_resolve_raises_when_pat_unset(
    ...
    monkeypatch.setattr(lumen_pat, "get_sandbox_by_id", lambda *_: _FakeSandbox(None))
    with pytest.raises(CredentialUnavailableError):
        resolver.resolve(make_flow(host=_API_HOST).request, _ctx())

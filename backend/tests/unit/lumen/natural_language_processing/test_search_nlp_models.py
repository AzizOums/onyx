...
async def test_cohere_embed_supports_v3_response_format(
    ...
    with patch(
        "lumen.natural_language_processing.search_nlp_models.CohereAsyncClient"
    ) as mock_cohere:
        mock_client = AsyncMock()
...
async def test_rate_limit_handling() -> None:
    with patch(
        "lumen.natural_language_processing.search_nlp_models.CloudEmbedding.embed"
    ) as mock_embed:
...
_SEARCH_NLP_MODULE = "lumen.natural_language_processing.search_nlp_models"

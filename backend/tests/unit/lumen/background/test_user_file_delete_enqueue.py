...
from lumen.background.task_utils import drain_delete_loop, enqueue_user_file_deletes

MODULE = "lumen.background.task_utils"
TENANT = "tenant_test"
...
def _enqueue(file_count: int, lite: bool) -> tuple[MagicMock, BackgroundTasks]:
    ...
    with (
        ...
        patch.dict(
            "sys.modules",
            {"lumen.background.celery.versioned_apps.client": MagicMock(app=client)},
        ),
    ):

...
from lumen.background.periodic_poller import _build_periodic_tasks, _run_license_reclaim


def _task_names() -> list[str]:
    with patch("lumen.utils.variable_functionality.global_version") as mock_version:
...
def test_license_reclaim_lock_id_is_unique() -> None:
    ...
    with patch("lumen.utils.variable_functionality.global_version") as mock_version:
        mock_version.is_ee_version.return_value = True
        with patch("lumen.configs.app_configs.AUTO_LLM_CONFIG_URL", "http://llm"):
            with patch("lumen.configs.app_configs.SCHEDULED_EVAL_DATASET_NAMES", ["ds"]):
...
@patch("lumen.utils.variable_functionality.fetch_versioned_implementation")

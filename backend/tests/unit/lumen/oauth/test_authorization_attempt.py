...
from lumen.oauth.authorization_attempt import AuthorizationAttemptStore
from lumen.oauth.models import AuthorizationAttempt
...
def test_store_generates_isolated_one_time_attempts() -> None:
    ...
    with pytest.raises(LumenError) as error:
        store.consume(owner_id="user-1", state=first.state)
    assert error.value.error_code is LumenErrorCode.INVALID_INPUT
...
def test_store_does_not_overwrite_duplicate_state() -> None:
    ...
    assert error.value.error_code is LumenErrorCode.CONFLICT

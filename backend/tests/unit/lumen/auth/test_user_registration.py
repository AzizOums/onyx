...
from lumen.auth.schemas import UserCreate
...
def _no_pinned_persona_seeding() -> Iterator[None]:
    ...
    with patch(
        "lumen.auth.users.seed_pinned_personas_from_featured", new_callable=AsyncMock
    ):
        yield
...
class TestDisposableEmailValidation:
    ...
    @pytest.mark.asyncio
    @patch("lumen.auth.users.is_disposable_email")
    @patch("lumen.auth.users.fetch_ee_implementation_or_noop")
    @patch("lumen.auth.users.get_async_session_context_manager")
    @patch("lumen.auth.users.get_user_count", new_callable=AsyncMock)
    ...
    @pytest.mark.asyncio
    @patch("lumen.auth.users.is_disposable_email")
    @patch("lumen.auth.users.verify_email_domain")
    @patch("lumen.auth.users.fetch_ee_implementation_or_noop")
    @patch("lumen.auth.users.get_async_session_context_manager")
    @patch("lumen.auth.users.get_user_count", new_callable=AsyncMock)
    @patch("lumen.auth.users.MULTI_TENANT", False)
...
class TestMultiTenantInviteLogic:
    ...
    @patch("lumen.auth.users.SQLAlchemyUserDatabase")
    @patch("lumen.auth.users.is_disposable_email", return_value=False)
    @patch("lumen.auth.users.verify_email_domain")
    @patch("lumen.auth.users.fetch_ee_implementation_or_noop")
    @patch("lumen.auth.users.get_async_session_context_manager")
    @patch("lumen.auth.users.get_user_count", new_callable=AsyncMock)
    @patch("lumen.auth.users.verify_email_is_invited")
    @patch("lumen.auth.users.MULTI_TENANT", True)
    @patch("lumen.auth.users.CURRENT_TENANT_ID_CONTEXTVAR")
    @pytest.mark.asyncio
...
class TestWhitelistBehavior:
    ...
    @patch("lumen.auth.users.workspace_invite_only_enabled", return_value=False)
    @patch("lumen.auth.users.get_invited_users")
    def test_empty_whitelist_allows_all(
        ...
        from lumen.auth.users import verify_email_is_invited
    ...
    @patch("lumen.auth.users.workspace_invite_only_enabled", return_value=True)
    @patch("lumen.auth.users.get_invited_users")
...
class TestSeatLimitEnforcement:
    ...
    def test_adding_user_fails_when_seats_full(self) -> None:
        from lumen.auth.users import enforce_seat_limit
    ...
    def test_cloud_locked_variant_forwards_session_to_billing(self) -> None:
        from lumen.auth.users import enforce_seat_limit_locked
        ...
        with (
            patch("lumen.auth.users.MULTI_TENANT", True),
            patch(
                "lumen.auth.users.get_current_tenant_id",
...
class TestOAuthDottedGmail:
    ...
    @pytest.mark.asyncio
    @patch("lumen.auth.users.MULTI_TENANT", False)
    @patch("lumen.auth.users.verify_email_in_whitelist")
    @patch("lumen.auth.users.verify_email_domain")
    @patch("lumen.auth.users.fetch_ee_implementation_or_noop")
    @patch("lumen.auth.users.get_async_session_context_manager")
    @patch("lumen.auth.users.remove_user_from_invited_users")
    @patch("lumen.auth.users.SQLAlchemyUserDatabase")
...
class TestOAuthNoAutoLinkExemptions:
    ...
    @patch("lumen.auth.users.get_async_session_context_manager")
    @patch("lumen.auth.users.remove_user_from_invited_users")
    @patch("lumen.auth.users.assign_user_to_default_groups__no_commit")
    @patch("lumen.auth.users._upgrade_will_add_seat", return_value=False)
    @patch("lumen.auth.users.get_session_with_current_tenant")
    @patch("lumen.auth.users.SQLAlchemyUserDatabase")
...
class TestPasswordAuthKillSwitch:
    ...
    @pytest.mark.asyncio
    @patch("lumen.auth.users.get_security_settings")
    async def test_signup_disabled_blocks_public_registration(
        ...
        assert exc.value.error_code is LumenErrorCode.REGISTRATION_DISABLED
        assert exc.value.status_code == 403
    ...
    async def test_signup_disabled_allows_sso_provisioning(
        ...
        try:
            ...
            assert e.error_code is not LumenErrorCode.REGISTRATION_DISABLED
        except Exception:
            pass
    ...
    @pytest.mark.asyncio
    @patch("lumen.auth.users.emit_audit_event")
    @patch("lumen.auth.users.fetch_ee_implementation_or_noop")
    @patch("lumen.auth.users.get_security_settings")

"""The SSO web-error decorator turns LumenError into a readable /auth/error
...
from lumen.auth.users import get_pkce_cookie_name
...
async def _handler(*, request: Request, raise_error: bool) -> RedirectResponse:  # noqa: ARG001
    if raise_error:
        raise LumenError(LumenErrorCode.UNAUTHORIZED, "invite only")
...
async def test_non_browser_gets_standard_json_error() -> None:
    ...
    assert resp.status_code == LumenErrorCode.UNAUTHORIZED.status_code
    body = json.loads(bytes(resp.body))
    assert body["error_code"] == LumenErrorCode.UNAUTHORIZED.code
...
async def test_no_request_kwarg_reraises() -> None:
    ...
    async def _no_request_handler(**_: Any) -> RedirectResponse:
        raise LumenError(LumenErrorCode.UNAUTHORIZED, "nope")
...
async def test_enum_detail_uses_value_in_url() -> None:
    ...
    async def _enum_handler(*, request: Request) -> RedirectResponse:  # noqa: ARG001
        raise LumenError(LumenErrorCode.VALIDATION_ERROR, _Code.SAMPLE)

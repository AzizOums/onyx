...
import lumen.auth.users as users
from lumen.auth.users import verify_email_domain
...
def test_verify_email_domain_allows_plus_for_lumen_app(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(users, "MULTI_TENANT", True, raising=False)

    # Should not raise for lumen.app domain
    verify_email_domain("user+tag@lumen.app", valid_email_domains=[])

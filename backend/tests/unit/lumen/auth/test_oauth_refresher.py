...
from lumen.auth import oauth_refresher
from lumen.auth.oauth_refresher import (
...
from lumen.db.models import OAuthAccount
...
async def test_refresh_oauth_token_success(
    ...
    with patch("lumen.auth.oauth_refresher.httpx.AsyncClient") as client_class_mock:
...
async def test_check_and_refresh_oauth_tokens(
    ...
    with patch(
        "lumen.auth.oauth_refresher.refresh_oauth_token", AsyncMock(return_value=True)
    ) as mock_refresh:
        # Call the function under test
...
async def test_check_and_refresh_oauth_tokens_coalesces_concurrent_refresh(
    ...
    with patch(
        "lumen.auth.oauth_refresher.refresh_oauth_token",

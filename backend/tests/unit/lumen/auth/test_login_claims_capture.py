...
import lumen.auth.login_claims_capture as claims_capture
from lumen.auth.login_claims_capture import (
...
async def test_capture_stores_id_token_claims_and_token_meta() -> None:
    ...
    with patch(
        "lumen.auth.login_claims_capture.get_async_redis_connection",
        return_value=redis,
    ):
...
async def test_capture_fetches_userinfo_when_endpoint_available() -> None:
    ...
    with (
        ...
        patch(
            "lumen.auth.login_claims_capture._fetch_userinfo",
...
async def test_capture_fetches_directory_profile_for_entra() -> None:
    ...
    with (
        ...
        patch(
            "lumen.auth.login_claims_capture._fetch_ms_graph_profile",
...
def test_get_idp_profile_fields_maps_directory_profile() -> None:
    ...
    with patch("lumen.redis.redis_pool.get_raw_redis_client", return_value=redis):
...
async def test_multi_tenant_capture_keys_by_mapped_tenant(
    ...
    with (
        ...
        patch(
            "lumen.auth.login_claims_capture.fetch_ee_implementation_or_noop",

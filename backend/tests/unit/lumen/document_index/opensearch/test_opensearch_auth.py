...
import lumen.configs.app_configs as app_configs
from lumen.document_index.opensearch.client import OpenSearchClient
...
def test_iam_auth_uses_sigv4_signer() -> None:
    ...
    """
    with (
        patch("lumen.document_index.opensearch.client.OpenSearch") as mock_os,
        patch("lumen.document_index.opensearch.client.boto3.Session") as mock_session,
    ):
...
def test_basic_auth_uses_tuple() -> None:
    ...
    """
    with patch("lumen.document_index.opensearch.client.OpenSearch") as mock_os:
        OpenSearchClient(
            host="h",
            auth=("u", "p"),
...
def test_iam_without_credentials_raises() -> None:
    with patch("lumen.document_index.opensearch.client.boto3.Session") as mock_session:

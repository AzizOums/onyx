...
from lumen.lumenbot.discord.cache import DiscordCacheManager, TenantDiscordData
...
class TestCacheInitialization:
    ...
    async def test_cache_refresh_all_loads_guilds(self) -> None:
        ...
        with (
            patch(
                "lumen.lumenbot.discord.cache.get_all_tenant_ids",
                return_value=["tenant1"],
            ),
            patch(
                "lumen.lumenbot.discord.cache.fetch_ee_implementation_or_noop",
                return_value=lambda: set(),
            ),
            patch("lumen.lumenbot.discord.cache.get_session_with_tenant") as mock_session,
            patch(
                "lumen.lumenbot.discord.cache.get_guild_configs",
                return_value=[mock_config1, mock_config2],
            ),
            patch(
                "lumen.lumenbot.discord.cache.get_or_create_discord_service_api_key",

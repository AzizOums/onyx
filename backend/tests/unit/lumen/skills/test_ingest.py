...
def test_ingest_rejects_missing_frontmatter() -> None:
    ...
    with pytest.raises(LumenError, match="must start with YAML frontmatter") as exc_info:
...
def test_ingest_rejects_built_in_name() -> None:
    ...
    with pytest.raises(LumenError, match="skill name 'pptx' is reserved"):
        ingest_skill_bundle(skill_md, "SKILL.md", file_store)
...
def test_ingest_replacement_rejects_changed_name() -> None:
    ...
    with pytest.raises(LumenError, match="must remain 'existing-skill'"):
        ingest_skill_bundle(
            skill_md,
            "SKILL.md",
            file_store,
...
def test_ingested_skill_bundle_deletes_new_blob_on_failure(
    ...
    monkeypatch.setattr(
        "lumen.skills.ingest.ingest_skill_bundle",
    ...
    monkeypatch.setattr(
        "lumen.skills.ingest.delete_bundle_blob",
        delete_bundle_blob,
    )

...
from lumen.skills import ingest_from_github
from lumen.skills.bundle import NormalizedSkillBundle
...
def _mock_repository(
    ...
    monkeypatch.setattr("lumen.utils.github.ssrf_safe_get", get)
...
def test_invalid_and_reserved_skills_do_not_block_valid_skills(
    ...
    assert by_name["pptx"].unavailable_reason == (
        "A built-in Lumen skill already uses this name."
    )
...
def test_rejects_repositories_with_too_many_skills(
    ...
    with pytest.raises(LumenError, match="more than 500 skills") as exc_info:
...
def test_truncated_archive_returns_retryable_error(
    ...
    with pytest.raises(LumenError, match="unreadable repository download") as exc_info:
        fetch_github_skill_bundles("owner/repository")
...
def test_rejects_repository_symlinks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(LumenError, match="symbolic link"):
        fetch_github_skill_bundles("owner/repository")

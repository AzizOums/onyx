...
from lumen.skills.bundle import _ZIP_UNIX_CREATE_SYSTEM, normalize_custom_bundle
...
def test_validator_rejects_path_traversal(bad_path: str) -> None:
    ...
    with pytest.raises(LumenError, match="escapes root"):
...
def test_validator_rejects_symlink() -> None:
    ...
    with pytest.raises(LumenError, match="symlink"):
        normalize_custom_bundle(zip_bytes)

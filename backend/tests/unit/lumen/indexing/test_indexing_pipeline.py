...
from lumen.hooks.executor import HookSkipped, HookSoftFailed
...
from lumen.llm.model_response import Choice, Message, ModelResponse
...
def test_filter_documents_exceeding_max_chars() -> None:
    ...
    with patch("lumen.indexing.indexing_pipeline.MAX_DOCUMENT_CHARS", limit):
        docs, failures = filter_documents([doc])
    assert len(docs) == 0
...
@patch("lumen.llm.model_capabilities.GEN_AI_MAX_TOKENS", 4096)
...
_PATCH_EXECUTE_HOOK = "lumen.indexing.indexing_pipeline.execute_hook"
_PATCH_GET_SESSION = "lumen.indexing.indexing_pipeline.get_session_with_current_tenant"
...
_PATCH_MULTI_TENANT = "lumen.indexing.indexing_pipeline.MULTI_TENANT"
_PATCH_GET_CC_PAIR = "lumen.indexing.indexing_pipeline.get_connector_credential_pair"
_PATCH_GET_SESSION_AW = (
    "lumen.indexing.indexing_pipeline.get_session_with_current_tenant"
)
_PATCH_EXECUTE_HOOK = "lumen.indexing.indexing_pipeline.execute_hook"
...
def _make_insertion_records(doc_ids: list[str]) -> list[Any]:
    from lumen.document_index.interfaces_new import DocumentInsertionRecord
...
def test_document_push_skipped_when_from_beginning() -> None:
    from lumen.indexing.indexing_pipeline import _maybe_push_documents
...
def test_document_push_skipped_in_multi_tenant_mode() -> None:
    ...
    with (
        ...
        patch(
            "lumen.indexing.indexing_pipeline.get_document_push_config",
            return_value=None,
        ),
        patch(_PATCH_EXECUTE_HOOK) as mock_hook,
    ):
...
def test_document_push_config_sink_skipped_in_multi_tenant_mode() -> None:
    from lumen.indexing.indexing_pipeline import _maybe_push_documents
    from lumen.utils.external_endpoint import ExternalEndpointConfig
    ...
    with (
        ...
        patch(_PATCH_EXECUTE_HOOK) as mock_hook,
        patch(
            "lumen.indexing.indexing_pipeline.push_document_via_config"
        ) as mock_config_push,
    ):
        _maybe_push_documents(_make_adapter(), [doc], _make_insertion_records(["doc1"]))
...
def test_document_push_fires_execute_hook_for_public_doc() -> None:
    from lumen.db.enums import HookPoint
    from lumen.indexing.document_push import DocumentPushResponse
    from lumen.indexing.indexing_pipeline import _maybe_push_documents
...
_PATCH_PREFIX = "lumen.indexing.indexing_pipeline"
...
def test_content_hash_changes_with_owners() -> None:
    from lumen.connectors.models import BasicExpertInfo

...
from lumen.hooks.points.query_processing import QueryProcessingResponse
...
def test_null_query_raises_query_rejected() -> None:
    ...
    assert exc_info.value.error_code is LumenErrorCode.QUERY_REJECTED
...
def test_fallback_rejection_message_when_none() -> None:
    """No rejection_message → generic fallback used in LumenError detail."""
    with pytest.raises(LumenError) as exc_info:
        _resolve_query_processing_hook_result(

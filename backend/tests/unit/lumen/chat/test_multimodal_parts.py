...
from lumen.chat.models import ChatLoadedFile, ChatMessageSimple
...
def test_audio_part_built_for_capable_model() -> None:
    ...
    with (
        patch("lumen.chat.llm_step.model_supports_image_input", return_value=False),
        patch("lumen.chat.llm_step.model_supports_audio_input", return_value=True),
        patch("lumen.chat.llm_step.model_supports_video_input", return_value=False),
    ):
...
def test_video_part_built_for_capable_model() -> None:
    ...
    with (
        patch("lumen.chat.llm_step.model_supports_image_input", return_value=False),
        patch("lumen.chat.llm_step.model_supports_audio_input", return_value=False),
        patch("lumen.chat.llm_step.model_supports_video_input", return_value=True),
    ):
        from typing import cast
...
def test_opencode_oversized_video_degrades_to_marker() -> None:
    ...
    with (
        ...
        patch("lumen.chat.llm_step.model_supports_audio_input", return_value=False),
        patch("lumen.chat.llm_step.model_supports_video_input", return_value=True),
        patch("lumen.chat.llm_step.OPENCODE_MAX_VIDEO_BASE64_CHARS", 4),
    ):
        from typing import cast

...
from lumen.db.engine.async_sql_engine import reset_sqlalchemy_async_engine
...
async def test_lifespan_shutdown_disposes_all_three_engines() -> None:
    ...
    """
    from lumen import main as lumen_main
    ...
    with ExitStack() as stack:
        ...
        reset_async = stack.enter_context(
            patch.object(lumen_main, "reset_sqlalchemy_async_engine", new=AsyncMock())
        )
        stack.enter_context(
            patch.object(
                lumen_main,
        ...
        stack.enter_context(
            patch.object(lumen_main, "setup_postgres_connection_pool_metrics")
        )
        stack.enter_context(patch.object(lumen_main, "validate_no_vector_db_settings"))
        stack.enter_context(patch.object(lumen_main, "validate_cache_backend_settings"))
        stack.enter_context(patch.object(lumen_main, "validate_registry"))
        stack.enter_context(patch.object(lumen_main, "verify_user_auth_secret"))
        stack.enter_context(
            patch.object(
                lumen_main,
        ...
        stack.enter_context(patch.object(lumen_main, "setup_tracing"))
        stack.enter_context(
            patch.object(lumen_main, "warm_up_connections", new=AsyncMock())
        )
        stack.enter_context(patch.object(lumen_main, "get_session_with_current_tenant"))
        stack.enter_context(patch.object(lumen_main, "setup_lumen"))
        stack.enter_context(patch.object(lumen_main, "get_default_file_store"))
        stack.enter_context(patch.object(lumen_main, "get_or_generate_uuid"))
        stack.enter_context(patch.object(lumen_main, "optional_telemetry"))
        stack.enter_context(patch.object(lumen_main, "MULTI_TENANT", False))
        stack.enter_context(patch.object(lumen_main, "RATE_LIMITING_ENABLED", False))
        stack.enter_context(patch.object(lumen_main, "DISABLE_VECTOR_DB", False))
        stack.enter_context(patch.object(lumen_main, "OAUTH_CLIENT_ID", ""))
        stack.enter_context(patch.object(lumen_main, "OAUTH_CLIENT_SECRET", ""))
        stack.enter_context(patch.object(lumen_main, "SYSTEM_RECURSION_LIMIT", None))

        async with lumen_main.lifespan(MagicMock()):

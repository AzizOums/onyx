...
from lumen.configs.constants import SLACK_SERVICE_ACCOUNT_EMAIL
from lumen.db.enums import AccountType
from lumen.db.users import get_or_create_slack_service_account


def test_create_slack_service_account() -> None:
    db_session = MagicMock()
    with (
        patch("lumen.db.users.get_user_by_email", return_value=None),
        patch("lumen.db.users._generate_password_hash", return_value="hash"),
    ):
...
def test_reuse_slack_service_account() -> None:
    db_session = MagicMock()
    existing = MagicMock()
    with patch("lumen.db.users.get_user_by_email", return_value=existing):

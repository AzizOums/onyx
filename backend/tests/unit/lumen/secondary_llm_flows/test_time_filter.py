...
from lumen.secondary_llm_flows.time_filter import (
    DocumentTimeField,
    TimeFilter,
...
def _run_decision(
    ...
    with (
        patch(
            "lumen.secondary_llm_flows.time_filter.llm_generation_span",
            return_value=nullcontext(MagicMock()),
        ),
        patch("lumen.secondary_llm_flows.time_filter.record_llm_response"),
    ):

...
from lumen.server.query_and_chat.chat_backend import (
...
def test_rate_limited_chat_naming_returns_fallback() -> None:
    ...
    with (
        patch(
            "lumen.server.query_and_chat.chat_backend.check_token_rate_limits",
            side_effect=LumenError(LumenErrorCode.RATE_LIMITED),
        ),
        patch(
            "lumen.server.query_and_chat.chat_backend.get_llm_for_persona"

...
class TestValidateNoVectorDbSettings:
    @patch("lumen.main.DISABLE_VECTOR_DB", False)
    def test_no_error_when_vector_db_enabled(self) -> None:
        from lumen.main import validate_no_vector_db_settings

        validate_no_vector_db_settings()

    @patch("lumen.main.DISABLE_VECTOR_DB", True)
    @patch("lumen.main.MULTI_TENANT", False)
    @patch("lumen.server.features.build.configs.ENABLE_CRAFT", False)
    ...
    @patch("lumen.main.DISABLE_VECTOR_DB", True)
    @patch("lumen.main.MULTI_TENANT", True)
    ...
    @patch("lumen.main.DISABLE_VECTOR_DB", True)
    @patch("lumen.main.MULTI_TENANT", False)
    @patch("lumen.server.features.build.configs.ENABLE_CRAFT", True)

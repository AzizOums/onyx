...
import lumen.llm.models
from lumen.configs.app_configs import MOCK_LLM_RESPONSE
from lumen.llm.constants import LlmProviderNames
from lumen.llm.interfaces import LLMUserIdentity
from lumen.llm.model_capabilities import get_max_input_tokens
from lumen.llm.model_response import ModelResponse, ModelResponseStream, Usage
...
from lumen.llm.multi_llm import (
    LitellmLLM,
    LLMTimeoutError,
...
def test_claude_adaptive_thinking_uses_output_config(
    ...
    with (
        ...
        patch("lumen.llm.multi_llm.model_is_reasoning_model", return_value=True),
    ):
        mock_completion.return_value = []
...
def test_openai_auto_reasoning_effort_maps_to_medium() -> None:
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch("lumen.llm.multi_llm.model_is_reasoning_model", return_value=True),
        patch("lumen.llm.multi_llm.is_true_openai_model", return_value=True),
    ):
        mock_completion.return_value = []
...
def test_aliased_claude_model_still_reasons() -> None:
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch("lumen.llm.multi_llm.model_is_reasoning_model", return_value=False),
    ):
        mock_completion.return_value = []
...
def test_openai_chat_omits_reasoning_params() -> None:
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch(
            "lumen.llm.multi_llm.model_is_reasoning_model", return_value=True
        ) as mock_is_reasoning,
        patch(
            "lumen.llm.multi_llm.is_true_openai_model", return_value=True
        ) as mock_is_openai,
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
...
def _stream_and_get_completion_kwargs(
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch("lumen.llm.multi_llm.is_true_openai_model", return_value=is_openai),
    ):
        mock_completion.return_value = []
        messages: LanguageModelInput = [UserMessage(content="Hi")]
        list(llm.stream(messages))
...
def test_reasoning_effort_omitted_for_models_rejecting_it(
    ...
    with (
        ...
        patch("lumen.llm.multi_llm.model_is_reasoning_model", return_value=True),
        patch(
            "lumen.llm.multi_llm.is_true_openai_model",
...
def test_user_identity_metadata_enabled(default_multi_llm: LitellmLLM) -> None:
    with (
        patch("litellm.completion") as mock_completion,
        patch("lumen.llm.utils.SEND_USER_METADATA_TO_LLM_PROVIDER", True),
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
                id="chatcmpl-123",
...
def test_user_identity_metadata_disabled_omits_identity(
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch("lumen.llm.utils.SEND_USER_METADATA_TO_LLM_PROVIDER", False),
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
                id="chatcmpl-123",
...
def test_multithreaded_invoke_without_custom_config_does_not_inject_env() -> None:
    ...
    """
    from lumen.llm import multi_llm as multi_llm_module
...
def test_messages_contain_tool_content_with_tool_role() -> None:
    from lumen.llm.multi_llm import _messages_contain_tool_content
...
def test_strip_tool_content_converts_assistant_tool_calls_to_text() -> None:
    from lumen.llm.multi_llm import _strip_tool_content_from_messages
...
def test_prompt_contains_tool_call_history_true() -> None:
    from lumen.llm.multi_llm import _prompt_contains_tool_call_history
...
def test_injection_disabled_maps_kwargs_and_never_touches_env(
    ...
    with (
        ...
        patch(
            "lumen.llm.multi_llm._env_injection_enabled",
            return_value=False,
        ),
        patch.object(
            multi_llm_module,
...
def test_ui_only_keys_never_injected_or_warned(
    ...
    for injection_enabled in (True, False):
        with (
            ...
            patch("lumen.llm.multi_llm._warn_dropped_env_only_keys") as mock_warn,
        ):
            llm.invoke([UserMessage(content="Hi")])
...
def _completion_message_roles(llm: LitellmLLM) -> list[str]:
    with (
        patch("litellm.completion") as mock_completion,
        patch("lumen.llm.multi_llm.is_true_openai_model", return_value=False),
    ):
        mock_completion.return_value = []
...
def test_track_llm_cost_prices_cache_creation_at_write_rate(
    ...
    monkeypatch.setattr(
        "lumen.server.usage_limits.is_usage_limits_enabled", lambda: True
    )
    monkeypatch.setattr(
        "lumen.server.usage_limits.is_lumen_managed_api_key", lambda _key: True
    )
    monkeypatch.setattr(
        "lumen.db.engine.sql_engine.get_session_with_current_tenant", _fake_session
    )
    monkeypatch.setattr(
        "lumen.llm.cost.cost_overrides.get_override",
        lambda _session, _model, _provider: None,
    )
    increment_usage = MagicMock()
    monkeypatch.setattr("lumen.db.usage.increment_usage", increment_usage)

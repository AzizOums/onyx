"""Tests for the token-limit lookup helpers in `lumen.llm.model_capabilities`:
`llm_max_input_tokens`, `get_llm_max_output_tokens`, and `get_max_input_tokens`."""

from unittest.mock import patch

from lumen.configs.model_configs import GEN_AI_MODEL_FALLBACK_MAX_TOKENS
...
class TestLlmMaxInputTokens:
    ...
    def test_override_env_var_wins(self) -> None:
        ...
        with patch("lumen.llm.model_capabilities.GEN_AI_MAX_TOKENS", 5000):
            assert (
                llm_max_input_tokens(
                    model_map=model_map,
                    model_name="gpt-4o",
...
class TestGetMaxInputTokens:
    def test_subtracts_reserved_output_tokens(self) -> None:
        model_map = {"openai/gpt-4o": {"max_input_tokens": 128000}}
        with patch("lumen.llm.model_capabilities.get_model_map", return_value=model_map):
            assert (
                get_max_input_tokens(
                    model_name="gpt-4o",
                    model_provider="openai",

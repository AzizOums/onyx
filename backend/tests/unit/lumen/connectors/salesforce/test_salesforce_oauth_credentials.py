...
from lumen.connectors.interfaces import CredentialsProviderInterface
from lumen.connectors.salesforce import auth, salesforce_calls
...
from lumen.connectors.salesforce.models import (
...
@patch("lumen.connectors.salesforce.auth.LumenSalesforce")
...
@patch("lumen.connectors.salesforce.connector.build_salesforce_client")
...
def test_authorization_code_exchange_uses_typed_form(
    ...
    credentials = exchange_salesforce_authorization_code(
        ...
        redirect_uri="https://lumen.example/oauth/callback",
...
@patch("lumen.utils.retry_wrapper.requests.request")
...
def test_refresh_callback_persists_rotated_credentials(
    ...
    with patch(
        "lumen.connectors.salesforce.auth.refresh_salesforce_oauth_credentials",
...
def test_lumen_salesforce_refreshes_invalid_session_mid_run() -> None:
...
def test_lumen_salesforce_routes_malformed_unauthorized_response(
    payload: Any,
) -> None:
...
def test_lumen_salesforce_routes_non_json_unauthorized_response() -> None:
    unauthorized = MagicMock(
        status_code=401,
        url=f"{INSTANCE_URL}/services/data/v59.0/sobjects",
        text="unauthorized",
...
def test_bulk_download_rebuilds_client_after_session_refresh(tmp_path: Path) -> None:
    ...
    sf_client = MagicMock(spec=LumenSalesforce)
...
def test_salesforce_authorization_url_uses_my_domain_and_pkce(
    ...
    authorization_url = SalesforceConnector.oauth_authorization_url(
        base_domain="https://lumen.example/",
        state="oauth-state",
    ...
    assert parse_qs(parsed.query) == {
        "client_id": ["client-id"],
        "response_type": ["code"],
        "redirect_uri": ["https://lumen.example/connector/oauth/callback/salesforce"],

...
from lumen.connectors.cross_connector_utils.miscellaneous_utils import time_str_to_utc
from lumen.connectors.models import BasicExpertInfo, Document, ImageSection, TextSection
from lumen.connectors.salesforce.doc_conversion import ID_PREFIX, _extract_section
from lumen.connectors.salesforce.lumen_salesforce import LumenSalesforce
from lumen.connectors.salesforce.salesforce_calls import (
    _bulk_retrieve_from_salesforce,
    _make_time_filter_for_sf_type,
    _make_time_filtered_query,
    get_object_by_id_query,
)
from lumen.connectors.salesforce.sqlite_functions import LumenSalesforceSQLite
from lumen.connectors.salesforce.utils import (
    ACCOUNT_OBJECT_TYPE,
    MODIFIED_FIELD,
    USER_OBJECT_TYPE,
)
...
# from lumen.connectors.salesforce.lumen_salesforce_type import LumenSalesforceType
# from lumen.connectors.salesforce.salesforce_calls import get_children_of_sf_type
...
def _create_csv_with_example_data(sf_db: LumenSalesforceSQLite) -> None:
    """
...
def _test_query(sf_db: LumenSalesforceSQLite) -> None:
    """
...
def _test_upsert(sf_db: LumenSalesforceSQLite) -> None:
    """
...
def _test_relationships(sf_db: LumenSalesforceSQLite) -> None:
    """
...
def _test_account_with_children(sf_db: LumenSalesforceSQLite) -> None:
    """
...
def _test_relationship_updates(sf_db: LumenSalesforceSQLite) -> None:
    """
...
def _test_get_affected_parent_ids(sf_db: LumenSalesforceSQLite) -> None:
    """
...
def test_salesforce_sqlite() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        sf_db = LumenSalesforceSQLite(filename)
        sf_db.connect()
        sf_db.apply_schema()
...
def test_salesforce_bulk_retrieve() -> None:
    ...
    # lumen_sf_type = LumenSalesforceType("Contact", sf_client)
...
def test_normalize_record() -> None:
    ...
    with open(current_dir / "test_account.csv", "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            normalized_record, parent_ids = LumenSalesforceSQLite.normalize_record(row)
...
def test_salesforce_connector_single() -> None:
    ...
    # lumen_parent_sf_type = LumenSalesforceType(parent_type, sf_client)
    ...
    for parent_type_working in parent_types:
        ...
        for child_type, child_relationship in child_types_working.items():
            # lumen_sf_type = LumenSalesforceType(child_type, sf_client)
    ...
    normalized_record, _ = LumenSalesforceSQLite.normalize_record(record)

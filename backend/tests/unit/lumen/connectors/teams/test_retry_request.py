...
from lumen.connectors.teams.utils import GRAPH_API_RETRYABLE_STATUSES, _retry
...
def test_returns_json_without_retry_on_success() -> None:
    ...
    with patch("lumen.connectors.teams.utils.time.sleep") as mock_sleep:
...
def test_raises_when_json_is_not_an_object() -> None:
    ...
    with patch("lumen.connectors.teams.utils.time.sleep"):

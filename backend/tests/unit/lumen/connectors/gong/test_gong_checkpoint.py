...
from lumen.connectors.gong.connector import GongConnector, GongConnectorCheckpoint
from lumen.connectors.models import ConnectorFailure, Document
...
class TestGongConnectorCheckpoint:
    ...
    def test_missing_call_details_yields_failure(
        ...
        with patch(
            "lumen.connectors.gong.connector.time.time", side_effect=_advance_clock
        ):
    ...
    def test_backoff_gate_prevents_retry_too_soon(
        ...
        with patch("lumen.connectors.gong.connector.time.time", return_value=fixed_now):
    ...
    def test_pending_retry_does_not_block_on_time_sleep(
        ...
        with (
            patch("lumen.connectors.gong.connector.time.sleep") as mock_sleep,
            patch(
                "lumen.connectors.gong.connector.time.time", side_effect=_advance_clock
            ),
        ):

...
from lumen.connectors.github.utils import normalize_github_base_url
...
def _ssrf_env(
    ...
    with patch("lumen.connectors.github.utils.get_security_settings") as settings:
        settings.return_value.ssrf_protection_level = level
        with patch("lumen.utils.url.socket.getaddrinfo") as getaddrinfo:
...
def _connector() -> GithubConnector:
    return GithubConnector(repo_owner="lumen-dot-app", repositories="lumen")


def test_credential_base_url_is_used(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "lumen.connectors.github.connector.GITHUB_CONNECTOR_BASE_URL", None
    )
...
def test_credential_base_url_wins_over_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "lumen.connectors.github.connector.GITHUB_CONNECTOR_BASE_URL",
...
class TestCredentialBaseUrlSSRF:
    ...
    def test_private_server_allowed_when_admin_relaxes_the_level(self) -> None:
        """Self-hosted Lumen usually reaches Enterprise Server over the LAN."""

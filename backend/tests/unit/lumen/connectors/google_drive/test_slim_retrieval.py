...
from lumen.connectors.google_drive.file_retrieval import DriveFileFieldType
from lumen.connectors.google_drive.models import (
    DriveRetrievalStage,
    GoogleDriveCheckpoint,
    StageCompletion,
)
from lumen.connectors.google_utils.resources import ImpersonationError
from lumen.connectors.interfaces import SlimConnector, SlimConnectorWithPermSync
...
_DOC_CONVERSION_MODULE = "lumen.connectors.google_drive.doc_conversion"
...
class TestFailedFolderIdsByEmail:
    ...
    def test_skips_api_call_for_known_failed_pair(self) -> None:
        ...
        with patch(
            "lumen.connectors.google_drive.connector.get_folder_metadata"
        ) as mock_api:
...
class TestImpersonateUserRefreshError:
    def test_user_removed_error_skips_silently(self) -> None:
        ...
        with (
            ...
            patch(
                "lumen.connectors.google_drive.connector.get_root_folder_id",
                side_effect=RefreshError("invalid_grant: Invalid email or User ID"),
            ),
            patch(
                "lumen.connectors.google_drive.connector.retry_builder",

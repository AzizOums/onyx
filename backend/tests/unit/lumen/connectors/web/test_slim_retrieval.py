...
def _skip_web_connector_ssrf_check(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "lumen.connectors.web.connector.protected_url_check", lambda _url: None
    )
...
@patch("lumen.connectors.web.connector.check_internet_connection")
@patch("lumen.connectors.web.connector.requests.head")
@patch("lumen.connectors.web.connector.start_playwright")
...
def test_slim_skips_content_extraction(
    ...
    with patch("lumen.connectors.web.connector.web_html_cleanup") as mock_cleanup:
...
@patch("lumen.connectors.web.connector.time")
@patch("lumen.connectors.web.connector.check_internet_connection")
@patch("lumen.connectors.web.connector.requests.head")
@patch("lumen.connectors.web.connector.start_playwright")

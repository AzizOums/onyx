...
from lumen.connectors.gmail.connector import (
    GmailCheckpoint,
    GmailConnector,
...
def test_thread_to_document() -> None:
    ...
    doc = thread_to_document(full_email_thread, "admin@lumen-test.com")
...
def test_gmail_checkpoint_progression() -> None:
    ...
    with patch.object(GmailConnector, "_get_all_user_emails", return_value=user_emails):
        with patch(
            "lumen.connectors.gmail.connector.get_gmail_service",
            side_effect=fake_get_gmail_service,
        ):
            with patch(
                "lumen.connectors.gmail.connector.thread_to_document",

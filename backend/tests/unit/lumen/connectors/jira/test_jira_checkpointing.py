...
from lumen.connectors.jira.connector import JiraConnector, JiraConnectorCheckpoint
from lumen.connectors.jira.utils import JIRA_SERVER_API_VERSION
from lumen.connectors.models import ConnectorFailure, Document, SlimDocument
...
def jira_connector(
    ...
    with patch("lumen.connectors.jira.connector._JIRA_FULL_PAGE_SIZE", 2):
        yield connector
...
def test_load_credentials(jira_connector: JiraConnector) -> None:
    ...
    with patch("lumen.connectors.jira.connector.build_jira_client") as mock_build_client:
...
def test_load_from_checkpoint_with_issue_processing_error(
    ...
    with patch("lumen.connectors.jira.connector.process_jira_issue") as mock_process:

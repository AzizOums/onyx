...
from lumen.connectors.registry import CONNECTOR_CLASS_MAP, ConnectorMapping
...
class TestConnectorClassLoading:
    ...
    def test_load_connector_class_import_error(self, mock_import: Mock) -> None:
        ...
        assert (
            "Failed to import WebConnector from lumen.connectors.web.connector"
...
class TestConnectorMappingIntegrity:
    ...
    def test_mapping_format_consistency(self) -> None:
        ...
        for source, mapping in CONNECTOR_CLASS_MAP.items():
            ...
            assert mapping.module_path.startswith("lumen.connectors."), (
                f"{source.value} module_path doesn't start with lumen.connectors."
            )
...
class TestInstantiateConnectorIntegration:
    ...
    def test_instantiate_connector_loads_class_lazily(self) -> None:
        ...
        from lumen.utils.sensitive import make_mock_sensitive_value
...
class TestValidateCCPairForUser:
    def test_validate_settings_error_raises_connector_validation_error(self) -> None:
        ...
        with (
            patch(
                "lumen.connectors.factory.fetch_connector_by_id",
            ...
            patch(
                "lumen.connectors.factory.fetch_credential_by_id",
                return_value=MagicMock(),
            ),
            patch(
                "lumen.connectors.factory.instantiate_connector",

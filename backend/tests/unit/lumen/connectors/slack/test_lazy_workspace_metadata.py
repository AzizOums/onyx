...
from lumen.connectors.exceptions import InsufficientPermissionsError
from lumen.connectors.slack.connector import SlackConnector
from lumen.connectors.slack.source_operations import SlackApiError
...
class TestLazyResolution:
    ...
    def test_grid_team_enumeration_failure_raises(self) -> None:
        ...
        with patch(
            "lumen.connectors.slack.connector.list_grid_team_ids",
    ...
    def test_grid_user_email_failure_raises(self) -> None:
        ...
        with (
            ...
            patch(
                "lumen.connectors.slack.connector.fetch_team_url",
                return_value="https://t1.slack.com",
            ),
            patch(
                "lumen.connectors.slack.connector.fetch_team_user_emails",
                side_effect=SlackApiError("missing_scope", MagicMock()),
            ),
        ):
            with pytest.raises(SlackApiError):
...
class TestLazyGridResolution:
    ...
    def test_non_grid_workspace_skips_team_enumeration(self) -> None:
        ...
        with patch("lumen.connectors.slack.connector.list_grid_team_ids") as list_teams:
            connector._ensure_workspace_metadata()

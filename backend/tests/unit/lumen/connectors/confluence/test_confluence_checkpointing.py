...
from lumen.connectors.confluence.connector import (
...
from tests.unit.lumen.connectors.utils import (
...
def confluence_connector(
    confluence_base_url: str, space_key: str, mock_confluence_client: LumenConfluence
    ...
    with patch("lumen.connectors.confluence.connector._SLIM_DOC_BATCH_SIZE", 2):
        yield connector
...
def test_load_from_checkpoint_with_page_processing_error(
    ...
    with patch(
        "lumen.connectors.confluence.connector.ConfluenceConnector._convert_page_to_document",
...
def test_validate_connector_settings_errors(
    ...
    with patch(
        "lumen.connectors.confluence.lumen_confluence.LumenConfluence.retrieve_confluence_spaces"
    ) as mock_retrieve:

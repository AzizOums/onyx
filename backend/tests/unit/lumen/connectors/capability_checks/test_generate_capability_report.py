...
from lumen.connectors.capability_checks import runner as runner_module
...
from lumen.connectors.capability_checks.runner import generate_capability_report
from lumen.connectors.exceptions import CredentialInvalidError
from lumen.connectors.interfaces import BaseConnector
from lumen.connectors.source_operations import SourceOperations
...
def test_real_config_unlocks_config_requiring_checks(
    ...
    instantiate = _patch_runner_environment(monkeypatch, [check])
    connector_specific_config = {"repositories": "lumen"}
...
def test_hung_instantiation_is_bounded_and_surfaces_indeterminate(
    ...
    # Under test.
    report = generate_capability_report(
        _make_credential(), connector_specific_config={"repo": "lumen"}
    )

...
from lumen.connectors.notion.connector import (
    NotionConnector,
    NotionDataSource,
    NotionPage,
)
...
class TestFetchDataSourcesForDatabase:
    def test_multi_source_database(self) -> None:
        ...
        with patch(
            "lumen.connectors.notion.connector.rl_requests.get", return_value=resp
        ):
...
class TestFetchDataSource:
    def test_query_returns_pages(self) -> None:
        ...
        with patch(
            "lumen.connectors.notion.connector.rl_requests.post", return_value=resp
        ):

...
LumenError always did (see test_exceptions.py). These are the three paths that
...
from lumen.main import log_http_error, validation_exception_handler, value_error_handler
...
class TestCanonicalCodeForStatus:
    ...
    def test_known_statuses(self, status_code: int, expected: str) -> None:
        assert LumenErrorCode.for_status(status_code).code == expected

    def test_unlisted_statuses_fall_back_by_class(self) -> None:
        assert LumenErrorCode.for_status(418) is LumenErrorCode.BAD_REQUEST
        assert LumenErrorCode.for_status(507) is LumenErrorCode.INTERNAL_ERROR
    ...
    def test_every_mapped_code_agrees_with_its_own_status(self) -> None:
        ...
        which is the right default for an LumenError raise site, and it is still
        the right name for a rejected request body.
        """
        for status_code in (400, 401, 404, 409, 429, 500, 502, 503, 504):
            assert LumenErrorCode.for_status(status_code).status_code == status_code

        assert LumenErrorCode.for_status(422) is LumenErrorCode.VALIDATION_ERROR

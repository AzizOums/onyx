...
from lumen.configs.constants import MilestoneRecordType
from lumen.utils import telemetry as telemetry_utils
...
class CloudIdentifyHarness:
    ...
    def fetch_versioned_implementation_with_fallback(
        ...
        if module == "lumen.utils.posthog_client" and attribute == "alias_user":
            return self.alias_user
        if module == "lumen.utils.telemetry" and attribute == "identify_user":
...
def test_mt_cloud_telemetry_noop_when_not_multi_tenant(monkeypatch: Any) -> None:
    ...
    monkeypatch.setattr("lumen.utils.telemetry.MULTI_TENANT", False)
...
def test_mt_cloud_telemetry_calls_event_telemetry_when_multi_tenant(
    ...
    # mt_cloud_telemetry reads the module-local imported symbol, so patch this path.
    monkeypatch.setattr("lumen.utils.telemetry.MULTI_TENANT", True)
...
def test_mt_cloud_identify_user_resolves_tenant_by_email_when_not_provided(
    ...
    telemetry_utils.mt_cloud_identify_user(
        distinct_id="user-id",
        email="wenxi@lumen.app",
    )

    assert harness.lookup_emails == ["wenxi@lumen.app"]
    assert harness.identify_calls == [
        ("user-id", {"email": "wenxi@lumen.app", "tenant_id": "tenant_dev"})
    ]
...
def test_mt_cloud_identify_user_omits_public_tenant(
    ...
    assert harness.lookup_emails == ["wenxi@lumen.app"]
    assert harness.identify_calls == [("user-id", {"email": "wenxi@lumen.app"})]

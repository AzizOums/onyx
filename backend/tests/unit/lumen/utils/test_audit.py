"""Unit tests for the generalized audit-event subsystem (``lumen.utils.audit``).
...
def test_class_selects_the_child_logger(caplog: pytest.LogCaptureFixture) -> None:
    ...
    records = [r for r in caplog.records if r.name.startswith("lumen.audit")]
    assert len(records) == 1
    assert records[0].name == "lumen.audit.group_management"
...
def test_routes_to_per_class_child_logger(caplog: pytest.LogCaptureFixture) -> None:
    ...
    auth_records = [r for r in caplog.records if r.name == "lumen.audit.authentication"]
...
def test_dedup_degrades_to_emit_when_redis_down(
    ...
    import lumen.redis.redis_pool as redis_pool
...
def test_audit_logger_self_contained_without_root_handler() -> None:
    ...
    assert own_handlers, "lumen.audit must attach its own handler"
    ...
    assert lines, "audit event must reach lumen.audit handlers without root"

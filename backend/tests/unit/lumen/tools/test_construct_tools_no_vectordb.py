...
APP_CONFIGS_MODULE = "lumen.configs.app_configs"
FILE_READER_MODULE = "lumen.tools.tool_implementations.file_reader.file_reader_tool"
OPEN_URL_MODULE = "lumen.tools.tool_implementations.open_url.open_url_tool"
...
class TestSearchToolAvailability:
    ...
    @patch("lumen.db.connector.check_user_files_exist", return_value=True)
    @patch(
...
class TestForceAddSearchToolGuard:
    def test_force_add_block_checks_disable_vector_db(self) -> None:
        ...
        from lumen.tools.tool_constructor import _construct_tools_impl
...
class TestValidateVectorDbKnowledge:
    @patch(
        "lumen.server.features.persona.api.DISABLE_VECTOR_DB",
        True,
    )
    def test_rejects_document_set_ids(self) -> None:
        from fastapi import HTTPException

        from lumen.server.features.persona.api import _validate_vector_db_knowledge

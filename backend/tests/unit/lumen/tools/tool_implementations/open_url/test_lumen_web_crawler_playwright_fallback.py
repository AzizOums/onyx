"""Tests for LumenWebCrawler's Playwright fallback on Cloudflare/bot challenges.
...
import lumen.tools.tool_implementations.open_url.lumen_web_crawler as crawler_module
from lumen.tools.tool_implementations.open_url.lumen_web_crawler import (
    FailureReason,
    LumenWebCrawler,
)
from lumen.utils.playwright_fetch import RenderedPage
...
@patch("lumen.tools.tool_implementations.open_url.lumen_web_crawler.fetch_rendered_html")
@patch("lumen.tools.tool_implementations.open_url.lumen_web_crawler.ssrf_safe_get")
def test_403_triggers_playwright_fallback(
    ...
    result = LumenWebCrawler()._fetch_url("https://example.com/")
...
def test_4xx_with_cf_mitigated_triggers_playwright_fallback(
    ...
    LumenWebCrawler()._fetch_url("https://example.com/")
...
def test_404_without_cloudflare_signals_skips_playwright_fallback(
    ...
    result = LumenWebCrawler()._fetch_url("https://example.com/missing")
...
def test_401_without_cloudflare_signals_skips_playwright_fallback(
    ...
    LumenWebCrawler()._fetch_url("https://example.com/private")
...
def test_fallback_disabled_skips_playwright(
    ...
    crawler = LumenWebCrawler(playwright_fallback_enabled=False)
...
def test_fallback_respects_max_html_size(
    ...
    crawler = LumenWebCrawler(max_html_size_bytes=100)
    result = crawler._fetch_url("https://example.com/")
...
def test_ssrf_failure_reason(mock_get: MagicMock) -> None:
    ...
    result = LumenWebCrawler()._fetch_url("http://internal.local/")

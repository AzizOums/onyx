...
_APP_CONFIG_MODULES = [
    f"lumen.background.celery.configs.{name}"
    for name in (
        "beat",
        "client",
        "docfetching",
...
def _reload_celery_configs() -> None:
    import lumen.background.celery.configs.base as celery_base
...
def test_sentinel_urls_keep_the_configured_redis_db() -> None:
    ...
    with _celery_config_env(env):
        ...
        beat = importlib.import_module("lumen.background.celery.configs.beat")

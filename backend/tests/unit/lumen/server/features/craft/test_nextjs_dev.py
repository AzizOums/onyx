...
from lumen.server.features.build.sandbox import nextjs_dev
...
def test_start_script_exports_allowed_dev_origins() -> None:
    with patch.object(nextjs_dev, "WEB_DOMAIN", "https://cloud.lumen.app"):
        script = build_nextjs_start_script(_SESSION_PATH, 3010)

    assert 'export LUMEN_WEBAPP_ALLOWED_DEV_ORIGINS="cloud.lumen.app"' in script
    assert (
        'export LUMEN_WEBAPP_BASE_PATH="/api/build/sessions/'
...
def test_start_script_port_is_env_driven_not_a_dup_flag() -> None:
    ...
    the port must flow via LUMEN_WEBAPP_PORT."""
    script = build_nextjs_start_script(_SESSION_PATH, 3010)

    assert "export LUMEN_WEBAPP_PORT=3010" in script
...
def test_template_dev_script_wires_port_to_managed_env_and_port_file() -> None:
    ...
    managed launch flows the port via LUMEN_WEBAPP_PORT, and a hand-started
    ...
    assert (
        "${LUMEN_WEBAPP_PORT:-$(cat ../../.nextjs-port 2>/dev/null || echo 3000)}"
        in dev_script
    )
...
def test_start_script_passes_port_flag_when_dev_script_ignores_env() -> None:
    """A `dev` script without the LUMEN_WEBAPP_PORT marker (legacy scaffold, or
    rewritten by the agent) ignores the env var and would bind 3000; the
    managed launch must pass -p explicitly for those."""
    script = build_nextjs_start_script(_SESSION_PATH, 3010)

    assert 'if ! grep -q "LUMEN_WEBAPP_PORT" package.json 2>/dev/null; then' in script
...
def test_start_script_allowed_dev_origins_is_hostname_only() -> None:
    ...
    assert 'export LUMEN_WEBAPP_ALLOWED_DEV_ORIGINS="localhost"' in script
...
def test_nextjs_ready_probe_targets_base_path_dev_asset() -> None:
    ...
    with (
        ...
        patch(
            "lumen.server.features.build.session.manager.httpx.Client",

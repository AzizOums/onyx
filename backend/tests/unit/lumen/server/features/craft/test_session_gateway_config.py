...
from lumen.server.features.build.configs import OPENCODE_DISABLED_TOOLS
...
from lumen.server.features.build.session import manager as manager_module
from lumen.server.features.build.session.manager import SessionManager
from lumen.server.gateway import model_catalog
from lumen.server.gateway.models import GatewayModelDescriptor
...
def test_gateway_config_qualifies_collisions_and_selects_exact_default() -> None:
    ...
    with patch.object(llm_config, "LUMEN_SERVER_URL", "https://lumen.test/api"):
        config = llm_config.build_lumen_gateway_config(
            [bedrock, direct],
            llm_config.GatewaySelection(7, "anthropic/claude-sonnet"),
        )

    assert config is not None
    assert config.provider == "lumen"
    assert config.model_name == "7/anthropic/claude-sonnet"
    assert config.api_base == "https://lumen.test/api/gateway/v1"
...
def test_gateway_config_fallback_supports_any_provider() -> None:
    ...
    with patch.object(llm_config, "LUMEN_SERVER_URL", "https://lumen.test"):
        config = llm_config.build_lumen_gateway_config([bedrock, anthropic])
...
def test_gateway_model_capabilities_reach_opencode_catalog() -> None:
    ...
    with patch.object(llm_config, "LUMEN_SERVER_URL", "https://lumen.test"):
        gateway_config = llm_config.build_lumen_gateway_config([provider])
...
def test_gateway_config_can_target_direct_api_service() -> None:
    with patch.object(llm_config, "LUMEN_SERVER_URL", "http://api:8080/"):
        config = llm_config.build_lumen_gateway_config(
...
def test_gateway_config_preserves_url_path_prefix() -> None:
    with patch.object(llm_config, "LUMEN_SERVER_URL", "https://lumen.example/api"):
        config = llm_config.build_lumen_gateway_config(
            [_provider(2, "bedrock", [_model("bedrock-model")])]
        )

    assert config is not None
    assert config.api_base == "https://lumen.example/api/gateway/v1"
...
def test_custom_azure_alias_renders_deployment_limits_for_opencode() -> None:
    ...
    with (
        patch.object(llm_config, "LUMEN_SERVER_URL", "https://lumen.test"),
    ...
    assert opencode_config["provider"]["lumen"]["models"]["300/GPT-5.6-SOL"][
        "limit"
    ] == {
        "context": 272_000,
        "input": 272_000,
...
def test_gateway_config_preserves_explicit_context_override() -> None:
    ...
    assert gateway_config is not None
    opencode_config = build_provider_opencode_config(gateway_config)
    assert opencode_config["provider"]["lumen"]["models"]["300/Customer Alias"][
        "limit"
    ] == {
        "context": 200_000,
        "input": 200_000,
...
def test_unknown_model_omits_unverified_token_limits() -> None:
    ...
    assert gateway_config is not None
    opencode_config = build_provider_opencode_config(gateway_config)
    model_config = opencode_config["provider"]["lumen"]["models"]["7/unknown-model"]
...
def test_small_input_override_does_not_reduce_provider_output() -> None:
    ...
    assert gateway_config is not None
    opencode_config = build_provider_opencode_config(gateway_config)
    assert opencode_config["provider"]["lumen"]["models"]["7/tiny-model"]["limit"] == {
        "context": 25_000,
        "input": 25_000,
        "output": 128_000,
    }
...
def _gateway_config() -> CraftLLMProviderConfig:
    return CraftLLMProviderConfig(
        ...
        api_base="https://lumen.test/gateway/v1",
        models=[

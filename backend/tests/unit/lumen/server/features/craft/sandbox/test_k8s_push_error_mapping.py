...
from lumen.server.features.build.sandbox.kubernetes.sidecar_client import SidecarClient
...
_HTTPX_CLIENT_PATH = (
    "lumen.server.features.build.sandbox.kubernetes.sidecar_client.httpx.Client"
)
_MANAGER_MODULE = (
    "lumen.server.features.build.sandbox.kubernetes.kubernetes_sandbox_manager"
)
...
def test_provision_cleans_up_pod_when_opencode_history_restore_fails(
    ...
    monkeypatch.setattr(ksm, "LUMEN_SERVER_URL", "http://api-server")
    monkeypatch.setattr(ksm, "validate_sandbox_api_url", lambda *_: None)

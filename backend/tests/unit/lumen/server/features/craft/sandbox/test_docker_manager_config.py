...
import lumen.server.features.build.sandbox.docker.docker_sandbox_manager as dsm
...
def test_volume_name_is_per_sandbox_and_short() -> None:
    ...
    assert vol.startswith("lumen-craft-sandbox-")
...
def test_labels_include_required_fields() -> None:
    ...
    assert labels[LABEL_K8S_MANAGED_BY] == LABEL_K8S_MANAGED_BY_LUMEN
...
def test_immutable_sandbox_image_uses_cached_image_when_present() -> None:
    mgr, docker = _bare_manager_with_image("lumendotapp/sandbox:v4.1.2")

    mgr._ensure_sandbox_image()

    docker.images.get.assert_called_once_with("lumendotapp/sandbox:v4.1.2")
...
def test_immutable_sandbox_image_pulls_when_missing() -> None:
    ...
    docker.images.pull.assert_called_once_with("lumendotapp/sandbox:v4.1.2")


def test_mutable_sandbox_image_refreshes_once() -> None:
    mgr, docker = _bare_manager_with_image("lumendotapp/sandbox:latest")

    mgr._ensure_sandbox_image()
    mgr._ensure_sandbox_image()

    docker.images.pull.assert_called_once_with("lumendotapp/sandbox:latest")
...
def test_mutable_sandbox_image_uses_cache_once_if_refresh_fails() -> None:
    ...
    mgr, docker = _bare_manager_with_image("lumendotapp/sandbox:edge")
    docker.images.pull.side_effect = dsm.APIError("registry unavailable")

    mgr._ensure_sandbox_image()
    mgr._ensure_sandbox_image()

    docker.images.pull.assert_called_once_with("lumendotapp/sandbox:edge")
    docker.images.get.assert_called_once_with("lumendotapp/sandbox:edge")


def test_mutable_sandbox_image_raises_if_refresh_fails_without_cache() -> None:
    mgr, docker = _bare_manager_with_image("lumendotapp/sandbox:beta")
    docker.images.pull.side_effect = dsm.APIError("registry unavailable")
    docker.images.get.side_effect = dsm.NotFound("missing")
...
def test_local_dev_sandbox_image_uses_cached_image_when_present() -> None:
    mgr, docker = _bare_manager_with_image("lumendotapp/sandbox:dev")

    mgr._ensure_sandbox_image()

    docker.images.get.assert_called_once_with("lumendotapp/sandbox:dev")
    docker.images.pull.assert_not_called()


def test_registry_port_untagged_image_refreshes_as_implicit_latest() -> None:
    image = "localhost:5001/lumen-sandbox"
    mgr, docker = _bare_manager_with_image(image)
...
def test_digest_sandbox_image_uses_cached_image_when_present() -> None:
    image = "lumendotapp/sandbox@sha256:abc123"
    mgr, docker = _bare_manager_with_image(image)
...
def kwargs() -> ContainerCreateKwargs:
    ...
    return build_container_create_kwargs(
        ...
        user_id=USER_ID,
        tenant_id=TENANT_ID,
        image="lumendotapp/sandbox:test",
        lumen_pat="pat-redacted",
        api_server_url="http://api_server:8080",
        network="lumen_craft_sandbox",
        volume_name="lumen-craft-sandbox-12345678",
        memory_limit="2g",
        cpu_limit=1.0,
...
def proxy_kwargs() -> ContainerCreateKwargs:
    ...
    Note: ``lumen_pat`` is the proxy-injected placeholder because in proxy mode
    ...
    return build_container_create_kwargs(
        ...
        tenant_id=TENANT_ID,
        image="lumendotapp/sandbox:test",
        lumen_pat=SANDBOX_PROXY_INJECTED_PLACEHOLDER,
        api_server_url="https://lumen.example.com/api",
        network="lumen_craft_sandbox",
        volume_name="lumen-craft-sandbox-12345678",
        memory_limit="2g",
        cpu_limit=1.0,
...
def test_container_kwargs_env_allowlist_excludes_storage_credentials(
    ...
    # Required env
    assert env["LUMEN_PAT"] == "pat-redacted"
    assert env["LUMEN_SERVER_URL"] == "http://api_server:8080"
    assert env["LUMEN_API_PREFIX"] == ""
    ...
    forbidden = {
        ...
        "LUMEN_SANDBOX_PUSH_PRIVATE_KEY",
    }
...
def test_container_kwargs_labels_and_volume(kwargs: ContainerCreateKwargs) -> None:
    ...
    assert "lumen-craft-sandbox-12345678" in kwargs["volumes"]
    assert (
        kwargs["volumes"]["lumen-craft-sandbox-12345678"]["bind"]
        == "/workspace/sessions"
    )
...
def test_container_kwargs_uses_sandbox_network(kwargs: ContainerCreateKwargs) -> None:
    ...
    assert kwargs["network"] == "lumen_craft_sandbox"
...
def test_container_kwargs_env_is_a_minimal_allowlist(
    ...
    assert set(env.keys()) == {
        ...
        "LUMEN_WEBAPP_ALLOWED_DEV_ORIGINS",
    }
...
def test_container_kwargs_warns_on_internal_compose_host(
    ...
    """Deployers that point LUMEN_SERVER_URL at compose DNS get warned."""
    import logging
...
def test_container_kwargs_no_warning_for_craft_api_alias(
    ...
    with caplog.at_level(logging.WARNING):
        build_container_create_kwargs(
            ...
            image="lumendotapp/sandbox:test",
            lumen_pat="pat",
            api_server_url="http://lumen-craft-api:8080",
            network="lumen_craft_sandbox",
            volume_name="vol",
            memory_limit="2g",
            cpu_limit=1.0,
...
def test_proxy_kwargs_mounts_ca_volume_read_only(
    ...
    assert "lumen-craft-sandbox-12345678" in volumes
...
def test_proxy_kwargs_env_contains_proxy_and_ca_keys(
    ...
    # The compatibility core is preserved; LUMEN_PAT is the proxy placeholder
    ...
    assert env["LUMEN_SERVER_URL"] == "https://lumen.example.com/api"
    assert env["LUMEN_API_PREFIX"] == ""
    assert env["OPENCODE_SERVER_PASSWORD"] == _OPENCODE_PASSWORD
    assert env["OPENCODE_CONFIG_CONTENT"] == _OPENCODE_CONFIG_JSON
...
def test_compose_uses_internal_api_alias_for_craft() -> None:
    ...
    api_network = services["api_server"]["networks"]["lumen_craft_sandbox"]
    assert api_network["aliases"] == ["lumen-craft-api"]

    expected_url = "LUMEN_SERVER_URL=${LUMEN_SERVER_URL:-http://lumen-craft-api:8080}"
    ...
    assert "lumen_craft_sandbox" in services["sandbox-proxy"]["networks"]
...
def test_embedded_craft_compose_copy_is_in_sync() -> None:
    """lumen-cli go:embeds a byte-identical copy (go:embed can't reach outside

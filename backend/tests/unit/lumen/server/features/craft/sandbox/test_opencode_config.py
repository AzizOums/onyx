...
from lumen.server.features.build.configs import MCP_SESSION_TAG_HEADER
from lumen.server.features.build.sandbox.models import (
    CraftLLMProviderConfig,
    CraftMCPServerConfig,
)
from lumen.server.features.build.sandbox.util.mcp_config import craft_mcp_fingerprint
from lumen.server.features.build.sandbox.util.opencode_config import (
    build_opencode_base_config,
    build_provider_opencode_config,
)
from lumen.server.gateway.models import (
...
def _gateway(*, default: str = "7/gpt-5.5") -> CraftLLMProviderConfig:
    return CraftLLMProviderConfig(
        ...
        api_base="https://lumen.test/api/gateway/v1",
        display_name="Lumen",
        models=[
            GatewayModelDescriptor(
                id="7/gpt-5.5",
...
def test_gateway_is_the_only_enabled_provider() -> None:
    ...
    assert config["model"] == "lumen/7/gpt-5.5"
    assert config["enabled_providers"] == ["lumen"]
    provider = config["provider"]["lumen"]
    assert provider["npm"] == "@ai-sdk/openai-compatible"
    assert provider["options"] == {
        "apiKey": "proxy-placeholder",
        "baseURL": "https://lumen.test/api/gateway/v1",
    }
...
def test_gateway_capabilities_are_adapted_instead_of_hardcoded() -> None:
    ...
    model = build_provider_opencode_config(gateway)["provider"]["lumen"]["models"][
        "4/custom-model"
    ]

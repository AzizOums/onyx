...
from lumen.server.features.build.sandbox.util import api_url_check
...
def test_missing_api_prefix_raises_with_corrected_url() -> None:
    def fake_get(url: str, **_: object) -> MagicMock:
        if url == "https://lumen.example/health":
            return _response(404)
        assert url == "https://lumen.example/api/health"
        return _health_response()

    with patch.object(api_url_check.httpx, "get", side_effect=fake_get):
        with pytest.raises(LumenError) as exc_info:
            api_url_check.validate_sandbox_api_url("https://lumen.example")

    assert "'https://lumen.example/api'" in str(exc_info.value)
...
def test_auth_guarded_health_counts_as_reachable() -> None:
    with patch.object(api_url_check.httpx, "get", return_value=_response(403)):
        api_url_check.validate_sandbox_api_url("https://lumen.example/api")

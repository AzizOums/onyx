...
_REPO_ROOT = find_ancestor_containing("backend/lumen")
_DAEMON_DIR = (
    _REPO_ROOT / "backend/lumen/server/features/build/sandbox/image/sandbox_daemon"
)
...
def _point_opencode_paths(
    ...
    opencode_restored_path = (
        sessions_root.parent / "managed" / ".lumen" / "opencode-history-restored"
    )
...
def configured_sandbox_daemon(
    ...
    monkeypatch.setenv("LUMEN_SANDBOX_PUSH_PUBLIC_KEY", pub_b64)
...
def test_push_missing_public_key_raises_lumen_error(
    configured_sandbox_daemon: tuple[ModuleType, ModuleType, Ed25519PrivateKey, Path],
    client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    Note: the in-pod daemon is standalone Python (no Lumen imports) so it
    surfaces this via ``HTTPException(status_code=500, ...)`` rather than
    ``LumenError``. The behavior asserted here is the operator-facing error
    ...
    monkeypatch.delenv("LUMEN_SANDBOX_PUSH_PUBLIC_KEY", raising=False)
    monkeypatch.setattr(server_mod, "_public_key", None, raising=False)

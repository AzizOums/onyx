...
from lumen.server.features.build.sandbox.models import FatalWriteError, FileSet
...
def test_oversized_bundle_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "lumen.server.features.build.sandbox.kubernetes.kubernetes_sandbox_manager._MAX_BUNDLE_BYTES",
        100,
    )

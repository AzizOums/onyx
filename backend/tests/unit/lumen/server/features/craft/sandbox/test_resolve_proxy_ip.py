...
def test_fqdn_resolved_to_clusterip_via_api(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ksm, "SANDBOX_PROXY_HOST", "lumen-sandbox-proxy.lumen.svc.cluster.local"
    )
    mgr, core_api = _mgr()
    svc = MagicMock()
    ...
    core_api.read_namespaced_service.assert_called_once_with(
        name="lumen-sandbox-proxy", namespace="lumen"
    )


def test_namespace_parsed_from_fqdn(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ksm, "SANDBOX_PROXY_HOST", "lumen-sandbox-proxy.myrelease.svc.cluster.local"
    )
    monkeypatch.setattr(ksm, "SANDBOX_PROXY_NAMESPACE", "lumen")
    mgr, core_api = _mgr()
    svc = MagicMock()
    ...
    core_api.read_namespaced_service.assert_called_once_with(
        name="lumen-sandbox-proxy", namespace="myrelease"
    )
...
def test_namespace_falls_back_to_config_for_bare_host(
    ...
    monkeypatch.setattr(ksm, "SANDBOX_PROXY_HOST", "lumen-sandbox-proxy")
    ...
    core_api.read_namespaced_service.assert_called_once_with(
        name="lumen-sandbox-proxy", namespace="proxy-ns"
    )

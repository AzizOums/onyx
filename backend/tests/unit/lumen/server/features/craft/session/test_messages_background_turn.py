...
from lumen.server.features.build.interactive_turns.state import get_turn
from lumen.server.features.build.session import manager as manager_module
from lumen.server.features.build.session import messages as messages_api
...
def test_send_message_starts_background_turn(
    ...
    assert session.agent_provider == "lumen"
...
def test_send_message_blocked_when_over_token_budget(
    ...
    def _over_budget(_user: object) -> None:
        raise LumenError(LumenErrorCode.RATE_LIMITED, "You've reached the usage budget.")
    ...
    assert ei.value.error_code is LumenErrorCode.RATE_LIMITED

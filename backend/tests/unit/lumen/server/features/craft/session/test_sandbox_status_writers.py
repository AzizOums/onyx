...
ALLOWED_REFERENCES: set[str] = {
    ...
    "lumen/server/features/build/db/sandbox.py",
    # Substrate-invariant transitions + invariants.
    "lumen/server/features/build/session/sandbox_lifecycle.py",
}


def _modules_referencing_status_mutation() -> set[str]:
    backend_dir = find_ancestor_containing("backend/lumen") / "backend"
    scan_roots = (
        backend_dir / "lumen" / "server" / "features" / "build",
        backend_dir / "lumen" / "background",
    )
    found: set[str] = set()

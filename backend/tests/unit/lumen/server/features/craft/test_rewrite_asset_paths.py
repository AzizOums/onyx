...
from lumen.server.features.build import webapp_proxy as api
...
class TestNextjsProxyMountContract:
    def test_webapp_prefix_is_used_as_next_base_path(self) -> None:
        ...
        assert "LUMEN_WEBAPP_BASE_PATH" in config_source
    ...
    def test_template_cwd_fallback_pins_session_base_path(self) -> None:
        """When LUMEN_WEBAPP_BASE_PATH is unset (hand-started server), the
    ...
    def test_sandbox_start_script_exports_next_base_path(self) -> None:
        ...
        assert (
            'export LUMEN_WEBAPP_BASE_PATH="/api/build/sessions/$(basename '
            '{session_path})/webapp"'
        ) in source
...
class TestProxyRequestWiring:
    ...
    async def test_proxy_request_strips_sensitive_viewer_headers(
        ...
        sensitive_headers = {
            ...
            "Sec-Fetch-User": "?1",
            # x-lumen-* prefix matcher (not literal deny-list entries).
            "X-Lumen-Authorization": "Bearer alt-victim-token",
            "X-Lumen-Tenant-ID": "victim-tenant",
            "X-Lumen-Request-ID": "abc-123",
            "X-Lumen-Future-Header": "should-be-stripped-by-prefix",
        }

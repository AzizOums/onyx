...
from lumen.server.features.build import configs
...
def _restore_configs() -> Iterator[None]:
    ...
    os.environ.pop("LUMEN_VERSION", None)
...
def test_sandbox_image_fallback_is_not_derived_from_lumen_version(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("SANDBOX_CONTAINER_IMAGE", raising=False)
    monkeypatch.setenv("LUMEN_VERSION", "v4.1.2")

    reloaded = importlib.reload(configs)

    assert reloaded.SANDBOX_CONTAINER_IMAGE == "lumendotapp/sandbox:latest"
...
def test_sandbox_image_override_wins(
    ...
    monkeypatch.setenv("LUMEN_VERSION", "v4.1.2")
    monkeypatch.setenv("SANDBOX_CONTAINER_IMAGE", "lumendotapp/sandbox:ctx-123")

    reloaded = importlib.reload(configs)

    assert reloaded.SANDBOX_CONTAINER_IMAGE == "lumendotapp/sandbox:ctx-123"

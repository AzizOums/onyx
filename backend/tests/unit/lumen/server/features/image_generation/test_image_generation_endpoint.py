...
from lumen.image_gen.exceptions import ImageGenerationNotConfiguredError
from lumen.image_gen.generation import GeneratedImageData
from lumen.image_gen.interfaces import ImageShape
from lumen.server.features.image_generation.api import (
...
_HELPER = (
    "lumen.server.features.image_generation.api.generate_images_with_default_config"
)
_ENSURE = "lumen.server.features.image_generation.api.ensure_image_generation_configured"
_KEEPALIVE = "lumen.server.features.image_generation.api._KEEPALIVE_INTERVAL_S"
_MAX_DURATION = "lumen.server.features.image_generation.api._MAX_STREAM_DURATION_S"
...
def test_oversized_reference_rejected() -> None:
    with patch(
        "lumen.server.features.image_generation.api._MAX_REFERENCE_IMAGE_BYTES", 4
    ):
        with pytest.raises(LumenError) as exc:
            generate_image(
                ImageGenerationRequest(
...
def test_admission_limit_rejects_with_rate_limited() -> None:
    with patch(
        "lumen.server.features.image_generation.api._admission_semaphore",
        threading.BoundedSemaphore(0),
    ):
        with pytest.raises(LumenError) as exc:
            generate_image(ImageGenerationRequest(prompt="a cat"))
    assert exc.value.error_code == LumenErrorCode.RATE_LIMITED
...
def test_not_configured_raises_before_stream() -> None:
    with patch(_ENSURE, side_effect=ImageGenerationNotConfiguredError("none")):
        with pytest.raises(LumenError) as exc:
            generate_image(ImageGenerationRequest(prompt="a cat"))
    assert exc.value.error_code == LumenErrorCode.NOT_FOUND
...
def test_not_configured_mid_generation_maps_to_not_found_envelope() -> None:
    ...
    assert parsed["error_code"] == LumenErrorCode.NOT_FOUND.code
...
def test_reference_unsupported_maps_to_invalid_input_envelope() -> None:
    ...
    assert parsed["error_code"] == LumenErrorCode.INVALID_INPUT.code
...
def test_provider_error_maps_to_llm_provider_error_envelope() -> None:
    ...
    assert parsed["error_code"] == LumenErrorCode.LLM_PROVIDER_ERROR.code
...
def test_stream_deadline_yields_timeout_envelope() -> None:
    ...
    try:
        ...
        release.set()
    assert parsed["error_code"] == LumenErrorCode.GATEWAY_TIMEOUT.code

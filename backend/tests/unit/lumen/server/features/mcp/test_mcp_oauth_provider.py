...
class _RecordingAuthorizationStorage(oauth.LumenTokenStorage):
    def __init__(
        self,
        token_endpoint_auth_method: Literal[
            "client_secret_post", "client_secret_basic"
...
def _provider(
    ...
    *,
    ...
    provider = oauth.LumenOAuthClientProvider(
...
def test_failed_token_exchange_does_not_persist_authorization(
    ...
    assert exc_info.value.error_code is LumenErrorCode.BAD_GATEWAY
...
def test_noninteractive_provider_requires_reconnection() -> None:
    ...
    with pytest.raises(
        ...
        match="Please reconnect to the server through Lumen",
    ):
        asyncio.run(run())

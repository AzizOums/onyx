...
Exercises the real OAuthClientProvider/OAuthContext/LumenTokenStorage code —
...
import lumen.server.features.mcp.oauth as mcp_oauth
from lumen.cache.interface import CacheLockAcquisitionError
from lumen.db.enums import MCPOAuthProviderMode, MCPTransport
from lumen.db.models import MCPServer as DbMCPServer
from lumen.server.features.mcp.models import MCPOAuthKeys
from lumen.server.features.mcp.oauth import refresh_mcp_oauth_token_if_expired

_TOKEN_ENDPOINT = "https://gitlab.example.com/oauth/token"
_REDIRECT_URI = "https://lumen.example.com/mcp/oauth/callback"
...
def _install_mocks(
    ...
    *,
    ...
    """Patch the DB layer LumenTokenStorage uses and the outbound network call;
    the real OAuthClientProvider/OAuthContext/LumenTokenStorage code runs
...
def test_refreshes_expired_token_with_client_secret_post(
    ...
    # Persisted via the real LumenTokenStorage.set_tokens.
...
def test_refresh_persists_via_real_lumen_token_storage(
    monkeypatch: pytest.MonkeyPatch,
) -> None:

...
from lumen.server.features.mcp import oauth, oauth_flow
...
def test_discovery_timeout_cancels_probe(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(LumenError, match="Timed out during MCP OAuth discovery"):
        _start_flow(_server())
...
def test_multiple_authorization_handoffs_fail_as_an_ordinary_application_error(
    ...
    with pytest.raises(LumenError, match="multiple authorization redirects"):
        _start_flow(_server())
...
def test_reverse_order_completions_restore_their_own_registration(
    ...
    providers: list[oauth.LumenOAuthClientProvider] = []

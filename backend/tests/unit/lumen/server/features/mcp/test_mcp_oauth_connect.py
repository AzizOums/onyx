...
from lumen.server.features.mcp import api
...
def test_tool_discovery_reports_reauthentication_as_unauthenticated(
    ...
    assert exc_info.value.error_code is LumenErrorCode.UNAUTHENTICATED
    assert exc_info.value.detail == "Please reconnect to the server through Lumen."
...
def test_server_url_change_invalidates_old_credentials_without_losing_new_client(
    ...
    client_information = OAuthClientInformationFull(
        ...
        redirect_uris=["https://lumen.example.com/mcp/oauth/callback"],
    )
    server = _server()

...
from lumen.server.features.mcp.models import MCPOAuthFlowState
...
def test_start_rejects_invalid_known_provider_configuration() -> None:
    ...
    with pytest.raises(LumenError, match="oauth_token_endpoint"):
    ...
    with pytest.raises(LumenError, match="client_id"):
        oauth_flow.start_known_provider_oauth_flow(
            mcp_server=_server(),
            user_id="user-1",
            connection_config_id=101,
    ...
    with pytest.raises(LumenError, match="cannot override: state"):
        oauth_flow.start_known_provider_oauth_flow(

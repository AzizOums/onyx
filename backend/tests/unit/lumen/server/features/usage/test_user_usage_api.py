...
import lumen.server.features.usage.api as usage_api
...
from lumen.db.user_usage import get_cost_window_reset, get_cost_window_start
from lumen.error_handling.exceptions import register_lumen_exception_handlers
from lumen.llm import cost_overrides
from lumen.llm.cost import ModelPrice, get_model_price_per_million
from lumen.server.features.usage.api import user_usage_router
...
def _no_configured_providers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "lumen.server.features.usage.api.fetch_all_llm_providers_accessible_in_any_context",
        lambda *_a, **_k: [],
    )
...
def _seed_current_window(db_session: Session, user_id: str) -> datetime.datetime:
    from lumen.db.user_usage import USER_USAGE_BUCKET_SECONDS
...
def test_returns_only_callers_rows_and_aggregates(
    ...
    monkeypatch.setattr(
        "lumen.server.features.usage.api.fetch_default_llm_model", lambda _db: None
    )
...
def test_selected_model_price_known_model(
    ...
    monkeypatch.setattr(
        "lumen.server.features.usage.api.fetch_default_llm_model",

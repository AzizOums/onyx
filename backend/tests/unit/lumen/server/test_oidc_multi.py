...
def test_validate_config_accepts_legacy_callback_for_oauth_types() -> None:
    from lumen.db.sso_provider import validate_sso_config
...
def test_login_callback_uri_saml_is_fixed_acs() -> None:
    from lumen.db.sso_provider import sso_login_callback_uri

    provider = _provider(name="corp-saml", provider_type=SSOProviderType.SAML)
    uri = sso_login_callback_uri(provider, {}, "https://lumen.example.com")
    assert uri == "https://lumen.example.com/auth/saml/callback"

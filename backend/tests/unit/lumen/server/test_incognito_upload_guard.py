...
from lumen.chat.incognito_context import _TOMBSTONE
from lumen.error_handling.error_codes import LumenErrorCode
from lumen.error_handling.exceptions import LumenError
from lumen.server.features.projects.api import upload_user_files

MODULE = "lumen.server.features.projects.api"
CONTEXT_MODULE = "lumen.chat.incognito_context"
...
def test_an_upload_into_an_ended_session_never_writes_a_row() -> None:
    ...
    assert isinstance(attempt.error, LumenError)
    assert attempt.error.error_code is LumenErrorCode.INVALID_INPUT
...
def test_a_user_without_incognito_is_refused() -> None:
    ...
    assert isinstance(attempt.error, LumenError)
    assert attempt.error.error_code is LumenErrorCode.UNAUTHORIZED
    assert not attempt.wrote_rows

...
def test_image_generation_error_is_actionable_and_redacts_credentials() -> None:
    ...
    with (
        patch(
            "lumen.server.manage.image_generation.api.get_image_generation_provider",
            return_value=image_provider,
        ),
        pytest.raises(LumenError) as exc_info,
    ):
        run_test_image_generation(request, MagicMock(), MagicMock())

    assert exc_info.value.error_code is LumenErrorCode.VALIDATION_ERROR

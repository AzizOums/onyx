...
class TestFetchOpenAICompatibleModelIds:
    def test_returns_sorted_ids_without_embeddings(self) -> None:
        ...
        with (
            patch(
                "lumen.server.manage.llm.utils.httpx.get",
                return_value=_models_response(
                    ["whisper-1", "tts-1", "flux-schnell", "text-embedding-3-small"]
                ),
            ),
            patch(
                "lumen.server.manage.llm.utils.is_embedding_model",
...
class TestImageGenAvailableModelsEndpoint:
    def test_returns_names(self) -> None:
        from lumen.server.manage.image_generation.api import list_available_models
        from lumen.server.manage.image_generation.models import (
            AvailableImageModelsRequest,
        )

        with patch(
            "lumen.server.manage.image_generation.api.fetch_openai_compatible_model_ids",
...
class TestVoiceAvailableModelsEndpoint:
    def test_returns_ids(self) -> None:
        from lumen.server.manage.voice.api import list_available_models
        from lumen.server.manage.voice.models import AvailableVoiceModelsRequest

        with patch(
            "lumen.server.manage.voice.api.fetch_openai_compatible_model_ids",

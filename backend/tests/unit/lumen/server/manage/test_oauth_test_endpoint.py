...
from lumen.server.manage.oauth_test import get_oauth_login_claims
...
async def test_not_found_returns_found_false() -> None:
    with patch(
        "lumen.server.manage.oauth_test.get_captured_oauth_claims",
...
async def test_found_snapshot_maps_to_response() -> None:
    ...
    with (
        ...
        patch(
            "lumen.server.manage.oauth_test.get_idp_profile_fields",

...
from lumen.server.manage.models import EmailInviteStatus, UserByEmail
from lumen.server.manage.users import bulk_invite_users, remove_invited_user
...
@patch(
    "lumen.server.manage.users.get_session_with_shared_schema",
    new_callable=lambda: _make_shared_session_mock(next_total=6),
)
@patch("lumen.server.manage.users.enforce_invite_rate_limit")
@patch("lumen.server.manage.users.MULTI_TENANT", True)
@patch("lumen.server.manage.users.is_tenant_on_trial_fn", return_value=True)
@patch("lumen.server.manage.users.get_current_tenant_id", return_value="test_tenant")
@patch("lumen.server.manage.users.get_invited_users", return_value=[])
@patch("lumen.server.manage.users.get_all_users", return_value=[])
@patch("lumen.server.manage.users.enforce_seat_limit_locked")
@patch("lumen.server.manage.users.NUM_FREE_TRIAL_USER_INVITES", 5)
def test_trial_tenant_cannot_exceed_invite_limit(*_mocks: None) -> None:
    """Post-upsert total of 6 exceeds cap=5 — must raise LumenError."""
    emails = [f"user{i}@example.com" for i in range(6)]

    with pytest.raises(LumenError) as exc_info:
        bulk_invite_users(emails=emails, current_user=MagicMock())

    assert exc_info.value.error_code == LumenErrorCode.TRIAL_INVITE_LIMIT_EXCEEDED
...
@patch("lumen.server.manage.users.enforce_invite_rate_limit")
@patch("lumen.server.manage.users.get_redis_client")
@patch("lumen.server.manage.users.MULTI_TENANT", True)
@patch("lumen.server.manage.users.DEV_MODE", True)
@patch("lumen.server.manage.users.ENABLE_EMAIL_INVITES", False)
@patch("lumen.server.manage.users.is_tenant_on_trial_fn", return_value=True)
@patch("lumen.server.manage.users.get_current_tenant_id", return_value="test_tenant")
@patch("lumen.server.manage.users.get_invited_users", return_value=[])
@patch("lumen.server.manage.users.get_all_users", return_value=[])
@patch("lumen.server.manage.users.write_invited_users", return_value=3)
@patch("lumen.server.manage.users.enforce_seat_limit_locked")
@patch("lumen.server.manage.users.NUM_FREE_TRIAL_USER_INVITES", 5)
@patch(
    "lumen.server.manage.users.fetch_ee_implementation_or_noop",
...
@patch("lumen.server.manage.users.get_session_with_shared_schema")
@patch("lumen.server.manage.users.enforce_invite_rate_limit")
@patch("lumen.server.manage.users.MULTI_TENANT", True)
@patch("lumen.server.manage.users.DEV_MODE", True)
@patch("lumen.server.manage.users.ENABLE_EMAIL_INVITES", False)
@patch("lumen.server.manage.users.is_tenant_on_trial_fn", return_value=False)
@patch("lumen.server.manage.users.get_current_tenant_id", return_value="test_tenant")
@patch("lumen.server.manage.users.get_invited_users", return_value=[])
@patch("lumen.server.manage.users.get_all_users", return_value=[])
@patch("lumen.server.manage.users.write_invited_users", return_value=3)
...
_COMMON_PATCHES = [
    patch("lumen.server.manage.users.MULTI_TENANT", False),
    patch("lumen.server.manage.users.get_current_tenant_id", return_value="test_tenant"),
    patch("lumen.server.manage.users.get_invited_users", return_value=[]),
    patch("lumen.server.manage.users.get_all_users", return_value=[]),
    patch("lumen.server.manage.users.write_invited_users", return_value=1),
    patch("lumen.server.manage.users.enforce_seat_limit_locked"),
    patch("lumen.server.manage.users.enforce_invite_rate_limit"),
]
...
def test_bulk_invite_emits_user_create(
    ...
    with caplog.at_level(logging.INFO, logger="lumen.audit"):
...
@patch("lumen.server.manage.users.ENABLE_EMAIL_INVITES", False)
@patch("lumen.server.manage.users.get_current_tenant_id", return_value="test_tenant")
@patch("lumen.server.manage.users.get_invited_users", return_value=["known@example.com"])
@patch("lumen.server.manage.users.get_all_users", return_value=[])
@patch("lumen.server.manage.users.write_invited_users", return_value=1)
@patch("lumen.server.manage.users.enforce_seat_limit_locked")
@patch("lumen.server.manage.users.enforce_invite_rate_limit")
...
@_with_common_patches
@patch("lumen.server.manage.users.ENABLE_EMAIL_INVITES", True)
@patch("lumen.server.manage.users.EMAIL_CONFIGURED", False)
...
@_with_common_patches
@patch("lumen.server.manage.users.ENABLE_EMAIL_INVITES", True)
@patch("lumen.server.manage.users.EMAIL_CONFIGURED", True)
@patch("lumen.server.manage.users.send_user_email_invite")
...
@patch("lumen.server.manage.users.EMAIL_CONFIGURED", True)
@patch(
    "lumen.server.manage.users.send_user_email_invite",
...
@patch("lumen.server.manage.users.enforce_remove_invited_rate_limit")
@patch("lumen.server.manage.users.remove_user_from_invited_users", return_value=0)
@patch("lumen.server.manage.users.MULTI_TENANT", True)
@patch("lumen.server.manage.users.DEV_MODE", True)
@patch("lumen.server.manage.users.is_tenant_on_trial_fn", return_value=False)
@patch("lumen.server.manage.users.get_current_tenant_id", return_value="test_tenant")
...
def test_cloud_seat_decline_propagates_from_add_users(*_mocks: MagicMock) -> None:
    ...
    the caller as LumenError — the generic except must not swallow it."""

    def _declining_add_users(*_args: object) -> None:
        raise LumenError(LumenErrorCode.SEAT_LIMIT_EXCEEDED, "card declined")
    ...
    assert exc_info.value.error_code == LumenErrorCode.SEAT_LIMIT_EXCEEDED

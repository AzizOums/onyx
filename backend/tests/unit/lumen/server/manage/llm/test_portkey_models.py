...
Verifies the mapping from Portkey's OpenAI-shaped /v1/models response to Lumen
...
from lumen.server.manage.llm.api import get_portkey_available_models
from lumen.server.manage.llm.models import PortkeyModelsRequest
...
def _fetch(api_base: str = "https://api.portkey.ai/v1") -> dict:
    with (
        patch("lumen.server.manage.llm.api._resolve_api_key", return_value="pk-key"),
        patch(
            "lumen.server.manage.llm.api._get_portkey_models_response",

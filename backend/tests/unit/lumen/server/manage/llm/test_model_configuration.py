...
class TestModelConfigurationViewVisionFallback:
    ...
    def _view(
        ...
        with patch(
            "lumen.server.manage.llm.models.litellm_thinks_model_supports_image_input",
...
class TestModelConfigurationViewReasoningFallback:
    ...
    def _view(self, mc: MagicMock, litellm_reasoning: bool) -> ModelConfigurationView:
        with patch(
            "lumen.server.manage.llm.models.model_is_reasoning_model",
...
class TestModelConfigurationViewFromModelStatic:
    ...
    def test_deployment_alias_reveals_reasoning_support(self) -> None:
        ...
        with (
            patch(
                "lumen.server.manage.llm.models.get_max_input_tokens",
                return_value=128000,
            ),
            patch(
                "lumen.server.manage.llm.models.litellm_thinks_model_supports_image_input",
            ...
            patch(
                "lumen.llm.model_name_parser.parse_litellm_model_name",
            ) as mock_parse,
        ):
...
class TestLLMProviderDescriptorRecommendedDefault:
    ...
    def _from_model(
        ...
        with (
            patch(
                "lumen.server.manage.llm.models.filter_model_configurations",
                return_value=views,
            ),
            patch(
                "lumen.llm.well_known_providers.llm_provider_options."

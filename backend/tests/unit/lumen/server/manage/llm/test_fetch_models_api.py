...
class TestGetOllamaAvailableModels:
    ...
    def test_returns_model_list(
        ...
        from lumen.server.manage.llm.api import get_ollama_available_models

        mock_session = MagicMock()

        with patch("lumen.server.manage.llm.api.httpx") as mock_httpx:
    ...
    def test_unreachable_server_returns_400(self) -> None:
        ...
        with patch(
            "lumen.server.manage.llm.api.httpx.get",
    ...
    def test_upstream_error_status_returns_502(self) -> None:
        ...
        assert exc_info.value.error_code == LumenErrorCode.BAD_GATEWAY
    ...
    def test_syncs_to_db_when_provider_id_specified(
        ...
        with (
            patch("lumen.server.manage.llm.api.httpx") as mock_httpx,
            patch(
                "lumen.db.llm.fetch_existing_llm_provider_by_id",
...
class TestGetOpenRouterAvailableModels:
    ...
    def test_returns_model_list(self, mock_openrouter_response: dict) -> None:
        """Test that endpoint returns properly formatted model list."""
        from lumen.server.manage.llm.api import get_openrouter_available_models

        mock_session = MagicMock()

        with patch("lumen.server.manage.llm.api.httpx.get") as mock_get:
    ...
    def test_syncs_to_db_when_provider_id_specified(
        ...
        with (
            patch("lumen.server.manage.llm.api.httpx.get") as mock_get,
            patch(
                "lumen.db.llm.fetch_existing_llm_provider_by_id",
                return_value=mock_provider,
            ),
...
class TestGetLMStudioAvailableModels:
    ...
    def test_returns_model_list(self, mock_lm_studio_response: dict) -> None:
        ...
        from lumen.server.manage.llm.api import get_lm_studio_available_models
    ...
    def test_falls_back_to_stored_api_key(self) -> None:
        ...
        with (
            patch("lumen.server.manage.llm.api.httpx") as mock_httpx,
            patch(
                "lumen.server.manage.llm.api.fetch_existing_llm_provider_by_id",
                return_value=mock_provider,
            ),
        ):
            mock_response = MagicMock()
...
class TestGetLitellmAvailableModels:
    ...
    def test_returns_model_list(self, mock_litellm_response: dict) -> None:
        """Test that endpoint returns properly formatted model list."""
        from lumen.server.manage.llm.api import get_litellm_available_models
    ...
    def test_empty_data_raises_lumen_error(self) -> None:
        """Test that empty model list raises LumenError."""
        from lumen.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("lumen.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(LumenError, match="No models found"):
                get_litellm_available_models(request, MagicMock(), mock_session)

    def test_missing_data_key_raises_lumen_error(self) -> None:
        """Test that response without 'data' key raises LumenError."""
        from lumen.server.manage.llm.api import get_litellm_available_models
    ...
    def test_all_entries_unparseable_raises_lumen_error(self) -> None:
        """Test that LumenError is raised when all entries fail to parse."""
        from lumen.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("lumen.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(LumenError, match="No compatible models"):
                get_litellm_available_models(request, MagicMock(), mock_session)
    ...
    def test_401_raises_authentication_error(self) -> None:
        """Test that a 401 response raises LumenError with authentication message."""
        from lumen.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("lumen.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(LumenError, match="Authentication failed"):
                get_litellm_available_models(request, MagicMock(), mock_session)

    def test_404_raises_not_found_error(self) -> None:
        """Test that a 404 response raises LumenError with endpoint not found message."""
        from lumen.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("lumen.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(LumenError, match="endpoint not found"):
                get_litellm_available_models(request, MagicMock(), mock_session)
...
class TestGetBifrostAvailableModels:
    ...
    def test_returns_model_list(self, mock_bifrost_response: dict) -> None:
        ...
        from lumen.server.manage.llm.api import get_bifrost_available_models
    ...
    def test_request_failure_is_logged_and_returns_400(self) -> None:
        ...
        with (
            patch("lumen.server.manage.llm.api.httpx.get") as mock_get,
            patch("lumen.server.manage.llm.api.logger.warning") as mock_warning,
        ):
            mock_get.side_effect = httpx.ConnectError(
                "Connection refused", request=MagicMock()
            )
...
class TestGetBedrockAvailableModels:
    ...
    def test_bearer_token_delivered_via_per_session_provider(
        ...
        admin requests."""
        from lumen.server.manage.llm.api import get_bedrock_available_models
        ...
        with patch(
            "lumen.server.manage.llm.api.boto3.Session",
    ...
    def test_masked_bearer_token_resolved_from_existing_provider(
        ...
        with (
            ...
            patch("lumen.server.manage.llm.api._sync_fetched_models"),
        ):
            request = BedrockModelsRequest(
                aws_region_name="us-west-2",
    ...
    def test_real_client_signs_with_bearer_token(self) -> None:
        ...
        from lumen.server.manage.llm.api import _build_bedrock_bearer_token_session

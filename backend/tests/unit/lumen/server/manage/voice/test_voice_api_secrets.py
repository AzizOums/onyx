...
from lumen.db.models import VoiceProvider
from lumen.error_handling.exceptions import LumenError
from lumen.server.manage.voice.api import (
...
async def test_upsert_rejects_custom_config_credentials() -> None:
    with pytest.raises(LumenError, match="custom_config cannot contain"):
...
async def test_voice_provider_test_uses_stored_secret_independently(
    ...
    monkeypatch.setattr(
        "lumen.server.manage.voice.api.get_voice_provider", mock_get_voice_provider
    )
...
async def test_voice_provider_test_requires_id_for_stored_secret() -> None:
    with pytest.raises(LumenError, match="Provider id is required"):
        await voice_provider_test_endpoint(
            VoiceProviderTestRequest(
                provider_type="openai",
                api_key="provided-key",
...
async def test_voice_provider_test_rejects_stored_secret_type_mismatch() -> None:
    ...
    with pytest.raises(LumenError, match="does not match"):
        await voice_provider_test_endpoint(
            VoiceProviderTestRequest(
                id=1,
                provider_type="openai",

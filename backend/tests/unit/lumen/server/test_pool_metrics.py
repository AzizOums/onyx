...
from lumen.utils.middleware import _build_route_map, _match_route
...
def test_pool_state_collector_reports_pool_stats() -> None:
    ...
    assert metrics["lumen_db_pool_checked_out:sync"] == 5
    assert metrics["lumen_db_pool_checked_in:sync"] == 35
    assert metrics["lumen_db_pool_overflow:sync"] == 2
    assert metrics["lumen_db_pool_size:sync"] == 40
...
def test_checkout_event_stores_endpoint_and_increments_gauge() -> None:
    ...
    with patch("lumen.server.metrics.postgres_connection_pool.event") as mock_event:
    ...
    with (
        patch(
            "lumen.server.metrics.postgres_connection_pool.CURRENT_ENDPOINT_CONTEXTVAR"
        ) as mock_ctx,
        patch(
            "lumen.server.metrics.postgres_connection_pool.CURRENT_TENANT_ID_CONTEXTVAR"
        ) as mock_tenant_ctx,
        patch(
            "lumen.server.metrics.postgres_connection_pool._connections_held"
        ) as mock_gauge,
        patch("lumen.server.metrics.postgres_connection_pool._checkout_total"),
    ):
...
def test_checkin_event_observes_hold_duration() -> None:
    ...
    with (
        ...
        patch(
            "lumen.server.metrics.postgres_connection_pool._hold_seconds"
        ) as mock_hist,
        patch("lumen.server.metrics.postgres_connection_pool._checkin_total"),
        patch(
            "lumen.server.metrics.postgres_connection_pool.time.monotonic",
...
def test_checkin_with_missing_endpoint_uses_unknown() -> None:
    ...
    with (
        ...
        patch("lumen.server.metrics.postgres_connection_pool._hold_seconds"),
        patch("lumen.server.metrics.postgres_connection_pool._checkin_total"),
    ):
        mock_labels = MagicMock()
        mock_gauge.labels.return_value = mock_labels
...
def test_setup_skips_null_pool_engines() -> None:
    ...
    with (
        patch("lumen.server.metrics.postgres_connection_pool.REGISTRY"),
        patch(
            "lumen.server.metrics.postgres_connection_pool._register_pool_events"
        ) as mock_register,
    ):

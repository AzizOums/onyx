...
so it enforces at N * 1000 tokens (the Lumen convention).
"""
...
import lumen.server.query_and_chat.token_limit as token_limit
from lumen.db.models import TokenRateLimit, TokenRateLimitScope, UserUsage
...
from lumen.server.query_and_chat.token_limit import _token_budget_reset
...
class TestIsRateLimitedUnit:
    def test_budget_is_in_thousands(self) -> None:
        # token_budget=12 means 12,000 tokens (the Lumen thousands convention).
...
def _structured_reset_at(
    exc: LumenError, scope: TokenRateLimitScope
) -> datetime.datetime:
    assert exc.error_code is LumenErrorCode.RATE_LIMITED
...
def _assert_token_reset(
    exc: LumenError, scope: TokenRateLimitScope, period_hours: int
) -> None:
...
class TestRaiseRateLimited:
    def test_global_scope_shape(self) -> None:
        ...
        with pytest.raises(LumenError) as ei:
...
class TestTokenRateLimitArgsValidation:
    ...
    def test_neither_budget_rejected(self) -> None:
        from lumen.server.token_rate_limits.models import TokenRateLimitArgs

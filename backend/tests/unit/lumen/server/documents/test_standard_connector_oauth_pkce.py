...
from lumen.auth.pkce import compute_s256_challenge, generate_pkce_pair
from lumen.configs.constants import DocumentSource
from lumen.connectors.egnyte.connector import EgnyteConnector
from lumen.connectors.interfaces import OAuthConnector
from lumen.connectors.linear.connector import LinearConnector
...
def test_authorize_passes_pkce_challenge(
    ...
    monkeypatch.setattr(standard_oauth, "WEB_DOMAIN", "https://lumen.example")
    monkeypatch.setattr(
...
def test_callback_is_bound_to_the_initiating_user(
    ...
    with pytest.raises(LumenError, match="Invalid or expired"):
...
def test_callback_revalidates_stored_connector_arguments(
    ...
    with pytest.raises(LumenError, match="Invalid OAuth configuration"):
        standard_oauth.oauth_callback(
            source=DocumentSource.LINEAR,
            code="authorization-code",
            state=state,
...
def test_authorize_rejects_non_local_return_path(
    ...
    with pytest.raises(LumenError, match="OAuth return"):
        standard_oauth.oauth_authorize(
            request=_request(),
            source=DocumentSource.LINEAR,
...
def test_callback_normalizes_same_origin_return_and_replaces_credential_id(
    ...
    standard_oauth.oauth_authorize(
        ...
        source=DocumentSource.LINEAR,
        desired_return_url=(
            "https://lumen.example/return?step=0&credentialId=999#setup"
        ),
        user=_user(),
    )
    state = authorization_url.call_args.args[1]
    ...
    assert result.redirect_url == (
        "https://lumen.example/return?step=0&credentialId=7#setup"
    )

...
from lumen.auth.captcha import CaptchaVerificationError
from lumen.error_handling.exceptions import register_lumen_exception_handlers
from lumen.server.auth import captcha_api as captcha_api_module
from lumen.server.auth.captcha_api import CaptchaCookieMiddleware
...
def test_verify_endpoint_returns_ok_when_captcha_disabled() -> None:
    ...
    from lumen.auth.captcha import CAPTCHA_COOKIE_NAME
...
def test_verify_endpoint_raises_lumen_error_on_failure() -> None:
    app = build_app_with_middleware()
    client = TestClient(app)
    with (
        patch.object(captcha_api_module, "is_captcha_enabled", return_value=True),

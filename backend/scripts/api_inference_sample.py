...
# In this case, the equivalent of asking a question in Lumen Chat in a new chat session
import argparse
import json
import os
...
def create_new_chat_session(lumen_url: str, api_key: str | None) -> int:
    headers = {"Authorization": f"Bearer {api_key}"} if api_key else None
    session_endpoint = lumen_url + "/api/chat/create-chat-session"
...
def process_question(lumen_url: str, question: str, api_key: str | None) -> None:
    message_endpoint = lumen_url + "/api/chat/send-chat-message"

    chat_session_id = create_new_chat_session(lumen_url, api_key)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Lumen URL, should point to Lumen nginx.",
    )
    parser.add_argument(
        "--test-question",
        type=str,
        default="What is Lumen?",
        help="Test question for new Chat Session.",
    )

    # Not needed if Auth is disabled
    # Or for Lumen MIT API key must be replaced with session cookie
    api_key = os.environ.get("DANSWER_API_KEY")

    args = parser.parse_args()
    process_question(
        lumen_url=args.lumen_url, question=args.test_question, api_key=api_key
    )

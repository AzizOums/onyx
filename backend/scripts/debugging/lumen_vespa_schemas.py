"""Tool to generate all supported schema variations for Lumen Cloud's Vespa database.

Usage:

```
PYTHONPATH=. python scripts/debugging/lumen_vespa_schemas.py
```
...
from lumen.configs.embedding_configs import SUPPORTED_EMBEDDING_MODELS
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="lumen/document_index/vespa/app_config/schemas/danswer_chunk.sd.jinja",
    )
    parser.add_argument(
        "--cloud-services-template",
        help="The cloud-services.xml.jinja template path",
        default="ee/lumen/document_index/vespa/app_config/cloud-services.xml.jinja",
    )
    parser.add_argument(
        "--output-path",

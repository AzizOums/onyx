# Terraform Provider for Lumen

Manages **Lumen application configuration** declaratively via the Lumen admin API: LLM
...
> Lumen runs on (EKS, RDS, ...). This provider configures what runs *inside* an Lumen
> deployment.
...
|---|---|---|
| `lumen_api_key` | API keys (`/admin/api-key`) | numeric id |
| `lumen_llm_provider` | LLM providers + their model list (`/admin/llm/provider`) | numeric id |
| `lumen_llm_provider_default` | The deployment default (and vision) model — a singleton | `default` |
| `lumen_settings` | Workspace settings — a singleton, partially managed | `settings` |
| `lumen_embedding_provider` | Cloud embedding provider credentials | provider type (e.g. `openai`) |
| `lumen_credential` | Connector credentials (`/manage/credential`) | numeric id |
| `lumen_connector` | Connector definitions (`/manage/admin/connector`) | numeric id |
| `lumen_cc_pair` | Connector-credential pairs (`/manage/connector/.../credential/...`) | numeric id |
| `lumen_document_set` | Document sets (`/manage/admin/document-set`) | numeric id |
| `lumen_custom_tool` | Custom actions (`/admin/tool/custom`) | numeric id |
| `lumen_persona` | Agents / assistants (`/persona`) | numeric id |
| `lumen_mcp_server` | MCP servers Lumen connects to (`/admin/mcp`) | numeric id |
| `lumen_user_group` | User groups: roster, managers, permission grants (**EE only**) | numeric id |
| `data.lumen_llm_providers` | Read-only list of providers + defaults | — |
| `data.lumen_embedding_providers` | Read-only list of embedding providers | — |
| `data.lumen_settings` | Read-only current settings (incl. license `tier`) | — |
| `data.lumen_connectors` | Read-only list of connectors | — |
...
by an admin user). Create one in the Lumen admin panel (*API Keys*) or via the API — pass the
Admin group id, since a key with no group has no admin permissions:

```bash
curl -X POST https://your-lumen/api/admin/api-key \
...
requires `LUMEN_ADMIN_EMAIL` and `LUMEN_ADMIN_PASSWORD` rather than defaulting them: on a
...
```hcl
provider "lumen" {
  endpoint = "https://your-lumen.example.com" # or LUMEN_SERVER_URL
  api_key  = var.lumen_api_key                # or LUMEN_API_KEY
  # api_prefix defaults to "/api" (the web proxy). Set to "" when pointing
  # directly at the backend (e.g. http://localhost:8080). Also: LUMEN_API_PREFIX.
}
```

API keys work regardless of the deployment's human `AUTH_TYPE` (basic/OIDC/SAML/cloud),
and on Lumen Cloud the tenant is embedded in the key itself.
...
| --- | --- | --- |
| `lumen_llm_provider` | `api_key`, `custom_config` | `api_key_wo`, `custom_config_wo` |
| `lumen_embedding_provider` | `api_key` | `api_key_wo` |
| `lumen_credential` | `credential_json` | `credential_json_wo` |
| `lumen_mcp_server` | `api_token`, `admin_credentials` | `api_token_wo`, `admin_credentials_wo` |
| `lumen_custom_tool` | `custom_headers` | `custom_headers_wo` |

Set one or the other, never both. `lumen_credential` needs exactly one of the two, because
the payload is mandatory.

Three secrets have no twin, and cannot get one:

- **`lumen_api_key.api_key`** is the key Lumen mints, not one you supply. Terraform can only
  hand back a generated value through state. Treat the state file as holding it.
- **`lumen_mcp_server.auth_template_headers`** is computed — Lumen writes the template itself
...
- **The provider's own `api_key`** is provider configuration, which Terraform does not
  write to state at all. Supply it from `LUMEN_API_KEY` rather than in a `.tf` file.
...
The counter only decides when an apply is *triggered*. Lumen replaces all fields on update,
so the provider sends the secret on every apply it runs, whatever moved the plan.

### Two things to know

**`lumen_custom_tool` stops refreshing its headers.** Lumen returns action headers in full
...
**Importing takes one extra apply.** Import reads what the server has, so a secret Lumen
...
- **`lumen_settings` and `lumen_llm_provider_default` don't really delete.** Lumen has no
...
- **`lumen_embedding_provider` updates replace all fields.** Keep `api_key` (or
...
- **`model_configurations` is the list of record.** Models omitted from it are removed
  ...
  `lumen_llm_provider_default` first (references order this correctly).
- **`lumen_credential` payloads are never read back.** The API always returns the payload
...
- **`lumen_connector` does not own its access control.** `access_type` and `groups` are
  validated on write but stored on the cc-pair, so Terraform cannot refresh them. Lumen also
  rewrites an unset `prune_freq` to 7 days on the first update, which the provider then
  keeps as the value of record.
- **`lumen_connector` does not set access control.** Lumen applies it when a credential is
...
- **Deleting an agent leaves a tombstone.** Lumen marks the row deleted instead of removing
...
- **A deleted agent answers 400, not 404.** The lookup raises a plain `ValueError`, which
  Lumen renders as a bad request, so "gone" cannot be read off the status. The provider
...
- **`lumen_persona` does not own every field on an agent.** Attached folders and documents
  ...
  `search_start_date` is sent but never read back, because Lumen returns it as a parsed
  timestamp that would not match a plain date. Avatar images are not managed at all.
- **`display_priority` is create-only on the upsert.** Lumen reads it when an agent is
...
- **`lumen_user_group` is Enterprise Edition only.** The routes live in the EE application
...
- **`lumen_user_group` does not manage what a group can see.** Connectors, document sets,
...
- **A roster change must not disturb those links, and how it avoids that depends on the
  change.** Lumen's update endpoint replaces connector links along with members. A roster
  that only gains members therefore goes through the add-users endpoint instead, which
  takes members alone and lets Lumen preserve the links itself, inside the transaction that
...
- **A group's computed links lag by one apply.** Terraform creates a group before the
  `lumen_cc_pair` that references it, so `cc_pair_ids` is still empty in the state written
  by that first apply and fills in on the next refresh.
- **Lumen refuses membership, rename and delete while a group is syncing**, and a newly
...
- **`lumen_user_group` permissions use Lumen's wire tokens**, for example `manage:connectors`,
  not the enum names. Only toggleable permissions can be set; `basic`, `admin`,
  `craft_sandbox`, `manage:skills` and the implied read tokens are managed by Lumen and are
...
- **Lumen refuses a membership removal that would strand someone**, leaving them in no group
...
- **`lumen_mcp_server` manages only servers that need no interactive sign-in.** `NONE` and
...
- **Which tools an MCP server exposes is not managed.** Lumen only learns them by calling
...
- **An MCP server's `description` left out of the configuration is cleared, not kept.** Lumen
...
- **An MCP server added from the admin panel but never configured imports with empty strings.**
  ...
  in one apply.
- **A configured `auth_template_headers` is never refreshed from Lumen.** A header value may be
  a literal rather than a `{placeholder}`, and Lumen masks those on the way out, so refreshing
  would store the mask and leave a difference that never settles. Lumen's own template is read
...
- **`auth_performer = "PER_USER"` credentials belong to the identity that applied them.**
  Lumen stores `admin_credentials` against the applying user rather than the server, so a
...
- **An MCP server's header template is never reset.** Lumen keeps the stored template
...
- **`groups` and `users` on an MCP server are owned by the configuration.** Lumen reads a
  ...
  admin panel.
- **An MCP server URL cannot point at the Lumen host.** The SSRF guard refuses `localhost`
...
```bash
go build ./...        # build
go test ./...         # unit tests (no Lumen needed)
```
...
provider_installation {
  dev_overrides {
    "lumen-dot-app/lumen" = "/path/to/lumen/terraform-provider-lumen"
  }
  direct {}
}
```
...
Acceptance tests run real CRUD cycles against a live Lumen deployment (they create and
destroy providers/keys and briefly modify workspace settings — use a dev deployment):

```bash
TF_ACC=1 LUMEN_TF_ACC_SERVER_URL=http://localhost:8080 go test ./internal/provider/ -v
```

- `LUMEN_TF_ACC_API_PREFIX` defaults to `""` (direct backend). Set `/api` when targeting
  the web server.
- Auth: set `LUMEN_TF_ACC_API_KEY` to an existing admin key, or let the harness bootstrap
  one by logging in as `LUMEN_TF_ACC_ADMIN_EMAIL`/`LUMEN_TF_ACC_ADMIN_PASSWORD` (defaults:
  `admin_user@example.com` / `TestPassword123!`; on a fresh deployment the first
  registered user becomes admin automatically).

Without `TF_ACC` these tests skip, so plain `go test ./...` stays green with no Lumen
...
```bash
docker exec lumen-relational_db-1 psql -U postgres -c "CREATE DATABASE lumen_tf_acc;"
cd backend && POSTGRES_DB=lumen_tf_acc uv run alembic upgrade head
POSTGRES_DB=lumen_tf_acc AUTH_TYPE=basic LICENSE_ENFORCEMENT_ENABLED=false \
  ENABLE_PAID_ENTERPRISE_EDITION_FEATURES=true \
  USER_AUTH_SECRET="$(openssl rand -hex 32)" \
  uv run uvicorn lumen.main:app --port 8081
```
...
The `lumen_cc_pair`, `lumen_document_set` and `lumen_user_group` tests also need Celery,
...
export PYTHONPATH=/path/to/lumen/backend
celery -A lumen.background.celery.versioned_apps.beat beat --loglevel=INFO &
celery -A lumen.background.celery.versioned_apps.primary worker \
  --pool=threads --concurrency=4 --loglevel=INFO --hostname=tfacc-primary@%n -Q celery &
celery -A lumen.background.celery.versioned_apps.light worker \
...
**Beat is required for the user group tests specifically.** Lumen refuses to change or
...
connector's real `validate_connector_settings`, which reaches the source system; Lumen
...
releases go out through a mirror, `lumen-dot-app/terraform-provider-lumen`. The mirror holds
...
- [ ] Create the public repo `lumen-dot-app/terraform-provider-lumen`. Leave it empty or
...
- [ ] Create the release GPG key and register its public half with the Terraform Registry
      under the `lumen-dot-app` namespace.
...
- [ ] Link the mirror on registry.terraform.io as the `lumen-dot-app` organisation.

...
resource "lumen_mcp_server" "docs" {
  name        = "Docs"
  description = "Public documentation search"
  server_url  = "https://mcp.example.com/mcp"
}

# A server behind one shared API token. Lumen returns the token masked, so the
# configuration is the only record of it: rotate it here, never in the UI.
resource "lumen_mcp_server" "weather" {
  name           = "Weather"
...
resource "lumen_mcp_server" "tickets" {
  name           = "Tickets"

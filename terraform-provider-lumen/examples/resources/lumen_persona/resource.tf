...
resource "lumen_persona" "support" {
  name        = "Support"
  ...
  document_set_ids = [lumen_document_set.handbook.id]
  tool_ids         = [lumen_custom_tool.weather.id]
...
# Lumen promotes a featured agent to users. Ignoring documents from before a
# migration keeps an agent off stale material.
resource "lumen_persona" "onboarding" {
  name          = "Onboarding"

...
resource "lumen_document_set" "engineering" {
  name        = "engineering"
  description = "Design docs, runbooks and the internal wiki"

  cc_pair_ids = [
    lumen_cc_pair.confluence_wiki.id,
    lumen_cc_pair.github_docs.id,
  ]
}

# A set only two groups may use.
resource "lumen_document_set" "hr_private" {
  name        = "hr-private"
  description = "Handbook and policies"

  cc_pair_ids = [lumen_cc_pair.hr_handbook.id]
...
# Lumen rejects a set that holds nothing, so a set built only from federated
...
resource "lumen_document_set" "support_channels" {

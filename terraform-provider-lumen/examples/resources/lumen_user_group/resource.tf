...
resource "lumen_user_group" "everyone_uk" {
  name = "UK"
}
...
# managing the roster.
#
# Permissions use Lumen's own tokens, not the enum names. Only toggleable ones
# can be set: `basic`, `admin`, `craft_sandbox` and `manage:skills` are managed
# by Lumen and are refused here.
resource "lumen_user_group" "data_platform" {
  name = "Data Platform"
  ...
  # Every manager must also appear in user_ids: Lumen stores the flag on the
...
resource "lumen_cc_pair" "sales_drive" {
  name          = "Sales Drive"
  connector_id  = lumen_connector.drive.id
  credential_id = lumen_credential.drive.id
  access_type   = "private"
  groups        = [lumen_user_group.data_platform.id]
}

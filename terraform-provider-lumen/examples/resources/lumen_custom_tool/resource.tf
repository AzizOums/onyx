# A custom action lets an assistant call an external HTTP API. Lumen derives one
# callable method per operation, so every operation needs an operationId and
# either a summary or a description.
resource "lumen_custom_tool" "weather" {
  name        = "weather"
...
# configuration.
resource "lumen_custom_tool" "billing" {
  name       = "billing"
  definition = file("${path.module}/openapi/billing.json")

  # Forward the calling user's Lumen credentials instead of a fixed key, so the
...
resource "lumen_custom_tool" "legacy_lookup" {
  name       = "legacy-lookup"

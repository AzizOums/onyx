#!/usr/bin/env bash
# Mints an Lumen API key in the seeded "Admin" group and prints it.
#
...
server_url="${LUMEN_SERVER_URL:-http://localhost:8080}"
email="${LUMEN_ADMIN_EMAIL:-}"
password="${LUMEN_ADMIN_PASSWORD:-}"
key_name="${LUMEN_API_KEY_NAME:-terraform}"

if [ -z "$email" ] || [ -z "$password" ]; then
  echo "set LUMEN_ADMIN_EMAIL and LUMEN_ADMIN_PASSWORD" >&2
...
base="${server_url%/}"
if [ -n "${LUMEN_API_PREFIX:-}" ]; then
  base="${base}/${LUMEN_API_PREFIX#/}"
fi

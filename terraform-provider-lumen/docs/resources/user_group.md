...
page_title: "lumen_user_group Resource - terraform-provider-lumen"
subcategory: ""
description: |-
  ...
  Permissions in Lumen come only from group grants, so this resource is how a person gets any authority at all.
...
- `name` (String) Group name, unique across the deployment. Renaming is a separate call that Lumen refuses while the group is syncing, so the provider waits first.
...
Lumen refuses a removal that would leave someone in no group at all, because a person with no group has no permissions and would keep a login that can do nothing.

### Read-Only

- `cc_pair_ids` (Set of String) Connector-credential pairs shared with this group. Read-only here: `lumen_cc_pair` owns the link through its own `groups` attribute.
- `document_set_ids` (Set of String) Document sets shared with this group. Read-only here: `lumen_document_set` owns the link.
- `id` (String) Group id, assigned by Lumen.
- `is_default` (Boolean) Whether this is one of the seeded system groups (`Admin`, `Basic`). A default group holds members and nothing else: Lumen refuses to rename it, delete it, or change its permissions or incognito setting. Importing one and managing its roster works; anything else fails at apply time.
- `persona_ids` (Set of String) Agents shared with this group. Read-only here: `lumen_persona` owns the link.

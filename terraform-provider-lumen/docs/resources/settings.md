...
page_title: "lumen_settings Resource - terraform-provider-lumen"
subcategory: ""
description: |-
  The Lumen workspace settings singleton. Only attributes set in configuration are managed: unset attributes are left untouched server-side, and removing an attribute from configuration stops managing it rather than resetting it. Deleting the resource only removes it from state — the live settings are not changed. At most one lumen_settings resource should exist per deployment.
---

# lumen_settings (Resource)
...
- `craft_default_enabled` (Boolean) Workspace default for Lumen Craft access (per-user overrides win).
...
- `opensearch_indexing_enabled` (Boolean) OpenSearch migration flag. Read-only: controlled by the ENABLE_OPENSEARCH_INDEXING_FOR_LUMEN backend env var.
...
terraform import lumen_settings.workspace settings
```

...
- `admin_public` (Boolean) Whether every admin can use this credential. Lumen has no API to change it later. Leaving it `true` also keeps the credential readable: the API hides a private credential from admins other than its creator, and Terraform cannot tell that apart from a deleted one.
...
- `name` (String) Display name. Lumen has no API to clear a name, so removing this attribute keeps the last value instead of planning a change.

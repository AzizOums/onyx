...
page_title: "lumen_mcp_server Resource - terraform-provider-lumen"
subcategory: ""
description: |-
  An MCP server Lumen connects to, so its tools can be attached to agents.
  ...
  Which tools the server exposes is not part of this resource. Lumen learns them by calling the server, and both the tool selection and the Craft approval policies are rejected for a tool it has never seen.
---

# lumen_mcp_server (Resource)
...
- `name` (String) Display name. Lumen does not require it to be unique, so two servers may share a name.
- `server_url` (String) URL Lumen calls the server on. Lumen refuses loopback and link-local addresses whatever the SSRF protection level, so a server on the Lumen host itself cannot be reached by name.

### Optional

> **NOTE**: [Write-only arguments](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments) are supported in Terraform 1.11 and later.

- `admin_credentials` (Map of String, Sensitive) Values for the `auth_template_headers` placeholders, required with `auth_performer = "PER_USER"` and rejected otherwise — a shared token is set through `api_token`. Lumen stores them against the identity that applied, not the server, and returns them masked. Prefer `admin_credentials_wo`, which keeps the value out of state entirely; the two cannot be set together.
...
- `api_token` (String, Sensitive) Shared API token, for `auth_type = "API_TOKEN"` with `auth_performer = "ADMIN"`. Lumen returns it masked, so Terraform never reads it back: the configured value is the only record, and an imported server has none. Prefer `api_token_wo`, which keeps the value out of state entirely; the two cannot be set together.
...
- `auth_template_headers` (Map of String, Sensitive) Headers Lumen sends to the server, for `auth_performer = "PER_USER"`. A `{placeholder}` in a value names a field each user fills in. Lumen writes this itself for a shared token, and keeps whatever it holds when a request states none, so switching a server from per-user to a shared token leaves the per-user headers in place. Recreate the server to start over.
- `auth_type` (String) `NONE` or `API_TOKEN`.
- `available_in_craft` (Boolean) Whether the Craft agent may use this server. Lumen keeps this on a different endpoint from the rest, so setting it costs a second call.
- `description` (String) Free-text description.
- `groups` (Set of Number) User group ids that may use the server when it is not public. Lumen refuses the built-in `Admin` group here and asks for a public server instead. The configuration owns this list: removing it clears the groups on the server, including any added from the admin panel.
...
- `id` (String) Server id, assigned by Lumen.
- `last_refreshed_at` (String) When Lumen last listed the server's tools.
...
- `status` (String) Connection state, which Lumen cycles on its own: `CREATED`, `AWAITING_AUTH`, `FETCHING_TOOLS`, `CONNECTED` or `DISCONNECTED`.
- `tool_count` (Number) How many tools Lumen has discovered on the server.

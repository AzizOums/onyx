...
func TestAccLLMProviderResource(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_llm_provider" "test" {
  name          = "tf-acc-openai"
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("lumen_llm_provider.test", "id"),
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "name", "tf-acc-openai"),
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "provider_type", "openai"),
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "is_public", "true"),
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "is_auto_mode", "false"),
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "model_configurations.#", "1"),
					// The configured secret must live in state untouched by the
					// masked read-back.
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "api_key", "sk-tf-acc-fake-key"),
				),
			},
			{
				ResourceName:      "lumen_llm_provider.test",
				ImportState:       true,
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "name", "tf-acc-openai-renamed"),
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "model_configurations.#", "2"),
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "api_key", "sk-tf-acc-fake-key-rotated"),
					testAccCheckLLMProviderModelNames(t, "lumen_llm_provider.test", []string{"gpt-5-mini", "gpt-5-nano"}),
				),
			},
			{
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_llm_provider.test", "model_configurations.#", "1"),
					testAccCheckLLMProviderModelNames(t, "lumen_llm_provider.test", []string{"gpt-5-mini"}),
				),
			},
		},
	})

...
func TestAccSettingsResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_settings" "test" {
  company_name = "tf-acc-corp"
  auto_scroll  = true
}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_settings.test", "id", "settings"),
					resource.TestCheckResourceAttr("lumen_settings.test", "company_name", "tf-acc-corp"),
					resource.TestCheckResourceAttr("lumen_settings.test", "auto_scroll", "true"),
					// License-derived fields must be populated read-only.
					resource.TestCheckResourceAttrSet("lumen_settings.test", "tier"),
					resource.TestCheckResourceAttrSet("lumen_settings.test", "application_status"),
					// Unmanaged fields stay null: no value, no drift.
					resource.TestCheckNoResourceAttr("lumen_settings.test", "company_description"),
				),
			},
			{
				Config: `
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_settings.test", "company_name", "tf-acc-corp-renamed"),
					resource.TestCheckResourceAttr("lumen_settings.test", "auto_scroll", "false"),
				),
			},
			{
				ResourceName:      "lumen_settings.test",
				ImportState:       true,

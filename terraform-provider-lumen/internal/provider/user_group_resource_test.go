...
func TestAccUserGroupResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_user_group" "test" {
  name = "` + name + `"
}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_user_group.test", "name", name),
					resource.TestCheckResourceAttr("lumen_user_group.test", "is_default", "false"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "incognito_enabled", "false"),
					// Every collection defaults to empty rather than null, so an
					// unstated one does not report a change back to null.
					resource.TestCheckResourceAttr("lumen_user_group.test", "user_ids.#", "0"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "manager_ids.#", "0"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "permissions.#", "0"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "cc_pair_ids.#", "0"),
					resource.TestCheckResourceAttrSet("lumen_user_group.test", "id"),
				),
			},
			{
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_user_group.test", "name", name+"-renamed"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "user_ids.#", "1"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "manager_ids.#", "1"),
					resource.TestCheckTypeSetElemAttr("lumen_user_group.test", "user_ids.*", userID),
					resource.TestCheckTypeSetElemAttr("lumen_user_group.test", "manager_ids.*", userID),
					resource.TestCheckResourceAttr("lumen_user_group.test", "permissions.#", "2"),
					resource.TestCheckTypeSetElemAttr("lumen_user_group.test", "permissions.*", "manage:connectors"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "incognito_enabled", "true"),
				),
			},
			{
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_user_group.test", "user_ids.#", "1"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "manager_ids.#", "0"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "permissions.#", "1"),
					resource.TestCheckResourceAttr("lumen_user_group.test", "incognito_enabled", "false"),
				),
			},
			{
...
`,
			...
			{
				ResourceName:      "lumen_user_group.test",
				ImportState:       true,
...
// lumen_cc_pair owns those links. This is the end-to-end guard for that: the
...
func TestAccUserGroupKeepsConnectorLinksWhenTheRosterChanges(t *testing.T) {
	...
	dependencies := `
resource "lumen_connector" "links" {
  name                      = "` + name + `-connector"
...
resource "lumen_credential" "links" {
  name            = "` + name + `-credential"
  source          = "mock_connector"
  credential_json = jsonencode({})
}

resource "lumen_cc_pair" "links" {
  name          = "` + name + `-pair"
  connector_id  = lumen_connector.links.id
  credential_id = lumen_credential.links.id
  access_type   = "private"
  groups        = [lumen_user_group.links.id]
}
`
		...
		Steps: []resource.TestStep{
			{
				...
				Config: dependencies + `
resource "lumen_user_group" "links" {
  name = "` + name + `"
}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_user_group.links", "user_ids.#", "0"),
					resource.TestCheckResourceAttrSet("lumen_cc_pair.links", "id"),
				),
			},
			{
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_user_group.links", "user_ids.#", "1"),
					resource.TestCheckResourceAttr("lumen_user_group.links", "cc_pair_ids.#", "1"),
					resource.TestCheckResourceAttrPair(
						"lumen_user_group.links", "cc_pair_ids.0", "lumen_cc_pair.links", "id"),
				),
			},
			{
...
func TestAccUserGroupRejectsADuplicateName(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_user_group" "first" {
  name = "` + name + `"
}
`,
			},
...
resource "lumen_user_group" "second" {
  name = "` + name + `"
}
`,
...
// A manager is stored on the membership row, so Lumen cannot make one out of a
...
func TestAccUserGroupAcceptsIDsThatAreUnknownAtPlanTime(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_user_group" "anchor" {
  name = "` + name + `-anchor"
}

resource "lumen_user_group" "test" {
  name        = "` + name + `"
  user_ids    = [lumen_user_group.anchor.id]
  manager_ids = [lumen_user_group.anchor.id]
}
`,
				PlanOnly:           true,
...
func TestAccUserGroupRefusesToRenameADefaultGroup(t *testing.T) {
	...
	// LUMEN_TF_ACC_API_KEY short-circuits the bootstrap, which then never
	// records the id.
...
func TestAccUserGroupReportsAFailedFollowUpCall(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_user_group" "followup" {
  name        = "` + name + `"

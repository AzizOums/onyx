...
func TestAccAPIKeyResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
resource "lumen_api_key" "test" {
  name      = "tf-acc-test-key"
  group_ids = [%s]
}
`, adminGroupID),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("lumen_api_key.test", "id"),
					resource.TestCheckResourceAttr("lumen_api_key.test", "name", "tf-acc-test-key"),
					resource.TestCheckResourceAttr("lumen_api_key.test", "group_ids.#", "1"),
					resource.TestCheckTypeSetElemAttr("lumen_api_key.test", "group_ids.*", adminGroupID),
					resource.TestCheckResourceAttrSet("lumen_api_key.test", "api_key"),
					resource.TestCheckResourceAttrSet("lumen_api_key.test", "api_key_display"),
					resource.TestCheckResourceAttrSet("lumen_api_key.test", "user_id"),
				),
			},
			{
				ResourceName:      "lumen_api_key.test",
				ImportState:       true,
...
`, adminGroupID),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_api_key.test", "name", "tf-acc-test-key-renamed"),
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("lumen_api_key.test", "group_ids.#", "0"),
				),
			},
		},
	})

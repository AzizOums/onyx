...
// NewSettingsResource returns the lumen_settings resource.
...
func (r *settingsResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The Lumen workspace settings singleton. Only attributes set in " +
			...
			"At most one `lumen_settings` resource should exist per deployment.",
		Attributes: map[string]schema.Attribute{
			...
			"craft_default_enabled": schema.BoolAttribute{
				Optional:            true,
				MarkdownDescription: "Workspace default for Lumen Craft access (per-user overrides win).",
			},
			...
			"opensearch_indexing_enabled": schema.BoolAttribute{
				...
				MarkdownDescription: "OpenSearch migration flag. Read-only: controlled by the " +
					"ENABLE_OPENSEARCH_INDEXING_FOR_LUMEN backend env var.",
			},
		},
	}
}
...
func settingsErrorDetail(err error) string {
	...
	if errors.As(err, &apiErr) && apiErr.ErrorCode == "FEATURE_NOT_AVAILABLE" {
		return err.Error() + "\n\nThis setting requires a higher Lumen license tier (see the `tier` attribute)."
	}
	return err.Error()
}
...
func (r *settingsResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Failed to apply Lumen settings", settingsErrorDetail(err))
		if !patched {
			return
		}
...
func (r *settingsResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Failed to read Lumen settings", err.Error())
		return
	}
...
func (r *settingsResource) Delete(ctx context.Context, _ resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	resp.Diagnostics.AddWarning(
		"Lumen settings left unchanged",
		"lumen_settings was removed from Terraform state, but the live workspace settings were NOT "+
...
func (r *settingsResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	if req.ID != settingsResourceID {
		resp.Diagnostics.AddError(
			"Invalid import id",
			"lumen_settings is a singleton; import it with the fixed id \"settings\".",
		)
		return
	}

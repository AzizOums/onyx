		...
		Steps: []resource.TestStep{
			{
				...
				Config: `
resource "lumen_embedding_provider" "test" {
  provider_type = "voyage"
  api_key       = "pa-tf-acc-fake-key"
}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_embedding_provider.test", "id", "voyage"),
					resource.TestCheckResourceAttr("lumen_embedding_provider.test", "provider_type", "voyage"),
					// The configured secret must be in state, not the masked
					// read-back value.
					resource.TestCheckResourceAttr("lumen_embedding_provider.test", "api_key", "pa-tf-acc-fake-key"),
				),
			},
			{
				ResourceName:      "lumen_embedding_provider.test",
				ImportState:       true,
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_embedding_provider.test", "api_key", "pa-tf-acc-fake-key-rotated"),
					resource.TestCheckResourceAttr("lumen_embedding_provider.test", "api_url", "https://api.voyageai.example.com/v1"),
...
func testAccCheckEmbeddingProviderDestroyed(t *testing.T) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "lumen_embedding_provider" {
				continue
			}

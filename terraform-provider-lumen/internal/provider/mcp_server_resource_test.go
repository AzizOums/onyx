...
// external, because Lumen refuses loopback whatever the SSRF setting.
const mcpServerURL = "https://mcp.example.com/mcp"

func TestAccMCPServerResource(t *testing.T) {
	// Lumen does not require server names to be unique, so this is for legible
		...
		Steps: []resource.TestStep{
			{
				...
				Config: fmt.Sprintf(`
resource "lumen_mcp_server" "test" {
  name               = %q
...
`, name, mcpServerURL),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "name", name),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "description", "Weather tools"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "server_url", mcpServerURL),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "auth_type", "NONE"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "auth_performer", "ADMIN"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "transport", "STREAMABLE_HTTP"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "is_public", "true"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "available_in_craft", "true"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "status", "CREATED"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "tool_count", "0"),
					resource.TestCheckResourceAttrSet("lumen_mcp_server.test", "id"),
					resource.TestCheckResourceAttrSet("lumen_mcp_server.test", "owner"),
					// A server with no auth has no header template, and unset
					// collections must stay unset or every plan reports a change.
					resource.TestCheckNoResourceAttr("lumen_mcp_server.test", "auth_template_headers.%"),
					resource.TestCheckNoResourceAttr("lumen_mcp_server.test", "groups.#"),
					resource.TestCheckNoResourceAttr("lumen_mcp_server.test", "users.#"),
				),
			},
			{
				// Rename, drop the description, and turn both flags around.
				// Dropping the description is the real test: Lumen preserves the
...
`, name),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "name", name+"-renamed"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "description", ""),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "server_url", "https://mcp2.example.com/mcp"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "transport", "SSE"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "is_public", "false"),
					resource.TestCheckResourceAttr("lumen_mcp_server.test", "available_in_craft", "false"),
				),
			},
			{
				ResourceName:      "lumen_mcp_server.test",
				ImportState:       true,
...
// A shared API token is write-only: Lumen returns it masked, so Terraform holds
...
func TestAccMCPServerResourceAPIToken(t *testing.T) {
	...
	config := func(description, token string) string {
		return fmt.Sprintf(`
resource "lumen_mcp_server" "token" {
  name           = %q
  description    = %q
  server_url     = %q
  auth_type      = "API_TOKEN"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_mcp_server.token", "auth_type", "API_TOKEN"),
					resource.TestCheckResourceAttr("lumen_mcp_server.token", "api_token", "token-one"),
					// Lumen writes the header template itself for a shared token.
					resource.TestCheckResourceAttr("lumen_mcp_server.token", "auth_template_headers.Authorization", "Bearer {api_key}"),
				),
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_mcp_server.token", "description", "second"),
					resource.TestCheckResourceAttr("lumen_mcp_server.token", "api_token", "token-one"),
					testAccCheckMCPServerHasStoredCredentials(t, "lumen_mcp_server.token"),
				),
			},
			{
				Config: config("second", "token-two"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_mcp_server.token", "api_token", "token-two"),
					testAccCheckMCPServerHasStoredCredentials(t, "lumen_mcp_server.token"),
				),
			},
			{
				ResourceName:      "lumen_mcp_server.token",
				ImportState:       true,
				ImportStateVerify: true,
				// Lumen returns the token masked, so an imported server carries
...
func TestAccMCPServerResourceRejectsOAuth(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
resource "lumen_mcp_server" "oauth" {
...
func TestAccMCPServerResourceRequiresATokenForSharedAuth(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
resource "lumen_mcp_server" "missing" {
...
// fields, and supplies their own values for them. Lumen stores those against the
...
func TestAccMCPServerResourcePerUserAuth(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
resource "lumen_mcp_server" "per_user" {
  name           = %q
  server_url     = %q
  auth_type      = "API_TOKEN"
...
`, name, mcpServerURL),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_mcp_server.per_user", "auth_performer", "PER_USER"),
					// The template survives the round trip as written, rather
					// than being replaced by the shared-token one Lumen writes
					// for an ADMIN performer.
					resource.TestCheckResourceAttr("lumen_mcp_server.per_user", "auth_template_headers.X-Api-Key", "{api_key}"),
					resource.TestCheckResourceAttr("lumen_mcp_server.per_user", "admin_credentials.api_key", "the-admins-own-key"),
				),
			},
			{
				// Lumen keeps a stored header template whenever a request omits
...
`, name, mcpServerURL),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_mcp_server.per_user", "auth_performer", "ADMIN"),
					resource.TestCheckResourceAttr("lumen_mcp_server.per_user", "auth_template_headers.X-Api-Key", "{api_key}"),
					resource.TestCheckNoResourceAttr("lumen_mcp_server.per_user", "auth_template_headers.Authorization"),
				),
			},
			{
				ResourceName:      "lumen_mcp_server.per_user",
				ImportState:       true,
				ImportStateVerify: true,
				// Lumen returns these masked, so an imported server carries none.
...
// server. Lumen reads a missing list as "leave it alone", so an omitted one
...
`, name, mcpServerURL, userID),
				Check: resource.TestCheckResourceAttr("lumen_mcp_server.acl", "users.#", "1"),
			},
			{
...
`, name, mcpServerURL),
				Check: resource.TestCheckNoResourceAttr("lumen_mcp_server.acl", "users.#"),
			},
		},
	})
}
...
// A header template value may be a literal rather than a placeholder, and Lumen
...
func TestAccMCPServerResourceKeepsALiteralHeaderValue(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
resource "lumen_mcp_server" "literal" {
  name           = %q
  server_url     = %q
  auth_type      = "API_TOKEN"
  auth_performer = "PER_USER"
...
`, name, mcpServerURL),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_mcp_server.literal", "auth_template_headers.X-Tenant", "a-literal-value"),
					resource.TestCheckResourceAttr("lumen_mcp_server.literal", "auth_template_headers.X-Api-Key", "{api_key}"),
				),
			},
		},
	})
...
func TestAccMCPServerResourceRejectsAnUnknownPerformer(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
resource "lumen_mcp_server" "bad_performer" {
...
func TestAccMCPServerResourceRejectsAnUnknownPerformerWhileAuthTypeIsUnknown(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
resource "lumen_mcp_server" "source" {
  name       = "%s-source"
  server_url = %q
}

resource "lumen_mcp_server" "unknown_type" {
  name           = "%s-target"
  server_url     = %q
  auth_type      = lumen_mcp_server.source.status
  auth_performer = "EVERYONE"
}

		...
		Steps: []resource.TestStep{
			{
				...
				// deployment default.
				Config: `
resource "lumen_llm_provider" "for_default" {
...
resource "lumen_llm_provider_default" "test" {
  provider_id = lumen_llm_provider.for_default.id
  model_name  = "gpt-5-mini"
}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_llm_provider_default.test", "id", "default"),
					resource.TestCheckResourceAttrPair(
						"lumen_llm_provider_default.test", "provider_id",
						"lumen_llm_provider.for_default", "id",
					),
					resource.TestCheckResourceAttr("lumen_llm_provider_default.test", "model_name", "gpt-5-mini"),
					testAccCheckServerDefaultModel(t, "lumen_llm_provider.for_default", "gpt-5-mini"),
				),
			},
			{
				Config: `
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_llm_provider_default.test", "model_name", "gpt-5-nano"),
					testAccCheckServerDefaultModel(t, "lumen_llm_provider.for_default", "gpt-5-nano"),
				),
			},
			{
				ResourceName:      "lumen_llm_provider_default.test",
				ImportState:       true,

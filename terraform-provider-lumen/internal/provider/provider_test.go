...
// Acceptance tests need TF_ACC=1 plus LUMEN_TF_ACC_SERVER_URL (a live
// deployment). Auth uses LUMEN_TF_ACC_API_KEY when set; otherwise the harness
...
var testAccProtoV6ProviderFactories = map[string]func() (tfprotov6.ProviderServer, error){
	"lumen": providerserver.NewProtocol6WithError(New("test")()),
}
...
func testAccServerURL() string {
	return os.Getenv("LUMEN_TF_ACC_SERVER_URL")
}

func testAccAPIPrefix() string {
	if v, ok := os.LookupEnv("LUMEN_TF_ACC_API_PREFIX"); ok {
		return v
	}
	return ""
}
...
func testAccPreCheck(t *testing.T) {
	...
	if serverURL == "" {
		t.Skip("LUMEN_TF_ACC_SERVER_URL is not set; set it (e.g. http://localhost:8080) to run acceptance tests")
	}
	...
	t.Setenv("LUMEN_API_KEY", bootstrapKey)
	t.Setenv("LUMEN_API_PREFIX", testAccAPIPrefix())
}
...
func testAccAdminGroupID(t *testing.T) int64 {
	...
	// Only reached when LUMEN_TF_ACC_API_KEY short-circuited the bootstrap.
...
func bootstrapAPIKey(serverURL, apiPrefix string) (string, error) {
	if key := os.Getenv("LUMEN_TF_ACC_API_KEY"); key != "" {
		return key, nil
	}
	...
	email := envOr("LUMEN_TF_ACC_ADMIN_EMAIL", "admin_user@example.com")
	password := envOr("LUMEN_TF_ACC_ADMIN_PASSWORD", "TestPassword123!")
	...
	if loginResp.StatusCode >= 300 {
		return "", fmt.Errorf("login as %s failed with HTTP %d (set LUMEN_TF_ACC_ADMIN_EMAIL/LUMEN_TF_ACC_ADMIN_PASSWORD or LUMEN_TF_ACC_API_KEY)", email, loginResp.StatusCode)
	}

...
// A definition Lumen accepts: it needs info.title, info.description, a server
...
func TestAccCustomToolResource(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_custom_tool" "test" {
  name        = "tf-acc-weather"
  description = "Looks up the weather"
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_custom_tool.test", "name", "tf-acc-weather"),
					resource.TestCheckResourceAttr("lumen_custom_tool.test", "description", "Looks up the weather"),
					resource.TestCheckResourceAttr("lumen_custom_tool.test", "passthrough_auth", "false"),
					resource.TestCheckResourceAttr("lumen_custom_tool.test", "enabled", "true"),
					resource.TestCheckResourceAttr("lumen_custom_tool.test", "custom_headers.X-Api-Key", "tf-acc-secret"),
					resource.TestCheckResourceAttrSet("lumen_custom_tool.test", "id"),
					resource.TestCheckResourceAttrSet("lumen_custom_tool.test", "display_name"),
					resource.TestCheckNoResourceAttr("lumen_custom_tool.test", "oauth_config_id"),
				),
			},
			{
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lumen_custom_tool.test", "name", "tf-acc-weather-renamed"),
					resource.TestCheckResourceAttr("lumen_custom_tool.test", "description", ""),
					resource.TestCheckResourceAttr("lumen_custom_tool.test", "enabled", "false"),
					resource.TestCheckNoResourceAttr("lumen_custom_tool.test", "custom_headers.%"),
				),
			},
			{
				ResourceName:      "lumen_custom_tool.test",
				ImportState:       true,
...
// Lumen rejects a definition it cannot turn into callable methods, and the
...
func TestAccCustomToolResourceRejectsABadDefinition(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_custom_tool" "bad" {
  name       = "tf-acc-bad-definition"
  definition = jsonencode({ openapi = "3.0.0" })
}
`,
				ExpectError: regexp.MustCompile(`(?s)Lumen rejected the action definition`),
			},
		},
	})
}
...
func TestAccCustomToolResourceRejectsConflictingAuth(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_custom_tool" "conflict" {
...
// Lumen answers the read endpoint for built-in actions but refuses every write
...
func TestAccCustomToolResourceRejectsImportingABuiltIn(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
resource "lumen_custom_tool" "builtin" {
  name       = "tf-acc-import-target"
  definition = ` + customToolDefinition + `
}
`,
				ResourceName:  "lumen_custom_tool.builtin",
				ImportState:   true,
				ImportStateId: builtInID,
				ExpectError:   regexp.MustCompile(`(?s)Not a custom Lumen action`),
			},
		},
	})
}

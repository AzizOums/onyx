...
// ee/lumen/main.py and simply do not exist on Community Edition, where every
...
// connector ids: Lumen keeps the stored ones itself.
...
//
// Lumen's update endpoint replaces connector links along with members, and
// those links belong to lumen_cc_pair, so they have to survive a roster change.
// Which call does that best depends on the change:
//
// A roster that only gains members goes through the add-users endpoint. That
// one takes members alone, and Lumen preserves the connector links itself,
...
// between is overwritten by the older list. Lumen offers nothing narrower —
...
func (c *Client) SetUserGroupMembers(ctx context.Context, id int64, userIDs []string) (*UserGroup, error) {
	...
	// The whole roster goes out, not just the new names: Lumen works out which
...
// Grants Lumen manages itself are excluded, which matches what the provider
// is able to write.
...
// the stored set. Lumen refuses any permission it does not let a group toggle.
...
// DeleteUserGroup asks Lumen to delete a group. The row usually survives the
...
//
// Lumen refuses a membership change, a rename and a delete while a group is

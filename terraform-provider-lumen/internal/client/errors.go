...
// LumenError {"error_code", "detail"}, HTTPException {"detail"}, the ValueError
...
func (e *APIError) Error() string {
	if e.ErrorCode != "" {
		return fmt.Sprintf("lumen API error (HTTP %d, %s): %s", e.StatusCode, e.ErrorCode, e.Detail)
	}
	return fmt.Sprintf("lumen API error (HTTP %d): %s", e.StatusCode, e.Detail)
}

...
// A document set write leaves the set syncing, and Lumen rejects a change to a
...
func TestUserAgentIsVersioned(t *testing.T) {
	...
	if got := captured.Header.Get("User-Agent"); got != "terraform-provider-lumen/dev" {
		t.Errorf("User-Agent = %q, want terraform-provider-lumen/dev", got)
	}

	versioned := NewClient(Config{ServerURL: "http://example.invalid", Version: "1.2.3"})
	if versioned.userAgent != "terraform-provider-lumen/1.2.3" {
		t.Errorf("User-Agent = %q, want terraform-provider-lumen/1.2.3", versioned.userAgent)
	}
}

// Package client is a minimal hand-written HTTP client for the Lumen admin
...
// Config holds everything NewClient needs to reach an Lumen deployment.
type Config struct {
	// ServerURL is the server origin, e.g. https://cloud.lumen.app.
	ServerURL string
...
// Client talks to the Lumen backend API.
type Client struct {
	baseURL    string
	apiKey     string
	userAgent  string
...
func NewClient(cfg Config) *Client {
	...
	return &Client{
		baseURL:    base,
		apiKey:     cfg.APIKey,
		userAgent:  "terraform-provider-lumen/" + version,
		retry:      retry,
...
func (c *Client) newRequest(ctx context.Context, method, path string, body io.Reader) (*http.Request, error) {
	...
	if c.apiKey != "" {
		// X-Lumen-Authorization is checked first server-side and survives

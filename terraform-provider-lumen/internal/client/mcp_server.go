...
// MCPAuthTemplate is the header set Lumen sends to the MCP server. Values may
...
type MCPServerWrite struct {
	...
	// so it is never refreshed from the server. APITokenChanged tells Lumen to
...
// covers per-tool Craft policies, but Lumen rejects a policy for a tool it has

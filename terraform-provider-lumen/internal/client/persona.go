...
//
// Lumen hides a few built-in tools from this list, so an agent that holds one
...
//
// Lumen matches a create by name: a name another live agent holds is rejected,
...
// ValueError, which Lumen renders as a bad request. Callers that need to tell

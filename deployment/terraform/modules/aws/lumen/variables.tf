variable "name" {
  type        = string
  description = "Name of the Lumen resources. Example: 'lumen'"
  default     = "lumen"
}
...
variable "irsa_additional_service_account_names" {
  type        = list(string)
  description = "Additional service accounts in the Lumen namespace that may assume the workload IRSA role. Use the rendered ServiceAccount name for chart-created workloads, such as lumen-sandbox-proxy, that also need RDS IAM auth."
  default     = []
}

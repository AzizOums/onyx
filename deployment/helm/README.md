...
Chart tarballs are attached to GitHub releases named `helm/lumen-<version>`, and
...
```bash
helm install lumen oci://ghcr.io/lumen-dot-app/charts/lumen \
  --version 0.8.16 -n lumen --create-namespace
```
...
`helm/lumen-<version>` [releases](https://github.com/lumen-dot-app/lumen/releases)
or from the Helm repository index:

```bash
helm search repo lumen/lumen --versions   # needs `helm repo add` from above
```

To inspect one version before installing it:

```bash
helm show chart oci://ghcr.io/lumen-dot-app/charts/lumen --version 0.8.16
```
...
CRD and controller automatically as part of `helm install lumen lumen/lumen`.
...
## Lumen Craft with Kubernetes sandboxes
...
To point Lumen at an externally-managed Redis (e.g. AWS ElastiCache) and skip
...
externalSecret:
  ...
  refPath: "lumen/<customer>/app-secrets"
  # Name of the materialized k8s Secret. Point auth.*.existingSecret here.
  secretName: lumen-app-secrets
  secretStoreRef:
    name: aws-secrets-manager
    kind: ClusterSecretStore
  refreshInterval: 1h

# Inject every key from lumen-app-secrets as an env var on backend pods.
# Use for secrets that today live as plaintext in `configMap:` blocks.
extraEnvFromSecret: lumen-app-secrets

auth:
  postgresql:
    existingSecret: lumen-app-secrets
    secretKeys:
...
The Lumen docker-compose stack uses service-name hostnames like `api_server`,
...
* `DOMAIN` should be your public hostname (e.g. `lumen.example.com`), not
  `localhost`. It affects cookies and CORS.
* `WEB_DOMAIN` should be the full origin (e.g. `https://lumen.example.com`).
  Watch for typos like `hhttps://...`; they silently break email links and
  OAuth redirects.

# Local testing

> This section covers chart-maintainer testing; for the Lumen Craft local-kind developer workflow, see [docs/craft/dev/local-kubernetes.md](/docs/craft/dev/local-kubernetes.md).
...
* cd charts/lumen
* helm install lumen . -n lumen --set postgresql.primary.persistence.enabled=false --set auth.opensearch.values.opensearch_admin_password='StrongPassword123!'
...
* kubectl -n lumen port-forward service/lumen-nginx 8080:80
...
* When you are finished
  ...
  * Vespa leaves behind a PVC. Delete it if you are completely done.
    * k -n lumen get pvc
    * k -n lumen delete pvc vespa-storage-da-vespa-0
  * If you didn't disable Postgres persistence earlier, you may want to delete that PVC too.

## Run as non-root user
By default, some lumen containers run as root. If you'd like to explicitly run the lumen containers as a non-root user, update the values.yaml file for the following components:
...
## Resourcing
In the helm charts, we have resource suggestions for all Lumen-owned components. 
...
1. Install and manage the KEDA operator in your cluster yourself (for example via the official KEDA Helm chart). KEDA is no longer packaged as a dependency of the Lumen chart.

...
export type LumenSortDirection = "none" | "ascending" | "descending";
export type LumenSelectionState = "none" | "partial" | "all";
...
/**
 * Convert a TanStack sort direction to an Lumen sort direction string.
 *
 ...
 */
export function toLumenSortDirection(
  dir: false | "asc" | "desc"
): LumenSortDirection {
...
interface UseDataTableReturn<TData extends RowData> {
  ...
  // Pagination (1-based, matching Lumen Footer)
  ...
  // Selection (pre-computed for Lumen Footer)
  /** Aggregate selection state for the current page. */
  selectionState: LumenSelectionState;
...
/**
 * Wraps TanStack `useReactTable` with Lumen-specific defaults and derived
...
export default function useDataTable<TData extends RowData>(
  ...
  const selectionState: LumenSelectionState = isAllPageRowsSelected
    ? "all"

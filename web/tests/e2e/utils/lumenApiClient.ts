...
/**
 * API Client for Lumen backend operations in E2E tests.
 *
 * Provides a type-safe, abstracted interface for interacting with the Lumen backend API.
 ...
 * const client = new LumenApiClient(ctx, baseURL);
 * ```
 *
...
export class LumenApiClient {
  ...
  private async handleResponseSoft(
    ...
    if (!response.ok()) {
      const errorText = await response.text();
      console.error(
        `[LumenApiClient] ${errorMessage}: ${response.status()} - ${errorText}`
      );
      return false;
    }
    return true;
   ...
   *
   * @param message - The message to log (will be prefixed with "[LumenApiClient]")
   */
  private log(message: string): void {
    console.log(`[LumenApiClient] ${message}`);
  }

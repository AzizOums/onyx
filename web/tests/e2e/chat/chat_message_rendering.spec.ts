...
const SHORT_USER_MESSAGE = "What is Lumen?";
...
const LONG_USER_MESSAGE = `I've been evaluating several enterprise search and AI platforms for our organization, and I have a number of detailed questions about Lumen that I'd like to understand before we make a decision.

First, can you explain how Lumen handles document indexing across multiple data sources? We currently use Confluence, Google Drive, Slack, and GitHub, and we need to ensure that all of these can be indexed simultaneously without performance degradation.

Second, I'm interested in understanding the security model. Specifically, how does Lumen handle document-level permissions when syncing from sources that have their own ACL systems? Does it respect the original source permissions, or does it create its own permission layer?

Third, we have a requirement for real-time or near-real-time indexing. What is the typical latency between a document being updated in a source system and it becoming searchable in Lumen?
...
const SHORT_AI_RESPONSE =
  "Lumen is an open-source AI-powered enterprise search platform that connects to your company's documents, apps, and people.";

const LONG_AI_RESPONSE = `Lumen is an open-source Gen-AI and Enterprise Search platform designed to connect to your company's documents, applications, and people. Let me address each of your questions in detail.

## Document Indexing

Lumen uses a **connector-based architecture** where each data source has a dedicated connector. These connectors run as background workers and can index simultaneously without interfering with each other. The supported connectors include:
...
Lumen implements a **document-level permission system** that syncs with source ACLs. When documents are indexed, their permissions are preserved:

\`\`\`
Source Permission → Lumen ACL Sync → Query-time Filtering
\`\`\`
...
\`\`\`python
from lumen import LumenClient

client = LumenClient(api_key="your-key")
...
> **Note**: Lumen supports both cloud and self-hosted deployments. The self-hosted option gives you full control over your data.
  ...
  test.describe(`Chat Message Rendering (${theme} mode)`, () => {
    ...
    test.describe("Short Messages", () => {
      test("short user message with short AI response renders correctly", async ({
        ...
        const userMessage = page.locator("#lumen-human-message").first();
        await expect(userMessage).toContainText(SHORT_USER_MESSAGE);

        const aiMessage = page.getByTestId("lumen-ai-message").first();
    ...
    test.describe("Markdown and Code Rendering", () => {
      test("AI response with tables and code blocks renders correctly", async ({
        ...
        await sendMessage(page, "Give me an overview of Lumen features");

        const aiMessage = page.getByTestId("lumen-ai-message").first();
        await expect(aiMessage).toContainText("Key Features");
        await expect(aiMessage).toContainText("LumenClient");
    ...
    test.describe("Multi-Turn Conversation", () => {
      test("multi-turn conversation renders all messages correctly", async ({
        ...
        const responses = [
          SHORT_AI_RESPONSE,
          "Yes, Lumen supports over 30 data source connectors including Confluence, Google Drive, Slack, GitHub, Jira, Notion, and many more.",
          "To get started, you can deploy Lumen using Docker Compose with a single command. The setup takes about 5 minutes.",
        ];

        await mockChatEndpointSequence(page, responses);

        await sendMessage(page, SHORT_USER_MESSAGE);
        await expect(page.getByTestId("lumen-ai-message").first()).toContainText(
          "open-source AI-powered"
        );

        await sendMessage(page, "What connectors does it support?");
        await expect(page.getByTestId("lumen-ai-message")).toHaveCount(2, {
          timeout: 30000,
        });

        await sendMessage(page, "How do I get started?");
        await expect(page.getByTestId("lumen-ai-message")).toHaveCount(3, {
          timeout: 30000,
        });

        const userMessages = page.locator("#lumen-human-message");
      ...
      const WEB_SEARCH_DOCUMENTS: MockDocument[] = [
        {
          ...
          semantic_identifier: "Lumen Documentation - Getting Started",
          link: "https://docs.lumen.app/getting-started",
          source_type: "web",
          blurb:
            "Lumen is an open-source enterprise search and AI platform. Deploy in minutes with Docker Compose.",
          is_internet: true,
        },
        {
          document_id: "web-doc-2",
          semantic_identifier: "Lumen GitHub Repository",
          link: "https://github.com/lumen-dot-app/lumen",
          source_type: "web",
          blurb:
        ...
        },
        {
          ...
          source_type: "web",
          blurb:
            "Comparing top enterprise search platforms including Lumen, Glean, and Coveo.",
          is_internet: true,
        },
      ];

      const WEB_SEARCH_RESPONSE = `Based on my web search, here's what I found about Lumen:

Lumen is an open-source enterprise search and AI platform that can be deployed in minutes using Docker Compose [[D1]](https://docs.lumen.app/getting-started). The project is hosted on GitHub and is MIT licensed for the community edition, with over 30 connectors available [[D2]](https://github.com/lumen-dot-app/lumen).

In comparisons with other enterprise search platforms, Lumen stands out for its open-source nature and self-hosted deployment option [[D3]](https://example.com/enterprise-search-comparison). Unlike proprietary alternatives, you maintain full control over your data and infrastructure.

Key advantages include:

- **Self-hosted**: Deploy on your own infrastructure
- **Open source**: Full visibility into the codebase [[D2]](https://github.com/lumen-dot-app/lumen)
- **Quick setup**: Get running in under 5 minutes [[D1]](https://docs.lumen.app/getting-started)
- **Extensible**: 30+ pre-built connectors with custom connector support`;
      ...
      test("web search response with citations renders correctly", async ({
        ...
        await page.route("**/api/chat/send-chat-message", async (route) => {
          await route.fulfill({
            ...
            body: buildMockSearchStream({
              ...
              queries: ["Lumen enterprise search platform overview"],
        ...
        await sendMessage(page, "Search the web for information about Lumen");
      ...
      test("internal document search response renders correctly", async ({
        ...
        const internalDocs: MockDocument[] = [
          ...
          },
          {
            ...
            blurb:
              "Lumen platform architecture document covering microservices, data flow, and deployment topology.",
            is_internet: false,
          },
        ];

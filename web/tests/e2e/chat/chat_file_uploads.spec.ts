...
async function uploadFilesToChat(
  ...
  const inputWrapper = page.locator("#lumen-chat-input");
...
test.describe("Chat File Uploads", () => {
  ...
  test.describe("User-Uploaded Files", () => {
    test("uploaded text file renders in user message", async ({ page }) => {
      ...
      const fileDisplay = userMessage.locator("#lumen-file");
    ...
    test("uploaded image renders in user message", async ({ page }) => {
      ...
      const userMessage = page.locator("#lumen-human-message").first();
      const imageDisplay = userMessage.locator("#lumen-image");
    ...
    test("multiple uploaded files render together", async ({ page }) => {
      ...
      const userMessage = page.locator("#lumen-human-message").first();
      await expect(
        userMessage.locator("#lumen-file").getByText(textName)
      ).toBeVisible();
      await expect(
        userMessage.locator("#lumen-image").locator("img").first()
      ).toBeVisible();
  ...
  test.describe("LLM-Generated Images", () => {
    test("AI response with generated image renders correctly", async ({
      ...
      await mockChatEndpoint(
        ...
        buildMockImageGenStream({
          fileId,
          revisedPrompt: "A friendly cartoon lumen stone with a smile",
      ...
      await sendMessage(
        page,
        "Generate an image of a friendly cartoon lumen stone"
      );

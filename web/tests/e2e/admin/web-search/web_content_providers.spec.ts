...
test.describe("Web Content Provider Configuration", () => {
  ...
  test.describe("Firecrawl Provider", () => {
    ...
    test("should switch back to Lumen Web Crawler from Firecrawl", async ({
      page,
    }) => {
      ...
      console.log(
        "[web-content-test] Firecrawl configured, now switching to Lumen Web Crawler..."
      );

      // Switch to Lumen Web Crawler
      const lumenCrawlerCard = findProviderCard(page, "Lumen Web Crawler");
      await lumenCrawlerCard.waitFor({ state: "visible", timeout: 10000 });

      const lumenSetDefault = lumenCrawlerCard.getByRole("button", {
        name: "Set as Default",
      });

      if (await lumenSetDefault.isVisible()) {
        await lumenSetDefault.click();
        await page.waitForLoadState("networkidle");
      }

      await expect(
        lumenCrawlerCard.getByRole("button", { name: "Current Crawler" })
      ).toBeVisible({ timeout: 15000 });

      console.log("[web-content-test] Switched back to Lumen Web Crawler");
    });
  });
});

...
**[Lumen](https://www.lumen.app/?utm_source=lumen_repo&utm_medium=github&utm_campaign=readme)** 是面向 LLM 的应用层，提供功能丰富的界面，任何人都可以轻松自托管。
Lumen 通过 RAG、Web 搜索、代码执行、文件生成、深度研究等高级能力增强 LLM。

通过 50+ 内置的索引型 connector 或 MCP，将你的应用接入 Lumen。
...
- **🔬 Deep Research：** 通过多步骤研究流程生成深度报告
  - 截至 2026 年 2 月，位列 [leaderboard](https://github.com/lumen-dot-app/lumen_deep_research_bench) 榜首。
...
- **▶️ Actions 与 MCP：** 让 Lumen Agent 与外部应用交互，支持灵活的 Auth 配置。
- **💻 代码执行：** 在沙箱中执行代码，用于数据分析、绘图或修改文件。
- **🎙️ 语音模式：** 通过 TTS / STT 与 Lumen 对话。
- **🎨 图像生成：** 根据用户 prompt 生成图像。

Lumen 支持所有主流 LLM 提供商，包括自托管方案 (Ollama、LiteLLM、vLLM 等) 和闭源服务 (Anthropic、OpenAI、Gemini 等)。

更多内容详见我们的[文档](https://docs.lumen.app/welcome?utm_source=lumen_repo&utm_medium=github&utm_campaign=readme)！

---

## 🚀 部署方式

> Lumen 支持 Docker、Kubernetes、Helm / Terraform 部署，并为主流云厂商提供部署指南。
> 详细部署指南见[这里](https://docs.lumen.app/deployment/overview)。

Lumen 提供两种部署模式：Standard 与 Lite。

#### Lumen Lite

Lite 模式可以理解为一个轻量级的 Chat UI。资源占用更低 (内存低于 1GB)，技术栈也更简单。
适合想快速试用 Lumen 的用户，或只关心 Chat UI 与 Agent 功能的团队。

#### Standard Lumen

Lumen 的完整功能集，推荐正式使用的用户与较大规模团队选用。相比 Lite 模式额外包含：
- 用于 RAG 的向量 + 关键词索引。
...
> [!TIP]
> **想免部署免费试用 Lumen？访问 [Lumen Cloud](https://cloud.lumen.app/signup?utm_source=lumen_repo&utm_medium=github&utm_campaign=readme)。**

---

## 🏢 面向企业的 Lumen

Lumen 适合各种规模的团队，从个人用户到全球最大的企业：
...
- Lumen Community Edition (CE) 基于 MIT 许可证免费开放，涵盖 Chat、RAG、Agent 与 Action 的全部核心功能。
- Lumen Enterprise Edition (EE) 提供面向大型组织的额外功能。

功能详情请见[官网](https://www.lumen.app/pricing?utm_source=lumen_repo&utm_medium=github&utm_campaign=readme)。

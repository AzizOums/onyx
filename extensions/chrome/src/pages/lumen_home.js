...
import { getLumenDomain } from "../utils/storage.js";

(function () {
  let mainIframe = document.getElementById("lumen-iframe");
  ...
  function checkLumenPreference() {
    chrome.storage.local.get(
      ...
      (items) => {
        let useLumenAsDefaultNewTab =
          items[CHROME_SPECIFIC_STORAGE_KEYS.USE_LUMEN_AS_DEFAULT_NEW_TAB];

        if (useLumenAsDefaultNewTab === undefined) {
          useLumenAsDefaultNewTab = !!(
        ...
        setIframeSrc(items[CHROME_SPECIFIC_STORAGE_KEYS.LUMEN_DOMAIN] + "/nrf");
      }
    );
  }
  ...
  chrome.storage.onChanged.addListener(function (changes, namespace) {
    if (namespace === "local" && changes.useLumenAsDefaultNewTab) {
      checkLumenPreference();
    }
  });
  ...
  window.addEventListener("message", function (event) {
    if (event.data.type === CHROME_MESSAGE.SET_DEFAULT_NEW_TAB) {
      chrome.storage.local.set({ useLumenAsDefaultNewTab: event.data.value });
    } else if (event.data.type === CHROME_MESSAGE.LUMEN_APP_LOADED) {
      clearTimeout(iframeLoadTimeout);
      hideErrorModal();
      fadeInContent();
      iframeLoaded = true;

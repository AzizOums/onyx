...
```html
<!-- Load the widget -->
<script type="module" src="https://cdn.lumen.app/widget/1.0/dist/lumen-widget.js"></script>

<!-- Configure and display -->
<lumen-chat-widget
  backend-url="https://cloud.lumen.app/api"
...
| ------------- | ------ | -------------------------------------------------------------------- |
| `backend-url` | string | Your Lumen backend API URL (or set `VITE_WIDGET_BACKEND_URL` in .env) |
...
| `logo`             | string | Lumen logo     | URL to custom logo image                 |
...
```html
<lumen-chat-widget backend-url="https://cloud.lumen.app/api" api-key="on_abc123">
</lumen-chat-widget>
```
...
```html
<lumen-chat-widget mode="launcher"></lumen-chat-widget>
```
...
<div class="chat-container">
  <lumen-chat-widget mode="inline"></lumen-chat-widget>
</div>
```
...
│   └── assets/
│       └── logo.ts              # Default Lumen logo (base64)
├── dist/                        # Build output
├── index.html
├── package.json
├── vite.config.ts
...
The widget communicates with these Lumen backend endpoints:
...
3. **Deploy `dist/lumen-widget.js` to your server**

4. **Customer embed:**
   ```html
   <script type="module" src="https://your-cdn.com/lumen-widget.js"></script>
   <lumen-chat-widget
     agent-id="1"
     agent-name="Support"
...
```css
--theme-primary-05: #1c1c1c; /* Buttons, accents (lumen-ink-95) */
--theme-primary-06: #000000; /* Hover state (lumen-ink-100) */
